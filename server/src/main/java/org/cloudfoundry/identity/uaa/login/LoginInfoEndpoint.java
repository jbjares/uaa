 ...
 *
 * <p>This product includes a number of subcomponents wit separate copyright notices and license
...
public class LoginInfoEndpoint {
  ...
  private MapCollector<IdentityProvider, String, AbstractXOAuthIdentityProviderDefinition>
      idpsMapCollector =
          new MapCollector<>(
  ...
  private Links globalLinks =
      new Links().setSelfService(new Links.SelfService().setPasswd(null).setSignup(null));
  ...
  public LoginInfoEndpoint setXoAuthProviderConfigurator(
  ...
  private void updateLoginPageModel(
    ...
    ofNullable(oauthIdentityProviders)
        .orElse(emptyMap())
        .entrySet()
        .stream()
            ...
            e ->
                oauthLinks.put(
                    xoAuthProviderConfigurator.getCompleteAuthorizationURI(
                        e.getKey(), UaaUrlUtils.getBaseURL(request), e.getValue()),
  ...
  private String redirectToExternalProvider(
      AbstractIdentityProviderDefinition idpForRedirect, String alias, HttpServletRequest request) {
    if (idpForRedirect != null) {
      if (idpForRedirect instanceof SamlIdentityProviderDefinition) {
        String url =
            SamlRedirectUtils.getIdpRedirectUrl(
                (SamlIdentityProviderDefinition) idpForRedirect, entityID);
        return "redirect:/" + url;
      } else if (idpForRedirect instanceof AbstractXOAuthIdentityProviderDefinition) {
        try {
          String redirectUrl =
              getRedirectUrlForXOAuthIDP(
                  request, alias, (AbstractXOAuthIdentityProviderDefinition) idpForRedirect);
  ...
  private String getRedirectUrlForXOAuthIDP(
      HttpServletRequest request, String alias, AbstractXOAuthIdentityProviderDefinition definition)
      throws UnsupportedEncodingException {
    return xoAuthProviderConfigurator.getCompleteAuthorizationURI(
        alias, UaaUrlUtils.getBaseURL(request), definition);
  }

  protected Map<String, SamlIdentityProviderDefinition> getSamlIdentityProviderDefinitions(
  ...
  protected Map<String, AbstractXOAuthIdentityProviderDefinition>
    ...
    Map<String, AbstractXOAuthIdentityProviderDefinition> identityProviderDefinitions =
        identityProviders
            .stream()
  ...
  public AutologinResponse generateAutologinCode(
    ...
    String credentials =
        new String(new Base64().decode(base64Credentials.getBytes()), UTF_8.name());
