...
public class JdbcScimGroupProvisioning extends AbstractQueryable<ScimGroup>
  ...
  public static final String GET_GROUP_BY_NAME_SQL =
      String.format(
          "select %s from %s where displayName=? and identity_zone_id=?",
          GROUP_FIELDS, GROUP_TABLE);

  public static final String QUERY_FOR_FILTER =
  ...
  public static final String DELETE_GROUP_BY_ZONE =
      String.format("delete from %s where identity_zone_id=?", GROUP_TABLE);

  public static final String DELETE_GROUP_MEMBERSHIP_BY_ZONE =
      String.format("delete from %s where identity_zone_id = ?", GROUP_MEMBERSHIP_TABLE);

  public static final String DELETE_EXTERNAL_GROUP_BY_ZONE =
      String.format("delete from %s where identity_zone_id = ?", EXTERNAL_GROUP_TABLE);

  public static final String DELETE_ZONE_ADMIN_MEMBERSHIP_BY_ZONE =
      String.format(
  ...
  public static final String DELETE_ZONE_ADMIN_GROUPS_BY_ZONE =
      String.format("delete from %s where identity_zone_id=? and displayName like ?", GROUP_TABLE);

  public static final String DELETE_GROUP_MEMBERSHIP_BY_PROVIDER =
      String.format(
          "delete from %s where identity_zone_id = ? and origin = ?", GROUP_MEMBERSHIP_TABLE);

  public static final String DELETE_EXTERNAL_GROUP_BY_PROVIDER =
      String.format(
  ...
  public void onApplicationEvent(AbstractUaaEvent event) {
    if (event != null && event instanceof IdentityZoneModifiedEvent) {
      ...
      if (zevent.getEventType() == AuditEventType.IdentityZoneCreatedEvent) {
        ...
        getSystemScopes().stream().forEach(scope -> createAndIgnoreDuplicate(scope, zoneId));
      }
    }
  ...
  public int deleteByIdentityZone(String zoneId) {
    jdbcTemplate.update(
        ...
        "zones." + zoneId + ".%");
    jdbcTemplate.update(
        DELETE_ZONE_ADMIN_GROUPS_BY_ZONE, IdentityZone.getUaa().getId(), "zones." + zoneId + ".%");
