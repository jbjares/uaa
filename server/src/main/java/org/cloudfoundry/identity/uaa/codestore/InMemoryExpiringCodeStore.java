...
public class InMemoryExpiringCodeStore implements ExpiringCodeStore {
  ...
  public ExpiringCode generateCode(String data, Timestamp expiresAt, String intent, String zoneId) {
    ...
    ExpiringCode duplicate =
        store.putIfAbsent(code + IdentityZoneHolder.get().getId(), expiringCode);
    if (duplicate != null) {
  ...
  public void expireByIntent(String intent, String zoneId) {
    ...
    store
        .values()
        .stream()
        .filter(c -> intent.equals(c.getIntent()))
        .forEach(c -> store.remove(c.getCode() + IdentityZoneHolder.get().getId()));
  }
