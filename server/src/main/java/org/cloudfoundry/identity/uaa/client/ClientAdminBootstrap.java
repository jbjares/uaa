...
public class ClientAdminBootstrap
  ...
  /**
   * Flag to indicate that client details should override existing values by default. If true and
   * the override flag is not set in the client details input then the details will override any
   * existing details with the same id.
   *
   * @param defaultOverride the default override flag to set (default true, so flag does not have to
  ...
  /**
   * A set of client ids that are unconditionally to be autoapproved (independent of the settings in
   * the client details map). These clients will have <code>autoapprove=true</code> when they are
   * inserted into the client details store.
   *
  ...
  private void updateAutoApproveClients() {
    ...
    Collection<String> autoApproveList =
        new LinkedList(ofNullable(autoApproveClients).orElse(emptyList()));
        ...
        BaseClientDetails base =
            (BaseClientDetails)
                clientRegistrationService.loadClientByClientId(
                    clientId, IdentityZone.getUaa().getId());
        ...
        if (override == null || override) {
          ...
          if (didPasswordChange(clientId, client.getClientSecret())) {
            clientRegistrationService.updateClientSecret(
                clientId, client.getClientSecret(), IdentityZone.getUaa().getId());
          }
        } else {
          // ignore it
