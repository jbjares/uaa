 ...
 *
 * <p>Important methods like splitPacket, getHeader not extensible
 */
public class SyslogAppender extends AppenderSkeleton {
  ...
  private void initSyslogFacilityStr() {
    ...
    if (facilityStr == null) {
      System.err.println(
          "\"" + syslogFacility + "\" is an unknown syslog facility. Defaulting to \"USER\".");
   ...
   * recognized.
   *
   * @param facilityName one of the strings KERN, USER, MAIL, DAEMON, AUTH, SYSLOG, LPR, NEWS, UUCP,
   *     CRON, AUTHPRIV, FTP, LOCAL0, LOCAL1, LOCAL2, LOCAL3, LOCAL4, LOCAL5, LOCAL6, LOCAL7. The
   ...
   * non-default port can be specified by appending a colon and port number to a host name, an IPv4
   * address or an IPv6 address enclosed in square brackets.
   *
   * <p><b>WARNING</b> If the SyslogHost is not set, then this appender will fail.
   */
   ...
   *
   * <p>The <code>facilityName</code> parameter must be one of the strings KERN, USER, MAIL, DAEMON,
  ...
  public void setFacility(String facilityName) {
    ...
    if (syslogFacility == -1) {
      System.err.println(
          "[" + facilityName + "] is an unknown syslog facility. Defaulting to [USER].");
   ...
   * syslog packet. Default value is false for compatibility with existing behavior, however should
   * be true unless there is a specific justification.
   *
   * @since 1.2.15
   */
  ...
  /**
   * Returns whether the appender produces the HEADER part (that is, timestamp and host name) of the
   * syslog packet.
   *
   * @since 1.2.15
   */
