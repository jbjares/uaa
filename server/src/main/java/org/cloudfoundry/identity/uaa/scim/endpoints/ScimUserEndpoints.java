...
public class ScimUserEndpoints implements InitializingBean, ApplicationEventPublisherAware {
  ...
  public ScimUser getUser(@PathVariable String userId, HttpServletResponse response) {
    ScimUser scimUser =
        syncApprovals(
            syncGroups(scimUserProvisioning.retrieve(userId, IdentityZoneHolder.get().getId())));
  ...
  public ScimUser createUser(
    ...
    if (!isUaaUser(user)) {
      ...
      if (!idpsForEmailDomain.isEmpty()) {
        List<String> idpOrigins =
            idpsForEmailDomain.stream().map(idp -> idp.getOriginKey()).collect(Collectors.toList());
    ...
    ScimUser scimUser =
        scimUserProvisioning.createUser(user, user.getPassword(), IdentityZoneHolder.get().getId());
  ...
  public ScimUser deleteUser(
    ...
    membershipManager.removeMembersByMemberId(
        userId, user.getOrigin(), IdentityZoneHolder.get().getId());
    scimUserProvisioning.delete(userId, version, IdentityZoneHolder.get().getId());
    scimDeletes.incrementAndGet();
    if (publisher != null) {
      publisher.publishEvent(
          new EntityDeletedEvent<>(user, SecurityContextHolder.getContext().getAuthentication()));
  ...
  public ResponseEntity<VerificationResponse> getUserVerificationLink(
    ...
    ExpiringCode expiringCode =
        ScimUtils.getExpiringCode(
            codeStore, userId, user.getPrimaryEmail(), clientId, redirectUri, REGISTRATION);
  ...
  public ScimUser verifyUser(
    ...
    int version = etag == null ? -1 : getVersion(userId, etag);
    ScimUser user =
        scimUserProvisioning.verifyUser(userId, version, IdentityZoneHolder.get().getId());
    scimUpdates.incrementAndGet();
    ...
    try {
      result =
          scimUserProvisioning.query(
              filter, sortBy, sortOrder.equals("ascending"), IdentityZoneHolder.get().getId());
      for (ScimUser user : UaaPagingUtils.subList(result, startIndex, count)) {
        if (attributes.isEmpty()
            || attributes.stream().anyMatch(p -> "groups".equalsIgnoreCase(p))) {
          syncGroups(user);
        }
        if (attributes.isEmpty()
            || attributes.stream().anyMatch(p -> "approvals".equalsIgnoreCase(p))) {
          syncApprovals(user);
        }
        input.add(user);
      }
  ...
  public UserAccountStatus updateAccountStatus(
    ...
    if (status.isPasswordChangeRequired() != null && status.isPasswordChangeRequired()) {
      scimUserProvisioning.updatePasswordChangeRequired(
          userId, true, IdentityZoneHolder.get().getId());
    }
  ...
  protected ScimUser syncGroups(ScimUser user) {
    ...
    Set<ScimGroup> indirectGroups =
        membershipManager.getGroupsWithMember(user.getId(), true, IdentityZoneHolder.get().getId());
  ...
  protected ScimUser syncApprovals(ScimUser user) {
    ...
    Set<Approval> approvals =
        new HashSet<Approval>(
            approvalStore.getApprovalsForUser(user.getId(), IdentityZoneHolder.get().getId()));
  ...
  public void setIdentityProviderProvisioning(
  ...
  public void setMfaCredentialsProvisioning(
      UserMfaCredentialsProvisioning mfaCredentialsProvisioning) {
