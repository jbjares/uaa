...
public class ScimEventPublisher implements ApplicationEventPublisherAware {
  ...
  public void groupCreated(ScimGroup group) {
    publish(
        GroupModifiedEvent.groupCreated(group.getId(), group.getDisplayName(), getMembers(group)));
  }

  public void groupModified(ScimGroup group) {
    publish(
        GroupModifiedEvent.groupModified(group.getId(), group.getDisplayName(), getMembers(group)));
  }

  public void groupDeleted(ScimGroup group) {
    publish(
        GroupModifiedEvent.groupDeleted(group.getId(), group.getDisplayName(), getMembers(group)));
  }
