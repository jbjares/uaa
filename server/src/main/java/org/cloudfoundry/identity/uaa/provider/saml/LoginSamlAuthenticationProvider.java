...
public class LoginSamlAuthenticationProvider extends SAMLAuthenticationProvider
  ...
  public Authentication authenticate(Authentication authentication) throws AuthenticationException {
    ...
    publish(
        new IdentityProviderAuthenticationSuccessEvent(
            user, resultUaaAuthentication, OriginKeys.SAML));
  ...
  public Collection<? extends GrantedAuthority> retrieveSamlAuthorities(
    ...
    if (definition.getAttributeMappings().get(GROUP_ATTRIBUTE_NAME) != null) {
      ...
      } else if (definition.getAttributeMappings().get(GROUP_ATTRIBUTE_NAME)
      ...
      for (Attribute attribute : credential.getAttributes()) {
        if ((attributeNames.contains(attribute.getName()))
            || (attributeNames.contains(attribute.getFriendlyName()))) {
          if (attribute.getAttributeValues() != null && attribute.getAttributeValues().size() > 0) {
            for (XMLObject group : attribute.getAttributeValues()) {
              authorities.add(
                  new SamlUserAuthority(getStringValue(attribute.getName(), definition, group)));
            }
          }
        }
      }
