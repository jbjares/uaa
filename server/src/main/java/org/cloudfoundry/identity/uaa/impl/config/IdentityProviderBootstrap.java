...
public class IdentityProviderBootstrap
  ...
  public IdentityProviderBootstrap(
      IdentityProviderProvisioning provisioning, Environment environment) {
    ...
    */
    ...
    if (!override) {
      IdentityProvider existing =
          getProviderByOriginIgnoreActiveFlag(LDAP, IdentityZone.getUaa().getId());
      ...
      if (!UAA.equals(origin) && !LDAP.equals(origin)) {
        ...
        if (provider != null) {
          EntityDeletedEvent<IdentityProvider> event =
              new EntityDeletedEvent<>(provider, SYSTEM_AUTHENTICATION);
  ...
  protected void updateDefaultZoneUaaIDP() throws JSONException {
    ...
    IdentityProvider internalIDP =
        getProviderByOriginIgnoreActiveFlag(UAA, IdentityZone.getUaa().getId());
