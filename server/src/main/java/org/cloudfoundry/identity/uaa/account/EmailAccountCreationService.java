...
public class EmailAccountCreationService implements AccountCreationService {
  ...
  private void generateAndSendCode(
      String email, String clientId, String subject, String userId, String redirectUri)
      throws IOException {
    ExpiringCode expiringCode =
        ScimUtils.getExpiringCode(codeStore, userId, email, clientId, redirectUri, REGISTRATION);
  ...
  public AccountCreationResponse completeActivation(String code) throws IOException {
    ...
    ScimUser user =
        scimUserProvisioning.retrieve(data.get("user_id"), IdentityZoneHolder.get().getId());
    user =
        scimUserProvisioning.verifyUser(
            user.getId(), user.getVersion(), IdentityZoneHolder.get().getId());
  ...
  public ScimUser createUser(String username, String password, String origin) {
    ...
    try {
      ScimUser userResponse =
          scimUserProvisioning.createUser(scimUser, password, IdentityZoneHolder.get().getId());
      return userResponse;
  ...
  private String getEmailHtml(String code, String email) {
    ...
    ctx.setVariable(
        "servicePhrase",
        StringUtils.hasText(companyName) && IdentityZoneHolder.isUaa()
            ? companyName + " account"
            : "an account");
