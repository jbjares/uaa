...
public class ScimGroupEndpoints {
  ...
  public void setExternalMembershipManager(
      JdbcScimGroupExternalMembershipManager externalMembershipManager) {
    this.externalMembershipManager = externalMembershipManager;
  }

  public ScimGroupEndpoints(
      ScimGroupProvisioning scimGroupProvisioning, ScimGroupMembershipManager membershipManager) {
  ...
  private List<ScimGroup> filterForCurrentUser(
    ...
    while (needMore && startIndex <= input.size()) {
      for (ScimGroup group : UaaPagingUtils.subList(input, startIndex, count)) {
        if (includeMembers) {
          group.setMembers(
              membershipManager.getMembers(group.getId(), false, IdentityZoneHolder.get().getId()));
        }
        response.add(group);
  ...
  public SearchResults<?> listGroups(
    ...
    try {
      ...
      throw new ScimException(
          "Invalid filter expression: [" + filter + "]", HttpStatus.BAD_REQUEST);
    }
  ...
  public SearchResults<?> getExternalGroups(
    ...
    try {
      result =
          new ArrayList(
              externalMembershipManager.getExternalGroupMappings(IdentityZoneHolder.get().getId()));
    } catch (IllegalArgumentException e) {
      throw new ScimException(
  ...
  public ScimGroupExternalMember unmapExternalGroup(
    ...
    try {
      ...
      return externalMembershipManager.unmapExternalGroup(
          groupId, externalGroup.trim(), origin, IdentityZoneHolder.get().getId());
    } catch (IllegalArgumentException e) {
      throw new ScimException(e.getMessage(), HttpStatus.BAD_REQUEST);
    } catch (ScimResourceNotFoundException e) {
      throw new ScimException(e.getMessage(), HttpStatus.NOT_FOUND);
  ...
  public ScimGroupExternalMember unmapExternalGroupUsingName(
    ...
    try {
      ...
      return externalMembershipManager.unmapExternalGroup(
          getGroupId(displayName), externalGroup.trim(), origin, IdentityZoneHolder.get().getId());
    } catch (IllegalArgumentException e) {
      throw new ScimException(e.getMessage(), HttpStatus.BAD_REQUEST);
    } catch (ScimResourceNotFoundException e) {
      throw new ScimException(e.getMessage(), HttpStatus.NOT_FOUND);
  ...
  private String getGroupId(String displayName) {
    ...
    List<ScimGroup> result =
        dao.query("displayName eq \"" + displayName + "\"", IdentityZoneHolder.get().getId());
  ...
  public ScimGroup getGroup(@PathVariable String groupId, HttpServletResponse httpServletResponse) {
    ...
    group.setMembers(
        membershipManager.getMembers(groupId, false, IdentityZoneHolder.get().getId()));
  ...
  public ScimGroup createGroup(
    ...
    created.setMembers(
        membershipManager.getMembers(created.getId(), false, IdentityZoneHolder.get().getId()));
    ...
    try {
      ...
      if (group.getMembers() != null && group.getMembers().size() > 0) {
        membershipManager.updateOrAddMembers(
            updated.getId(), group.getMembers(), IdentityZoneHolder.get().getId());
      } else {
        membershipManager.removeMembersByGroupId(updated.getId(), IdentityZoneHolder.get().getId());
      }
      updated.setMembers(
          membershipManager.getMembers(updated.getId(), false, IdentityZoneHolder.get().getId()));
  ...
  public ResponseEntity<ScimGroupMember> getGroupMembership(
    ...
    ScimGroupMember membership =
        membershipManager.getMemberById(groupId, memberId, IdentityZoneHolder.get().getId());
  ...
  public ResponseEntity<List<ScimGroupMember>> listGroupMemberships(
    ...
    List<ScimGroupMember> members =
        membershipManager.getMembers(groupId, returnEntities, IdentityZoneHolder.get().getId());
  ...
  public ScimGroupMember deleteGroupMembership(
      @PathVariable String groupId, @PathVariable String memberId) {
    ScimGroupMember membership =
        membershipManager.removeMemberById(groupId, memberId, IdentityZoneHolder.get().getId());
    return membership;
  }
