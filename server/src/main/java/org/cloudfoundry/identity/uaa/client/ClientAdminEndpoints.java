...
public class ClientAdminEndpoints implements InitializingBean, ApplicationEventPublisherAware {
  ...
  public ClientDetails createClientDetails(@RequestBody BaseClientDetails client) throws Exception {
    ...
    ClientDetails ret =
        removeSecret(clientDetailsService.create(details, IdentityZoneHolder.get().getId()));
  ...
  public List<String> getRestrictedClientScopes() throws Exception {
    if (restrictedScopesValidator instanceof RestrictUaaScopesClientValidator) {
      return ((RestrictUaaScopesClientValidator) restrictedScopesValidator)
          .getUaaScopes()
          .getUaaScopes();
    } else {
      return null;
  ...
  public ClientDetailsModification[] modifyClientDetailsTx(
      @RequestBody ClientDetailsModification[] details) throws Exception {
    ...
    for (int i = 0; i < details.length; i++) {
      publish(
          new EntityDeletedEvent<>(
              details[i], SecurityContextHolder.getContext().getAuthentication()));
  ...
  protected void deleteApprovals(String clientId) {
    if (approvalStore != null) {
      ...
      throw new UnsupportedOperationException(
          "No approval store configured on " + getClass().getName());
    }
  }
  ...
  public SearchResults<?> listClientDetails(
    ...
    if (!StringUtils.hasLength(attributesCommaSeparated)) {
      return new SearchResults<>(
          Arrays.asList(SCIM_CLIENTS_SCHEMA_URI), result, startIndex, count, clients.size());
    }
    ...
    try {
      return SearchResultsFactory.buildSearchResultFrom(
          ...
          attributes,
          attributeNameMapper,
          Arrays.asList(SCIM_CLIENTS_SCHEMA_URI));
  ...
  public void setClientDetailsResourceMonitor(
      ResourceMonitor<ClientDetails> clientDetailsResourceMonitor) {
