...
public class JdbcUaaUserDatabase implements UaaUserDatabase {
  ...
  public static final String PRE_DEFAULT_USER_BY_USERNAME_QUERY =
      "select "
          + USER_FIELDS
          + "from users where %s = ? and active=? and origin=? and identity_zone_id=?";
  public static final String DEFAULT_CASE_SENSITIVE_USER_BY_USERNAME_QUERY =
      String.format(PRE_DEFAULT_USER_BY_USERNAME_QUERY, "lower(username)");
  public static final String DEFAULT_CASE_INSENSITIVE_USER_BY_USERNAME_QUERY =
      String.format(PRE_DEFAULT_USER_BY_USERNAME_QUERY, "username");

  public static final String PRE_DEFAULT_USER_BY_EMAIL_AND_ORIGIN_QUERY =
      "select "
          + USER_FIELDS
          + "from users where %s=? and active=? and origin=? and identity_zone_id=?";
  public static final String DEFAULT_CASE_SENSITIVE_USER_BY_EMAIL_AND_ORIGIN_QUERY =
      String.format(PRE_DEFAULT_USER_BY_EMAIL_AND_ORIGIN_QUERY, "lower(email)");
  public static final String DEFAULT_CASE_INSENSITIVE_USER_BY_EMAIL_AND_ORIGIN_QUERY =
  ...
  public static final String DEFAULT_USER_BY_ID_QUERY =
  ...
  public UaaUser retrieveUserById(String id) throws UsernameNotFoundException {
    try {
      return jdbcTemplate.queryForObject(
          DEFAULT_USER_BY_ID_QUERY, mapper, id, true, IdentityZoneHolder.get().getId());
    } catch (EmptyResultDataAccessException e) {
  ...
  private final class UserInfoRowMapper implements RowMapper<UserInfo> {
    ...
    public UserInfo mapRow(ResultSet rs, int rowNum) throws SQLException {
      ...
      UserInfo userInfo =
          hasText(info) ? JsonUtils.readValue(info, UserInfo.class) : new UserInfo();
      return userInfo;
    }
  }
  ...
  private final class UaaUserRowMapper implements RowMapper<UaaUser> {
    ...
    protected void getAuthorities(Set<String> authorities, final List<String> memberIdList) {
      ...
      Object[] parameterList =
          ArrayUtils.addAll(
              new Object[] {IdentityZoneHolder.get().getId()}, memberIdList.toArray());
