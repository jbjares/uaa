...
public class AccessController {
  ...
  /**
   * Explicitly requests caller to point back to an authorization endpoint on "https", even if the
   * incoming request is "http" (e.g. when downstream of the SSL termination behind a load
   * balancer).
   *
  ...
  private List<Map<String, String>> getScopes(ArrayList<String> scopes) {
    ...
    for (String scope : scopes) {
      ...
      Optional<ScimGroup> group =
          groupProvisioning
              .query(
                  String.format("displayName eq \"%s\"", scope), IdentityZoneHolder.get().getId())
              .stream()
              .findFirst();
      group.ifPresent(
          g -> {
