...
public class JdbcScimGroupMembershipManager
    implements ScimGroupMembershipManager, InitializingBean {
  ...
  public static final String MEMBERSHIP_FIELDS =
      "group_id,member_id,member_type,authorities,added,origin";
  ...
  public static final String DELETE_MEMBER_WITH_ORIGIN_SQL =
      String.format(
  ...
  public static final String DELETE_MEMBERS_WITH_ORIGIN_GROUP_SQL =
  ...
  public static final String DELETE_MEMBER_IN_GROUPS_SQL_USER =
      String.format(
          "delete from %s where member_id=? and member_type='USER' and identity_zone_id=?",
          MEMBERSHIP_TABLE);

  public static final String DELETE_MEMBER_IN_GROUPS_SQL_GROUP =
      String.format(
  ...
  public static final String GET_GROUPS_BY_EXTERNAL_MEMBER_SQL =
      String.format(
  ...
  private TimeBasedExpiringValueMap<String, ScimGroup> defaultGroupCache =
      new TimeBasedExpiringValueMap<>(timeService);;
    ...
    if (includeEntities) {
      for (ScimGroupMember member : result) {
        if (member.getType().equals(ScimGroupMember.Type.USER)) {
          ...
          ScimGroup group =
              groupProvisioning.retrieve(member.getMemberId(), IdentityZoneHolder.get().getId());
  ...
  private void getGroupsWithMember(
      List<ScimGroup> results, final String memberId, boolean transitive, final String zoneId) {
    if (results == null) {
      return;
    }
  ...
  @Override
  public Set<ScimGroup> getGroupsWithExternalMember(final String memberId, final String origin)
      throws ScimResourceNotFoundException {
  ...
  public ScimGroupMember getMemberById(String groupId, String memberId, String zoneId)
    ...
    try {
      ScimGroupMember u =
          jdbcTemplate.queryForObject(GET_MEMBER_SQL, rowMapper, memberId, groupId, zoneId);
      return u;
