...
public class UaaResetPasswordServiceTests {
  ...
  public void forgotPassword_ResetCodeIsReturnedSuccessfully() throws Exception {
    ...
    when(scimUserProvisioning.query(contains("origin"), eq(zoneID)))
    ...
    when(codeStore.generateCode(
            eq(
                "{\"user_id\":\"user-id-001\",\"username\":\"exampleUser\",\"passwordModifiedTime\":1234,\"client_id\":\"example\",\"redirect_uri\":\"redirect.example.com\"}"),
            any(Timestamp.class),
            anyString(),
            anyString()))
    ...
    ForgotPasswordInfo forgotPasswordInfo =
        uaaResetPasswordService.forgotPassword("exampleUser", "example", "redirect.example.com");

    verify(codeStore).expireByIntent(captor.capture(), anyString());
    assertEquals(
        UaaResetPasswordService.FORGOT_PASSWORD_INTENT_PREFIX + user.getId(), captor.getValue());
  ...
  public void forgotPassword_PublishesResetPasswordRequestEvent() throws Exception {
    ...
    ArgumentCaptor<ResetPasswordRequestEvent> captor =
        ArgumentCaptor.forClass(ResetPasswordRequestEvent.class);
  ...
  public void forgotPassword_ThrowsConflictException() throws Exception {
    ...
    String zoneId = IdentityZoneHolder.get().getId();
    when(scimUserProvisioning.query(contains("origin"), eq(zoneId)))
        .thenReturn(Arrays.asList(new ScimUser[] {}));
    when(scimUserProvisioning.query(eq("userName eq \"exampleUser\""), eq(zoneId)))
        .thenReturn(Arrays.asList(new ScimUser[] {user}));
    when(codeStore.generateCode(anyString(), any(Timestamp.class), eq(null), anyString()))
        .thenReturn(
            new ExpiringCode(
                "code", new Timestamp(System.currentTimeMillis()), "user-id-001", null));
    when(codeStore.retrieveCode(anyString(), anyString()))
        .thenReturn(
            new ExpiringCode(
                "code", new Timestamp(System.currentTimeMillis()), "user-id-001", null));
  ...
  public void resetPassword_validatesNewPassword() {
    ...
    ExpiringCode code1 =
        new ExpiringCode(
            "secret_code", new Timestamp(System.currentTimeMillis() + 1000 * 60 * 10), "{}", null);
  ...
  public void resetPassword_InvalidPasswordException_NewPasswordSameAsOld() {
    ...
    ExpiringCode expiringCode =
        new ExpiringCode(
            ...
            "{\"user_id\":\"user-id\",\"username\":\"username\",\"passwordModifiedTime\":null,\"client_id\":\"\",\"redirect_uri\":\"\"}",
            null);
    when(codeStore.retrieveCode("good_code", IdentityZoneHolder.get().getId()))
        .thenReturn(expiringCode);
    when(scimUserProvisioning.retrieve("user-id", IdentityZoneHolder.get().getId()))
        .thenReturn(user);
    when(scimUserProvisioning.checkPasswordMatches(
            "user-id", "Passwo3dAsOld", IdentityZoneHolder.get().getId()))
        .thenThrow(
  ...
  public void resetPassword_WithInvalidClientId() {
    ...
    doThrow(new NoSuchClientException("no such client"))
        .when(clientDetailsService)
        .loadClientByClientId("invalid_client", "uaa");
    ResetPasswordResponse response = uaaResetPasswordService.resetPassword(code, "new_secret");
  ...
  public void resetPassword_ForcedChange() {
    ...
    verify(scimUserProvisioning, times(1))
        .updatePasswordChangeRequired(userId, false, IdentityZoneHolder.get().getId());
    verify(scimUserProvisioning, times(1))
        .changePassword(userId, null, "password", IdentityZoneHolder.get().getId());
  }
  ...
  public void resetPassword_ForcedChange_NewPasswordSameAsOld() {
    ...
    when(scimUserProvisioning.retrieve(userId, IdentityZoneHolder.get().getId())).thenReturn(user);
    when(scimUserProvisioning.checkPasswordMatches(
            "user-id", "password", IdentityZoneHolder.get().getId()))
        .thenThrow(
            new InvalidPasswordException(
                "Your new password cannot be the same as the old password.", UNPROCESSABLE_ENTITY));
    uaaResetPasswordService.resetUserPassword(userId, "password");
  ...
  public void resetPassword_forcedChange_must_verify_password_policy() {
    ...
    user.setPrimaryEmail("foo@example.com");
    when(scimUserProvisioning.retrieve(userId, IdentityZoneHolder.get().getId())).thenReturn(user);
    doThrow(new InvalidPasswordException("Password cannot contain whitespace characters."))
  ...
  private ExpiringCode setupResetPassword(String clientId, String redirectUri) {
    ...
    ExpiringCode code =
        new ExpiringCode(
            ...
            "{\"user_id\":\"usermans-id\",\"username\":\"userman\",\"passwordModifiedTime\":null,\"client_id\":\""
                + clientId
                + "\",\"redirect_uri\":\""
                + redirectUri
                + "\"}",
