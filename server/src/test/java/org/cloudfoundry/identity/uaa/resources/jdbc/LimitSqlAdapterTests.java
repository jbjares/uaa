...
public class LimitSqlAdapterTests extends JdbcTestBase {
  ...
  public void revocable_token_delete_syntax() throws Exception {
    ...
    String query =
        limitSqlAdapter.getDeleteExpiredQuery("revocable_tokens", "token_id", "expires_at", 500);
  ...
  public void test_delete_top_rows() throws Exception {
    assertEquals(
        ...
        (int)
            jdbcTemplate.queryForObject(
                "select count(*) from delete_top_rows_test where id = 'X'", Integer.class));
    assertEquals(
        1,
        (int)
            jdbcTemplate.queryForObject(
                "select count(*) from delete_top_rows_test where id = 'A'", Integer.class));
    jdbcTemplate.update(
        limitSqlAdapter.getDeleteExpiredQuery("delete_top_rows_test", "id", "expires", 2), 5);
    assertEquals(
        1,
        (int)
            jdbcTemplate.queryForObject(
                "select count(*) from delete_top_rows_test where id = 'K'", Integer.class));
    assertEquals(
        1,
        (int)
            jdbcTemplate.queryForObject(
                "select count(*) from delete_top_rows_test where id = 'D'", Integer.class));
    assertEquals(
        1,
        (int)
            jdbcTemplate.queryForObject(
    ...
    assertEquals(
        ...
        (int)
            jdbcTemplate.queryForObject(
                "select count(*) from delete_top_rows_test", Integer.class));
  }
}
