...
public class LdapLoginAuthenticationManagerTests {
  ...
  public void update_existingUser_if_attributes_different() throws Exception {
    ExtendedLdapUserImpl authDetails =
        getAuthDetails(LDAP_EMAIL, "MarissaChanged", "BloggsChanged", "8675309");
    when(auth.getPrincipal()).thenReturn(authDetails);

    UaaUser user = getUaaUser();
    UaaUser userFromRequest = am.getUser(auth, null);
    am.userAuthenticated(auth, userFromRequest, user);
    ArgumentCaptor<ExternalGroupAuthorizationEvent> captor =
  ...
  public void dontUpdate_existingUser_if_attributes_same() throws Exception {
    ...
    ExtendedLdapUserImpl authDetails =
        getAuthDetails(
            user.getEmail(), user.getGivenName(), user.getFamilyName(), user.getPhoneNumber());
    when(auth.getPrincipal()).thenReturn(authDetails);
  ...
  public void test_group_white_list_with_wildcard() {
    ...
    assertThat(am.getExternalUserAuthorities(authDetails), containsInAnyOrder());

    definition.setExternalGroupsWhitelist(null);
    assertThat(am.getExternalUserAuthorities(authDetails), containsInAnyOrder());

    definition.setExternalGroupsWhitelist(Arrays.asList("ldap.role.1.a"));
    assertThat(am.getExternalUserAuthorities(authDetails), containsInAnyOrder("ldap.role.1.a"));

    definition.setExternalGroupsWhitelist(Arrays.asList("ldap.role.1.a", "ldap.role.2.*"));
    assertThat(
        am.getExternalUserAuthorities(authDetails),
  ...
  private ExtendedLdapUserImpl getAuthDetails(
      String email,
      String givenName,
      String familyName,
      String phoneNumber,
