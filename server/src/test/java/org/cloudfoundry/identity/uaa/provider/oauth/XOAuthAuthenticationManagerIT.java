...
public class XOAuthAuthenticationManagerIT {
  ...
  private static final List<String> SCOPES_LIST =
      Arrays.asList("openid", "some.other.scope", "closedid");
  ...
  public void setUp() throws Exception {
    ...
    header = map(entry("alg", "HS256"), entry("kid", keyName), entry("typ", "JWT"));
    signer = new RsaSigner(PRIVATE_KEY);
    IdentityZoneHolder.get()
        .getConfig()
        .getTokenPolicy()
        .setKeys(Collections.singletonMap(keyName, PRIVATE_KEY));
    ...
    for (String scope : SCOPES_LIST) {
      ...
      when(externalMembershipManager.getExternalGroupMapsByExternalGroup(
              eq(scope), anyString(), anyString()))
    ...
    xoAuthProviderConfigurator =
        spy(new XOAuthProviderConfigurator(provisioning, oidcMetadataFetcher));
    xoAuthAuthenticationManager =
        spy(
            new XOAuthAuthenticationManager(
                xoAuthProviderConfigurator,
                trustingRestTemplate,
                nonTrustingRestTemplate,
                tokenEndpointBuilder,
                new KeyInfoService(UAA_ISSUER_URL)));
    ...
    claims =
        map(
            ...
            entry(
                ClaimConstants.ACR,
                JsonUtils.readValue(
                    "{\"values\": [\"urn:oasis:names:tc:SAML:2.0:ac:classes:Password\"] }",
                    Map.class)));
  ...
  public void get_response_type_for_oauth2() {
    ...
    assertEquals(
        "id_token", xoAuthAuthenticationManager.getResponseType(oidcIdentityProviderDefinition));
  }
  ...
  public void unknown_config_class() {
    assertThrows(
        ...
        () -> {
          xoAuthAuthenticationManager.getResponseType(
              new AbstractXOAuthIdentityProviderDefinition() {
                @Override
  ...
  public void verify_hmac_256_signature() throws Exception {
    ...
    assertThat(
        new String(Base64.encodeBase64URLSafe(hmacData)),
  ...
  @Test
  public void
      when_a_null_id_token_is_provided_resolveOriginProvider_should_throw_a_jwt_validation_exception() {
    assertThrows(
  ...
  public void unable_to_resolve_to_single_provider() {
    ...
    assertThrows(
        ...
        String.format(
            "Unable to map issuer, %s , to a single registered provider", claims.get(ISS)));
  }
  ...
  public void origin_is_resolved_based_on_issuer_and_id_token() {
    CompositeToken token = getCompositeAccessToken();
    xCodeToken = new XOAuthCodeToken(null, null, null, token.getIdTokenValue(), null, null);
    when(provisioning.retrieveAll(eq(true), anyString()))
        .thenReturn(Collections.singletonList(getProvider()));
    // perform test
    xoAuthAuthenticationManager.getExternalAuthenticationDetails(xCodeToken);
    ...
    verify(xoAuthProviderConfigurator, times(1))
        .retrieveByIssuer(eq("http://localhost/oauth/token"), anyString());
  ...
  @Test
  public void
      when_exchanging_an_id_token_retrieved_from_the_internal_uaa_idp_for_an_access_token_then_auth_data_should_contain_oidc_sub_claim() {
    ...
    XOAuthAuthenticationManager.AuthenticationData externalAuthenticationDetails =
        xoAuthAuthenticationManager.getExternalAuthenticationDetails(xCodeToken);

    assertThat(username, is(externalAuthenticationDetails.getUsername()));
    assertThat(
        externalAuthenticationDetails.getClaims().get(ClaimConstants.ORIGIN), is(UAA_ORIGIN));
  ...
  @Test
  public void
      when_exchanging_an_id_token_retrieved_by_uaa_via_an_oidc_idp_for_an_access_token_origin_should_be_kept() {
    IdentityProvider<AbstractXOAuthIdentityProviderDefinition> idpProvider = getProvider();
    when(provisioning.retrieveAll(eq(true), anyString()))
        .thenReturn(Collections.singletonList(idpProvider));
    ...
    assertThat(username, is(externalAuthenticationDetails.getUsername()));
    assertThat(
        externalAuthenticationDetails.getClaims().get(ClaimConstants.ORIGIN),
        is(idpProvider.getOriginKey()));
    assertThat(xoAuthAuthenticationManager.getOrigin(), is(idpProvider.getOriginKey()));
  }

  @Test
  public void
      when_exchanging_an_id_token_retrieved_by_uaa_via_an_registered_oidc_idp_for_an_access_token_origin_should_be_taken_from_token() {
    IdentityProvider<AbstractXOAuthIdentityProviderDefinition> idpProvider = getProvider();
    ...
    assertThat(username, is(externalAuthenticationDetails.getUsername()));
    assertThat(
        externalAuthenticationDetails.getClaims().get(ClaimConstants.ORIGIN), is(OriginKeys.UAA));
    assertThat(xoAuthAuthenticationManager.getOrigin(), is(idpProvider.getOriginKey()));
  }

  @Test
  public void
      when_exchanging_an_id_token_retrieved_by_an_external_oidc_idp_for_an_access_token_then_auth_data_should_contain_oidc_sub_claim() {
    IdentityProvider<AbstractXOAuthIdentityProviderDefinition> idpProvider = getProvider();
    when(provisioning.retrieveAll(eq(true), anyString()))
        .thenReturn(Collections.singletonList(idpProvider));
  ...
  public void discoveryURL_is_used() throws MalformedURLException {
    ...
    mockUaaServer
        .expect(requestTo("http://some.discovery.url"))
        .andRespond(
            withStatus(OK)
                .contentType(APPLICATION_JSON)
                .body(JsonUtils.writeValueAsBytes(discoveryContent)));
  ...
  public void clientAuthInBody_is_used() {
    ...
    mockUaaServer
        .expect(requestTo(config.getTokenUrl().toString()))
        .andExpect(
            request ->
                assertThat(
                    "Check Auth header not present",
                    request.getHeaders().get("Authorization"),
                    nullValue()))
        .andExpect(content().string(containsString("client_id=" + config.getRelyingPartyId())))
        .andExpect(
            content().string(containsString("client_secret=" + config.getRelyingPartySecret())))
  ...
  public void idToken_In_Redirect_Should_Use_it() {
    ...
    ArgumentCaptor<ApplicationEvent> userArgumentCaptor =
    ...
    NewUserAuthenticatedEvent event =
        (NewUserAuthenticatedEvent) userArgumentCaptor.getAllValues().get(0);
  ...
  public void test_single_key_response() throws Exception {
    configureTokenKeyResponse("http://localhost/token_key", PRIVATE_KEY, "correctKey", false);
    addTheUserOnAuth();
    xoAuthAuthenticationManager.authenticate(xCodeToken);
  }
  ...
  public void test_multi_key_response_without_value() throws Exception {
    ...
    Map<String, Object> mapValid =
        JsonUtils.readValue(jsonValid, new TypeReference<Map<String, Object>>() {});
    Map<String, Object> mapInvalid =
        JsonUtils.readValue(jsonInvalid, new TypeReference<Map<String, Object>>() {});
    mapValid.remove("value");
    mapInvalid.remove("value");
    String json =
        JsonUtils.writeValueAsString(
            new JsonWebKeySet<>(
                Arrays.asList(new JsonWebKey(mapInvalid), new JsonWebKey(mapValid))));
    configureTokenKeyResponse("http://localhost/token_key", json);
    addTheUserOnAuth();
    xoAuthAuthenticationManager.authenticate(xCodeToken);
  }
  ...
  public void test_multi_key_all_invalid() throws Exception {
    ...
    Map<String, Object> mapInvalid =
        JsonUtils.readValue(jsonInvalid, new TypeReference<Map<String, Object>>() {});
    Map<String, Object> mapInvalid2 =
        JsonUtils.readValue(jsonInvalid2, new TypeReference<Map<String, Object>>() {});
    String json =
        JsonUtils.writeValueAsString(
            new JsonWebKeySet<>(
                Arrays.asList(new JsonWebKey(mapInvalid), new JsonWebKey(mapInvalid2))));
  ...
  public void test_multi_key_response() throws Exception {
    configureTokenKeyResponse("http://localhost/token_key", PRIVATE_KEY, "correctKey", true);
    addTheUserOnAuth();
    xoAuthAuthenticationManager.authenticate(xCodeToken);
  }
  ...
  public void doesNotCreateShadowUserAndFailsAuthentication_IfAddShadowUserOnLoginIsFalse() {
    ...
    assertThrows(
        AccountNotPreCreatedException.class,
        () -> xoAuthAuthenticationManager.authenticate(xCodeToken));
  }
  ...
  public void rejectTokenWithInvalidSignature() {
    ...
    assertThrows(
        InvalidTokenException.class, () -> xoAuthAuthenticationManager.authenticate(xCodeToken));
  }
  ...
  public void updateShadowUser_IfAlreadyExists() {
    ...
    ExternalGroupAuthorizationEvent event =
        (ExternalGroupAuthorizationEvent) userArgumentCaptor.getAllValues().get(0);
  ...
  public void invitedUser_becomesVerifiedOnAccept() {
    ...
    assertEquals(3, userArgumentCaptor.getAllValues().size());
    assertThat(
        userArgumentCaptor.getAllValues().get(0), instanceOf(InvitedUserAuthenticatedEvent.class));
  ...
  public void loginAndValidateSignatureUsingTokenKeyEndpoint() throws Exception {
    ...
    VerificationKeyResponse verificationKeyResponse =
        TokenKeyEndpoint.getVerificationKeyResponse(key);
    String response = JsonUtils.writeValueAsString(verificationKeyResponse);

    mockToken();
    mockUaaServer
        .expect(requestTo("http://localhost/token_key"))
        .andExpect(
            header(
                "Authorization",
                "Basic " + new String(Base64.encodeBase64("identity:identitysecret".getBytes()))))
  ...
  public void authenticatedUser_hasConfigurableUsernameField() {
    ...
    UaaUser uaaUser =
        xoAuthAuthenticationManager.getUser(
            xCodeToken, xoAuthAuthenticationManager.getExternalAuthenticationDetails(xCodeToken));
  ...
  public void testGetUserSetsTheRightOrigin() {
    ...
    XOAuthCodeToken otherToken =
        new XOAuthCodeToken(CODE, "other_origin", "http://localhost/callback/the_origin");
    xoAuthAuthenticationManager.getUser(
        otherToken, xoAuthAuthenticationManager.getExternalAuthenticationDetails(otherToken));
  ...
  public void test_authentication_context_transfers_to_authentication() {
    ...
    UaaAuthentication authentication =
        (UaaAuthentication) xoAuthAuthenticationManager.authenticate(xCodeToken);
    assertNotNull(authentication);
    assertNotNull(authentication.getAuthContextClassRef());
    assertThat(
        authentication.getAuthContextClassRef(),
        containsInAnyOrder("urn:oasis:names:tc:SAML:2.0:ac:classes:Password"));
  }
  ...
  public void unableToAuthenticate_whenProviderIsNotOIDCOrOAuth() {
    when(provisioning.retrieveByOrigin(eq(ORIGIN), anyString()))
        .thenReturn(MultitenancyFixture.identityProvider("the_origin", "uaa"));
  ...
  public void tokenCannotBeFetchedFromCodeBecauseOfServerError() {
    ...
    assertThrows(
        HttpServerErrorException.class, () -> xoAuthAuthenticationManager.authenticate(xCodeToken));
  }
  ...
  public void tokenCannotBeFetchedFromInvalidCode() {
    ...
    assertThrows(
        HttpClientErrorException.class, () -> xoAuthAuthenticationManager.authenticate(xCodeToken));
  }
  ...
  public void test_user_existing_attributes_mapping() {
    ...
    UaaAuthentication authentication =
        (UaaAuthentication) xoAuthAuthenticationManager.authenticate(xCodeToken);
    UaaUser actualUaaUser =
        xoAuthAuthenticationManager
            .getUserDatabase()
            .retrieveUserById(authentication.getPrincipal().getId());
  ...
  public void test_custom_user_attributes_are_stored() {
    ...
    UaaUser actualUser =
        xoAuthAuthenticationManager.getUserDatabase().retrieveUserByName("12345", "the_origin");
  ...
  private void configureTokenKeyResponse(String keyUrl, String signingKey, String keyId)
      throws MalformedURLException {
    configureTokenKeyResponse(keyUrl, signingKey, keyId, false);
  }

  private void configureTokenKeyResponse(
      String keyUrl, String signingKey, String keyId, boolean list) throws MalformedURLException {
  ...
  private String getKeyJson(String signingKey, String keyId, boolean list) {
    ...
    Object verificationKeyResponse =
        list
            ? new VerificationKeysListResponse(Collections.singletonList(keyResponse))
            : keyResponse;
    return JsonUtils.writeValueAsString(verificationKeyResponse);
  }

  private void configureTokenKeyResponse(String keyUrl, String response)
      throws MalformedURLException {
    config.setTokenKey(null);
    config.setTokenKeyUrl(new URL(keyUrl));
    mockToken();
    mockUaaServer
        .expect(requestTo(keyUrl))
        .andExpect(
            header(
                "Authorization",
                "Basic " + new String(Base64.encodeBase64("identity:identitysecret".getBytes()))))
  ...
  private void mockToken() {
    ...
    mockUaaServer
        .expect(requestTo("http://localhost/oauth/token"))
        .andExpect(
            header(
                "Authorization",
                "Basic " + new String(Base64.encodeBase64("identity:identitysecret".getBytes()))))
        ...
        .andExpect(
            content()
                .string(
                    containsString("redirect_uri=http%3A%2F%2Flocalhost%2Fcallback%2Fthe_origin")))
