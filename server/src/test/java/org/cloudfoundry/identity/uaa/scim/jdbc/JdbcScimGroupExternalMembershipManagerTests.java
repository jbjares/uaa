...
public class JdbcScimGroupExternalMembershipManagerTests extends JdbcTestBase {
  ...
  private void addGroup(String id, String name) {
    ...
    jdbcTemplate.execute(
        String.format(addGroupSqlFormat, id, name, IdentityZoneHolder.get().getId()));
  }

  private void validateCount(int expected) {
    int existingMemberCount =
        jdbcTemplate.queryForObject("select count(*) from external_group_mapping", Integer.class);
  ...
  public void deleteGroupAndMappings() {
    ...
    int mappingsCount =
        jdbcTemplate.queryForObject(
            "select count(1) from "
                + JdbcScimGroupExternalMembershipManager.EXTERNAL_GROUP_MAPPING_TABLE,
            Integer.class);
  ...
  public void test_group_mapping() throws Exception {
    ...
    assertEquals(
        1,
        edao.getExternalGroupMapsByExternalGroup(
    ...
    assertEquals(
        0,
        edao.getExternalGroupMapsByExternalGroup(
                "cn=engineering,ou=groups,dc=example,dc=com", origin, "id")
            .size());
  }

  private void createGroupMapping() {
    ScimGroup group =
        gdao.retrieve("g1-" + IdentityZoneHolder.get().getId(), IdentityZoneHolder.get().getId());
    assertNotNull(group);

    ScimGroupExternalMember member =
        edao.mapExternalGroup(
            "g1-" + IdentityZoneHolder.get().getId(),
            "cn=engineering,ou=groups,dc=example,dc=com",
            origin,
            IdentityZoneHolder.get().getId());
    ...
    List<ScimGroupExternalMember> externalMapping =
        edao.getExternalGroupMapsByGroupId(
            "g1-" + IdentityZoneHolder.get().getId(), origin, IdentityZoneHolder.get().getId());
  ...
  public void cannot_Retrieve_ById_For_OtherZone() {
    edao.getExternalGroupMapsByGroupId(
        "g1-" + otherZone.getId(), origin, IdentityZoneHolder.get().getId());
  }
  ...
  public void addExternalMappingToGroupThatAlreadyExists() {
    ...
    ScimGroupExternalMember dupMember =
        edao.mapExternalGroup(
            "g1-" + IdentityZoneHolder.get().getId(),
            "cn=engineering,ou=groups,dc=example,dc=com",
            origin,
  ...
  public void addMultipleExternalMappingsToGroup() {
    ...
    List<ScimGroupExternalMember> externalMappings =
        edao.getExternalGroupMapsByGroupId(
            "g1-" + IdentityZoneHolder.get().getId(), origin, IdentityZoneHolder.get().getId());
  ...
  public void addMultipleExternalMappingsToMultipleGroup() {
    ...
    {
      ScimGroup group =
          gdao.retrieve("g2-" + IdentityZoneHolder.get().getId(), IdentityZoneHolder.get().getId());
      assertNotNull(group);

      {
        ScimGroupExternalMember member =
            edao.mapExternalGroup(
                "g2-" + IdentityZoneHolder.get().getId(),
                "cn=engineering,ou=groups,dc=example,dc=com",
                origin,
                IdentityZoneHolder.get().getId());
      ...
      List<ScimGroupExternalMember> externalMappings =
          edao.getExternalGroupMapsByGroupId(
              "g2-" + IdentityZoneHolder.get().getId(), origin, IdentityZoneHolder.get().getId());
      assertEquals(externalMappings.size(), 3);
    }
    {
      ScimGroup group =
          gdao.retrieve("g3-" + IdentityZoneHolder.get().getId(), IdentityZoneHolder.get().getId());
      assertNotNull(group);

      {
        ScimGroupExternalMember member =
            edao.mapExternalGroup(
                "g3-" + IdentityZoneHolder.get().getId(),
                "cn=engineering,ou=groups,dc=example,dc=com",
                origin,
                IdentityZoneHolder.get().getId());
      ...
      List<ScimGroupExternalMember> externalMappings =
          edao.getExternalGroupMapsByGroupId(
              "g3-" + IdentityZoneHolder.get().getId(), origin, IdentityZoneHolder.get().getId());
      assertEquals(externalMappings.size(), 3);
    }
...

    {
      ...
      List<ScimGroupExternalMember> externalMappings =
          edao.getExternalGroupMapsByExternalGroup(
              "cn=hr,ou=groups,dc=example,dc=com", origin, IdentityZoneHolder.get().getId());
      for (ScimGroupExternalMember member : externalMappings) {
        assertEquals(member.getExternalGroup(), "cn=hr,ou=groups,dc=example,dc=com");
        assertNotNull(testGroups.remove(member.getGroupId()));
      }
      ...
      List<ScimGroupExternalMember> externalMappings2 =
          edao.getExternalGroupMapsByExternalGroup(
              "cn=HR,ou=groups,dc=example,dc=com", origin, IdentityZoneHolder.get().getId());
...

    {
      ...
      List<ScimGroupExternalMember> externalMappings =
          edao.getExternalGroupMapsByExternalGroup(
              "cn=mgmt,ou=groups,dc=example,dc=com", origin, IdentityZoneHolder.get().getId());
      for (ScimGroupExternalMember member : externalMappings) {
        assertEquals(member.getExternalGroup(), "cn=mgmt,ou=groups,dc=example,dc=com");
        assertNotNull(testGroups.remove(member.getGroupId()));
      }
