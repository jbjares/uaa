...
public class MultitenantJdbcClientDetailsServiceTests extends JdbcTestBase {
  ...
  private static final String INSERT_SQL =
      "insert into oauth_client_details (client_id, client_secret, resource_ids, scope, authorized_grant_types, web_server_redirect_uri, authorities, access_token_validity, refresh_token_validity, autoapprove, identity_zone_id, lastmodified, required_user_groups) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)";
  ...
  public void event_calls_delete_method() throws Exception {
    ...
    verify(service, times(1))
        .deleteByClient(eq(client.getClientId()), eq(IdentityZoneHolder.get().getId()));
  }
  ...
  public void delete_by_client_id_and_zone() throws Exception {
    ...
    for (IdentityZone zone : Arrays.asList(IdentityZone.getUaa(), otherIdentityZone)) {
      ...
      clients.removeIf(
          client -> {
            assertEquals(
                "We deleted exactly one row",
                1,
                service.deleteByClient(client.getClientId(), zone.getId()));
            assertEquals(
                "Our client count decreased by 1",
                (isUaa() ? 5 : 0) + (clients.size() - 1),
                countClientsInZone(zone.getId()));
            assertFalse(
                "Client " + client.getClientId() + " was deleted.",
                clientExists(client.getClientId(), zone.getId()));
            return true;
          });
  ...
  public void testLoadingClientIdWithAdditionalInformation() {
    ...
    additionalInfoMap.put(
        REQUIRED_USER_GROUPS, StringUtils.commaDelimitedListToSet(dbRequestedUserGroups));
  ...
  public void autoApproveOnlyReturnedInField_andNotInAdditionalInfo() throws Exception {
    ...
    jdbcTemplate.update(
        "update oauth_client_details set additional_information=? where client_id=?",
        "{\"autoapprove\":[\"bar.read\"]}",
        clientId);
    BaseClientDetails clientDetails = (BaseClientDetails) service.loadClientByClientId(clientId);
    ...
    jdbcTemplate.update(
        ...
        "{\"autoapprove\":true}",
        clientId);
    clientDetails = (BaseClientDetails) service.loadClientByClientId(clientId);
    assertNull(clientDetails.getAdditionalInformation().get(ClientConstants.AUTO_APPROVE));
  ...
  public void testLoadingClientIdWithSingleDetails() {
    ...
    assertEquals(
        "myAuthorizedGrantType", clientDetails.getAuthorizedGrantTypes().iterator().next());
  ...
  public void additional_information_does_not_override_user_group_column() throws Exception {
    ...
    assertEquals(
        ...
        jdbcTemplate.update(
            "UPDATE oauth_client_details SET additional_information = ? WHERE client_id = ?",
            JsonUtils.writeValueAsString(clientDetails.getAdditionalInformation()),
    ...
    assertThat(
        (Collection<String>) updateClient.getAdditionalInformation().get(REQUIRED_USER_GROUPS),
        containsInAnyOrder("group1", "group2", "group3"));
  }
  ...
  public void testLoadingClientIdWithMultipleDetails() {
    ...
    assertThat(
        (Collection<String>) requiredUserGroups, containsInAnyOrder("uaa.user", "uaa.something"));
  ...
  public void testAddClientWithSalt() throws Exception {
    ...
    assertNotNull(clientDetails);
    assertEquals(
        "newsalt", clientDetails.getAdditionalInformation().get(ClientConstants.TOKEN_SALT));
  }
  ...
  private Authentication authenticateAsUserAndReturnOldAuth(String userId) {
    Authentication authentication =
        new OAuth2Authentication(
            ...
            UaaAuthenticationTestFactory.getAuthentication(userId, "joe", "joe@test.org"));
