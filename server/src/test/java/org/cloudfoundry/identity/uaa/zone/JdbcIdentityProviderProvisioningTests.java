...
public class JdbcIdentityProviderProvisioningTests extends JdbcTestBase {
  ...
  public void testCreateAndUpdateIdentityProviderInDefaultZone() throws Exception {
    ...
    IdentityProvider<UaaIdentityProviderDefinition> idp =
        MultitenancyFixture.identityProvider(originKey, zoneId);
    ...
    idp.setType(UAA);
    IdentityProvider createdIdp = db.create(idp, zoneId);
    Map<String, Object> rawCreatedIdp =
        jdbcTemplate.queryForMap(
            "select * from identity_provider where id = ?", createdIdp.getId());
    ...
    assertEquals(
        idp.getConfig(),
        JsonUtils.readValue(
            (String) rawCreatedIdp.get("config"), UaaIdentityProviderDefinition.class));
    ...
    assertEquals(idp.getConfig(), createdIdp.getConfig());
    assertTrue(
        Math.abs(idp.getLastModified().getTime() - createdIdp.getLastModified().getTime()) < 1001);
    assertEquals(
        Integer.valueOf(rawCreatedIdp.get("version").toString()) + 1, createdIdp.getVersion());
  ...
  public void testCreateIdentityProviderInOtherZone() throws Exception {
    ...
    assertEquals(
        idp.getConfig(),
        JsonUtils.readValue(
            (String) rawCreatedIdp.get("config"), AbstractIdentityProviderDefinition.class));
  ...
  public void testUpdateIdentityProviderInDefaultZone() throws Exception {
    ...
    Map<String, Object> rawUpdatedIdp =
        jdbcTemplate.queryForMap(
            "select * from identity_provider where id = ?", updatedIdp.getId());

    assertEquals(definition, updatedIdp.getConfig());
    assertEquals(
        definition,
        JsonUtils.readValue(
            (String) rawUpdatedIdp.get("config"), LdapIdentityProviderDefinition.class));
  ...
  public void testUpdateIdentityProviderInOtherZone() throws Exception {
    ...
    assertEquals(
        definition,
        JsonUtils.readValue(
            (String) rawUpdatedIdp.get("config"), AbstractIdentityProviderDefinition.class));
  ...
  public void testRetrieveAll() throws Exception {
    ...
    IdentityZone otherZone =
        MultitenancyFixture.identityZone(UUID.randomUUID().toString(), "myzone");
    IdentityZoneHolder.set(otherZone);
    String originKey = RandomStringUtils.randomAlphabetic(6);
    IdentityProvider otherZoneIdp =
        MultitenancyFixture.identityProvider(originKey, otherZone.getId());
