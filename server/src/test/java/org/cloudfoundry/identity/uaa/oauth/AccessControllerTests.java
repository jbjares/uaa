...
public class AccessControllerTests {
  ...
  public void testSunnyDay() throws Exception {
    ...
    clientDetailsService.setClientDetailsStore(
        ...
        Collections.singletonMap("client", new BaseClientDetails()));
    controller.setClientDetailsService(clientDetailsService);
    controller.setApprovalStore(mock(ApprovalStore.class));
    Authentication auth =
        UaaAuthenticationTestFactory.getAuthentication("foo@bar.com", "Foo Bar", "foo@bar.com");
    String result =
        controller.confirm(
            new ModelMap(), new MockHttpServletRequest(), auth, new SimpleSessionStatus());
  ...
  public void testSchemePreserved() throws Exception {
    ...
    Map<String, Object> options =
        (Map<String, Object>) ((Map<String, Object>) model.get("options")).get("confirm");
  ...
  public void testClientDisplayName() throws Exception {
    ...
    clientDetailsService.setClientDetailsStore(
        IdentityZoneHolder.get().getId(), Collections.singletonMap("client-id", client));
    controller.setClientDetailsService(clientDetailsService);
  ...
  private void performAutoApprovedScopeTest(List<String> autoApprovedScopes) throws Exception {
    ...
    model.put(
        ...
        new AuthorizationRequest("client-id", Arrays.asList("resource.scope1", "resource.scope2")));

    controller.confirm(model, new MockHttpServletRequest(), auth, new SimpleSessionStatus());
    List<Map<String, String>> undecidedScopeDetails =
        (List<Map<String, String>>) model.get("undecided_scopes");
