...
public class DeprecatedUaaTokenServicesTests {
  ...
  public static Collection<Object[]> data() {
    return Arrays.asList(
        new Object[][] {{null, "old behavior"}, {new TestTokenEnhancer(), "using enhancer"}});
  }
  ...
  public void test_opaque_tokens_are_persisted() {
    ...
    IdentityZoneHolder.get()
        .getConfig()
        .getTokenPolicy()
        .setRefreshTokenFormat(JWT.getStringValue());
    CompositeToken result =
        tokenServices.persistRevocableToken(
            "id",
            persistToken,
    ...
    assertEquals(
        RevocableToken.TokenType.REFRESH_TOKEN, rt.getAllValues().get(1).getResponseType());
  ...
  public void test_refresh_tokens_are_uniquely_persisted() {
    ...
    IdentityZoneHolder.get()
        .getConfig()
        .getTokenPolicy()
        .setRefreshTokenFormat(OPAQUE.getStringValue());
    tokenServices.persistRevocableToken(
        "id",
        persistToken,
        ...
        true);
    ArgumentCaptor<RevocableToken> rt = ArgumentCaptor.forClass(RevocableToken.class);
    verify(tokenProvisioning, times(1))
        .deleteRefreshTokensForClientAndUserId(
  ...
  public void test_refresh_token_not_unique_when_set_to_false() {
    ...
    verify(tokenProvisioning, times(0))
        .deleteRefreshTokensForClientAndUserId(anyString(), anyString(), eq(currentZoneId));
    verify(tokenProvisioning, times(2)).create(rt.capture(), anyString());
    RevocableToken refreshToken = rt.getAllValues().get(1);
    assertEquals(RevocableToken.TokenType.REFRESH_TOKEN, refreshToken.getResponseType());
  }
  ...
  public void refreshAccessToken_buildsIdToken_withRolesAndAttributesAndACR() throws Exception {
    ...
    when(tokenValidationService.validateToken(anyString(), anyBoolean()))
    ...
    verify(idTokenCreator)
        .create(
            eq(TokenTestSupport.CLIENT_ID),
            eq(userId),
            userAuthenticationDataArgumentCaptor.capture());
  ...
  public void test_opaque_refresh_token_is_persisted() {
    ...
    assertEquals(
        RevocableToken.TokenType.REFRESH_TOKEN, rt.getAllValues().get(0).getResponseType());
    assertEquals(OPAQUE.getStringValue(), rt.getAllValues().get(0).getFormat());
  ...
  public void isOpaqueTokenRequired() {
    AuthorizationRequest authorizationRequest =
        new AuthorizationRequest(CLIENT_ID, tokenSupport.requestedAuthScopes);
    ...
    OAuth2Authentication authentication =
        new OAuth2Authentication(authorizationRequest.createOAuth2Request(), userAuthentication);
  ...
  public void testInvalidRefreshToken() {
    ...
    AuthorizationRequest authorizationRequest =
        new AuthorizationRequest(map, null, null, null, null, null, false, null, null, null);
    String refreshTokenValue = "dasdasdasdasdas";
    try {
      tokenServices.refreshAccessToken(
          refreshTokenValue,
          tokenSupport.requestFactory.createTokenRequest(authorizationRequest, "refresh_token"));
  ...
  public void testCreateAccessTokenForAClient() {
    ...
    AuthorizationRequest authorizationRequest =
        new AuthorizationRequest(CLIENT_ID, tokenSupport.clientScopes);
  ...
  public void testCreateAccessTokenForAnotherIssuer() throws Exception {
    ...
    identityZone.setConfig(
        JsonUtils.readValue(
            "{\"issuer\": \"http://uaamaster:8080/uaa\"}", IdentityZoneConfiguration.class));
    identityZone
        .getConfig()
        .getTokenPolicy()
        .setAccessTokenValidity(tokenSupport.accessTokenValidity);
  ...
  public void testCreateAccessTokenForInvalidIssuer() {
    ...
    try {
      identityZone.setConfig(
          JsonUtils.readValue("{\"issuer\": \"notAnURL\"}", IdentityZoneConfiguration.class));
      fail();
  ...
  public void test_refresh_token_is_opaque_when_requested() {
    ...
    assertThat(
        "Token value should be equal to or lesser than 36 characters",
    ...
    for (String s :
        Arrays.asList(refreshTokenValue, tokenSupport.tokens.get(refreshTokenValue).getValue())) {
      OAuth2AccessToken refreshedAccessToken =
          tokenServices.refreshAccessToken(s, refreshTokenRequest);
  ...
  public void test_using_opaque_parameter_on_refresh_grant() {
    ...
    for (String s :
      ...
      assertThat(
          "Token value should be equal to or lesser than 36 characters",
          refreshedAccessToken.getValue().length(),
          lessThanOrEqualTo(36));
      assertCommonUserAccessTokenProperties(
          new DefaultOAuth2AccessToken(tokenSupport.tokens.get(refreshedAccessToken).getValue()),
          CLIENT_ID);
  ...
  public void testCreateAccessTokenForAClientInAnotherIdentityZone() {
    ...
    assertThat(
        accessToken, issuerUri(is("http://" + subdomain + ".localhost:8080/uaa/oauth/token")));
    assertThat(accessToken.getRefreshToken(), is(nullValue()));
    validateExternalAttributes(accessToken);
  ...
  public void testCreateAccessTokenOnlyForClientWithoutRefreshToken() {
    AuthorizationRequest authorizationRequest =
        new AuthorizationRequest(
            CLIENT_ID_NO_REFRESH_TOKEN_GRANT, tokenSupport.requestedAuthScopes);
    authorizationRequest.setResourceIds(new HashSet<>(tokenSupport.resourceIds));
    Map<String, String> azParameters = new HashMap<>(authorizationRequest.getRequestParameters());
    azParameters.put(GRANT_TYPE, GRANT_TYPE_AUTHORIZATION_CODE);
    authorizationRequest.setRequestParameters(azParameters);
  ...
  public void testCreateAccessTokenPasswordGrant() {
    ...
    tokenServices.setExcludedClaims(
        new HashSet(
            Arrays.asList(
                ClaimConstants.AUTHORITIES, ClaimConstants.USER_NAME, ClaimConstants.EMAIL)));
  ...
  public void test_missing_required_user_groups() {
    ...
    tokenSupport.defaultClient.addAdditionalInformation(
        REQUIRED_USER_GROUPS, Arrays.asList("uaa.admin"));
    AuthorizationRequest authorizationRequest =
        new AuthorizationRequest(CLIENT_ID, tokenSupport.requestedAuthScopes);
    authorizationRequest.setResourceIds(new HashSet<>(tokenSupport.resourceIds));
    Map<String, String> azParameters = new HashMap<>(authorizationRequest.getRequestParameters());
  ...
  public void testClientSecret_Added_Token_Validation_Still_Works() {
    ...
    // add a 2nd secret
    tokenSupport.defaultClient.setClientSecret(
        tokenSupport.defaultClient.getClientSecret() + " newsecret");
    tokenServices.loadAuthentication(accessToken.getValue());
  ...
  public void testCreateAccessTokenExternalContext() {
    ...
    OAuth2AccessToken refreshedAccessToken =
        tokenServices.refreshAccessToken(
            accessToken.getRefreshToken().getValue(), refreshTokenRequest);
  ...
  @Test
  public void
      testCreateAccessTokenRefreshGrant_with_an_old_refresh_token_format_containing_scopes_claim() {
    // Given
    OAuth2AccessToken accessToken = getOAuth2AccessToken();
    String refreshTokenJwt = accessToken.getRefreshToken().getValue();

    String kid = JwtHelper.decode(refreshTokenJwt).getHeader().getKid();
    HashMap claimsWithScopeAndNotGrantedScopeMap =
        JsonUtils.readValue(JwtHelper.decode(refreshTokenJwt).getClaims(), HashMap.class);
    ...
    String refreshTokenWithOnlyScopeClaimNotGrantedScopeClaim =
        UaaTokenUtils.constructToken(
            tokenJwtHeaderMap,
            claimsWithScopeAndNotGrantedScopeMap,
            keyInfoService.getKey(kid).getSigner());

    // When
    OAuth2AccessToken refreshedAccessToken =
        tokenServices.refreshAccessToken(
            refreshTokenWithOnlyScopeClaimNotGrantedScopeClaim, getRefreshTokenRequest());
  ...
  public void createAccessToken_usingRefreshGrant_inOtherZone() {
    ...
    AuthorizationRequest refreshAuthorizationRequest =
        new AuthorizationRequest(CLIENT_ID, tokenSupport.requestedAuthScopes);
    refreshAuthorizationRequest.setResourceIds(new HashSet<>(tokenSupport.resourceIds));
    Map<String, String> refreshAzParameters =
        new HashMap<>(refreshAuthorizationRequest.getRequestParameters());
    ...
    OAuth2AccessToken refreshedAccessToken =
        tokenServices.refreshAccessToken(
            accessToken.getRefreshToken().getValue(),
            tokenSupport.requestFactory.createTokenRequest(
                refreshAuthorizationRequest, "refresh_token"));
    assertEquals(
        refreshedAccessToken.getRefreshToken().getValue(),
        accessToken.getRefreshToken().getValue());
  ...
  public void testCreateAccessTokenRefreshGrantAllScopesAutoApproved() {
    ...
    tokenSupport.clientDetailsService.setClientDetailsStore(
        IdentityZoneHolder.get().getId(), Collections.singletonMap(CLIENT_ID, clientDetails));
    ...
    this.assertCommonEventProperties(
        accessToken, tokenSupport.userId, buildJsonString(tokenSupport.requestedAuthScopes));
  ...
  public void testCreateAccessTokenRefreshGrantSomeScopesAutoApprovedDowngradedRequest() {
    ...
    AuthorizationRequest refreshAuthorizationRequest =
        new AuthorizationRequest(CLIENT_ID, tokenSupport.readScope);
    refreshAuthorizationRequest.setResourceIds(new HashSet<>(tokenSupport.resourceIds));
    Map<String, String> refreshAzParameters =
        new HashMap<>(refreshAuthorizationRequest.getRequestParameters());
    refreshAzParameters.put(GRANT_TYPE, GRANT_TYPE_REFRESH_TOKEN);
  ...
  public void create_id_token_with_profile_scope() {
    ...
    assertTrue(
        idTokenJwt
            .getClaims()
            .contains("\"given_name\":\"" + tokenSupport.defaultUser.getGivenName() + "\""));
    assertTrue(
        idTokenJwt
            .getClaims()
            .contains("\"family_name\":\"" + tokenSupport.defaultUser.getFamilyName() + "\""));
    assertTrue(
        idTokenJwt
            .getClaims()
            .contains("\"phone_number\":\"" + tokenSupport.defaultUser.getPhoneNumber() + "\""));
  }
  ...
  public void testCreateAccessWithNonExistingScopes() {
    ...
    AuthorizationRequest authorizationRequest =
        new AuthorizationRequest(CLIENT_ID, scopesThatDontExist);
    authorizationRequest.setResourceIds(new HashSet<>(tokenSupport.resourceIds));
    Map<String, String> azParameters = new HashMap<>(authorizationRequest.getRequestParameters());
    azParameters.put(GRANT_TYPE, GRANT_TYPE_IMPLICIT);
    authorizationRequest.setRequestParameters(azParameters);
    ...
    this.assertCommonEventProperties(
        accessToken, tokenSupport.userId, buildJsonString(scopesThatDontExist));
  }
  ...
  public void createAccessToken_forUser_inanotherzone() {
    ...
    this.assertCommonUserAccessTokenProperties(accessToken, CLIENT_ID);
    assertThat(
        accessToken, issuerUri(is("http://test-zone-subdomain.localhost:8080/uaa/oauth/token")));
    assertThat(accessToken, scope(is(tokenSupport.requestedAuthScopes)));
    assertThat(accessToken, validFor(is(3600)));
    assertThat(accessToken.getRefreshToken(), is(not(nullValue())));
    ...
    assertThat(
        refreshToken,
        OAuth2RefreshTokenMatchers.issuerUri(
            is("http://test-zone-subdomain.localhost:8080/uaa/oauth/token")));
  ...
  public void testCreateAccessTokenAuthcodeGrantNarrowerScopes() {
    ...
    assertThat(
        refreshToken, OAuth2RefreshTokenMatchers.scope(is(tokenSupport.requestedAuthScopes)));
    ...
    AuthorizationRequest reducedScopeAuthorizationRequest =
        new AuthorizationRequest(CLIENT_ID, tokenSupport.readScope);
    reducedScopeAuthorizationRequest.setResourceIds(new HashSet<>(tokenSupport.resourceIds));
    Map<String, String> refreshAzParameters =
        new HashMap<>(reducedScopeAuthorizationRequest.getRequestParameters());
    refreshAzParameters.put(GRANT_TYPE, GRANT_TYPE_REFRESH_TOKEN);
    reducedScopeAuthorizationRequest.setRequestParameters(refreshAzParameters);

    OAuth2Authentication reducedScopeAuthentication =
        new OAuth2Authentication(
            reducedScopeAuthorizationRequest.createOAuth2Request(), userAuthentication);
    OAuth2AccessToken reducedScopeAccessToken =
        tokenServices.refreshAccessToken(
            accessToken.getRefreshToken().getValue(),
            tokenSupport.requestFactory.createTokenRequest(
                reducedScopeAuthorizationRequest, "refresh_token"));
  ...
  public void testCreateAccessTokenAuthcodeGrantExpandedScopes() {
    ...
    assertThat(
        ...
        OAuth2RefreshTokenMatchers.scope(is(tokenSupport.requestedAuthScopes)));
    assertThat(
        accessToken.getRefreshToken(),
        OAuth2RefreshTokenMatchers.audience(is(tokenSupport.resourceIds)));

    // Second request with expanded scopes
    AuthorizationRequest expandedScopeAuthorizationRequest =
        new AuthorizationRequest(CLIENT_ID, tokenSupport.expandedScopes);
    expandedScopeAuthorizationRequest.setResourceIds(new HashSet<>(tokenSupport.resourceIds));
    Map<String, String> refreshAzParameters =
        new HashMap<>(expandedScopeAuthorizationRequest.getRequestParameters());
    refreshAzParameters.put(GRANT_TYPE, GRANT_TYPE_REFRESH_TOKEN);
    ...
    tokenServices.refreshAccessToken(
        accessToken.getRefreshToken().getValue(),
        tokenSupport.requestFactory.createTokenRequest(
            expandedScopeAuthorizationRequest, "refresh_token"));
  }
  ...
  public void testUserUpdatedAfterRefreshTokenIssued() {
    ...
    UaaUser user =
        tokenSupport.userDatabase.retrieveUserByName(tokenSupport.username, OriginKeys.UAA);
    UaaUser newUser =
        new UaaUser(
            new UaaUserPrototype()
  ...
  public void testRefreshTokenAfterApprovalsRevoked() {
    ...
    for (Approval approval :
        tokenSupport.approvalStore.getApprovals(
            tokenSupport.userId, CLIENT_ID, IdentityZoneHolder.get().getId())) {
  ...
  public void testReadAccessToken_When_Given_Refresh_token_should_throw_exception() {
    tokenServices.setExcludedClaims(
        new HashSet<>(Arrays.asList(ClaimConstants.EMAIL, ClaimConstants.USER_NAME)));
    AuthorizationRequest authorizationRequest =
        new AuthorizationRequest(CLIENT_ID, tokenSupport.requestedAuthScopes);
    authorizationRequest.setResourceIds(new HashSet<>(tokenSupport.resourceIds));
    Map<String, String> azParameters = new HashMap<>(authorizationRequest.getRequestParameters());
  ...
  public void testLoadAuthenticationForAUser() {
    ...
    OAuth2Authentication authentication =
        new OAuth2Authentication(authorizationRequest.createOAuth2Request(), userAuthentication);
    OAuth2AccessToken accessToken = tokenServices.createAccessToken(authentication);
    OAuth2Authentication loadedAuthentication =
        tokenServices.loadAuthentication(accessToken.getValue());
    ...
    UaaPrincipal uaaPrincipal =
        (UaaPrincipal) tokenSupport.defaultUserAuthentication.getPrincipal();
  ...
  public void load_Opaque_AuthenticationForAUser() {
    ...
    assertThat(
        "id_token should be JWT, thus longer than 36 characters",
        composite.getIdTokenValue().length(),
        greaterThan(36));
    assertThat(
        "Opaque access token must be shorter than 37 characters",
        accessToken.getValue().length(),
        lessThanOrEqualTo(36));
    assertThat(
        "Opaque refresh token must be shorter than 37 characters",
        accessToken.getRefreshToken().getValue().length(),
        lessThanOrEqualTo(36));

    String accessTokenValue =
        tokenProvisioning
            .retrieve(composite.getValue(), IdentityZoneHolder.get().getId())
            .getValue();
    Map<String, Object> accessTokenClaims =
        tokenSupport.tokenValidationService.validateToken(accessTokenValue, true).getClaims();
    assertTrue((Boolean) accessTokenClaims.get(ClaimConstants.REVOCABLE));

    String refreshTokenValue =
        tokenProvisioning
            .retrieve(composite.getRefreshToken().getValue(), IdentityZoneHolder.get().getId())
            .getValue();
    Map<String, Object> refreshTokenClaims =
        tokenSupport.tokenValidationService.validateToken(refreshTokenValue, false).getClaims();
    ...
    OAuth2AccessToken newAccessToken =
        tokenServices.refreshAccessToken(
            composite.getRefreshToken().getValue(),
            new TokenRequest(params, CLIENT_ID, Collections.EMPTY_SET, "refresh_token"));
    assertThat(
        "Opaque access token must be shorter than 37 characters",
        newAccessToken.getValue().length(),
        lessThanOrEqualTo(36));
    assertThat(
        "Opaque refresh token must be shorter than 37 characters",
        newAccessToken.getRefreshToken().getValue().length(),
        lessThanOrEqualTo(36));
  }
  ...
  public void loadAuthentication_when_given_an_opaque_refreshToken_should_throw_exception() {
    ...
    String refreshTokenValue =
        tokenProvisioning
            .retrieve(compositeToken.getRefreshToken().getValue(), IdentityZoneHolder.get().getId())
            .getValue();
  ...
  public void loadAuthentication_when_given_an_refresh_jwt_should_throw_exception() {
    ...
    TokenValidation refreshToken =
        tokenSupport.tokenValidationService.validateToken(
            compositeToken.getRefreshToken().getValue(), false);

    String refreshTokenValue =
        tokenProvisioning
            .retrieve(
                refreshToken.getClaims().get("jti").toString(), IdentityZoneHolder.get().getId())
            .getValue();
  ...
  public void testLoadAuthenticationWithAnExpiredToken() {
    ...
    tokenSupport.clientDetailsService.setClientDetailsStore(
        IdentityZoneHolder.get().getId(), Collections.singletonMap(CLIENT_ID, shortExpiryClient));
  ...
  public void testCreateAccessTokenAuthcodeGrantAdditionalAuthorizationAttributes() {
    ...
    this.assertCommonEventProperties(
        token, tokenSupport.userId, buildJsonString(tokenSupport.requestedAuthScopes));
  ...
  public void createRefreshToken_JwtDoesNotContainScopeClaim() {
    AuthorizationRequest authorizationRequest =
        new AuthorizationRequest(CLIENT_ID, tokenSupport.requestedAuthScopes);
    Map<String, String> authzParameters =
    ...
    assertThat(
        refreshTokenClaims.getGrantedScopes(),
        containsInAnyOrder(accessToken.getScope().toArray()));
  }
  ...
  private void readAccessToken(Set<String> excludedClaims) {
    ...
    try {
      ...
      assertThat(
          "Exception should be about approvals",
  ...
  private void validateExternalAttributes(OAuth2AccessToken accessToken) {
    Map<String, String> extendedAttributes =
        (Map<String, String>)
            accessToken.getAdditionalInformation().get(ClaimConstants.EXTERNAL_ATTR);
    if (tokenEnhancer != null) {
      String atValue =
          accessToken.getValue().length() < 40
              ? tokenSupport.tokens.get(accessToken.getValue()).getValue()
              : accessToken.getValue();
      Map<String, Object> claims =
          JsonUtils.readValue(
              JwtHelper.decode(atValue).getClaims(), new TypeReference<Map<String, Object>>() {});
  ...
  private TokenRequest getRefreshTokenRequest(Map<String, String> requestParameters) {
    ...
    refreshAzParameters.put(GRANT_TYPE, GRANT_TYPE_REFRESH_TOKEN);
    refreshAuthorizationRequest.setRequestParameters(refreshAzParameters);
    return tokenSupport.requestFactory.createTokenRequest(
        refreshAuthorizationRequest, "refresh_token");
  }
  ...
  private void assertCommonUserAccessTokenProperties(
      OAuth2AccessToken accessToken, String clientId) {
    assertThat(
        accessToken,
        allOf(
            username(is(tokenSupport.username)),
  ...
  private void assertCommonEventProperties(
      OAuth2AccessToken accessToken, String expectedPrincipalId, String expectedData) {
    Assert.assertEquals(1, tokenSupport.publisher.getEventCount());
