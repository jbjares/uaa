...
public class EncryptionKeyServiceTest {
  ...
  public void shouldFetchValidEncryptionActiveKeyPassphrase() throws EncryptionServiceException {
    ...
    assertThat(
        new String(encryptionKeyService.getActiveKey().decrypt(cipherValue)), is("plain-text"));
  }
  ...
  public void shouldThrowErrorIfPassphraseIsLessThan8Characters() {
    expectedException.expect(NoActiveEncryptionKeyProvided.class);
    expectedException.expectMessage(
        "The required length of the encryption passphrases for [label=key-1, label=key-2] need to be at least 8 characters long.");
  ...
  public void shouldThrowErrorIfDuplicateKeysAreProvided() {
    expectedException.expect(NoActiveEncryptionKeyProvided.class);
    expectedException.expectMessage(
        "UAA cannot be started as multiple keys have the same label in uaa.encryption.encryption_keys/[label=key-1]");

    EncryptionKeyService.EncryptionKey key1 =
        new EncryptionKeyService.EncryptionKey() {
          {
            put("label", "key-1");
  ...
  public void eachEncryptionKeyShouldHaveANonEmptyPassphrase() {
    ...
    expectedException.expectMessage(
        String.format(
            "UAA cannot be started as encryption key passphrase for uaa.encryption.encryption_keys/[label=%s, label=%s] is undefined",
            key2, key3));

    EncryptionKeyService.EncryptionKey activeEncryptionKey =
        new EncryptionKeyService.EncryptionKey();
    ...
    encryptionKeyService =
        new EncryptionKeyService(
            "key1", Lists.newArrayList(activeEncryptionKey, encryptionKey2, encryptionKey3));
  }
  ...
  public void shouldThrowAMeaningfulErrorWhenUnableToFindAKey() {
    Optional<EncryptionKeyService.EncryptionKey> missingKey =
        encryptionKeyService.getKey("key-that-does-not-exist");
