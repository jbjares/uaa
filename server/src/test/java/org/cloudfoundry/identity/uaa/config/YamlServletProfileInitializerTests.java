...
public class YamlServletProfileInitializerTests {
  ...
  private ConfigurableWebApplicationContext context =
      Mockito.mock(ConfigurableWebApplicationContext.class);
  ...
  public void testLoadDefaultResource() throws Exception {
    ...
    Mockito.when(context.getResource(ArgumentMatchers.contains("${APPLICATION_CONFIG_URL}")))
  ...
  public void testLoadSessionEventPublisher() throws Exception {
    ...
    ArgumentCaptor<HttpSessionEventPublisher> httpSessionEventPublisherArgumentCaptor =
        ArgumentCaptor.forClass(HttpSessionEventPublisher.class);
    verify(servletContext, atLeastOnce())
        .addListener(httpSessionEventPublisherArgumentCaptor.capture());
  ...
  public void testActiveProfiles() throws Exception {
    ...
    Mockito.when(context.getResource(ArgumentMatchers.anyString()))
  ...
  public void testLoadServletConfiguredFilename() throws Exception {
    ...
    Mockito.when(servletConfig.getInitParameter("APPLICATION_CONFIG_FILE"))
        .thenReturn("/config/path/foo.yml");
    Mockito.when(context.getResource(ArgumentMatchers.eq("file:/config/path/foo.yml")))
        .thenReturn(new ByteArrayResource("foo: bar\nspam:\n  foo: baz".getBytes()));
  ...
  public void testLoadServletConfiguredResource() throws Exception {
    ...
    Mockito.when(servletConfig.getInitParameter("environmentConfigLocations"))
        .thenReturn("foo.yml");
    Mockito.when(context.getResource(ArgumentMatchers.eq("foo.yml")))
        .thenReturn(new ByteArrayResource("foo: bar\nspam:\n  foo: baz".getBytes()));
  ...
  public void testLoadContextConfiguredResource() throws Exception {
    ...
    Mockito.when(servletContext.getInitParameter("environmentConfigLocations"))
        .thenReturn("foo.yml");
    Mockito.when(context.getResource(ArgumentMatchers.eq("foo.yml")))
        .thenReturn(new ByteArrayResource("foo: bar\nspam:\n  foo: baz".getBytes()));
  ...
  public void testLoadReplacedResource() throws Exception {
    ...
    Mockito.when(context.getResource(ArgumentMatchers.eq("file:foo/uaa.yml")))
        .thenReturn(new ByteArrayResource("foo: bar\nspam:\n  foo: baz".getBytes()));
  ...
  public void testIgnoreDashDTomcatLoggingConfigVariable() throws Exception {
    ...
    initializer.initialize(context);
    assertEquals(
        "-Djava.util.logging.config=/some/path/logging.properties",
        environment.getProperty("logging.config"));
    ...
    for (String s : servletLogCaptor.getAllValues()) {
      if (s.startsWith("Ignoring Log Config Location")
          && s.contains("Tomcat startup script environment variable")) {
        logEntryFound = true;
      }
    }
    assertTrue(
        "Expected to find a log entry indicating that the LOGGING_CONFIG variable was found.",
        logEntryFound);
  }
