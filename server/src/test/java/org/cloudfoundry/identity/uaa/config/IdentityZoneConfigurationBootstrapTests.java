...
public class IdentityZoneConfigurationBootstrapTests extends JdbcTestBase {
  ...
  public void configureProvisioning() {
    ...
    MfaProviderProvisioning mfaProvisoning =
        new JdbcMfaProviderProvisioning(jdbcTemplate, mfaProviderValidator);
    ...
    GeneralIdentityZoneConfigurationValidator configValidator =
        new GeneralIdentityZoneConfigurationValidator();
  ...
  public void test_multiple_keys() throws InvalidIdentityZoneDetailsException {
    ...
    assertEquals(
        SamlTestUtils.PROVIDER_PRIVATE_KEY_PASSWORD, config.getKeys().get("key1").getPassphrase());
  ...
  public void testDefaultSamlKeys() throws Exception {
    ...
    IdentityZone uaa = provisioning.retrieve(IdentityZone.getUaa().getId());
    assertEquals(
        SamlTestUtils.PROVIDER_PRIVATE_KEY, uaa.getConfig().getSamlConfig().getPrivateKey());
    assertEquals(
        SamlTestUtils.PROVIDER_PRIVATE_KEY_PASSWORD,
        uaa.getConfig().getSamlConfig().getPrivateKeyPassword());
    assertEquals(
        SamlTestUtils.PROVIDER_CERTIFICATE, uaa.getConfig().getSamlConfig().getCertificate());
  }
  ...
  public void set_home_redirect() throws Exception {
    ...
    IdentityZone zone = provisioning.retrieve(IdentityZone.getUaa().getId());
    assertEquals(
        "http://some.redirect.com/redirect", zone.getConfig().getLinks().getHomeRedirect());
  }
  ...
  public void test_logout_redirect() throws Exception {
    ...
    IdentityZoneConfiguration config =
        provisioning.retrieve(IdentityZone.getUaa().getId()).getConfig();
  ...
  public void test_prompts() throws Exception {
    List<Prompt> prompts =
        Arrays.asList(new Prompt("name1", "type1", "text1"), new Prompt("name2", "type2", "text2"));
