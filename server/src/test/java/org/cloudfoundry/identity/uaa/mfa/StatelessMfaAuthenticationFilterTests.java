...
public class StatelessMfaAuthenticationFilterTests {
  ...
  public void setup() throws Exception {
    ...
    UaaPrincipal uaaPrincipal =
        new UaaPrincipal("1", "marissa", "marissa@test.org", OriginKeys.UAA, null, zone.getId());
    uaaAuthentication =
        new UaaAuthentication(
            uaaPrincipal, Collections.emptyList(), mock(UaaAuthenticationDetails.class));
    ...
    when(googleAuthenticator.getUserGoogleMfaCredentials(anyString(), anyString()))
        .thenReturn(mock(UserGoogleMfaCredentials.class));
    grantTypes = new HashSet<>(Arrays.asList("password"));

    mfaProvider = mock(MfaProviderProvisioning.class);
    when(mfaProvider.retrieveByName(anyString(), anyString()))
        .thenReturn(
            new MfaProvider()
                .setName("mfa-provider-name")
                .setId("mfa-provider-id")
    ...
    commonLoginPolicy =
        new CommonLoginPolicy(
            jdbcAuditServiceMock,
            lockoutPolicyRetriever,
            AuditEventType.MfaAuthenticationSuccess,
            AuditEventType.MfaAuthenticationFailure,
            timeService,
            mfaLockoutPolicyEnabled);

    filter =
        new StatelessMfaAuthenticationFilter(
            googleAuthenticator, grantTypes, mfaProvider, userDatabase, commonLoginPolicy);
  ...
  public void mfa_validation_works() throws Exception {
    ...
    verify(chain).doFilter(same(request), same(response));
    assertThat(
        uaaAuthentication.getAuthenticationMethods(), containsInAnyOrder("pwd", "otp", "mfa"));
  ...
  public void mfa_code_missing_returns_json_error() throws Exception {
    ...
    JsonAssert.with(response.getContentAsString())
        ...
        .assertThat(
            "error_description",
            equalTo("A multi-factor authentication code is required to complete the request"));
  ...
  public void user_config_is_returning_error() throws Exception {
    ...
    assertThat(response.getStatus(), equalTo(400));
    assertThat(
        response.getHeader(HttpHeaders.CONTENT_TYPE), equalTo(MediaType.APPLICATION_JSON_VALUE));
    assertNotNull(response.getContentAsString());
    JsonAssert.with(response.getContentAsString())
        .assertThat("error", equalTo("invalid_request"))
        .assertThat(
            "error_description", equalTo("User must register a multi-factor authentication token"));
    verify(publisher, times(1)).publishEvent(any(MfaAuthenticationFailureEvent.class));
    verify(publisher, times(1)).publishEvent(any(ApplicationEvent.class));
  }
  ...
  public void when_valid_mfa_auth_code_given_but_mfa_is_locked_out_should_fail() {
    ...
    MfaAuthenticationFailureEvent event =
        new MfaAuthenticationFailureEvent(user, authentication, GOOGLE_AUTHENTICATOR.toValue());
    when(jdbcAuditServiceMock.find(user.getId(), fixedTime, zone.getId()))
        .thenReturn(Lists.newArrayList(event.getAuditEvent()));
  ...
  @Test
  public void
      when_valid_mfa_auth_code_given_with_previously_failed_mfa_auth_attempts_but_not_locked_out_should_pass() {
    long fixedTime = 1L;
    when(timeService.getCurrentTimeMillis()).thenReturn(fixedTime);
  ...
  @Test
  public void
      when_valid_mfa_auth_code_given_with_previously_failed_mfa_auth_attempts_interleaved_with_successful_mfa_auth_event_but_not_locked_out_should_pass() {
    long fixedTime = 1L;
    when(timeService.getCurrentTimeMillis()).thenReturn(fixedTime);

    AuditEvent failedMfaEvent =
        new MfaAuthenticationFailureEvent(user, authentication, GOOGLE_AUTHENTICATOR.toValue())
            .getAuditEvent();
    AuditEvent successfulMfaEvent =
        new MfaAuthenticationSuccessEvent(user, authentication, GOOGLE_AUTHENTICATOR.toValue())
            .getAuditEvent();
    ArrayList<AuditEvent> events =
        Lists.newArrayList(failedMfaEvent, failedMfaEvent, successfulMfaEvent, failedMfaEvent);
