...
public class ScimUserBootstrapTests extends JdbcTestBase {
  ...
  public static void addIdentityProvider(JdbcTemplate jdbcTemplate, String originKey) {
    jdbcTemplate.update(
        "insert into identity_provider (id,identity_zone_id,name,origin_key,type) values (?,'uaa',?,?,'UNKNOWN')",
  ...
  public void can_delete_users_but_only_in_default_zone() throws Exception {
    ...
    canAddUsers(
        OriginKeys.UAA,
        otherZone.getId()); // this is just an update of the same two users, zoneId is ignored
    ...
    doAnswer(
        ...
        .when(publisher)
        .publishEvent(any(EntityDeletedEvent.class));
    ...
    List<EntityDeletedEvent<ScimUser>> deleted =
        new LinkedList(ofNullable(captor.getAllValues()).orElse(emptyList()));
  ...
  public void canAddUsers(String origin, String zoneId) throws Exception {
    ...
    UaaUser mabel =
        new UaaUser("mabel", "password", "mabel@blah.com", "Mabel", "User", origin, zoneId);
    ScimUserBootstrap bootstrap = new ScimUserBootstrap(db, gdb, mdb, Arrays.asList(joe, mabel));
    bootstrap.afterPropertiesSet();
  }
  ...
  public void canUpdateUsers() throws Exception {
    ...
    String passwordHash =
        jdbcTemplate.queryForObject(
            "select password from users where username='joe'", new Object[0], String.class);
  ...
  public void uaaUserGetsVerifiedSetToTrue() throws Exception {
    ...
    ScimUser existingUser =
        db.retrieveAll(IdentityZone.getUaa().getId())
            .stream()
  ...
  public void nonExistentGroupThroughEvent(boolean add) throws Exception {
    ...
    List<ScimUser> users =
        db.query(
            "userName eq \"" + username + "\" and origin eq \"" + origin + "\"",
    ...
    bootstrap.onApplicationEvent(
        new ExternalGroupAuthorizationEvent(user, false, getAuthorities(externalAuthorities), add));
    ...
    validateAuthoritiesCreated(
        add ? externalAuthorities : new String[0], userAuthorities, origin, created);
  ...
  protected void validateAuthoritiesCreated(
      String[] externalAuthorities, String[] userAuthorities, String origin, ScimUser created) {
    Set<ScimGroup> groups =
        mdb.getGroupsWithMember(created.getId(), true, IdentityZoneHolder.get().getId());
    ...
    for (ScimGroup g : groups) {
      ScimGroupMember m =
          mdb.getMemberById(g.getId(), created.getId(), IdentityZoneHolder.get().getId());
      if (external.contains(g.getDisplayName())) {
        assertEquals(
            "Expecting relationship for Group[" + g.getDisplayName() + "] be of different origin.",
            origin,
            m.getOrigin());
      } else {
        assertEquals(
  ...
  public void canUpdateEmailThroughEvent() throws Exception {
    ...
    bootstrap.onApplicationEvent(
        new ExternalGroupAuthorizationEvent(user, true, getAuthorities(externalAuthorities), true));
    users =
        db.query(
            "userName eq \"" + username + "\" and origin eq \"" + origin + "\"",
            IdentityZoneHolder.get().getId());
    ...
    bootstrap.onApplicationEvent(
        new ExternalGroupAuthorizationEvent(
            user, false, getAuthorities(externalAuthorities), true));
    users =
        db.query(
            "userName eq \"" + username + "\" and origin eq \"" + origin + "\"",
            IdentityZoneHolder.get().getId());
  ...
  public void addUsersWithSameUsername() throws Exception {
    ...
    bootstrap =
        new ScimUserBootstrap(
            db, gdb, mdb, Arrays.asList(user, user.modifySource("newOrigin", "")));
    bootstrap.afterPropertiesSet();
    assertEquals(2, db.retrieveAll(IdentityZoneHolder.get().getId()).size());
  }
  ...
  public void testConcurrentAuthEventsRaceCondition() throws Exception {
    ...
    for (int i = 0; i < numgroups; i++) {
      gdb.create(
          new ScimGroup("group" + i, "group" + i, IdentityZoneHolder.get().getId()),
          IdentityZoneHolder.get().getId());
      String gid =
          gdb.query("displayName eq \"group" + i + "\"", IdentityZoneHolder.get().getId())
              .get(0)
              .getId();
    ...
    ExternalGroupAuthorizationEvent externalGroupAuthorizationEvent =
        new ExternalGroupAuthorizationEvent(user, false, getAuthorities(externalAuthorities), true);
