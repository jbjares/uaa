...
public class ChainedSignatureVerifierTests {
  ...
  public void setup() {
    ...
    validKey =
        new JsonWebKey(
            KeyInfoBuilder.build(null, rsaSigningKey, "http://localhost/uaa").getJwkMap());
    invalidKey =
        new JsonWebKey(
            KeyInfoBuilder.build(null, invalidRsaSigningKey, "http://localhost/uaa").getJwkMap());
  }
  ...
  public void test_multi_key_first_valid() {
    verifier =
        new ChainedSignatureVerifier(new JsonWebKeySet<>(Arrays.asList(validKey, invalidKey)));
    signedValidContent.verifySignature(verifier);
  }
  ...
  public void test_multi_key_last_valid() {
    verifier =
        new ChainedSignatureVerifier(new JsonWebKeySet<>(Arrays.asList(invalidKey, validKey)));
    signedValidContent.verifySignature(verifier);
  }
  ...
  public void test_multi_key_invalid() {
    verifier =
        new ChainedSignatureVerifier(new JsonWebKeySet<>(Arrays.asList(invalidKey, invalidKey)));
    signedValidContent.verifySignature(verifier);
  }
  ...
  public void check_that_we_use_common_signer() {
    ...
    verifier =
        new ChainedSignatureVerifier(
            new JsonWebKeySet<>(Arrays.asList(validKey, invalidKey, macKey)));
    List<SignatureVerifier> delegates =
        new ArrayList(
            (List<SignatureVerifier>)
                ReflectionTestUtils.getField(verifier, verifier.getClass(), "delegates"));
  ...
  public void unsupported_key_types_are_ignored() {
    ...
    JsonWebKeySet keySet =
        JsonUtils.convertValue(
            singletonMap("keys", Arrays.asList(validKey, p, q)), JsonWebKeySet.class);
  ...
  public void no_supported_key_types_causes_error() {
    ...
    q.put("k", "octkeyvalue");
    JsonWebKeySet keySet =
        JsonUtils.convertValue(singletonMap("keys", Arrays.asList(p, q)), JsonWebKeySet.class);
    verifier = new ChainedSignatureVerifier(keySet);
  }
  ...
  public void test_multi_key_both_valid() {
    ...
    verifier =
        new ChainedSignatureVerifier(new JsonWebKeySet<>(Arrays.asList(validKey, jsonWebKey)));
    signedValidContent.verifySignature(verifier);
    List<SignatureVerifier> delegates =
        new ArrayList(
            (List<SignatureVerifier>)
