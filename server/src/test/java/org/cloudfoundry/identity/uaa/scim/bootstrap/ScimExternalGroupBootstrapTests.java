...
public class ScimExternalGroupBootstrapTests extends JdbcTestBase {
  ...
  public void initScimExternalGroupBootstrapTests() {
    ...
    gDB.create(
        new ScimGroup(null, "acme", IdentityZone.getUaa().getId()),
        IdentityZoneHolder.get().getId());
    gDB.create(
        new ScimGroup(null, "acme.dev", IdentityZone.getUaa().getId()),
        IdentityZoneHolder.get().getId());
  ...
  public void canAddExternalGroups() throws Exception {
    ...
    externalGroupMap.put(
        "cn=Engineering Department,ou=groups,dc=example,dc=com", Arrays.asList("acme", "acme.dev"));
    ...
    assertEquals(
        ...
        eDB.getExternalGroupMapsByExternalGroup(
                "cn=Engineering Department,ou=groups,dc=example,dc=com",
                OriginKeys.LDAP,
                IdentityZoneHolder.get().getId())
            .size());
    assertEquals(
    ...
    assertEquals(
        3,
        eDB.getExternalGroupMapsByGroupName(
                "acme", OriginKeys.LDAP, IdentityZoneHolder.get().getId())
            .size());
    assertEquals(
        1,
        eDB.getExternalGroupMapsByGroupName(
                "acme.dev", OriginKeys.LDAP, IdentityZoneHolder.get().getId())
            .size());
  }
  ...
  public void cannotAddExternalGroupsThatDoNotExist() throws Exception {
    ...
    assertNull(
        eDB.getExternalGroupMapsByGroupName(
            "acme1", OriginKeys.LDAP, IdentityZoneHolder.get().getId()));
    assertNull(
        eDB.getExternalGroupMapsByGroupName(
            "acme1.dev", OriginKeys.LDAP, IdentityZoneHolder.get().getId()));
  }
