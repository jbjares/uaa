...
public class ScimGroupEndpointsTests extends JdbcTestBase {
  ...
  public void initScimGroupEndpointsTests() throws Exception {
    ...
    externalToInternalMap.put(
        "cn=test_org,ou=people,o=springsource,o=org",
        Collections.singletonList("organizations.acme"));
    externalToInternalMap.put(
        "cn=developers,ou=scopes,dc=test,dc=com", Collections.singletonList("internal.read"));
    externalToInternalMap.put(
        "cn=operators,ou=scopes,dc=test,dc=com", Collections.singletonList("internal.write"));
    externalToInternalMap.put(
        "cn=superusers,ou=scopes,dc=test,dc=com",
  ...
  private ScimGroupMember createMember(ScimGroupMember.Type t) {
    ...
    if (t == ScimGroupMember.Type.USER) {
      ...
      id =
          dao.create(
                  new ScimGroup(null, id, IdentityZoneHolder.get().getId()),
                  IdentityZoneHolder.get().getId())
              .getId();
      groupIds.add(id);
    }
  ...
  private void deleteGroup(String name) {
    for (ScimGroup g :
        dao.query("displayName eq \"" + name + "\"", IdentityZoneHolder.get().getId())) {
  ...
  public void testListGroups() throws Exception {
    validateSearchResults(
        endpoints.listGroups("id,displayName", "id pr", "created", "ascending", 1, 100), 11);
  }
  ...
  public void testListGroupsWithAttributesWithMembersDoesQueryMembers() throws Exception {
    ScimGroupMembershipManager memberManager = mock(ScimGroupMembershipManager.class);
    when(memberManager.getMembers(anyString(), eq(false), eq("uaa")))
        .thenReturn(Collections.emptyList());
    endpoints = new ScimGroupEndpoints(dao, memberManager);
    endpoints.setExternalMembershipManager(em);
    endpoints.setGroupMaxCount(20);
    validateSearchResults(
        endpoints.listGroups("id,displayName,members", "id pr", "created", "ascending", 1, 100),
        11);
  ...
  public void testListGroups_Without_Description() throws Exception {
    validateSearchResults(
        endpoints.listGroups("id,displayName,description", "id pr", "created", "ascending", 1, 100),
        11);
    validateSearchResults(
        endpoints.listGroups(
            "id,displayName,meta.lastModified", "id pr", "created", "ascending", 1, 100),
        11);
    validateSearchResults(
  ...
  public void testListExternalGroups() throws Exception {
    ...
    validateSearchResults(
        endpoints.getExternalGroups(1, 100, "", "", "cn=superusers,ou=scopes,dc=test,dc=com"), 4);
    validateSearchResults(
        endpoints.getExternalGroups(
            1, 100, "", OriginKeys.LDAP, "cn=superusers,ou=scopes,dc=test,dc=com"),
        2);
    validateSearchResults(
        endpoints.getExternalGroups(
            1, 100, "", "you-wont-find-me", "cn=superusers,ou=scopes,dc=test,dc=com"),
        0);

    validateSearchResults(
        endpoints.getExternalGroups(
            1, 100, "externalGroup eq \"cn=superusers,ou=scopes,dc=test,dc=com\"", "", ""),
        4);
    validateSearchResults(
        endpoints.getExternalGroups(1, 100, "origin eq \"" + OriginKeys.LDAP + "\"", "", ""), 5);
    validateSearchResults(
        endpoints.getExternalGroups(
            1,
            100,
            "externalGroup eq \"cn=superusers,ou=scopes,dc=test,dc=com\" and "
                + "origin eq \""
                + OriginKeys.LDAP
                + "\"",
            "",
  ...
  public void testListExternalGroupsInvalidFilter() throws Exception {
    for (String filter : Arrays.asList("dasda dasdas dasdas", "displayName eq \"test\""))
      try {
  ...
  public void unmapExternalGroupUsingName_truncatesLeadingAndTrailingSpaces_InExternalGroupName()
    ...
    ScimGroupExternalMember member = getScimGroupExternalMember();
    member =
        endpoints.unmapExternalGroupUsingName(member.getDisplayName(), "  \nexternal_group_id\n");
    assertEquals("external_group_id", member.getExternalGroup());
  }

  private ScimGroupExternalMember getScimGroupExternalMember() {
    ScimGroupExternalMember member =
        new ScimGroupExternalMember(groupIds.get(0), "  external_group_id  ");
  ...
  public void testListGroupsWithNameEqFilter() {
    validateSearchResults(
        endpoints.listGroups(
            "id,displayName", "displayName eq \"uaa.user\"", "created", "ascending", 1, 100),
        1);
  }
  ...
  public void testListGroupsWithNameCoFilter() {
    validateSearchResults(
        endpoints.listGroups(
            "id,displayName", "displayName co \"admin\"", "created", "ascending", 1, 100),
        1);
  }
  ...
  public void testListGroupsWithInvalidFilterFails() {
    ...
    endpoints.listGroups(
        "id,displayName", "displayName cr \"admin\"", "created", "ascending", 1, 100);
  }
  ...
  public void testListGroupsWithInvalidAttributes() {
    validateSearchResults(
        endpoints.listGroups(
            "id,displayNameee", "displayName co \"admin\"", "created", "ascending", 1, 100),
        1);
  }
  ...
  public void testUpdateWrongVersionFails() {
    ...
    try {
      endpoints.updateGroup(
          g1, g1.getId(), String.valueOf(g1.getVersion() + 23), new MockHttpServletResponse());
    } catch (ScimException ex) {
      validateSearchResults(
          endpoints.listGroups("id", "displayName eq \"clients.write\"", "id", "ASC", 1, 100), 0);
      validateSearchResults(
  ...
  public void testDeleteGroupRemovesMembershipsInZone() throws Exception {
    ...
    List<ScimGroupMember> members =
        endpoints.listGroupMemberships(group.getId(), true, "").getBody();
  ...
  public void testDeleteWrongVersionFails() {
    ...
    try {
      endpoints.deleteGroup(
          g.getId(), String.valueOf(g.getVersion() + 3), new MockHttpServletResponse());
    } catch (ScimException ex) {
      validateSearchResults(
          endpoints.listGroups("id", "displayName eq \"clients.read\"", "id", "ASC", 1, 100), 1);
    }
  ...
  public void testExceptionHandler() {
    ...
    validateView(
        endpoints.handleException(new ScimResourceNotFoundException(""), request),
        HttpStatus.NOT_FOUND);
    validateView(
        endpoints.handleException(new UnsupportedOperationException(""), request),
        HttpStatus.BAD_REQUEST);
    validateView(
        endpoints.handleException(new BadSqlGrammarException("", "", null), request),
        HttpStatus.BAD_REQUEST);
    validateView(
        endpoints.handleException(new IllegalArgumentException(""), request),
        HttpStatus.BAD_REQUEST);
    validateView(
        endpoints.handleException(new DataIntegrityViolationException(""), request),
        HttpStatus.BAD_REQUEST);
  }
  ...
  public void testPatch() {
    ...
    patch =
        endpoints.patchGroup(
            patch, patch.getId(), Integer.toString(g1.getVersion()), new MockHttpServletResponse());
  ...
  public void testPatchIncorrectEtagFails() {
    ...
    patch =
        endpoints.patchGroup(
            patch,
            patch.getId(),
            Integer.toString(g1.getVersion() + 1),
            new MockHttpServletResponse());
  }
}
