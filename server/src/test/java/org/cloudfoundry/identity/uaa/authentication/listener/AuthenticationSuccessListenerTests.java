...
public class AuthenticationSuccessListenerTests {
  ...
  public void setUp() {
    ...
    id = "user-id";
    userPrototype =
        new UaaUserPrototype().withId(id).withUsername("testUser").withEmail("test@email.com");
  ...
  public void unverifiedUserBecomesVerifiedIfTheyHaveLegacyFlag() {
    userPrototype.withVerified(false).withLegacyVerificationBehavior(true);
  ...
  public UserAuthenticationSuccessEvent getEvent() {
    user = new UaaUser(userPrototype);
    return new UserAuthenticationSuccessEvent(user, mockAuth);
  }
  ...
  public void userLastUpdatedGetsCalledOnEvent() {
    ...
    UserAuthenticationSuccessEvent event = getEvent();
    when(scimUserProvisioning.retrieve(id, IdentityZoneHolder.get().getId()))
        .thenReturn(getScimUser(event.getUser()));
    listener.onApplicationEvent(event);
    verify(scimUserProvisioning, times(1))
        .updateLastLogonTime(id, IdentityZoneHolder.get().getId());
  }
  ...
  @Test
  public void provider_authentication_success_triggers_user_authentication_success()
      throws Exception {
    when(checker.isMfaEnabled(any(), any())).thenReturn(false);
    IdentityProviderAuthenticationSuccessEvent event =
        new IdentityProviderAuthenticationSuccessEvent(user, mockAuth, OriginKeys.UAA);
    listener.onApplicationEvent(event);
    verify(publisher, times(1)).publishEvent(isA(UserAuthenticationSuccessEvent.class));
  }

  @Test
  public void provider_authentication_success_does_not_trigger_user_authentication_success()
      throws Exception {
  ...
  public void mfa_authentication_success_triggers_user_authentication_success() throws Exception {
    when(checker.isMfaEnabled(any(), any())).thenReturn(true);
    MfaAuthenticationSuccessEvent event =
        new MfaAuthenticationSuccessEvent(user, mockAuth, "mfa-type");
    listener.onApplicationEvent(event);
    verify(publisher, times(1)).publishEvent(isA(UserAuthenticationSuccessEvent.class));
  }
}
