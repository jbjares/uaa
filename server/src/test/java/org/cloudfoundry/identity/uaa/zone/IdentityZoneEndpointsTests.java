...
public class IdentityZoneEndpointsTests {
  ...
  public void groups_are_created() throws Exception {
    ...
    verify(groupProvisioning, times(defaultGroups.size()))
        .createOrGet(captor.capture(), eq(zone.getId()));
  ...
  private IdentityZone createZone() {
    ...
    zone.getConfig()
        .getSamlConfig()
        .addAndActivateKey("active", new SamlKey("private1", "passphrase1", "certificate1"));
