...
public class MfaProviderEndpointsTest {
  ...
  public void testCreateDefaultIssuer() {
    ...
    assertEquals(
        IdentityZoneHolder.get().getName(),
        mfaProviderResponseEntity.getBody().getConfig().getIssuer());
  }
  ...
  public void testGetMfaProviders() {
    ...
    when(provisioning.retrieveAll(anyString()))
        .thenReturn(Collections.singletonList(mockProviderResponse));
  ...
  public void testDeleteMFaProvider() {
    ...
    ArgumentCaptor<EntityDeletedEvent> entityDeletedCaptor =
    ...
    assertEquals(
        providerToDelete.getId(),
        ((MfaProvider) (entityDeletedCaptor.getAllValues().get(0)).getDeleted()).getId());
  }
  ...
  public void testDeleteActiveProviderThrowsException() {
    ...
    IdentityZoneHolder.get()
        ...
        .getMfaConfig()
        .setEnabled(true)
        .setProviderName(providerToDelete.getName());
