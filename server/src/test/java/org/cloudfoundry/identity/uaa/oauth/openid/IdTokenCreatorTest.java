...
public class IdTokenCreatorTest {
  ...
  public void setup() throws Exception {
    ...
    when(UaaTokenUtils.getRevocableTokenSignature(user, tokensalt, clientId, clientsecret))
        .thenReturn("Signature");

    PowerMockito.mockStatic(IdentityZoneHolder.class);
    when(IdentityZoneHolder.get())
        .thenReturn(
            new IdentityZone() {
              {
                setId(zoneId);
    ...
    userAuthenticationData =
        new UserAuthenticationData(
            authTime, amr, acr, scopes, roles, userAttributes, nonce, grantType, jti);
  ...
  public void create_setsRolesToNullIfRolesAreNull() throws IdTokenCreationException {
    userAuthenticationData =
        new UserAuthenticationData(
            authTime, amr, acr, scopes, null, userAttributes, nonce, grantType, jti);
  ...
  public void create_doesntSetUserAttributesIfTheyAreNull() throws IdTokenCreationException {
    userAuthenticationData =
        new UserAuthenticationData(authTime, amr, acr, scopes, roles, null, nonce, grantType, jti);
  ...
  public void whenUserIdNotFound_throwsException() throws Exception {
    when(uaaUserDatabase.retrieveUserById("missing-user"))
  ...
  public void idToken_containsZonifiedIssuerUrl() throws Exception {
    when(UaaTokenUtils.constructTokenEndpointUrl(uaaUrl))
        .thenReturn("http://myzone.localhost:8080/uaa/oauth/token");
