...
public class AuthzAuthenticationManagerTests {
  ...
  private static final String PASSWORD =
      "$2a$10$HoWPAUn9zqmmb0b.2TBZWe6cjQcxyo8TDwTX.5G46PBL347N3/0zO"; // "password"
  ...
  public void successfulAuthentication() {
    ...
    assertEquals(
        "auser",
        ((IdentityProviderAuthenticationSuccessEvent) events.get(0)).getUser().getUsername());
  }
  ...
  public void unsuccessfulPasswordExpired() {
    ...
    UaaIdentityProviderDefinition idpDefinition =
        new UaaIdentityProviderDefinition(new PasswordPolicy(6, 128, 1, 1, 1, 1, 6), null);
    provider.setConfig(idpDefinition);

    when(providerProvisioning.retrieveByOriginIgnoreActiveFlag(eq(OriginKeys.UAA), anyString()))
        .thenReturn(provider);
  ...
  public void missingUserPublishesNotFoundEvent() {
    when(db.retrieveUserByName(eq("aguess"), eq(OriginKeys.UAA)))
        .thenThrow(new UsernameNotFoundException("mocked"));
    try {
  ...
  AuthzAuthenticationRequest createAuthRequest(String username, String password) {
    ...
    return new AuthzAuthenticationRequest(
        userdata, new UaaAuthenticationDetails(mock(HttpServletRequest.class)));
  }
}
