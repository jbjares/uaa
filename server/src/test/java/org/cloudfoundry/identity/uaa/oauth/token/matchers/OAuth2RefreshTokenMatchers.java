...
public class OAuth2RefreshTokenMatchers
    extends AbstractOAuth2AccessTokenMatchers<OAuth2RefreshToken> {
  ...
  protected void describeMismatchSafely(
      OAuth2RefreshToken accessToken, Description mismatchDescription) {
    if (accessToken != null) {
      Map<String, Object> claims = getClaims(accessToken);
  ...
  public static Matcher<OAuth2RefreshToken> validFor(Matcher<?> validFor) {
    return new AbstractOAuth2AccessTokenMatchers<OAuth2RefreshToken>() {
      ...
      protected boolean matchesSafely(OAuth2RefreshToken token) {
        ...
        return validFor.matches(
            ((Integer) claims.get(ClaimConstants.EXP))
                - ((Integer) claims.get(ClaimConstants.IAT)));
      }
