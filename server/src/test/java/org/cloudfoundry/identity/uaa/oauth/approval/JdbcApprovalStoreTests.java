...
public class JdbcApprovalStoreTests extends JdbcTestBase {
  ...
  public void testAddAndGetApproval() {
    ...
    assertThat(
        (int)
            Math.abs(
                expiresAt.getTime() / 1000d - approvals.get(0).getExpiresAt().getTime() / 1000d),
        lessThan(2));
    assertThat(
        (int)
            Math.abs(
                lastUpdatedAt.getTime() / 1000d
                    - approvals.get(0).getLastUpdatedAt().getTime() / 1000d),
        lessThan(2));
  ...
  public void approvals_is_zone_aware() throws Exception {
    ...
    assertThat(
        dao.getApprovalsForClient("c1", IdentityZoneHolder.get().getId()).size(), equalTo(2));
    assertThat(
        dao.getApprovalsForClient("c2", IdentityZoneHolder.get().getId()).size(), equalTo(1));
    assertThat(
        dao.getApprovalsForClient("c3", IdentityZoneHolder.get().getId()).size(), equalTo(0));

    IdentityZoneHolder.set(otherZone);
    assertThat(
        dao.getApprovalsForClient("c1", IdentityZoneHolder.get().getId()).size(), equalTo(0));
    assertThat(
        dao.getApprovalsForClient("c2", IdentityZoneHolder.get().getId()).size(), equalTo(0));
    assertThat(
        dao.getApprovalsForClient("c3", IdentityZoneHolder.get().getId()).size(), equalTo(0));
  ...
  public void canRevokeSingleApproval() {
    ...
    List<Approval> approvalsAfterRevoke =
        dao.getApprovalsForUser("u1", IdentityZoneHolder.get().getId());
  ...
  public void addSameApprovalRepeatedlyUpdatesExpiry() {
    ...
    // time comparison - we're satisfied if it is within 2 seconds
    assertThat(
        (int) Math.abs(timeFromNow.getTime() / 1000d - app.getExpiresAt().getTime() / 1000d),
        lessThan(2));
  ...
  public void canRefreshApproval() {
    ...
    assertThat(
        (int) Math.abs(now.getTime() / 1000d - app.getExpiresAt().getTime() / 1000d), lessThan(2));
  }
