...
public class HttpHeadersFilterRequestWrapperTest {
  ...
  public static List<String> BAD_HEADERS =
      Arrays.asList(
          "X-Forwarded-For",
          "X-Forwarded-Host",
          "X-Forwarded-Proto",
  ...
  public void non_filtered_x_forwarded_headers_single_header() throws Exception {
    ...
    for (String header : BAD_HEADERS) {
      assertNotNull(
          String.format("Header %s should be present.", header), request.getHeader(header));
    }
  }
  ...
  public void filtered_x_forwarded_headers_multi_header() throws Exception {
    for (String header : BAD_HEADERS) {
      assertFalse(
          String.format("Header %s should return empty enumeration.", header),
          request.getHeaders(header).hasMoreElements());
      assertSame(
