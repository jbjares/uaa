...
public class UaaStringUtilsTest {
  ...
  public void test_replace_zone_variables(IdentityZone zone) throws Exception {
    String s =
        "https://{zone.subdomain}.domain.com/z/{zone.id}?id={zone.id}&domain={zone.subdomain}";
    String expect =
        String.format(
            "https://%s.domain.com/z/%s?id=%s&domain=%s",
            zone.getSubdomain(), zone.getId(), zone.getId(), zone.getSubdomain());
  ...
  public void testConstructSimpleWildcardPattern() throws Exception {
    assertEquals(
        "space\\.[^\\\\.]+\\.developer",
        UaaStringUtils.constructSimpleWildcardPattern("space.*.developer"));
    assertEquals(
        "space\\.developer", UaaStringUtils.constructSimpleWildcardPattern("space.developer"));
  }
  ...
  public void testSimpleWildcardPattern() throws Exception {
    ...
    String[] matching =
        new String[] {
          "space.1.developer", "space.13242323423423423.developer",
        };
    String[] notmatching =
        new String[] {
          "space.1",
  ...
  public void test_retainAllMatches() {
    assertThat(
        UaaStringUtils.retainAllMatches(
            Arrays.asList("saml.group.1", "saml.group.2", "saml.group1.3"),
    ...
    assertThat(
        UaaStringUtils.retainAllMatches(
            Arrays.asList("saml.group.1", "saml.group.2", "saml.group1.3", "saml.group1.3.1"),
            Arrays.asList("saml.group*.*")),
