...
public class UaaAuthorizationEndpointTest {
  ...
  public void setup() {
    ...
    when(openIdSessionStateCalculator.calculate("userid", null, "http://example.com"))
        .thenReturn("opbshash");
    when(authorizationCodeServices.createAuthorizationCode(any(OAuth2Authentication.class)))
        .thenReturn("code");
  }
  ...
  public void testGetGrantType_id_token_only_is_implicit() {
    ...
    assertEquals(
        GRANT_TYPE_IMPLICIT,
        uaaAuthorizationEndpoint.deriveGrantTypeFromResponseType(responseTypes));
  }
  ...
  public void testBuildRedirectURI_doesNotIncludeSessionStateWhenNotPromptNone() {
    ...
    authorizationRequest.setResponseTypes(
        new HashSet<String>() {
          {
            add("code");
            add("token");
    ...
    UaaPrincipal principal =
        new UaaPrincipal("userid", "username", "email", "origin", "extid", "zoneid");
    UaaAuthenticationDetails details =
        new UaaAuthenticationDetails(true, "clientid", "origin", "SOMESESSIONID");
    ...
    String result =
        uaaAuthorizationEndpoint.buildRedirectURI(authorizationRequest, accessToken, authUser);
  ...
  public void approveUnmodifiedRequest() {
    ...
    model.put(
        ...
        uaaAuthorizationEndpoint.unmodifiableMap(authorizationRequest));
  ...
  public void testApproveWithModifiedResponseTypes() {
    ...
    model.put(
        ...
        uaaAuthorizationEndpoint.unmodifiableMap(authorizationRequest));
    authorizationRequest.setResponseTypes(
        Collections.singleton("implicit")); // Modify authorization request
    Map<String, String> approvalParameters = new HashMap<>();
    approvalParameters.put("user_oauth_approval", "true");
  ...
  public void testApproveWithModifiedResourceIds() {
    ...
    model.put(
        "org.springframework.security.oauth2.provider.endpoint.AuthorizationEndpoint.ORIGINAL_AUTHORIZATION_REQUEST",
        uaaAuthorizationEndpoint.unmodifiableMap(authorizationRequest));
    authorizationRequest.setResourceIds(
        Collections.singleton("resource-other")); // Modify authorization request
    Map<String, String> approvalParameters = new HashMap<>();
    approvalParameters.put("user_oauth_approval", "true");
  ...
  public void testApproveWithModifiedAuthorities() {
    ...
    authorizationRequest.setAuthorities(
        AuthorityUtils.commaSeparatedStringToAuthorityList(
            "authority-other")); // Modify authorization request
    Map<String, String> approvalParameters = new HashMap<>();
    approvalParameters.put("user_oauth_approval", "true");
  ...
  private AuthorizationRequest getAuthorizationRequest(
      String clientId, String redirectUri, String state, String scope, Set<String> responseTypes) {
    ...
    return new AuthorizationRequest(
        ...
        null,
        false,
        parameters.get(OAuth2Utils.STATE),
