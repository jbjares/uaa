...
public class ChangePasswordControllerTest extends TestClassNullifier {
  ...
  public void changePassword_Returns302Found_SuccessfullyChangedPassword() throws Exception {
    ...
    assertThat(
        ((UaaAuthentication) afterAuth).getAuthenticationMethods(),
  ...
  public void changePassword_PasswordPolicyViolationReported() throws Exception {
    doThrow(new InvalidPasswordException(asList("Msg 2b", "Msg 1b")))
        .when(changePasswordService)
        .changePassword("bob", "secret", "new secret");
  ...
  public void changePassword_Returns401Unauthorized_WrongCurrentPassword() throws Exception {
    doThrow(new BadCredentialsException("401 Unauthorized"))
        .when(changePasswordService)
        .changePassword("bob", "wrong", "new secret");
  ...
  @Test
  public void changePassword_PasswordNoveltyViolationReported_NewPasswordSameAsCurrentPassword()
      throws Exception {
    doThrow(
    ...
    mockMvc
        ...
        .andExpect(
            model()
                .attribute("message", "Your new password cannot be the same as the old password."));
  }
