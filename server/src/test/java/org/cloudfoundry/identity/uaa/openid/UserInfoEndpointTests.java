...
public class UserInfoEndpointTests {
  ...
  private final UaaUser verifiedUser =
      new UaaUser(
          new UaaUserPrototype()
              .withId(ID + "v")
              .withPhoneNumber("8505551234")
  ...
  private InMemoryUaaUserDatabase userDatabase =
      new InMemoryUaaUserDatabase(Arrays.asList(user, verifiedUser));
  ...
  public void setup() {
    ...
    info = new UserInfo().setUserAttributes(customAttributes).setRoles(roles);
  ...
  public void testSunnyDay() {
    ...
    UaaAuthentication authentication =
        UaaAuthenticationTestFactory.getAuthentication(
            user.getId(), "olds", "olds@vmware.com", new HashSet<>(Arrays.asList("openid")));

    UserInfoResponse userInfoResponse =
        endpoint.loginInfo(
            new OAuth2Authentication(createOauthRequest(Arrays.asList("openid")), authentication));
  ...
  public void testVerifiedUser() {
    ...
    UaaAuthentication authentication =
        UaaAuthenticationTestFactory.getAuthentication(
            user.getId(), "somename", "comr@dstal.in", new HashSet<>(Arrays.asList("openid")));
  ...
  public void testSunnyDay_WithCustomAttributes() {
    UaaUser user = userDatabase.retrieveUserByName("olds", OriginKeys.UAA);
    UaaAuthentication authentication =
        UaaAuthenticationTestFactory.getAuthentication(user.getId(), "olds", "olds@vmware.com");
  ...
  public void testMissingUser() {
    UaaAuthentication authentication =
        UaaAuthenticationTestFactory.getAuthentication("nonexist-id", "Dale", "olds@vmware.com");
    endpoint.loginInfo(
        new OAuth2Authentication(createOauthRequest(Arrays.asList("openid")), authentication));
  }
}
