...
public class ClientAdminBootstrapTests extends JdbcTestBase {
  ...
  public void setUpClientAdminTests() throws Exception {
    ...
    clientMetadataProvisioning =
        new JdbcClientMetadataProvisioning(clientRegistrationService, jdbcTemplate);
  ...
  public ClientDetails testSimpleAddClient(String clientId) throws Exception {
    ...
    assertSet(
        (String) map.get("redirect-uri"), null, created.getRegisteredRedirectUri(), String.class);
    return created;
  }
  ...
  public void client_slated_for_deletion_does_not_get_inserted() throws Exception {
    String autoApproveId =
        "autoapprove-" + new RandomValueStringGenerator().generate().toLowerCase();
    ...
    Map<String, Map<String, Object>> clients =
        Collections.singletonMap(clientId, createClientMap(clientId));
  ...
  public void clientMetadata_getsBootstrapped() throws Exception {
    ...
    ClientMetadata clientMetadata =
        clientMetadataProvisioning.retrieve("foo", IdentityZoneHolder.get().getId());
  ...
  public void testSimpleAddClientWithAutoApprove() throws Exception {
    ...
    when(clientMetadataProvisioning.update(any(ClientMetadata.class), anyString()))
        .thenReturn(new ClientMetadata());
    ...
    verify(clientRegistrationService)
        .addClientDetails(expectedAdd, IdentityZoneHolder.get().getId());
    BaseClientDetails expectedUpdate = new BaseClientDetails(expectedAdd);
    expectedUpdate.setAdditionalInformation(
        Collections.singletonMap(ClientConstants.AUTO_APPROVE, true));
  ...
  public void testOverrideClient() throws Exception {
    ...
    BaseClientDetails foo =
        new BaseClientDetails("foo", "", "openid", "client_credentials,password", "uaa.none");
    ...
    doThrow(new ClientAlreadyExistsException("Planned"))
        .when(clientRegistrationService)
        .addClientDetails(any(ClientDetails.class), anyString());
    bootstrap.afterPropertiesSet();
    verify(clientRegistrationService, times(1))
        .addClientDetails(any(ClientDetails.class), anyString());
    ...
    verify(clientRegistrationService, times(1))
        .updateClientSecret("foo", "bar", IdentityZoneHolder.get().getId());
    assertEquals(
        new HashSet(Arrays.asList("client_credentials")),
        captor.getValue().getAuthorizedGrantTypes());
  }
  ...
  public void testOverrideClientWithEmptySecret() throws Exception {
    ...
    verify(clientRegistrationService, times(1)).updateClientDetails(captor.capture(), anyString());
    verify(clientRegistrationService, times(1))
        .updateClientSecret("foo", "", IdentityZoneHolder.get().getId());
    assertEquals(
        new HashSet(Arrays.asList("client_credentials")),
        captor.getValue().getAuthorizedGrantTypes());
  }
  ...
  public void testOverrideClientByDefault() throws Exception {
    ...
    verify(clientRegistrationService, times(1))
        .addClientDetails(any(ClientDetails.class), anyString());
    verify(clientRegistrationService, times(1))
        .updateClientDetails(any(ClientDetails.class), anyString());
    verify(clientRegistrationService, times(1))
        .updateClientSecret("foo", "bar", IdentityZoneHolder.get().getId());
  }
  ...
  public void testOverrideClientWithYaml() throws Exception {
    ...
    doThrow(new ClientAlreadyExistsException("Planned"))
        ...
        .addClientDetails(any(ClientDetails.class), anyString());
    bootstrap.afterPropertiesSet();
    verify(clientRegistrationService, times(2))
        .addClientDetails(any(ClientDetails.class), anyString());
    verify(clientRegistrationService, times(2))
        .updateClientDetails(any(ClientDetails.class), anyString());
    verify(clientRegistrationService, times(1))
        .updateClientSecret("foo", "bar", IdentityZoneHolder.get().getId());
    verify(clientRegistrationService, times(1))
        .updateClientSecret("bar", "bar", IdentityZoneHolder.get().getId());
  }
  ...
  public void testChangePasswordDuringBootstrap() throws Exception {
    ...
    assertTrue(
        "Password should match bar:",
        bootstrap.getPasswordEncoder().matches("bar", details.getClientSecret()));
    map.put("secret", "bar1");
    ...
    assertTrue(
        "Password should match bar1:",
        bootstrap.getPasswordEncoder().matches("bar1", details.getClientSecret()));
    assertFalse(
        "Password should not match bar:",
        bootstrap.getPasswordEncoder().matches("bar", details.getClientSecret()));
  }
  ...
  private ClientDetails doSimpleTest(Map<String, Object> map) throws Exception {
    ...
    for (Map.Entry<String, Object> entry : info.entrySet()) {
      assertTrue(
          "Client should contain additional information key:" + entry.getKey(),
          created.getAdditionalInformation().containsKey(entry.getKey()));
  ...
  private void assertSet(
      String expectedValue, Collection defaultValueIfNull, Collection actualValue, Class<?> type) {
