...
public class LdapGroupMappingAuthorizationManagerTests extends JdbcTestBase {
  ...
  public void initLdapGroupMappingAuthorizationManagerTests() throws Exception {
    ...
    gDB.create(
        new ScimGroup(null, "acme", IdentityZoneHolder.get().getId()),
        IdentityZoneHolder.get().getId());
    gDB.create(
        new ScimGroup(null, "acme.dev", IdentityZoneHolder.get().getId()),
        IdentityZoneHolder.get().getId());
    ...
    externalGroupMap.put(
        "cn=Engineering,ou=groups,dc=example,dc=com", Collections.singletonList("acme"));
    ...
    externalGroupMap.put(
        "cn=Engineering,ou=groups,dc=example,dc=com", Collections.singletonList("acme.dev"));
