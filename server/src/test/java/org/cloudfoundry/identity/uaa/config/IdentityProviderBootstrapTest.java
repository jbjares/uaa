...
public class IdentityProviderBootstrapTest extends JdbcTestBase {
  ...
  public void testLdapProfileBootstrap() throws Exception {
    ...
    IdentityProvider<LdapIdentityProviderDefinition> ldapProvider =
        provisioning.retrieveByOriginIgnoreActiveFlag(LDAP, IdentityZoneHolder.get().getId());
  ...
  public void validateGenericLdapProvider(
      String idpDescription, IdentityProvider<LdapIdentityProviderDefinition> ldapProvider) {
    assertNotNull(ldapProvider);
    assertNotNull(ldapProvider.getCreated());
    assertNotNull(ldapProvider.getLastModified());
    assertEquals(LDAP, ldapProvider.getType());
  ...
  public void testKeystoneProfileBootstrap() throws Exception {
    ...
    IdentityProvider<KeystoneIdentityProviderDefinition> keystoneProvider =
        provisioning.retrieveByOriginIgnoreActiveFlag(KEYSTONE, IdentityZoneHolder.get().getId());
  ...
  public void testKeystoneBootstrap() throws Exception {
    ...
    IdentityProvider keystoneProvider =
        provisioning.retrieveByOriginIgnoreActiveFlag(KEYSTONE, IdentityZoneHolder.get().getId());
    assertNotNull(keystoneProvider);
    assertEquals(
        new KeystoneIdentityProviderDefinition(keystoneConfig), keystoneProvider.getConfig());
    assertNotNull(keystoneProvider.getCreated());
    assertNotNull(keystoneProvider.getLastModified());
    assertEquals(KEYSTONE, keystoneProvider.getType());
  }
  ...
  public void test_oauth_and_oidc_provider_deletion() throws Exception {
    ...
    bootstrap.afterPropertiesSet();
    for (Map.Entry<String, AbstractXOAuthIdentityProviderDefinition> provider :
        oauthProviderConfig.entrySet()) {
      try {
        provisioning.retrieveByOriginIgnoreActiveFlag(
            provider.getKey(), IdentityZoneHolder.get().getId());
            ...
            p -> {
              ...
              wrappers.add(
                  OauthIDPWrapperFactoryBean.getIdentityProviderWrapper(
                      p.getKey(), p.getValue(), provider, true));
            });
  ...
  public void test_oauth_and_oidc_provider_activation() throws Exception {
    ...
    for (Map.Entry<String, AbstractXOAuthIdentityProviderDefinition> provider :
        oauthProviderConfig.entrySet()) {
      IdentityProvider<AbstractXOAuthIdentityProviderDefinition> bootstrapOauthProvider =
          provisioning.retrieveByOriginIgnoreActiveFlag(
              provider.getKey(), IdentityZoneHolder.get().getId());
    ...
    for (Map.Entry<String, AbstractXOAuthIdentityProviderDefinition> provider :
      ...
      assertThat(
          oauthProviderConfig.values(),
          PredicateMatcher.has(c -> c.equals(bootstrapOauthProvider.getConfig())));
  ...
  public void validateOauthOidcProvider(
      Map.Entry<String, AbstractXOAuthIdentityProviderDefinition> provider,
      IdentityProvider<AbstractXOAuthIdentityProviderDefinition> bootstrapOauthProvider) {
    assertNotNull(bootstrapOauthProvider);
    assertThat(
        oauthProviderConfig.values(),
  ...
  protected AbstractXOAuthIdentityProviderDefinition setCommonProperties(
      AbstractXOAuthIdentityProviderDefinition definition) throws MalformedURLException {
    return definition
  ...
  public void testSamlBootstrap() throws Exception {
    ...
    IdentityProvider samlProvider =
        provisioning.retrieveByOriginIgnoreActiveFlag(
            samlIdentityProviderDefinition.getIdpEntityAlias(), IdentityZoneHolder.get().getId());
  ...
  public void test_providers_deleted_and_not_created() throws Exception {
    ...
    List<String> originsToDelete =
        Arrays.asList(samlIdentityProviderDefinition.getIdpEntityAlias(), OIDC10);
    ...
    ArgumentCaptor<EntityDeletedEvent<IdentityProvider>> captor =
        ArgumentCaptor.forClass(EntityDeletedEvent.class);
    verify(publisher, times(2)).publishEvent(captor.capture());
    assertThat(
        captor.getAllValues().stream().map(p -> p.getDeleted().getOriginKey()).collect(toList()),
        containsInAnyOrder(originsToDelete.toArray()));
  }

  public void configureSamlProviders(
      boolean override, SamlIdentityProviderDefinition... definitions) {
    reset(configurator);
    ...
    for (SamlIdentityProviderDefinition def : definitions) {
      IdentityProviderWrapper w =
          new IdentityProviderWrapper(BootstrapSamlIdentityProviderData.parseSamlProvider(def));
  ...
  public void test_saml_provider_override_false() throws Exception {
    ...
    IdentityProvider<SamlIdentityProviderDefinition> samlProvider =
        provisioning.retrieveByOriginIgnoreActiveFlag(
            samlIdentityProviderDefinition.getIdpEntityAlias(), IdentityZoneHolder.get().getId());
    IdentityProvider<SamlIdentityProviderDefinition> samlProvider2 =
        provisioning.retrieveByOriginIgnoreActiveFlag(
            samlIdentityProviderDefinition1.getIdpEntityAlias(), IdentityZoneHolder.get().getId());
    assertNotNull(samlProvider);
  ...
  public void test_saml_provider_not_deactivated() throws Exception {
    ...
    configurator = mock(BootstrapSamlIdentityProviderData.class);
    when(configurator.getIdentityProviderDefinitions())
        .thenReturn(Arrays.asList(samlIdentityProviderDefinition1, samlIdentityProviderDefinition));
    bootstrap.setSamlProviders(configurator);
    bootstrap.afterPropertiesSet();
  ...
  private void setDisableInternalUserManagement(String expectedValue) throws Exception {
    ...
    bootstrap.afterPropertiesSet();
    IdentityProvider<UaaIdentityProviderDefinition> internalIDP =
        provisioning.retrieveByOriginIgnoreActiveFlag(
    ...
    assertEquals(
        Boolean.valueOf(expectedValue), internalIDP.getConfig().isDisableInternalUserManagement());
  }
