...
public class UserGoogleMfaCredentialsProvisioningTest {
  ...
  public void testSavesCredentialsNotInDatabase() {
    ...
    provisioner.saveUserCredentials(
        creds.getUserId(),
        creds.getSecretKey(),
        creds.getValidationCode(),
        creds.getScratchCodes());
  ...
  public void testSaveUserCredentials_updatesWhenUserExists() {
    ...
    UserGoogleMfaCredentials updatedCreds =
        new UserGoogleMfaCredentials("jabbahut", "different_key", 45678, Arrays.asList(1, 22));

    provisioner.saveUserCredentials(
        updatedCreds.getUserId(),
        updatedCreds.getSecretKey(),
        updatedCreds.getValidationCode(),
        updatedCreds.getScratchCodes());
    verify(jdbcProvisioner, times(0)).save(any(), anyString());
  }
  ...
  public void testPersist_ErrorsIfAlreadyExists() {
    ...
    verify(jdbcProvisioner, times(0)).save(any(), anyString());
    provisioner.saveUserCredentials(creds);
    doThrow(UserMfaConfigAlreadyExistsException.class)
        .when(jdbcProvisioner)
        .save(any(), anyString());
    provisioner.saveUserCredentials(creds);
  }
  ...
  public void testActiveUserCredentialExists() {
    ...
    when(jdbcProvisioner.retrieve(anyString(), eq(otherMfaProvider.getId())))
        .thenThrow(UserMfaConfigDoesNotExistException.class);
    ...
    assertTrue(
        "user not persisted for mfa provider",
        provisioner.activeUserCredentialExists("jabbahut", mfaProvider.getId()));
    assertFalse(
        "user persisted even though we switched mfaProvider",
        provisioner.activeUserCredentialExists("jabbahut", otherMfaProvider.getId()));
  }

  @Test
  public void testActiveUserCredentialDoesNotExistAcrossProvider() {
    UserGoogleMfaCredentials creds = creds();
    when(jdbcProvisioner.retrieve(anyString(), anyString()))
        .thenThrow(UserMfaConfigDoesNotExistException.class)
        .thenThrow(UserMfaConfigDoesNotExistException.class)
        .thenReturn(creds);

    assertFalse(
        "no user in db but activeCredentialExists returned true",
        provisioner.activeUserCredentialExists("jabbahut", mfaProvider.getId()));
    ...
    // provisioner.persistCredentials();
    assertTrue(
        "user not shown as active after persisting",
        provisioner.activeUserCredentialExists("jabbahut", mfaProvider.getId()));
  }
