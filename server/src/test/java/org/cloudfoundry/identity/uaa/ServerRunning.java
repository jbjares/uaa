...
/**
 * A rule that prevents integration tests from failing if the server application is not running or
 * not accessible. If the server is not running in the background all the tests here will simply be
 * skipped because of a violated assumption (showing as successful). Usage:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>The rule can be declared as static so that it only has to check once for all tests in the
 * enclosing test case, but there isn't a lot of overhead in making it non-static.
 *
 * @see Assume
...
public class ServerRunning implements MethodRule, RestTemplateHolder, UrlHelper {
  ...
  public ResponseEntity<String> postForString(
    ...
    return client.exchange(
        getUrl(path), HttpMethod.POST, new HttpEntity<>(formData, headers), String.class);
  }

  @SuppressWarnings("rawtypes")
  public ResponseEntity<Map> postForMap(
      String path, MultiValueMap<String, String> formData, HttpHeaders headers) {
    if (headers.getContentType() == null) {
      headers.setContentType(MediaType.APPLICATION_FORM_URLENCODED);
    }
    return client.exchange(
        getUrl(path), HttpMethod.POST, new HttpEntity<>(formData, headers), Map.class);
  }
  ...
  public ResponseEntity<Map> postForMap(String path, String requestBody, HttpHeaders headers) {
    ...
    return client.exchange(
        getUrl(path), HttpMethod.POST, new HttpEntity<>(requestBody, headers), Map.class);
  }
  ...
  public ResponseEntity<Void> getForResponse(
      String path, final HttpHeaders headers, Object... uriVariables) {
    HttpEntity<Void> request = new HttpEntity<>(null, headers);
    return client.exchange(getUrl(path), HttpMethod.GET, request, Void.class, uriVariables);
  }

  public ResponseEntity<Void> postForResponse(
      String path, HttpHeaders headers, MultiValueMap<String, String> params) {
    HttpHeaders actualHeaders = new HttpHeaders();
    actualHeaders.putAll(headers);
    actualHeaders.setContentType(MediaType.APPLICATION_FORM_URLENCODED);

    return client.exchange(
        getUrl(path), HttpMethod.POST, new HttpEntity<>(params, actualHeaders), Void.class);
  }

  public ResponseEntity<Void> postForRedirect(
      String path, HttpHeaders headers, MultiValueMap<String, String> params) {
    ...
    return client.exchange(
        location, HttpMethod.GET, new HttpEntity<Void>(null, headers), Void.class);
  }
