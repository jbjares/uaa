...
public class JdbcIdentityZoneProvisioningTests extends JdbcTestBase {
  ...
  public void test_delete_zone() {
    ...
    assertThat(
        jdbcTemplate.queryForObject(
            ...
            new Object[] {createdIdZone.getId()},
            Integer.class),
        is(1));
  ...
  public void test_cannot_delete_uaa_zone() {
    assertThat(
        jdbcTemplate.queryForObject(
            "select count(*) from identity_zone where id = ?",
            new Object[] {IdentityZone.getUaa().getId()},
            Integer.class),
        is(1));
  ...
  public void testCreateDuplicateIdentityZone() throws Exception {
    IdentityZone identityZone =
        MultitenancyFixture.identityZone("there-can-be-only-one", "there-can-be-only-one");
    identityZone.setId(generator.generate());
  ...
  public void testGetIdentityZone() {
    ...
    assertEquals(
        identityZone.getConfig().getTokenPolicy().getAccessTokenValidity(),
        retrievedIdZone.getConfig().getTokenPolicy().getAccessTokenValidity());
    assertEquals(
        identityZone.getConfig().getTokenPolicy().getRefreshTokenValidity(),
        retrievedIdZone.getConfig().getTokenPolicy().getRefreshTokenValidity());
