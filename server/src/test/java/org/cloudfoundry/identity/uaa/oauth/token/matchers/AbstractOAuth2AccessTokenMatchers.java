...
public abstract class AbstractOAuth2AccessTokenMatchers<T> extends TypeSafeMatcher<T> {
  ...
  public static ThreadLocal<Map<String, RevocableToken>> revocableTokens =
  ...
  protected Map<String, Object> getClaims(T token) {
    ...
    if (token instanceof OAuth2AccessToken) tokenValue = ((OAuth2AccessToken) token).getValue();
    else if (token instanceof OAuth2RefreshToken)
      tokenValue = ((OAuth2RefreshToken) token).getValue();
    else
      throw new IllegalArgumentException(
          "token must be instanceof OAuth2AccessToken or OAuth2RefreshToken");
