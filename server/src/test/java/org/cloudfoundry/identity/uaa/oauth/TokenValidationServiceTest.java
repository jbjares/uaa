...
public class TokenValidationServiceTest {
  ...
  public void setup() {
    ...
    IdentityZoneHolder.get()
        .getConfig()
        .getTokenPolicy()
        .setKeys(Collections.singletonMap("key1", PRIVATE_KEY));
    ...
    when(clientServicesExtension.loadClientByClientId(clientId, IdentityZoneHolder.get().getId()))
        .thenReturn(new BaseClientDetails(clientId, null, "foo.bar", null, null));
    UaaUser user =
        new UaaUser(
            userId,
            "marrisa",
            "koala",
            "marissa@gmail.com",
            buildGrantedAuthorities("foo.bar"),
            "Marissa",
            "Bloggs",
            null,
            null,
  ...
  public void validation_enforcesKeyId() {
    ...
    expectedException.expectMessage(
        "Token header claim [kid] references unknown signing key : [testKey]");
