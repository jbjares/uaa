...
public class UaaMetricsFilterTests {
  ...
  public void happy_path() throws Exception {
    ...
    for (String uri : Arrays.asList(path, MetricsUtil.GLOBAL_GROUP)) {
      MetricsQueue totals =
          readValue(summary.get(filter.getUriGroup(request).getGroup()), MetricsQueue.class);
      assertNotNull("URI:" + uri, totals);
      for (StatusCodeGroup status :
          Arrays.asList(StatusCodeGroup.SUCCESS, StatusCodeGroup.SERVER_ERROR)) {
  ...
  public void intolerable_request() throws Exception {
    TimeService slowRequestTimeService =
        new TimeService() {
            ...
            entry -> {
              for (String s : entry.getValue()) {
                ...
                assertEquals(
                    "Testing URL: " + s,
                    FALLBACK.getGroup(),
                    filter.getUriGroup(request).getGroup());
              }
            });
  }
