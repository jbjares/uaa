...
public class JdbcScimGroupMembershipManagerTests extends JdbcTestBase {
  ...
  private static final String addMemberSqlFormat =
      "insert into group_membership (group_id, member_id, member_type, origin, identity_zone_id) values ('%s', '%s', '%s', '%s', '%s')";

  private static final String addExternalMapSql =
      "insert into external_group_mapping (group_id, external_group, added, origin, identity_zone_id) values (?, ?, ?, ?, ?)";
  ...
  private void mapExternalGroup(String gId, String external, String origin) {
    ...
    jdbcTemplate.update(
        addExternalMapSql, gId, external, now, origin, IdentityZoneHolder.get().getId());
  }
  ...
  private void addMember(String gId, String mId, String mType, String origin) {
    ...
    jdbcTemplate.execute(
        String.format(
            addMemberSqlFormat, gId, mId, mType, origin, IdentityZoneHolder.get().getId()));
  }
  ...
  private void validateUserGroups(String id, String... gNm) {
    Set<ScimGroup> directGroups =
        dao.getGroupsWithMember(id, false, IdentityZoneHolder.get().getId());
    assertNotNull(directGroups);
    Set<ScimGroup> indirectGroups =
        dao.getGroupsWithMember(id, true, IdentityZoneHolder.get().getId());
  ...
  public void default_groups_are_cached() throws Exception {
    ...
    defaultGroups
        .stream()
        .forEach(g -> verify(spy, times(1)).createAndIgnoreDuplicate(eq(g), eq(zone.getId())));
    reset(spy);
    defaultGroups.stream().forEach(g -> dao.createOrGetGroup(g, zone.getId()));
  ...
  public void user_delete_clears_memberships() throws Exception {
    UaaUserPrototype prototype =
        new UaaUserPrototype().withUsername("username").withEmail("test@test.com");
  ...
  public void test_provider_deleted() {
    ...
    assertThat(
        jdbcTemplate.queryForObject(
            ...
            new Object[] {IdentityZoneHolder.get().getId(), LOGIN_SERVER},
            Integer.class),
        is(4));
    assertThat(
        jdbcTemplate.queryForObject(
  ...
  public void canReadNullFromAuthoritiesColumn() {
    ...
    jdbcTemplate.execute(
        String.format(
            addNullAuthoritySQL, "g1", "m1", "USER", "uaa", IdentityZoneHolder.get().getId()));
  ...
  public void canUpdateOrAddMembers() {
    ...
    ScimGroupMember g2 =
        new ScimGroupMember("g2", ScimGroupMember.Type.GROUP); // update role member->admin
  ...
  public void canRemoveNestedGroupMember() {
    dao.addMember(
        "g1",
        new ScimGroupMember("m1", ScimGroupMember.Type.USER),
        IdentityZoneHolder.get().getId());
    dao.addMember(
        "g1",
        new ScimGroupMember("g2", ScimGroupMember.Type.GROUP),
        IdentityZoneHolder.get().getId());
    dao.addMember(
        "g2",
        new ScimGroupMember("m2", ScimGroupMember.Type.USER),
        IdentityZoneHolder.get().getId());
    validateCount(3);
    validateUserGroups("m1", "test1");
    validateUserGroups("m2", "test2", "test1.i");
