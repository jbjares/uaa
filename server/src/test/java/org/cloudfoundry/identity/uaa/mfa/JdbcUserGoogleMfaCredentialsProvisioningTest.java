...
public class JdbcUserGoogleMfaCredentialsProvisioningTest extends JdbcTestBase {
  ...
  private static final String MFA_ID = new RandomValueStringGenerator(36).generate();
  ...
  public void initJdbcScimUserProvisioningTests() throws Exception {
    ...
    encryptionKeyService =
        new EncryptionKeyService(
            activeKeyLabel, Lists.newArrayList(activeEncryptionKey, inActiveEncryptionKey));
  ...
  public void testSaveUserGoogleMfaCredentials() throws EncryptionServiceException {
    ...
    UserGoogleMfaCredentials userGoogleMfaCredentials =
        new UserGoogleMfaCredentials("jabbahut", "very_sercret_key", 74718234, Arrays.asList(1, 22))
            .setMfaProviderId(MFA_ID);

    db.save(userGoogleMfaCredentials, zoneId);
    List<Map<String, Object>> credentials =
        jdbcTemplate.queryForList("SELECT * FROM user_google_mfa_credentials");
    ...
    assertEquals(
        ...
        new String(
            activeEncryptionKey.decrypt(
                Base64Utils.decodeFromString((String) record.get("secret_key")))));
    assertEquals(
        74718234,
        Integer.parseInt(
            new String(
                activeEncryptionKey.decrypt(
                    Base64Utils.decodeFromString(
                        String.valueOf(record.get("encrypted_validation_code")))))));
    assertEquals(
        "1,22",
        new String(
            activeEncryptionKey.decrypt(
                Base64Utils.decodeFromString(String.valueOf(record.get("scratch_codes"))))));
  ...
  public void testUpdateUserGoogleMfaCredentials_noUser() {
    ...
    UserGoogleMfaCredentials userGoogleMfaCredentials =
        new UserGoogleMfaCredentials(
            "jabbahut", "very_sercret_key", 74718234, Arrays.asList(1, 22));
  ...
  public void testUpdateUserGoogleMfaCredentials() throws EncryptionServiceException {
    ...
    assertEquals(
        ...
        new String(
            activeEncryptionKey.decrypt(
                Base64Utils.decodeFromString((String) record.get("scratch_codes")))));
    assertEquals(activeKeyLabel, record.get("encryption_key_label"));
  }
  ...
  public void testRetrieveExisting() {
    db.save(
        new UserGoogleMfaCredentials("user1", "secret", 12345, Collections.singletonList(123))
            .setMfaProviderId(MFA_ID),
        zoneId);
  ...
  public void testRetrieveExistingWithANonActiveEncryptionKey() throws EncryptionServiceException {
    encryptionKeyService =
        new EncryptionKeyService(
            inactiveKeyLabel, Lists.newArrayList(activeEncryptionKey, inActiveEncryptionKey));
    db = new JdbcUserGoogleMfaCredentialsProvisioning(jdbcTemplate, encryptionKeyService);
    db.save(
        new UserGoogleMfaCredentials("user1", "secret", 12345, Collections.singletonList(123))
            .setMfaProviderId(MFA_ID),
  ...
  @Test
  public void whenSaving_AndFailsToEncrypt_ShouldThrowAMeaningfulException()
      throws EncryptionServiceException {
    EncryptionKeyService mockEncryptionKeyService =
        mock(EncryptionKeyService.class, Answers.RETURNS_DEEP_STUBS);
  ...
  @Test
  public void whenUpdating_AndFailsToEncrypt_ShouldThrowAMeaningfulException()
      throws EncryptionServiceException {
    UserGoogleMfaCredentials userGoogleMfaCredentials =
        new UserGoogleMfaCredentials("user1", "secret", 12345, Collections.singletonList(123))
            .setMfaProviderId(MFA_ID);
  ...
  @Test
  public void whenReading_AndFailsToDecrypt_ShouldThrowAMeaningfulException()
      throws EncryptionServiceException {
    UserGoogleMfaCredentials userGoogleMfaCredentials =
        new UserGoogleMfaCredentials("user1", "secret", 12345, Collections.singletonList(123))
            .setMfaProviderId(MFA_ID);
  ...
  public void whenDecryptingWithNonExistentKey_ShouldThrowMeaningfulException() {
    ...
    encryptionKeyService =
        new EncryptionKeyService(activeKeyLabel, Lists.newArrayList(activeEncryptionKey));
    db = new JdbcUserGoogleMfaCredentialsProvisioning(jdbcTemplate, encryptionKeyService);
  ...
  public void whenDecryptingRecordWithNoKeyLabel_ShouldNotAttemptToDecrypt() {
    String userId = "user1";
    int numInsertedRecords =
        jdbcTemplate.update(
            "INSERT INTO user_google_mfa_credentials (user_id, secret_key, validation_code, scratch_codes, mfa_provider_id, zone_id, encryption_key_label) VALUES (?,?,?,?,?,?,?)",
            userId,
            "secret_key",
            123456,
            "123",
