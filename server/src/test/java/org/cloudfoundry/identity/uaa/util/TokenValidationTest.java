...
public class TokenValidationTest {
  ...
  public void setup() {
    ...
    clientDetailsService.setClientDetailsStore(
        IdentityZone.getUaa().getId(), Collections.singletonMap(CLIENT_ID, uaaClient));
    revocableTokenProvisioning = mock(RevocableTokenProvisioning.class);

    when(revocableTokenProvisioning.retrieve(
            "8b14f193-8212-4af2-9927-e3ae903f94a6", IdentityZoneHolder.get().getId()))
        .thenReturn(
            new RevocableToken().setValue(UaaTokenUtils.constructToken(header, content, signer)));

    userDb =
        new MockUaaUserDatabase(
            u ->
                u.withUsername("marissa")
                    .withId(USER_ID)
                    .withEmail("marissa@test.org")
                    .withAuthorities(
                        Collections.singletonList(new SimpleGrantedAuthority("acme.dev"))));

    uaaUser = userDb.retrieveUserById(USER_ID);
    uaaUserGroups =
        uaaUser.getAuthorities().stream().map(a -> a.getAuthority()).collect(Collectors.toList());
  }
  ...
  public void validate_KeyId_actuallyExists() {
    ...
    expectedException.expectMessage(
        "Token header claim [kid] references unknown signing key : [garbage]");
  ...
  public void testGetClientById() {
    ...
    ClientDetails clientDetails =
        TokenValidation.buildAccessTokenValidator(token, new KeyInfoService("https://localhost"))
  ...
  public void validate_required_groups_is_invoked() {
    TokenValidation validation =
        spy(buildAccessTokenValidator(getToken(), new KeyInfoService("https://localhost")));

    validation.checkClientAndUser(uaaClient, uaaUser);
    verify(validation, times(1))
        .checkRequiredUserGroups(
            (Collection<String>) argThat(containsInAnyOrder(new String[0])),
    ...
    List<GrantedAuthority> authorities =
        AuthorityUtils.commaSeparatedStringToAuthorityList("group1,group2");

    authorities.addAll(
        AuthorityUtils.createAuthorityList(
            uaaUserGroups.toArray(new String[uaaUserGroups.size()])));
    uaaUser = uaaUser.authorities(authorities);

    validation.checkClientAndUser(uaaClient, uaaUser);
    verify(validation, times(1))
        .checkRequiredUserGroups(
            (Collection<String>) argThat(containsInAnyOrder(new String[] {"group1", "group2"})),
            (Collection<String>)
                argThat(
                    containsInAnyOrder(
                        uaaUser
  ...
  public void idTokenValidator_rejectsTokensWithRefreshTokenSuffix() {
    ...
    buildIdTokenValidator(
            getToken(),
            mock(ChainedSignatureVerifier.class),
                ...
                u.withUsername("marissa")
                    ...
                    .withAuthorities(
                        Collections.singletonList(
                            new SimpleGrantedAuthority("a.different.scope"))));
    ...
    clientDetailsService.setClientDetailsStore(
        Collections.singletonMap(
            ...
            new BaseClientDetails(
                "app", "acme", "a.different.scope", GRANT_TYPE_AUTHORIZATION_CODE, "")));
