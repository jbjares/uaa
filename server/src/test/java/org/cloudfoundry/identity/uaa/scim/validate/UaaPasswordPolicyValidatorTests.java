...
public class UaaPasswordPolicyValidatorTests {
  ...
  @Mock private IdentityProviderProvisioning provisioning;
  ...
  public void testValidateLongPassword() {
    validatePassword(
        RandomStringUtils.randomAlphanumeric(23) + "aA9",
        "Password must be no more than 23 characters in length.");
  }
  ...
  private void validatePassword(String password, String... expectedErrors) {
    ...
    try {
      ...
      for (int i = 0; i < expectedErrors.length; i++) {
        assertTrue(
            "Errors should contain:" + expectedErrors[i],
            e.getErrorMessages().contains(expectedErrors[i]));
      }
    }
  }
}
