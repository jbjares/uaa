...
public class UaaHttpRequestUtilsTest {
  ...
  public static void storeSystemProxyConfig() {
    for (String s :
        Arrays.asList(
            HTTP_HOST_PROPERTY, HTTP_PORT_PROPERTY, HTTPS_HOST_PROPERTY, HTTPS_PORT_PROPERTY)) {
  ...
  public void setup() throws Exception {
    ...
    httpsServer =
        NetworkTestUtils.startHttpsServer(keystore, NetworkTestUtils.keyPass, httpsResponseHandler);
  ...
  public void testHttpProxy(
      String url, int expectedPort, String expectedHost, boolean wantHandlerInvoked)
      throws Exception {
    HttpClientBuilder builder = UaaHttpRequestUtils.getClientBuilder(true);
    HttpRoutePlanner planner =
        (HttpRoutePlanner) ReflectionTestUtils.getField(builder.build(), "routePlanner");
    SystemProxyRoutePlanner routePlanner = new SystemProxyRoutePlanner(planner);
    builder.setRoutePlanner(routePlanner);
    RestTemplate template =
        new RestTemplate(UaaHttpRequestUtils.createRequestFactory(builder, Integer.MAX_VALUE));
    try {
  ...
  public void trustedOnly() {
    RestTemplate restTemplate =
        new RestTemplate(UaaHttpRequestUtils.createRequestFactory(false, 10_000));
    try {
