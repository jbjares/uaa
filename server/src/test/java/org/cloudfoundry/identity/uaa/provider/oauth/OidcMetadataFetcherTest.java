...
class OidcMetadataFetcherTest {
  ...
  class WithDiscoveryUrl {
    ...
    @Test
    public void shouldFavorUsingConfiguredIdentityProviderProperties()
        throws OidcMetadataFetchingException, MalformedURLException {
      ...
      assertThat(
          definition.getAuthUrl().toString(), is("http://authz.should.not.have.been.updated"));
      assertThat(
          definition.getTokenUrl().toString(), is("http://token.should.not.have.been.updated"));
      assertThat(
          definition.getUserInfoUrl().toString(),
          is("http://userinfo.should.not.have.been.updated"));
      assertThat(
          definition.getTokenKeyUrl().toString(), is("http://jwks.should.not.have.been.updated"));
      assertThat(definition.getIssuer(), is("should-not-have-been-updated"));
    }

    @Test
    public void givenConfiguredIdentityProviderPropertiesAreNotSet_shouldUseOidcMetadata()
    ...
    @Test
    public void shouldPerformDiscoveryUsingCache()
        throws OidcMetadataFetchingException, MalformedURLException {
      ...
      verify(urlContentCache, times(2))
          .getUrlContent(eq(definition.getDiscoveryUrl().toString()), eq(restTemplate));
    }
  }
  ...
  class WithoutDiscoveryUrl {
    ...
    @Test
    @DisplayName(
        "when the idp is configured without a discovery URL then it should retain the configured OAuth/OIDC endpoints")
    public void shouldNotPerformDiscovery()
        throws OidcMetadataFetchingException, MalformedURLException {
