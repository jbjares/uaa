...
public class IdentityProviderEndpointsTest {
  ...
  public IdentityProvider<LdapIdentityProviderDefinition> retrieve_oauth_provider_by_id(
    ...
    ResponseEntity<IdentityProvider> oauth =
        identityProviderEndpoints.retrieveIdentityProvider(id, true);
    assertNotNull(oauth);
    ...
    assertNull(
        ((AbstractXOAuthIdentityProviderDefinition) oauth.getBody().getConfig())
  ...
  public IdentityProvider<LdapIdentityProviderDefinition> retrieve_ldap_provider_by_id(String id)
      throws Exception {
    when(identityProviderProvisioning.retrieve(anyString(), anyString()))
        .thenReturn(getLdapDefinition());
    ResponseEntity<IdentityProvider> ldap =
        identityProviderEndpoints.retrieveIdentityProvider(id, true);
    assertNotNull(ldap);
  ...
  public void remove_client_secret() throws Exception {
    for (String type : Arrays.asList(OIDC10, OAUTH20)) {
      remove_sensitive_data(
          ...
          type,
          (spy) ->
              verify((AbstractXOAuthIdentityProviderDefinition) spy, times(1))
                  .setRelyingPartySecret(isNull()));
    }
  }

  public void remove_sensitive_data(
      Supplier<IdentityProvider> getProvider,
      String type,
      Consumer<AbstractIdentityProviderDefinition> validator) {
  ...
  public void remove_client_secret_wrong_origin() throws Exception {
    IdentityProvider provider = getXOAuthProvider();
    AbstractXOAuthIdentityProviderDefinition spy =
        Mockito.spy((AbstractXOAuthIdentityProviderDefinition) provider.getConfig());
    provider.setConfig(spy);
  ...
  public void remove_bind_password_non_ldap() throws Exception {
    ...
    LdapIdentityProviderDefinition spy =
        Mockito.spy((LdapIdentityProviderDefinition) provider.getConfig());
    provider.setConfig(spy);
  ...
  public void patch_bind_password() throws Exception {
    ...
    when(identityProviderProvisioning.retrieve(eq(provider.getId()), eq(zoneId)))
        .thenReturn(getLdapDefinition());
  ...
  public void patch_client_secret() throws Exception {
    for (String type : Arrays.asList(OIDC10, OAUTH20)) {
      ...
      String zoneId = IdentityZoneHolder.get().getId();
      when(identityProviderProvisioning.retrieve(eq(provider.getId()), eq(zoneId)))
          .thenReturn(getXOAuthProvider());
      identityProviderEndpoints.patchSensitiveData(provider.getId(), provider);
      verify(spy, times(1))
          .setRelyingPartySecret(eq(getXOAuthProvider().getConfig().getRelyingPartySecret()));
    }
  }
  ...
  public void retrieve_all_providers_redacts_data() throws Exception {
    ...
    ResponseEntity<List<IdentityProvider>> ldapList =
        identityProviderEndpoints.retrieveIdentityProviders("false", true);
  ...
  public void update_ldap_provider_patches_password() throws Exception {
    ...
    assertEquals(
        getLdapDefinition().getConfig().getBindPassword(),
        ((LdapIdentityProviderDefinition) captor.getValue().getConfig()).getBindPassword());
  ...
  public void create_ldap_provider_removes_password() throws Exception {
    ...
    ResponseEntity<IdentityProvider> response =
        identityProviderEndpoints.createIdentityProvider(ldapDefinition, true);
  ...
  public void create_oauth_provider_removes_password() throws Exception {
    String zoneId = IdentityZoneHolder.get().getId();
    for (String type : Arrays.asList(OIDC10, OAUTH20)) {
      IdentityProvider<AbstractXOAuthIdentityProviderDefinition> xoauthDefinition =
          getXOAuthProvider();
      ...
      ResponseEntity<IdentityProvider> response =
          identityProviderEndpoints.createIdentityProvider(xoauthDefinition, true);
      IdentityProvider created = response.getBody();
      assertNotNull(created);
      ...
      assertNull(
          ((AbstractXOAuthIdentityProviderDefinition) created.getConfig()).getRelyingPartySecret());
    }
  }
  ...
  public void testPatchIdentityProviderStatusInvalidPayload() {
    ...
    ResponseEntity responseEntity =
        identityProviderEndpoints.updateIdentityProviderStatus("123", identityProviderStatus);
