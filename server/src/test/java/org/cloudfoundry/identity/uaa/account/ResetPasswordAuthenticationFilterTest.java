...
public class ResetPasswordAuthenticationFilterTest {
  ...
  public void setup() throws Exception {
    ...
    when(service.resetPassword(any(ExpiringCode.class), eq(password)))
        .thenReturn(resetPasswordResponse);
    authenticationSuccessHandler = mock(AuthenticationSuccessHandler.class);
    entryPoint = mock(AuthenticationEntryPoint.class);
    filter =
        new ResetPasswordAuthenticationFilter(
            service, authenticationSuccessHandler, entryPoint, codeStore);
  }
  ...
  public void test_happy_day_password_reset_with_redirect() throws Exception {
    ...
    resetPasswordResponse =
        new ResetPasswordService.ResetPasswordResponse(user, "http://test.com", null);
    when(service.resetPassword(any(ExpiringCode.class), eq(password)))
        .thenReturn(resetPasswordResponse);
    happy_day_password_reset(resetPasswordResponse.getRedirectUri());
  }
  ...
  public void happy_day_password_reset(String redirectUri) throws Exception {
    ...
    verify(authenticationSuccessHandler, times(0))
        .onAuthenticationSuccess(same(request), same(response), any(Authentication.class));
    assertNull(SecurityContextHolder.getContext().getAuthentication());
    if (!StringUtils.hasText(redirectUri) || redirectUri.equals("home")) {
      verify(response, times(1))
          .sendRedirect(request.getContextPath() + "/login?success=password_reset");
    } else {
      verify(response, times(1))
          .sendRedirect(
  ...
  public void error_during_password_reset_invalid_password_exception() throws Exception {
    reset(service);
    InvalidPasswordException failed =
        new InvalidPasswordException("failed", HttpStatus.BAD_REQUEST);
    when(service.resetPassword(any(ExpiringCode.class), anyString())).thenThrow(failed);
    error_during_password_reset(failed.getClass());
    verify(service, times(1)).resetPassword(any(ExpiringCode.class), eq(password));
  }
  ...
  public AuthenticationException error_during_password_reset(Class<? extends Exception> failure)
      throws Exception {
    ArgumentCaptor<AuthenticationException> authenticationException =
    ...
    verify(authenticationSuccessHandler, times(0))
        .onAuthenticationSuccess(same(request), same(response), any(Authentication.class));
    verify(entryPoint, times(1))
        .commence(same(request), same(response), authenticationException.capture());
    assertNull(SecurityContextHolder.getContext().getAuthentication());
