...
class ScimUserUpdateDiffTest {
  ...
  private void assertUpdateIsNotAllowed() {
    assertThat(
        scimUserUpdateDiff.isAnythingOtherThanNameDifferent(scimUserID, scimUserFromRequest),
        is(false));
  }
  ...
  void setUp() {
    ...
    identityZone =
        MultitenancyFixture.identityZone(
            RandomStringUtils.randomAlphabetic(5), RandomStringUtils.randomAlphabetic(5));
    IdentityZoneHolder.set(identityZone);

    when(mockScimUserProvisioning.retrieve(scimUserID, identityZone.getId()))
        .thenReturn(scimUserFromDB);
  ...
  class WhenScimUserDoesNotExist {
    ...
    void setupNoUserInDB() {
      when(mockScimUserProvisioning.retrieve(scimUserID, identityZone.getId()))
          .thenThrow(ScimResourceNotFoundException.class);
    }
  ...
  class WhenNothingHasReallyChanged {
    ...
    class WhenSettingTheOriginToAStringWhichHasNoContent {
      ...
      class WhenTheOriginStoredInTheDbWasAnythingOtherThanUaa {
        ...
        @Test
        void
            isNotAllowedToUpdateField_BecauseAnEmptyOriginIsTreatedAsTheUaaOriginWhichIsNotTheSameAsWhatWasAlreadyStored() {
          assertUpdateIsNotAllowed();
        }
      }
    }
    ...
    class WhenThereAreAdditionalPhoneNumbersButTheFirstPhoneNumberIsTheSame {
      ...
      @Test
      void
          isAllowedToUpdateField_BecauseTheFirstPhoneNumberDidNotChangeAndOnlyTheFirstPhoneNumberActuallyGetsStored() {
        assertUpdateIsAllowed();
      }
    }
    ...
    class WhenThereAreAdditionalEmailsButThePrimaryEmailIsTheSame {
      ...
      @Test
      void
          isAllowedToUpdateField_BecauseThePrimaryEmailDidNotChangeAndOnlyThePrimaryEmailActuallyGetsStored() {
        assertUpdateIsAllowed();
      }
    }
  }
