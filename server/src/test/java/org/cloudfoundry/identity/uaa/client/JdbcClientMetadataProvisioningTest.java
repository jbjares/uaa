...
public class JdbcClientMetadataProvisioningTest extends JdbcTestBase {
  ...
  public void constraintViolation_WhenNoMatchingClientFound() throws Exception {
    ClientMetadata clientMetadata =
        createTestClientMetadata(
            generator.generate(), true, new URL("http://app.launch/url"), base64EncodedImg);
  ...
  public void retrieveClientMetadata() throws Exception {
    ...
    jdbcTemplate.execute(
        String.format(
            "insert into oauth_client_details(client_id, identity_zone_id, created_by) values ('%s', '%s', '%s')",
            clientId, IdentityZone.getUaa().getId(), randomGUID));
    ClientMetadata clientMetadata =
        createTestClientMetadata(
            clientId, true, new URL("http://app.launch/url"), base64EncodedImg);
    ClientMetadata createdClientMetadata =
        db.update(clientMetadata, IdentityZoneHolder.get().getId());

    ClientMetadata retrievedClientMetadata =
        db.retrieve(createdClientMetadata.getClientId(), IdentityZoneHolder.get().getId());
  ...
  public void retrieveAllClientMetadata() throws Exception {
    String clientId = generator.generate();
    jdbcTemplate.execute(
        "insert into oauth_client_details(client_id, identity_zone_id) values ('"
            + clientId
            + "', '"
    ...
    ClientMetadata clientMetadata2 =
        createTestClientMetadata(
            clientId2, true, new URL("http://app.launch/url"), base64EncodedImg);
    ...
    assertThat(
        clientMetadatas,
        PredicateMatcher.<ClientMetadata>has(m -> m.getClientId().equals(clientId)));
    assertThat(
        clientMetadatas,
        PredicateMatcher.<ClientMetadata>has(m -> m.getClientId().equals(clientId2)));
  }
  ...
  public void updateClientMetadata() throws Exception {
    ...
    ClientMetadata newClientMetadata =
        createTestClientMetadata(
            clientId, false, new URL("http://updated.app/launch/url"), base64EncodedImg);

    ClientMetadata updatedClientMetadata =
        db.update(newClientMetadata, IdentityZoneHolder.get().getId());
  ...
  public void test_set_and_get_ClientName() throws Exception {
    ...
    jdbcTemplate.execute(
        "insert into oauth_client_details(client_id, identity_zone_id) values ('"
            ...
            + "')");
    ClientMetadata data = createTestClientMetadata(clientId, false, null, null);
  ...
  private ClientMetadata createTestClientMetadata(
      String clientId, boolean showOnHomePage, URL appLaunchUrl, String appIcon)
