...
public class TestAccountSetup extends TestWatchman {
  ...
  private void createCfClient(RestOperations client) {
    BaseClientDetails clientDetails =
        new BaseClientDetails(
            "cf",
            "cloud_controller,openid,password",
  ...
  private List<? extends GrantedAuthority> extractAuthorities(
      Collection<Map<String, String>> groups) {
  ...
  private OAuth2RestTemplate createRestTemplate(
      OAuth2ProtectedResourceDetails resource, AccessTokenRequest accessTokenRequest) {
    ...
    client.setErrorHandler(
        new OAuth2ErrorHandler(client.getResource()) {
