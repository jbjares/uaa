...
public class InvitationsControllerTest {
  ...
  @Autowired InvitationsService invitationsService;
  ...
  @Autowired IdentityProviderProvisioning providerProvisioning;

  @Autowired UaaUserDatabase userDatabase;

  @Autowired DynamicZoneAwareAuthenticationManager zoneAwareAuthenticationManager;

  @Autowired CookieBasedCsrfTokenRepository loginCookieCsrfRepository;
  ...
  public void testAcceptInvitationsPage() throws Exception {
    ...
    when(expiringCodeStore.retrieveCode("code", IdentityZoneHolder.get().getId()))
        .thenReturn(createCode(codeData), null);
    when(expiringCodeStore.generateCode(
            anyString(), any(), eq(INVITATION.name()), eq(IdentityZoneHolder.get().getId())))
        .thenReturn(createCode(codeData));
    IdentityProvider provider = new IdentityProvider();
    provider.setType(OriginKeys.UAA);
    when(providerProvisioning.retrieveByOrigin(any(), any())).thenReturn(provider);

    mockMvc
        .perform(get("/invitations/accept").param("code", "code"))
  ...
  public void acceptInvitePage_for_unverifiedSamlUser() throws Exception {
    ...
    MvcResult result =
        mockMvc
            ...
            .andExpect(
                redirectedUrl(
                    "/saml/discovery?returnIDParam=idp&entityID=sp-entity-id&idp=test-saml&isPassive=true"))
            .andReturn();
  ...
  public void acceptInvitePage_for_unverifiedOIDCUser() throws Exception {
    ...
    MvcResult result =
        mockMvc
            ...
            .andExpect(
                redirectedUrl(
                    "https://oidc10.auth.url?client_id="
                        + definition.getRelyingPartyId()
                        + "&response_type=code&redirect_uri=http://null/login/callback/"
  ...
  public void unverifiedLdapUser_acceptsInvite_byLoggingIn() throws Exception {
    ...
    when(expiringCodeStore.retrieveCode("the_secret_code", IdentityZoneHolder.get().getId()))
        .thenReturn(
            new ExpiringCode(
                ...
                null));
    when(expiringCodeStore.generateCode(
            anyString(), any(), eq(null), eq(IdentityZoneHolder.get().getId())))
        .thenReturn(
            new ExpiringCode(
                "code",
                new Timestamp(System.currentTimeMillis()),
    ...
    when(zoneAwareAuthenticationManager.getLdapAuthenticationManager(any(), any()))
        .thenReturn(ldapAuthenticationManager);
    ...
    when(scimUserProvisioning.retrieve("user-id-001", IdentityZoneHolder.get().getId()))
        .thenReturn(invitedUser);
    when(invitationsService.acceptInvitation(anyString(), anyString()))
        .thenReturn(new InvitationsService.AcceptedInvitation("blah.test.com", new ScimUser()));
    when(expiringCodeStore.generateCode(
            anyString(), any(), eq(null), eq(IdentityZoneHolder.get().getId())))
        .thenReturn(
            new ExpiringCode(
    ...
    verify(scimUserProvisioning)
        .update(anyString(), userArgumentCaptor.capture(), eq(IdentityZoneHolder.get().getId()));
  ...
  @Test
  public void
      unverifiedLdapUser_acceptsInvite_byLoggingIn_whereEmailDoesNotMatchAuthenticatedEmail()
          throws Exception {
    Map<String, String> codeData = getInvitationsCode(LDAP);
    when(expiringCodeStore.retrieveCode("the_secret_code", IdentityZoneHolder.get().getId()))
        .thenReturn(
  ...
  public void acceptInvitePage_for_verifiedUser() throws Exception {
    ...
    when(expiringCodeStore.generateCode(
            anyString(), any(), eq(INVITATION.name()), eq(IdentityZoneHolder.get().getId())))
        .thenReturn(createCode(codeData));
    when(invitationsService.acceptInvitation(anyString(), eq("")))
        .thenReturn(new InvitationsService.AcceptedInvitation("blah.test.com", new ScimUser()));
    IdentityProvider provider = new IdentityProvider();
    provider.setType(OriginKeys.UAA);
    ...
    mockMvc.perform(get).andExpect(redirectedUrl("blah.test.com"));
  }

  private ExpiringCode createCode(Map<String, String> codeData) {
    return new ExpiringCode(
        "code",
        new Timestamp(System.currentTimeMillis()),
        JsonUtils.writeValueAsString(codeData),
        INVITATION.name());
    ...
    when(expiringCodeStore.generateCode(
        ...
        .thenReturn(
            new ExpiringCode(
                ...
                JsonUtils.writeValueAsString(codeData),
                "incorrect-code-intent"));
    doThrow(new HttpClientErrorException(BAD_REQUEST))
        .when(invitationsService)
        .acceptInvitation(eq("incorrect-code-intent"), eq(""));
  ...
  public void missing_code() throws Exception {
    ...
    verify(expiringCodeStore, never())
        .generateCode(anyString(), any(), anyString(), eq(IdentityZoneHolder.get().getId()));
  ...
  public void invalid_principal_id() throws Exception {
    ...
    when(expiringCodeStore.retrieveCode("thecode", IdentityZoneHolder.get().getId()))
        .thenReturn(
            new ExpiringCode("thecode", new Timestamp(1), codeDataString, INVITATION.name()), null);
  ...
  public void testAcceptInviteWithContraveningPassword() throws Exception {
    doThrow(new InvalidPasswordException(Arrays.asList("Msg 2c", "Msg 1c")))
    ...
    when(expiringCodeStore.retrieveCode("thecode", IdentityZoneHolder.get().getId()))
        .thenReturn(
            new ExpiringCode("thecode", new Timestamp(1), codeDataString, INVITATION.name()), null);
    when(expiringCodeStore.retrieveCode("thenewcode", IdentityZoneHolder.get().getId()))
        .thenReturn(
            new ExpiringCode("thenewcode", new Timestamp(1), codeDataString, INVITATION.name()),
            null);
    when(expiringCodeStore.generateCode(
            eq(codeDataString), any(), eq(INVITATION.name()), eq(IdentityZoneHolder.get().getId())))
        .thenReturn(
            new ExpiringCode("thenewcode", new Timestamp(1), codeDataString, INVITATION.name()),
    ...
    verify(expiringCodeStore).retrieveCode("thecode", IdentityZoneHolder.get().getId());
    verify(expiringCodeStore, times(2))
        .generateCode(anyString(), any(), anyString(), eq(IdentityZoneHolder.get().getId()));
    verify(invitationsService, never()).acceptInvitation(anyString(), anyString());
  }
  ...
  public void testAcceptInvite() throws Exception {
    ...
    Map<String, String> codeData = getInvitationsCode(OriginKeys.UAA);
    String codeDataString = JsonUtils.writeValueAsString(codeData);
    ExpiringCode thecode =
        new ExpiringCode("thecode", new Timestamp(1), codeDataString, INVITATION.name());
    ExpiringCode thenewcode =
        new ExpiringCode("thenewcode", new Timestamp(1), codeDataString, INVITATION.name());
    ExpiringCode thenewcode2 =
        new ExpiringCode("thenewcode2", new Timestamp(1), codeDataString, INVITATION.name());
    when(expiringCodeStore.retrieveCode("thecode", IdentityZoneHolder.get().getId()))
        .thenReturn(thecode, null);
    when(expiringCodeStore.retrieveCode("thenewcode", IdentityZoneHolder.get().getId()))
        .thenReturn(thenewcode, null);
    when(expiringCodeStore.generateCode(
            eq(codeDataString), any(), eq(INVITATION.name()), eq(IdentityZoneHolder.get().getId())))
        .thenReturn(thenewcode)
        .thenReturn(thenewcode2);

    when(invitationsService.acceptInvitation(anyString(), eq("passw0rd")))
        .thenReturn(new InvitationsService.AcceptedInvitation("/home", user));
  ...
  private MockHttpServletRequestBuilder startAcceptInviteFlow(
    ...
    UsernamePasswordAuthenticationToken token =
        new UsernamePasswordAuthenticationToken(uaaPrincipal, null, UaaAuthority.USER_AUTHORITIES);
  ...
  public void acceptInviteWithValidClientRedirect() throws Exception {
    ...
    when(expiringCodeStore.generateCode(
        ...
        .thenReturn(
            new ExpiringCode("thenewcode", new Timestamp(1), codeDataString, INVITATION.name()));
    when(invitationsService.acceptInvitation(anyString(), eq("password")))
        .thenReturn(new InvitationsService.AcceptedInvitation("valid.redirect.com", user));
    ...
    mockMvc
        ...
        .andExpect(status().isFound())
        .andExpect(
            redirectedUrl("/login?success=invite_accepted&form_redirect_uri=valid.redirect.com"));
  }
  ...
  public void invalidCodeOnAcceptPost() throws Exception {
    ...
    doThrow(new HttpClientErrorException(BAD_REQUEST))
        .when(invitationsService)
        .acceptInvitation(anyString(), anyString());
  ...
  public void testAcceptInvite_displaysConsentText() throws Exception {
    ...
    branding.setConsent(
        new Consent("paying Jaskanwal Pawar & Jennifer Hamon each a million dollars", null));
    defaultZone.getConfig().setBranding(branding);

    IdentityProvider identityProvider = new IdentityProvider();
    identityProvider.setType(OriginKeys.UAA);
    when(providerProvisioning.retrieveByOrigin(anyString(), anyString()))
    ...
    mockMvc
        .perform(get("/invitations/accept").param("code", "thecode"))
  ...
  @Test
  public void testAcceptInvite_doesNotDisplayConsentCheckboxWhenNotConfiguredForZone()
      throws Exception {
    IdentityProvider identityProvider = new IdentityProvider();
    identityProvider.setType(OriginKeys.UAA);
    when(providerProvisioning.retrieveByOrigin(anyString(), anyString()))
  ...
  public void testAcceptInvite_displaysErrorMessageIfConsentNotChecked() throws Exception {
    ...
    MvcResult mvcResult =
        mockMvc.perform(startAcceptInviteFlow("password", "password")).andReturn();

    mockMvc
        .perform(get("/invitations/" + mvcResult.getResponse().getHeader("Location")))
  ...
  public void testAcceptInvite_worksWithConsentProvided() throws Exception {
    ...
    MvcResult mvcResult =
        mockMvc
            .perform(
                startAcceptInviteFlow("password", "password").param("does_user_consent", "true"))
            .andReturn();
    assertThat(
        mvcResult.getResponse().getHeader("Location"),
        containsString(codeData.get("redirect_uri")));
