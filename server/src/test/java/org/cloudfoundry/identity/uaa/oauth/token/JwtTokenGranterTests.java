...
public class JwtTokenGranterTests {
  ...
  public void setUp() throws Exception {
    ...
    tokenRequest =
        new TokenRequest(
            Collections.emptyMap(), "client_ID", Collections.emptySet(), GRANT_TYPE_JWT_BEARER);
    ...
    UaaUser user =
        new UaaUser(
            ...
            new Date());
    uaaAuthentication =
        new UaaAuthentication(new UaaPrincipal(user), Collections.emptyList(), null);
    SecurityContextHolder.getContext().setAuthentication(authentication);

    client = new BaseClientDetails("clientID", null, "uaa.user", GRANT_TYPE_JWT_BEARER, null);
    when(clientDetailsService.loadClientByClientId(eq(client.getClientId()), anyString()))
        .thenReturn(client);
