...
public class TokenTestSupport {
  ...
  public static final String ALL_GRANTS_CSV =
  ...
  List<? extends GrantedAuthority> defaultUserAuthorities =
      Arrays.asList(
  ...
  public TokenTestSupport(UaaTokenEnhancer tokenEnhancer) throws Exception {
    ...
    defaultUserAuthentication =
        new UsernamePasswordAuthenticationToken(new UaaPrincipal(defaultUser), "n/a", null);
    ...
    when(tokenProvisioning.create(any(), anyString()))
        .thenAnswer(
            (Answer<RevocableToken>)
                invocation -> {
                  RevocableToken arg = (RevocableToken) invocation.getArguments()[0];
                  tokens.put(arg.getTokenId(), arg);
                  return arg;
                });
    when(tokenProvisioning.update(anyString(), any(), anyString()))
        .thenAnswer(
            (Answer<RevocableToken>)
                invocation -> {
    ...
    when(tokenProvisioning.retrieve(anyString(), anyString()))
        .thenAnswer(
            (Answer<RevocableToken>)
                invocation -> {
                  String id = (String) invocation.getArguments()[0];
    ...
    TokenValidityResolver refreshTokenValidityResolver =
        new TokenValidityResolver(
            new ClientRefreshTokenValidity(clientDetailsService), 12345, timeService);
    TokenValidityResolver accessTokenValidityResolver =
        new TokenValidityResolver(
            new ClientAccessTokenValidity(clientDetailsService), 1234, timeService);
    ...
    refreshTokenCreator =
        new RefreshTokenCreator(
            false, refreshTokenValidityResolver, tokenEndpointBuilder, timeService, keyInfoService);
    tokenServices =
    ...
    IdentityZoneHolder.get()
        .getConfig()
        .getUserConfig()
        .setDefaultGroups(new LinkedList<>(AuthorityUtils.authorityListToSet(USER_AUTHORITIES)));
  }
  ...
  public CompositeToken getCompositeAccessToken(List<String> scopes) {
    UaaPrincipal uaaPrincipal =
        new UaaPrincipal(
            ...
            defaultUser.getOrigin(),
            defaultUser.getExternalId(),
    ...
    userAuthentication.setAuthContextClassRef(
        new HashSet<>(Arrays.asList(AuthnContext.PASSWORD_AUTHN_CTX)));
    ...
    OAuth2Request oAuth2Request =
        new OAuth2Request(
            ...
            null,
            null,
            Sets.newHashSet("token", "id_token"),
            null);

    UaaOauth2Authentication uaaOauth2Authentication =
        new UaaOauth2Authentication(
            null, IdentityZoneHolder.get().getId(), oAuth2Request, userAuthentication);
  ...
  public void copyClients(String fromZoneId, String toZoneId) {
    getClientDetailsService()
        .setClientDetailsStore(toZoneId, getClientDetailsService().getInMemoryService(fromZoneId));
  }
}
