...
public class ApprovalsAdminEndpointsTests extends JdbcTestBase {
  ...
  public void initApprovalsAdminEndpointsTests() {
    ...
    MultitenantJdbcClientDetailsService clientDetailsService =
    ...
    endpoints.setSecurityContextAccessor(
        mockSecurityContextAccessor(marissa.getUsername(), marissa.getId()));
  }

  private void addApproval(
      String userName, String clientId, String scope, int expiresIn, ApprovalStatus status) {
    dao.addApproval(
        new Approval()
            .setUserId(userName)
  ...
  public void testApprovalsDeserializationIsCaseInsensitive() throws Exception {
    ...
    Set<Approval> deserializedApprovals =
        JsonUtils.readValue(
            "[{\"userid\":\"test-user-id\",\"clientid\":\"testclientid\",\"scope\":\"scope\",\"status\":\"APPROVED\",\"expiresat\":\"2015-08-25T14:35:42.512Z\",\"lastupdatedat\":\"2015-08-25T14:35:42.512Z\"}]",
            new TypeReference<Set<Approval>>() {});
  ...
  public void canGetApprovalsWithAutoApproveTrue() {
    ...
    assertEquals(
        3, endpoints.getApprovals("user_id eq \"" + marissa.getId() + "\"", 1, 100).size());
  ...
  public void canUpdateApprovals() {
    ...
    List<Approval> updatedApprovals =
        endpoints.getApprovals("user_id eq \"" + marissa.getId() + "\"", 1, 100);
  ...
  public void revokeApprovalsCountForClientAndUser(String clientId, String userId) {
    assertTrue(
        dao.revokeApprovalsForClientAndUser(clientId, userId, IdentityZoneHolder.get().getId()));
  }
  ...
  public void addApprovals(
      final Integer minUserId, final Integer maxUserId, final Integer countPerUser)
      throws Exception {
    jdbcTemplate.batchUpdate(
        "insert into authz_approvals (user_id, client_id, scope, expiresat, status, lastmodifiedat) values (?,?,?,?,?,?)",
        new BatchPreparedStatementSetter() {
          @Override
          public void setValues(PreparedStatement ps, int i) throws SQLException {
