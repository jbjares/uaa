...
import static org.cloudfoundry.identity.uaa.mfa.MfaUiRequiredFilter.MfaNextStep.*;
...
public class MfaUiRequiredFilterTests {
  ...
  public void setup() throws Exception {
    ...
    usernameAuthentication =
        new UsernamePasswordAuthenticationToken("fake-principal", "fake-credentials");
    anonymous =
        new AnonymousAuthenticationToken(
            "fake-key", "fake-principal", singletonList(new SimpleGrantedAuthority("test")));
    authentication =
  ...
  public void authentication_log_info_unknown() throws Exception {
    ...
    assertThat(
        spyFilter.getAuthenticationLogInfo(),
        containsString(
            "Unknown Auth=org.springframework.security.authentication.UsernamePasswordAuthenticationToken"));
  ...
  @Test
  public void next_step_mfa_not_needed_when_origin_key_does_not_match_valid_identity_provider()
      throws Exception {
    SecurityContextHolder.getContext().setAuthentication(authentication);
  ...
  @Test
  public void next_step_mfa_needed_when_origin_key_matches_valid_identity_provider()
      throws Exception {
  ...
  public void do_filter_mfa_required() throws Exception {
    ...
    verify(spyFilter, times(1))
        .sendRedirect(eq("/login/mfa/register"), same(request), same(response));
  }
}
