...
public class CheckTokenEndpointTests {
  ...
  private InMemoryClientServicesExtentions clientDetailsService =
      new InMemoryClientServicesExtentions();
  ...
  public void setUp(boolean opaque) throws Exception {
    ...
    clientDetailsService.setClientDetailsStore(
        IdentityZoneHolder.get().getId(), clientDetailsStore);
    ...
    tokenServices =
        new UaaTokenServices(
            ...
            new TokenValidityResolver(
                new ClientAccessTokenValidity(clientDetailsService),
                Integer.MAX_VALUE,
                timeService),
            userDatabase,
            Sets.newHashSet(),
  ...
  public void testClientWildcard() throws Exception {
    ...
    endpoint.checkToken(
        tokenServices.createAccessToken(authentication).getValue(),
  ...
  public void testValidateScopesMultipleNotPresent() throws Exception {
    try {
      authentication =
          new OAuth2Authentication(
              new AuthorizationRequest("client", Collections.singletonList("cat.pet"))
                  .createOAuth2Request(),
              null);
      OAuth2AccessToken accessToken = tokenServices.createAccessToken(authentication);

      endpoint.checkToken(
          accessToken.getValue(), Arrays.asList("scim.write", "scim.read"), request);
    } catch (InvalidScopeException ex) {
  ...
  public void testValidateScopesMultiplePresent() throws Exception {
    authentication =
        new OAuth2Authentication(
            new AuthorizationRequest("client", Arrays.asList("scim.read", "scim.write"))
                .createOAuth2Request(),
            null);
    OAuth2AccessToken accessToken = tokenServices.createAccessToken(authentication);
  ...
  public void testValidateScopesSomeNotPresent() throws Exception {
    try {
      ...
      endpoint.checkToken(
          accessToken.getValue(), Arrays.asList("scim.read", "ponies.ride"), request);
    } catch (InvalidScopeException ex) {
  ...
  public void testExpiredToken() throws Exception {
    ...
    Map<String, BaseClientDetails> clientDetailsStore =
    ...
    when(timeService.getCurrentTimeMillis())
        .thenReturn(nowMillis + validitySeconds.longValue() * 1000 + 1L);
    endpoint.checkToken(accessToken.getValue(), Collections.emptyList(), request);
  }
