...
public class JdbcScimUserProvisioningTests extends JdbcTestBase {
  ...
  private static final String SQL_INJECTION_FIELDS =
      "password,version,created,lastModified,username,email,givenName,familyName";

  private static final String ADD_USER_SQL_FORMAT =
      "insert into users (id, username, password, email, givenName, familyName, phoneNumber, identity_zone_id) values ('%s','%s','%s','%s','%s', '%s', '%s', '%s')";

  private static final String OLD_ADD_USER_SQL_FORMAT =
      "insert into users (id, username, password, email, givenName, familyName, phoneNumber) values ('%s','%s','%s','%s','%s', '%s', '%s')";
  ...
  private static final String INSERT_MEMBERSHIP =
      "insert into group_membership (group_id, member_id, member_type,authorities,added, origin, identity_zone_id) values (?,?,?,?,?,?,?)";
  ...
  protected void addMembership(String userId, String origin) {
    ...
    jdbcTemplate.update(
        INSERT_MEMBERSHIP, userId, userId, "USER", "authorities", timestamp, origin, zoneId);
  }
  ...
  public void test_can_delete_provider_users_in_other_zone() throws Exception {
    ...
    assertThat(
        jdbcTemplate.queryForObject(
            "select count(*) from users where origin=? and identity_zone_id=?",
            new Object[] {LOGIN_SERVER, zone.getId()},
            Integer.class),
        is(1));
    addMembership(created.getId(), created.getOrigin());
    assertThat(
  ...
  public void canCreateUserInDefaultIdentityZone() {
    ...
    assertTrue(
        Math.abs(
                created.getMeta().getCreated().getTime()
                    - created.getPasswordLastModified().getTime())
            < 1001); // 1 second at most given MySQL fractionless timestamp
  }
  ...
  public void canModifyPassword() throws Exception {
    ...
    Thread.sleep(10);
    db.changePassword(
        created.getId(), "j7hyqpassX", "j7hyqpassXXX", IdentityZoneHolder.get().getId());

    user = db.retrieve(created.getId(), IdentityZoneHolder.get().getId());
    assertNotNull(user.getPasswordLastModified());
    assertTrue(
        Math.abs(
                user.getMeta().getLastModified().getTime()
  ...
  public void testSetPasswordChangeRequired() {
    ...
    ScimUser created = db.createUser(user, "j7hyqpassX", IdentityZoneHolder.get().getId());
    assertFalse(
        db.checkPasswordChangeIndividuallyRequired(
            created.getId(), IdentityZoneHolder.get().getId()));
  ...
  public void canReadScimUserWithMissingEmail() {
    ...
    JdbcScimUserProvisioning noValidateProvisioning =
        new JdbcScimUserProvisioning(jdbcTemplate, pagingListFactory) {
          @Override
    ...
    String createdUserId =
        noValidateProvisioning.create(nohbdy, IdentityZoneHolder.get().getId()).getId();
  ...
  public void canChangePasswordWithoutOldPassword() throws Exception {
    ...
    String storedPassword =
        jdbcTemplate.queryForObject("SELECT password from users where ID=?", String.class, JOE_ID);
  ...
  public void canDeactivateExistingUser() {
    ...
    assertEquals(
        1,
        db.query(
                "username eq \"" + tmpUserId + "\" and active eq false",
  ...
  public void canDeleteExistingUserThroughEvent() {
    ...
    assertEquals(
        0, db.query("username eq \"" + tmpUserId + "\"", IdentityZoneHolder.get().getId()).size());
  }
  ...
  public void canDeleteExistingUserAndThenCreateHimAgain() {
    ...
    assertEquals(
        1, db.query("username eq \"" + tmpUserId + "\"", IdentityZoneHolder.get().getId()).size());
  ...
  public void testUpdatedIncorrectVersionUserVerified() {
    ...
    try {
      ...
      assertFalse(user.isVerified());
      user =
          db.verifyUser(tmpUserIdString, user.getVersion() + 50, IdentityZoneHolder.get().getId());
      assertTrue(user.isVerified());
    } finally {
      removeUser(tmpUserIdString);
    }
  ...
  public void canRetrieveUsersWithFilterGreater() {
    assertEquals(
        1 + existingUserCount,
        db.query("username gt \"joe\"", IdentityZoneHolder.get().getId()).size());
  }
  ...
  public void canRetrieveUsersWithGroupsFilter() {
    List<ScimUser> users =
        db.query("groups.display co \"uaa.user\"", IdentityZoneHolder.get().getId());
  ...
  public void canRetrieveUsersWithPhoneNumberFilter() {
    assertEquals(
        1, db.query("phoneNumbers.value sw \"+1-222\"", IdentityZoneHolder.get().getId()).size());
  }
  ...
  public void canRetrieveUsersWithMetaVersionFilter() {
    assertEquals(
        1,
        db.query("userName eq \"joe\" and meta.version eq 0", IdentityZoneHolder.get().getId())
            .size());
  }
  ...
  public void canRetrieveUsersWithMetaDateFilter() {
    assertEquals(
        2 + existingUserCount,
        db.query("meta.created gt \"1970-01-01T00:00:00.000Z\"", IdentityZoneHolder.get().getId())
            .size());
  }
  ...
  public void canRetrieveUsersWithBooleanFilter() {
    assertEquals(
        2 + existingUserCount,
        db.query("username pr and active eq true", IdentityZoneHolder.get().getId()).size());
  }
  ...
  public void canRetrieveUsersWithSortBy() {
    assertEquals(
        2 + existingUserCount,
        db.query("username pr", "username", true, IdentityZoneHolder.get().getId()).size());
  }
  ...
  public void canRetrieveUsersWithSortByEmail() {
    assertEquals(
        2 + existingUserCount,
        db.query("username pr", "emails.value", true, IdentityZoneHolder.get().getId()).size());
  }
  ...
  public void canRetrieveUsersWithFilterBooleanAnd() {
    assertEquals(
        2,
        db.query("username pr and emails.value co \".com\"", IdentityZoneHolder.get().getId())
            .size());
  }
  ...
  public void canRetrieveUsersWithFilterBooleanOr() {
    assertEquals(
        2,
        db.query(
                "username eq \"joe\" or emails.value co \".com\"", IdentityZoneHolder.get().getId())
            .size());
  }
  ...
  public void canRetrieveUsersWithFilterBooleanOrMatchesSecond() {
    assertEquals(
        1,
        db.query("username eq \"foo\" or username eq \"joe\"", IdentityZoneHolder.get().getId())
            .size());
  }
  ...
  public void cannotRetrieveUsersWithIllegalPhoneNumberFilterField() {
    assertEquals(
        2, db.query("phoneNumbers.type eq \"bar\"", IdentityZoneHolder.get().getId()).size());
  }
  ...
  public void cannotRetrieveUsersWithNativeSqlInjectionAttack() {
    String password =
        jdbcTemplate.queryForObject(
            "select password from users where username='joe'", String.class);
    assertNotNull(password);
    Collection<ScimUser> users =
        db.query(
            "username=\"joe\"; select " + SQL_INJECTION_FIELDS + " from users where username='joe'",
            IdentityZoneHolder.get().getId());
  ...
  public void cannotRetrieveUsersWithSqlInjectionAttackOnGt() {
    ...
    Collection<ScimUser> users =
        db.query(
            "username gt \"h\"; select "
                + SQL_INJECTION_FIELDS
  ...
  public void cannotRetrieveUsersWithAnotherSqlInjectionAttack() {
    ...
    Collection<ScimUser> users =
        db.query(
            "username eq \"joe\"\"; select id from users where id='''; select "
                + SQL_INJECTION_FIELDS
                + " from users where username='joe'",
            IdentityZoneHolder.get().getId());
    assertEquals(password, users.iterator().next().getId());
  ...
  public void checkPasswordMatches_ReturnsFalse_newPasswordSameAsOld() {
    assertFalse(
        db.checkPasswordMatches(JOE_ID, "notjoepassword", IdentityZoneHolder.get().getId()));
  }
