...
public class JdbcUnsuccessfulLoginCountingAuditServiceTests extends JdbcTestBase {
  ...
  public void userAuthenticationFailureAuditSucceeds() throws Exception {
    auditService.log(
        getAuditEvent(UserAuthenticationFailure, "1", "joe"),
        getAuditEvent(UserAuthenticationFailure, "1", "joe").getIdentityZoneId());
    Thread.sleep(100);
    auditService.log(
        getAuditEvent(UserAuthenticationFailure, "1", "joe"),
        getAuditEvent(UserAuthenticationFailure, "1", "joe").getIdentityZoneId());
  ...
  public void userAuthenticationFailureDeletesOldData() throws Exception {
    ...
    assertThat(
        jdbcTemplate.queryForObject(
            "select count(*) from sec_audit where principal_id='1'", Integer.class),
        is(1));
    ReflectionTestUtils.invokeMethod(
        ReflectionTestUtils.getField(auditService, "lastDelete"), "set", 0l);
  ...
  public void userAuthenticationSuccessResetsData() throws Exception {
    ...
    assertThat(
        ...
        is(1));
    auditService.log(
        getAuditEvent(UserAuthenticationSuccess, "1", "joe"),
        getAuditEvent(UserAuthenticationSuccess, "1", "joe").getIdentityZoneId());
    assertThat(
        jdbcTemplate.queryForObject(
            "select count(*) from sec_audit where principal_id='1'", Integer.class),
        is(0));
  ...
  public void userPasswordChangeSuccessResetsData() throws Exception {
    ...
    assertThat(
        ...
        is(1));
    auditService.log(
        getAuditEvent(PasswordChangeSuccess, "1", "joe"),
        getAuditEvent(PasswordChangeSuccess, "1", "joe").getIdentityZoneId());
    assertThat(
        jdbcTemplate.queryForObject(
            "select count(*) from sec_audit where principal_id='1'", Integer.class),
        is(0));
  ...
  public void findMethodOnlyReturnsEventsWithinRequestedPeriod() {
    ...
    auditService.log(
        ...
        getAuditEvent(UserAuthenticationFailure, "1", "joe").getIdentityZoneId());
    auditService.log(
        getAuditEvent(ClientAuthenticationFailure, "client", "testman"),
        getAuditEvent(ClientAuthenticationFailure, "client", "testman").getIdentityZoneId());
    ...
    auditService.log(
        ...
        getAuditEvent(UserAuthenticationFailure, "1", "joe").getIdentityZoneId());
    auditService.log(
        getAuditEvent(UserAuthenticationFailure, "2", "joe"),
        getAuditEvent(UserAuthenticationFailure, "2", "joe").getIdentityZoneId());
    auditService.log(
        getAuditEvent(ClientAuthenticationFailure, "client", "testman"),
        getAuditEvent(ClientAuthenticationFailure, "client", "testman").getIdentityZoneId());
    auditService.log(
        getAuditEvent(ClientAuthenticationFailure, "otherclient", "testman"),
        getAuditEvent(ClientAuthenticationFailure, "otherclient", "testman").getIdentityZoneId());
    // Find events within last 2 mins
    List<AuditEvent> userEvents =
        auditService.find("1", now - 120 * 1000, IdentityZoneHolder.get().getId());
    List<AuditEvent> clientEvents =
        auditService.find("client", now - 120 * 1000, IdentityZoneHolder.get().getId());
  ...
  public void mfaFailedEventsAreLogged() {
    ...
    assertThat(
        auditService.find(principalId, 0, mfaFailureEvent.getIdentityZoneId()), is(hasSize(1)));
  }
  ...
  public void nontAuthSuccessesShouldNotThrowAnException() {
    EnumSet<AuditEventType> userAuthenticationSuccess =
        EnumSet.of(
            UserAuthenticationSuccess,
            PasswordChangeSuccess,
            UserAccountUnlockedEvent,
            MfaAuthenticationSuccess);
    EnumSet<AuditEventType> complementOfUserAuthenticationSuccess =
        EnumSet.complementOf(userAuthenticationSuccess);

    for (AuditEventType ofUserAuthenticationSuccess : complementOfUserAuthenticationSuccess) {
      AuditEvent auditEvent =
          new AuditEvent(
              ofUserAuthenticationSuccess,
              "1",
              authDetails,
              "joe",
              System.currentTimeMillis(),
