...
public class SamlIdentityProviderConfiguratorTests {
  ...
  private String getSimpleSamlPhpMetadata(String domain) {
    return "<?xml version=\"1.0\"?>\n"
        + "<md:EntityDescriptor xmlns:md=\"urn:oasis:names:tc:SAML:2.0:metadata\" xmlns:ds=\"http://www.w3.org/2000/09/xmldsig#\" entityID=\""
        + domain
        + "/saml2/idp/metadata.php\" ID=\"pfx214e4cdb-8fb2-592d-27a1-32ff06dcda69\"><ds:Signature>\n"
  ...
  public static final String xml =
      String.format(xmlWithoutID, "http://www.okta.com/k2lw4l5bPODCMIIDBRYZ");

  public static final String xmlWithoutHeader =
      xmlWithoutID.replace("<?xml version=\"1.0\" encoding=\"UTF-8\"?>", "");
  ...
  public void testGetEntityID() throws Exception {
    ...
    Timer t = new Timer();
    bootstrap.setIdentityProviders(
        BootstrapSamlIdentityProviderDataTests.parseYaml(
            BootstrapSamlIdentityProviderDataTests.sampleYaml));
    ...
    for (SamlIdentityProviderDefinition def : bootstrap.getIdentityProviderDefinitions()) {
      switch (def.getIdpEntityAlias()) {
        case "okta-local":
          {
            ComparableProvider provider =
                (ComparableProvider)
                    configurator.getExtendedMetadataDelegateFromCache(def).getDelegate();
        ...
        case "simplesamlphp-url":
          {
            when(fixedHttpMetaDataProvider.fetchMetadata(any(), anyBoolean()))
                .thenReturn(
            ...
            ComparableProvider provider =
                (ComparableProvider)
                    configurator.getExtendedMetadataDelegateFromCache(def).getDelegate();
            assertEquals(
                "http://simplesamlphp.somewhere.com/saml2/idp/metadata.php",
  ...
  protected List<SamlIdentityProviderDefinition> getSamlIdentityProviderDefinitions(
      List<String> clientIdpAliases) {
    SamlIdentityProviderDefinition def1 =
        new SamlIdentityProviderDefinition()
  ...
  public void testGetIdentityProviderDefinititonsForAllowedProviders() throws Exception {
    ...
    List<SamlIdentityProviderDefinition> clientIdps =
        getSamlIdentityProviderDefinitions(clientIdpAliases);
  ...
  public void shouldTimeoutWhenFetchingMetadataURL() throws Exception {
    ...
    Assertions.assertTimeout(
        ...
        () -> {
          Assertions.assertThrows(
              NullPointerException.class, () -> configurator.configureURLMetadata(def));
        });
  }
}
