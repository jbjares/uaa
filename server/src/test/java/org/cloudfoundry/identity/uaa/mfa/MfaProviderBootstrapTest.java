...
public class MfaProviderBootstrapTest extends JdbcTestBase {
  ...
  public void testBootstrapWithSomeExistingProviders() throws Exception {
    ...
    reset(provisioning);
    bootstrap.setMfaProviders(
        parseMfaYaml(sampleMfaYaml.replace("mfa provider description", "new description")));
    bootstrap.afterPropertiesSet();
    ...
    assertEquals(
        "new description",
        ((GoogleMfaProviderConfig) captor.getValue().getConfig()).getProviderDescription());
    assertEquals(
        "new description",
        ((GoogleMfaProviderConfig)
                provisioning
                    .retrieveByName(expectedGoogleProviders.get(0).getName(), "uaa")
                    .getConfig())
  ...
  public Map<String, Map<String, Object>> parseMfaYaml(String sampleYaml) {
    ...
    for (Map.Entry<String, Map<String, Object>> entry :
        ((Map<String, Map<String, Object>>) tmpdata.get("mfa-providers")).entrySet()) {
