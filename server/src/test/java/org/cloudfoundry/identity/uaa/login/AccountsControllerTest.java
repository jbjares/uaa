...
public class AccountsControllerTest extends TestClassNullifier {
  ...
  @Autowired IdentityProviderProvisioning identityProviderProvisioning;
  ...
  public void tearDown() throws Exception {
    ...
    IdentityZoneHolder.get()
        ...
        .getLinks()
        .getSelfService()
        .setSelfServiceLinksEnabled(selfServiceToReset);
  }
  ...
  public void testSendActivationEmail() throws Exception {
    ...
    Mockito.verify(accountCreationService)
        .beginActivation("user1@example.com", "password", "app", "http://example.com/redirect");
  }
  ...
  public void testAttemptCreateAccountWithEmailDomainRestriction() throws Exception {
    ...
    MockHttpServletRequestBuilder post =
        post("/create_account.do")
            ...
            .param("client_id", "app")
            .param("redirect_uri", "http://example.com/redirect");
    IdentityProvider<OIDCIdentityProviderDefinition> oidcProvider =
    ...
    when(identityProviderProvisioning.retrieveAll(true, OriginKeys.UAA))
        .thenReturn(Collections.singletonList(oidcProvider));
    ...
    Mockito.verify(accountCreationService, times(0))
        .beginActivation("user1@example.com", "password", "app", "http://example.com/redirect");
  }
  ...
  public void testSendActivationEmailWithUserNameConflict() throws Exception {
    doThrow(new UaaException("username already exists", 409))
        .when(accountCreationService)
        .beginActivation("user1@example.com", "password", "app", null);
  ...
  public void testInvalidPassword() throws Exception {
    doThrow(new InvalidPasswordException(Arrays.asList("Msg 2", "Msg 1")))
        .when(accountCreationService)
        .beginActivation("user1@example.com", "password", "app", null);
  ...
  public void testVerifyUser() throws Exception {
    when(accountCreationService.completeActivation("the_secret_code"))
        .thenReturn(
            new AccountCreationService.AccountCreationResponse(
                "newly-created-user-id", "username", "user@example.com", "//example.com/callback"));

    MockHttpServletRequestBuilder get = get("/verify_user").param("code", "the_secret_code");
    ...
    mockMvc
        ...
        .andExpect(
            redirectedUrl(
                "/login?success=verify_success&form_redirect_uri=//example.com/callback"));
