...
public class UaaTokenServicesTests {
  @Autowired private UaaTokenServices tokenServices;
  ...
  @Autowired private JdbcUaaUserDatabase jdbcUaaUserDatabase;
  ...
  class WhenRequestingAnIdToken {
    ...
    public void ensureJKUHeaderIsSetWhenBuildingAnIdToken() {
      AuthorizationRequest authorizationRequest =
          constructAuthorizationRequest(clientId, GRANT_TYPE_PASSWORD, requestedScope);

      OAuth2Authentication auth2Authentication =
          constructUserAuthenticationFromAuthzRequest(authorizationRequest, "admin", "uaa");

      CompositeToken accessToken =
          (CompositeToken) tokenServices.createAccessToken(auth2Authentication);

      Jwt jwtToken = JwtHelper.decode(accessToken.getIdTokenValue());
      assertThat(jwtToken.getHeader().getJku(), startsWith(uaaUrl));
      assertThat(
          jwtToken.getHeader().getJku(), is("https://uaa.some.test.domain.com:555/uaa/token_keys"));
    }

    @Tag("oidc spec")
    @Tag("uaa oidc logic")
    @DisplayName(
        "ensureIdToken Returned when Client Has OpenId Scope and Scope=OpenId withGrantType")
    @ParameterizedTest
    @ValueSource(
        strings = {GRANT_TYPE_PASSWORD, GRANT_TYPE_AUTHORIZATION_CODE, GRANT_TYPE_IMPLICIT})
    public void ensureIdTokenReturned_withGrantType(String grantType) {
      AuthorizationRequest authorizationRequest =
          constructAuthorizationRequest(clientId, grantType, requestedScope);
        ...
        assertAll(
            ...
            () ->
                assertThat(
                    ...
                    containsString(
                        "an ID token was requested but 'openid' is missing from the requested scopes")));
      }
    }
    ...
    class WhenUserHasNotApprovedOpenIdScope {
      ...
      @Autowired private JdbcApprovalStore jdbcApprovalStore;
      ...
      void setupRequest() {
        ...
        Approval approvedNonOpenIdScope =
            new Approval()
                .setUserId("admin")
                .setScope("oauth.approvals")
                .setClientId(clientId)
                .setExpiresAt(DateTime.now().plusDays(1).toDate())
  ...
  public void ensureJKUHeaderIsSetWhenBuildingAnAccessToken() {
    AuthorizationRequest authorizationRequest =
        constructAuthorizationRequest(
            clientId, GRANT_TYPE_CLIENT_CREDENTIALS, Strings.split(clientScopes, ','));
    ...
    assertThat(
        decode.getHeader().getJku(), is("https://uaa.some.test.domain.com:555/uaa/token_keys"));
  }

  @Test
  public void ensureJKUHeaderIsSetWhenBuildingARefreshToken() {
    AuthorizationRequest authorizationRequest =
        constructAuthorizationRequest(clientId, GRANT_TYPE_PASSWORD, "oauth.approvals");
  ...
  class WhenRefreshGrant {
    ...
    class WhenAcrClaimIsPresent {
      ...
      void setup(Set<String> acrs) {
        ...
        UaaUser uaaUser = jdbcUaaUserDatabase.retrieveUserByName("admin", "uaa");
        refreshToken =
            refreshTokenCreator.createRefreshToken(uaaUser, refreshTokenRequestData, null);
        assertThat(refreshToken, is(notNullValue()));
      }

      @ParameterizedTest
      @MethodSource(
          "org.cloudfoundry.identity.uaa.oauth.UaaTokenServicesTests#authenticationTestParams")
      @DisplayName("an ID token is returned with ACR claim")
      public void happyCase(List<String> acrs) {
        setup(new HashSet<>(acrs));

        CompositeToken refreshedToken =
            (CompositeToken)
                tokenServices.refreshAccessToken(
        ...
        List<String> values =
            (List<String>) ((Map<String, Object>) claims.get(ClaimConstants.ACR)).get("values");
    ...
    class WhenOpenIdScopeNotRequested {
      ...
      public void idTokenNotReturned(String grantType) {
        ...
        AuthorizationRequest authorizationRequest =
            constructAuthorizationRequest(clientId, grantType, nonOpenIdScope);
        OAuth2Authentication auth2Authentication =
            constructUserAuthenticationFromAuthzRequest(authorizationRequest, "admin", "uaa");
    ...
    class WhenClientDoesNotHaveOpenIdScope {
      ...
      public void idTokenNotReturned(String grantType) {
        ...
        AuthorizationRequest authorizationRequest =
            constructAuthorizationRequest("client_without_openid", grantType, nonOpenIdScope);
        OAuth2Authentication auth2Authentication =
            constructUserAuthenticationFromAuthzRequest(authorizationRequest, "admin", "uaa");
        CompositeToken compositeToken =
            (CompositeToken) tokenServices.createAccessToken(auth2Authentication);
    ...
    class WhenScopingDownToExcludeOpenIdScope {
      ...
      public void idTokenNotReturned(String grantType) {
        AuthorizationRequest authorizationRequest =
            constructAuthorizationRequest("jku_test", grantType, "openid", "user_attributes");
        OAuth2Authentication auth2Authentication =
            constructUserAuthenticationFromAuthzRequest(authorizationRequest, "admin", "uaa");
        CompositeToken compositeToken =
            (CompositeToken) tokenServices.createAccessToken(auth2Authentication);
            ...
            (CompositeToken)
                tokenServices.refreshAccessToken(
                    ...
                    new TokenRequest(
                        Maps.newHashMap(),
                        "jku_test",
                        Sets.newHashSet("user_attributes"),
                        GRANT_TYPE_REFRESH_TOKEN));
  ...
  private OAuth2Authentication constructUserAuthenticationFromAuthzRequest(
  ...
  private AuthorizationRequest constructAuthorizationRequest(
      String clientId, String grantType, String... scopes) {
    AuthorizationRequest authorizationRequest =
        new AuthorizationRequest(clientId, Arrays.asList(scopes));
