...
public class PasswordResetEndpointTest extends TestClassNullifier {
  ...
  public void setUp() throws Exception {
    ...
    controller.setMessageConverters(
    ...
    when(expiringCodeStore.generateCode(
            eq("id001"), any(Timestamp.class), anyString(), anyString()))
        .thenReturn(
            new ExpiringCode(
                "secret_code",
                new Timestamp(
    ...
    when(expiringCodeStore.generateCode(
            eq(JsonUtils.writeValueAsString(change)),
            any(Timestamp.class),
            anyString(),
            anyString()))
        .thenReturn(
  ...
  public void password_reset_with_client_id_and_redirect_uri() throws Exception {
    ...
    when(scimUserProvisioning.query(
            "userName eq \"" + email + "\" and origin eq \"" + OriginKeys.UAA + "\"",
            IdentityZoneHolder.get().getId()))
        .thenReturn(Arrays.asList(user));

    PasswordChange change = new PasswordChange("id001", email, yesterday, clientId, redirectUri);
    when(expiringCodeStore.generateCode(
            anyString(), any(Timestamp.class), anyString(), anyString()))
        .thenReturn(
            new ExpiringCode(
                "secret_code",
                new Timestamp(
  ...
  public void testCreatingAPasswordResetWhenTheUsernameExists() throws Exception {
    ...
    user.setPasswordLastModified(yesterday);
    when(scimUserProvisioning.query(
            "userName eq \"user@example.com\" and origin eq \"" + OriginKeys.UAA + "\"",
            IdentityZoneHolder.get().getId()))
        .thenReturn(Arrays.asList(user));
  ...
  public void testCreatingAPasswordResetWhenTheUserHasNonUaaOrigin() throws Exception {
    ...
    when(scimUserProvisioning.query(
            "userName eq \"user@example.com\"", IdentityZoneHolder.get().getId()))
  ...
  @Test
  public void testCreatingAPasswordResetWithAUsernameContainingSpecialCharacters()
      throws Exception {
    ScimUser user = new ScimUser("id001", "user\"'@example.com", null, null);
    user.setMeta(new ScimMeta(yesterday, yesterday, 0));
    user.setPasswordLastModified(yesterday);
    ...
    PasswordChange change =
        new PasswordChange("id001", "user\"'@example.com", yesterday, null, null);
    when(expiringCodeStore.generateCode(
            eq(JsonUtils.writeValueAsString(change)),
            any(Timestamp.class),
            anyString(),
  ...
  public void testChangingAPasswordWithAValidCode() throws Exception {
    ExpiringCode code =
        new ExpiringCode(
            ...
            null);
    when(expiringCodeStore.retrieveCode("secret_code", IdentityZoneHolder.get().getId()))
        .thenReturn(code);
    ...
    when(scimUserProvisioning.retrieve("eyedee", IdentityZoneHolder.get().getId()))
        .thenReturn(scimUser);
    ExpiringCode autologinCode =
        new ExpiringCode(
            "autologin-code",
            ...
            AUTOLOGIN.name());
    when(expiringCodeStore.generateCode(
            anyString(), any(Timestamp.class), eq(AUTOLOGIN.name()), anyString()))
        .thenReturn(autologinCode);
    ...
    verify(scimUserProvisioning)
        .changePassword("eyedee", null, "new_secret", IdentityZoneHolder.get().getId());
  }
  ...
  public void testPasswordsMustSatisfyPolicy() throws Exception {
    doThrow(new InvalidPasswordException("Password flunks policy"))
        ...
        .andExpect(
            content()
                .string(
                    JsonObjectMatcherUtils.matchesJsonObject(
                        new JSONObject()
                            .put("error_description", "Password flunks policy")
                            .put("message", "Password flunks policy")
                            .put("error", "invalid_password"))));
  }
  ...
  public void changePassword_Returns422UnprocessableEntity_NewPasswordSameAsOld() throws Exception {
    ...
    when(scimUserProvisioning.checkPasswordMatches(
            "eyedee", "new_secret", IdentityZoneHolder.get().getId()))
        .thenReturn(true);
