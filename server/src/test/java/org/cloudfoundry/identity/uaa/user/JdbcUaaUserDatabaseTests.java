...
public class JdbcUaaUserDatabaseTests extends JdbcTestBase {
  ...
  private static final String addUserSql =
      "insert into users (id, username, password, email, givenName, familyName, phoneNumber, origin, identity_zone_id, created, lastmodified, passwd_lastmodified, passwd_change_required) values (?,?,?,?,?,?,?,?,?,?,?,?,?)";
  ...
  public static final String ADD_GROUP_SQL =
      "insert into groups (id, displayName, identity_zone_id) values (?,?,?)";
  public static final String ADD_MEMBER_SQL =
      "insert into group_membership (group_id, member_id, member_type, authorities) values (?,?,?,?)";
  ...
  public void initializeDb() throws Exception {
    defaultAuthorities =
        UserConfig.DEFAULT_ZONE_GROUPS
            .stream()
  ...
  public void is_the_right_query_used() throws Exception {
    ...
    verify(template)
        .queryForObject(
            eq(DEFAULT_CASE_SENSITIVE_USER_BY_USERNAME_QUERY),
            eq(db.getMapper()),
    ...
    verify(template)
        .query(
            eq(DEFAULT_CASE_SENSITIVE_USER_BY_EMAIL_AND_ORIGIN_QUERY),
            eq(db.getMapper()),
            eq(username.toLowerCase()),
            eq(true),
            eq(OriginKeys.UAA),
    ...
    verify(template)
        .queryForObject(
            eq(DEFAULT_CASE_INSENSITIVE_USER_BY_USERNAME_QUERY),
            eq(db.getMapper()),
            eq(username.toLowerCase()),
            eq(true),
            eq(OriginKeys.UAA),
    ...
    verify(template)
        .query(
            eq(DEFAULT_CASE_INSENSITIVE_USER_BY_EMAIL_AND_ORIGIN_QUERY),
            eq(db.getMapper()),
            eq(username.toLowerCase()),
            eq(true),
            eq(OriginKeys.UAA),
  ...
  public void getUserWithNestedAuthoritiesWorks() {
    ...
    assertThat(
        joe.getAuthorities(),
        containsInAnyOrder(defaultAuthorities.toArray(new SimpleGrantedAuthority[0])));
  ...
  protected void evaluateNestedJoe() {
    ...
    assertThat(
        joe.getAuthorities(), containsInAnyOrder(compareTo.toArray(new SimpleGrantedAuthority[0])));
  }
