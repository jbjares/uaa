...
public class UaaAuthorizationRequestManagerTests {
  ...
  public void initUaaAuthorizationRequestManagerTests() {
    ...
    factory =
        new UaaAuthorizationRequestManager(
            clientDetailsService, uaaUserDatabase, providerProvisioning);
    ...
    user =
        new UaaUser(
            ...
            AuthorityUtils.commaSeparatedStringToAuthorityList(
                "foo.bar,spam.baz,space.1.developer,space.2.developer,space.1.admin"),
            "givenname",
            "familyname",
            null,
            null,
  ...
  public void testClientIDPAuthorizationInUAAzoneListSucceeds() {
    when(providerProvisioning.retrieveByOrigin(anyString(), anyString()))
        .thenReturn(MultitenancyFixture.identityProvider("random", "random"));
  ...
  public void testTokenRequestIncludesResourceIds() {
    ...
    IdentityZoneHolder.get()
        .getConfig()
        .getUserConfig()
        .setDefaultGroups(Arrays.asList("aud1.test"));
    ...
    OAuth2Request request =
        factory.createTokenRequest(parameters, client).createOAuth2Request(client);
    assertEquals(
        StringUtils.commaDelimitedListToSet("aud1.test,aud2.test"),
        new TreeSet<>(request.getScope()));
    assertEquals(
        StringUtils.commaDelimitedListToSet("aud1,aud2"), new TreeSet<>(request.getResourceIds()));
  }
  ...
  public void test_user_token_request() {
    ...
    BaseClientDetails recipient =
        new BaseClientDetails("recipient", "requested", "requested.scope", "password", "");
    ...
    when(clientDetailsService.loadClientByClientId(recipient.getClientId(), "uaa"))
        .thenReturn(recipient);
    ReflectionTestUtils.setField(factory, "uaaUserDatabase", null);
    client.setClientId("requestingId");
    OAuth2Request request =
        factory.createTokenRequest(parameters, client).createOAuth2Request(recipient);
    ...
    assertEquals(
        client.getClientId(),
        request.getRequestParameters().get(TokenConstants.USER_TOKEN_REQUESTING_CLIENT_ID));
    assertEquals(
        StringUtils.commaDelimitedListToSet("requested.scope"), new TreeSet<>(request.getScope()));
    assertEquals(
        StringUtils.commaDelimitedListToSet(recipient.getClientId() + ",requested"),
        new TreeSet<>(request.getResourceIds()));
  ...
  public void testScopeIncludesAuthoritiesForUser() {
    ...
    assertEquals(
        StringUtils.commaDelimitedListToSet("foo.bar"), new TreeSet<String>(request.getScope()));
  ...
  public void testWildcardScopesIncludesAuthoritiesForUser() {
    ...
    AuthorizationRequest request = factory.createAuthorizationRequest(parameters);
    assertEquals(
        StringUtils.commaDelimitedListToSet("space.1.developer,space.2.developer"),
  ...
  public void testOpenidScopeIncludeIsAResourceId() {
    ...
    AuthorizationRequest request = factory.createAuthorizationRequest(parameters);
    assertEquals(
        StringUtils.commaDelimitedListToSet("openid,foo.bar"),
        new TreeSet<String>(request.getScope()));
    assertEquals(
        StringUtils.commaDelimitedListToSet("openid,foo"),
  ...
  public void testEmptyScopeFailsClientWithScopes() {
    ...
    expectedException.expectMessage(
        "[one, two] is invalid. This user is not allowed any of the requested scopes");
  ...
  public void testScopesValid() throws Exception {
    ...
    factory.validateParameters(
        parameters, new BaseClientDetails("foo", null, "read,write", "implicit", null));
  }
  ...
  public void testWildcardIntersect1() throws Exception {
    ...
    Set<String> user =
        new HashSet<>(
            Arrays.asList(
                "space.1.developer", "space.2.developer", "space.1.admin", "space.3.operator"));
  ...
  public void testWildcardIntersect5() throws Exception {
    Set<String> client =
        new HashSet<>(Arrays.asList("space.*.developer", "space.*.admin", "space.3.operator"));
    Set<String> requested = client;
    Set<String> user =
        new HashSet<>(
            Arrays.asList(
