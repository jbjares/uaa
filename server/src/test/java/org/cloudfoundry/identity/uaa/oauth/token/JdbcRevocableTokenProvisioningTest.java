...
public class JdbcRevocableTokenProvisioningTest extends JdbcTestBase {
  ...
  public void on_application_event_calls_internal_delete_method() throws Exception {
    ...
    for (IdentityZone zone : Arrays.asList(IdentityZone.getUaa(), otherZone)) {
      ...
      reset(dao);
      try {
        dao.onApplicationEvent(
            new EntityDeletedEvent<>(clientDetails, mock(UaaAuthentication.class)));
      } catch (Exception e) {
      }
      try {
        dao.onApplicationEvent(
  ...
  public void revocable_tokens_deleted_when_client_is() throws Exception {
    ...
    for (IdentityZone zone : Arrays.asList(IdentityZone.getUaa(), otherZone)) {
      ...
      countTokens(1);
      assertEquals(
          zone.getId(), dao.retrieve(tokenId, IdentityZoneHolder.get().getId()).getZoneId());
      dao.onApplicationEvent(
          (AbstractUaaEvent)
              new EntityDeletedEvent<>(clientDetails, mock(UaaAuthentication.class)));
      countTokens(0);
  ...
  public void revocable_tokens_deleted_when_user_is() throws Exception {
    ...
    for (IdentityZone zone : Arrays.asList(IdentityZone.getUaa(), otherZone)) {
      ...
      assertEquals(
          zone.getId(), dao.retrieve(tokenId, IdentityZoneHolder.get().getId()).getZoneId());
      dao.onApplicationEvent(
          (AbstractUaaEvent) new EntityDeletedEvent<>(user, mock(UaaAuthentication.class)));
      countTokens(0);
    }
  }
  ...
  public void listUserTokenForClient() throws Exception {
    ...
    List<RevocableToken> actualTokens =
        dao.getUserTokens(userId, clientId, IdentityZoneHolder.get().getId());
  ...
  public void listTokens(boolean client) throws Exception {
    ...
    List<RevocableToken> actualTokens =
        client
            ? dao.getClientTokens(clientId, IdentityZoneHolder.get().getId())
            : dao.getUserTokens(userId, IdentityZoneHolder.get().getId());
    assertThat(actualTokens, containsInAnyOrder(expectedTokens.toArray()));
  }
  ...
  public void testDeleteRefreshTokenForClientIdUserId() throws Exception {
    ...
    assertEquals(
        2,
        dao.deleteRefreshTokensForClientAndUserId(
            clientId, userId, IdentityZoneHolder.get().getId()));
    assertEquals(
        0,
        dao.deleteRefreshTokensForClientAndUserId(
            clientId, userId, IdentityZoneHolder.get().getId()));
    List<RevocableToken> userTokens =
        dao.getUserTokens(userId, clientId, IdentityZoneHolder.get().getId());
    assertEquals(
        0, userTokens.stream().filter(t -> t.getResponseType().equals(REFRESH_TOKEN)).count());
  }
  ...
  public void countTokens(int expected) {
    assertEquals(
        expected,
        (int) jdbcTemplate.queryForObject("select count(1) from revocable_tokens", Integer.class));
  }
    ...
    assertEquals(
        ...
        (int)
            jdbcTemplate.queryForObject(
                "select count(1) from revocable_tokens where token_id=?", Integer.class, tokenId));
  }
  ...
  public void test_periodic_deletion_of_expired_tokens() throws Exception {
    ...
    countTokens(2);
    jdbcTemplate.update(
        "UPDATE revocable_tokens SET expires_at=?", System.currentTimeMillis() - 10000);
    try {
