...
public class ChangeEmailEndpointsTest extends TestClassNullifier {
  ...
  public void setUp() throws Exception {
    ...
    ChangeEmailEndpoints changeEmailEndpoints =
        new ChangeEmailEndpoints(scimUserProvisioning, expiringCodeStore, clientDetailsService);
  ...
  public void testGenerateEmailChangeCode() throws Exception {
    ...
    when(expiringCodeStore.generateCode(
            eq(data), any(Timestamp.class), eq(EMAIL.name()), eq(IdentityZoneHolder.get().getId())))
        .thenReturn(
            new ExpiringCode(
                "secret_code",
  ...
  public void testGenerateEmailChangeCodeWithExistingUsernameChange() throws Exception {
    ...
    when(scimUserProvisioning.query(
            "userName eq \"new@example.com\" and origin eq \"" + OriginKeys.UAA + "\"",
  ...
  public void testChangeEmail() throws Exception {
    when(expiringCodeStore.retrieveCode("the_secret_code", IdentityZoneHolder.get().getId()))
        .thenReturn(
            new ExpiringCode(
                ...
                "{\"userId\":\"user-id-001\",\"email\":\"new@example.com\", \"client_id\":\"app\"}",
                EMAIL.name()));
    ...
    verify(scimUserProvisioning)
        .update(eq("user-id-001"), user.capture(), eq(IdentityZoneHolder.get().getId()));
    ...
    when(expiringCodeStore.retrieveCode("the_secret_code", IdentityZoneHolder.get().getId()))
        .thenReturn(
            new ExpiringCode(
                "the_secret_code",
                new Timestamp(System.currentTimeMillis()),
                "{\"userId\":\"user-id-001\",\"email\":\"new@example.com\",\"client_id\":null}",
                EMAIL.name()));
