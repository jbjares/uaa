...
public class XOAuthAuthenticationManagerTest {
  ...
  private String uaaIdentityZoneTokenSigningKey =
  ...
  private String oidcProviderTokenSigningKey =
      "-----BEGIN RSA PRIVATE KEY-----\n"
  ...
  private String changedOidcProviderTokenSigningKey =
      "-----BEGIN RSA PRIVATE KEY-----\n"
  ...
  public void setup() throws Exception {
    ...
    authManager =
        new XOAuthAuthenticationManager(
            ...
            new KeyInfoService(uaaIssuerBaseUrl));
  }
  ...
  @Test
  public void
      getExternalAuthenticationDetails_whenProviderHasSigningKey_throwsWhenIdTokenCannotBeValidated() {
    expectedException.expect(InvalidTokenException.class);
    expectedException.expectMessage("Could not verify token signature.");

    Map<String, Object> header = map(entry(ALG, "HS256"), entry(KID, "oidc-provider-key"));
    Signer signer = new RsaSigner(changedOidcProviderTokenSigningKey);
    Map<String, Object> claims = map(entry(EMAIL, "someuser@google.com"));
    IdentityZoneHolder.get()
        .getConfig()
        .getTokenPolicy()
        .setKeys(Collections.singletonMap("uaa-key", uaaIdentityZoneTokenSigningKey));
    String idTokenJwt = UaaTokenUtils.constructToken(header, claims, signer);

    XOAuthCodeToken oidcAuthentication =
        new XOAuthCodeToken(
            "thecode", origin, "http://google.com", idTokenJwt, "accesstoken", "signedrequest");
    authManager.getExternalAuthenticationDetails(oidcAuthentication);
  }

  @Test
  public void
      getExternalAuthenticationDetails_whenProviderIssuerMatchesUaaIssuer_throwsWhenIdTokenCannotBeValidated() {
    oidcConfig.setIssuer(tokenEndpointBuilder.getTokenEndpoint());
    expectedException.expect(InvalidTokenException.class);
    expectedException.expectMessage("Could not verify token signature.");

    Map<String, Object> header = map(entry(ALG, "HS256"), entry(KID, "uaa-key"));
    ...
    XOAuthCodeToken oidcAuthentication =
        new XOAuthCodeToken(
            null, origin, "http://google.com", idTokenJwt, "accesstoken", "signedrequest");
    authManager.getExternalAuthenticationDetails(oidcAuthentication);
  }
