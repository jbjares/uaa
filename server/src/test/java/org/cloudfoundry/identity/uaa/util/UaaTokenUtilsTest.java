...
public class UaaTokenUtilsTest {
  ...
  public void is_user_token() {
    ...
    for (String grantType :
        Arrays.asList(GRANT_TYPE_PASSWORD, GRANT_TYPE_IMPLICIT, GRANT_TYPE_AUTHORIZATION_CODE)) {
  ...
  public void test_required_user_authorities_invalid() {
    ...
    List<GrantedAuthority> userGroups =
        Arrays.asList(
  ...
  public void getClaims_throwsExceptionWhenClaimsCannotBeRead() {
    Jwt encoded =
        JwtHelper.encode(
            "great content", KeyInfoBuilder.build("foo", "bar", "https://localhost/uaa"));
