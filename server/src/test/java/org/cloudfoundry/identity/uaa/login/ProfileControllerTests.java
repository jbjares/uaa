...
public class ProfileControllerTests extends TestClassNullifier {
  ...
  public void setUp() throws Exception {
    ...
    Mockito.when(approvalStore.getApprovalsForUser(anyString(), anyString()))
        .thenReturn(allApprovals);

    BaseClientDetails appClient =
        new BaseClientDetails(
            "app", "thing", "thing.read,thing.write", GRANT_TYPE_AUTHORIZATION_CODE, "");
    ...
    BaseClientDetails otherClient =
        new BaseClientDetails(
            "other-client", "thing", "thing.read,thing.write", GRANT_TYPE_AUTHORIZATION_CODE, "");
    otherClient.addAdditionalInformation(ClientConstants.CLIENT_NAME, THE_ULTIMATE_APP);
    Mockito.when(clientDetailsService.loadClientByClientId("other-client", "uaa"))
  ...
  public void testGetProfile(String name) throws Exception {
    ...
    UsernamePasswordAuthenticationToken authentication =
        new UsernamePasswordAuthenticationToken(uaaPrincipal, null);
    ...
    mockMvc
        ...
        .andExpect(
            content()
                .string(
                    containsString("These applications have been granted access to your account.")))
        .andExpect(content().string(containsString("Change Password")))
        .andExpect(content().string(containsString("<h3>" + name)))
        .andExpect(
            content().string(containsString("Are you sure you want to revoke access to " + name)));
  }
  ...
  public void testSpecialMessageWhenNoAppsAreAuthorized() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            content()
                .string(
                    containsString("You have not yet authorized any third party applications.")));
  }
  ...
  public void testUpdateProfile() throws Exception {
    ...
    mockMvc.perform(post).andExpect(status().isFound()).andExpect(redirectedUrl("profile"));

    ArgumentCaptor<String> args = ArgumentCaptor.forClass(String.class);
    Mockito.verify(approvalStore, Mockito.times(2))
        .revokeApprovalsForClientAndUser(args.capture(), args.capture(), args.capture());
  ...
  public void testRevokeApp() throws Exception {
    ...
    Mockito.verify(approvalStore, Mockito.times(1))
        .revokeApprovalsForClientAndUser("app", USER_ID, zoneId);
  }
