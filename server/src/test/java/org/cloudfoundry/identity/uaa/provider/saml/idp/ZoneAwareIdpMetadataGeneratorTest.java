...
public class ZoneAwareIdpMetadataGeneratorTest {
  ...
  public String getMetadata() throws MarshallingException {
    ...
    return SAMLUtil.getMetadataAsString(
        mock(MetadataManager.class), keyManager, generator.generateMetadata(), extendedMetadata);
  }
  ...
  public void artifactBindingNotInSSOList() throws Exception {
    ...
    assertThat(
        ...
        not(hasItem(hasProperty("binding", equalTo(SAML2_ARTIFACT_BINDING_URI)))));
  }
  ...
  public void bindingOrderSSOList() {
    ...
    IDPSSODescriptor idpSSODescriptor =
        generator.buildIDPSSODescriptor(
            ...
            Arrays.asList("email"));
    assertEquals(
        SAML2_POST_BINDING_URI, idpSSODescriptor.getSingleSignOnServices().get(0).getBinding());
    ;
    assertEquals(
        SAML2_REDIRECT_BINDING_URI, idpSSODescriptor.getSingleSignOnServices().get(1).getBinding());
    ;
  }
  ...
  public void entityIDHonored() {
    ...
    assertEquals(
        ZONE_ID + "." + IdentityZoneHolder.getUaaZone().getConfig().getSamlConfig().getEntityID(),
        generator.getEntityId());
