...
public class ClientAdminEndpointsTests {
  ...
  private static final Set<String> SINGLE_REDIRECT_URL =
  ...
  public void setUp() throws Exception {
    ...
    clientRegistrationService =
        Mockito.mock(
            ClientServicesExtension.class, withSettings().extraInterfaces(SystemDeletable.class));
    ...
    for (int i = 0; i < details.length; i++) {
      ...
      // refresh token is added automatically by endpoint validation
      details[i].setAuthorizedGrantTypes(
          Arrays.asList(GRANT_TYPE_AUTHORIZATION_CODE, "refresh_token"));
  ...
  public void testMultipleCreateClientDetails() throws Exception {
    ...
    assertEquals(
        "We should have created " + inputs.length + " clients.", inputs.length, results.length);
    for (int i = 0; i < inputs.length; i++) {
  ...
  public void testFindClientDetails() throws Exception {
    Mockito.when(
            clientDetailsService.query("filter", "sortBy", true, IdentityZoneHolder.get().getId()))
        .thenReturn(Arrays.<ClientDetails>asList(detail));
  ...
  public void testFindClientDetails_Test_Attribute_Filter() throws Exception {
    when(clientDetailsService.query(
            anyString(), anyString(), anyBoolean(), eq(IdentityZoneHolder.get().getId())))
        .thenReturn(Arrays.asList(inputs));
    for (String attribute :
      ...
      SearchResults<Map<String, Object>> result =
          (SearchResults<Map<String, Object>>)
              endpoints.listClientDetails(attribute, "client_id pr", "sortBy", "ascending", 1, 100);
      validateAttributeResults(result, 5, Arrays.asList(attribute));
    }
  }

  protected void validateAttributeResults(
      SearchResults<Map<String, Object>> result, int size, List<String> attributes) {
  ...
  public void testUpdateClientDetailsWithNullCallerAndInvalidScope() throws Exception {
    Mockito.when(
            clientDetailsService.retrieve(input.getClientId(), IdentityZoneHolder.get().getId()))
        .thenReturn(new BaseClientDetails(input));
  ...
  public void testNonExistentClient1() throws Exception {
    Mockito.when(
            clientDetailsService.retrieve(input.getClientId(), IdentityZoneHolder.get().getId()))
        .thenThrow(new InvalidClientDetailsException(""));
  ...
  public void testNonExistentClient2() throws Exception {
    Mockito.when(
            clientDetailsService.retrieve(input.getClientId(), IdentityZoneHolder.get().getId()))
        .thenThrow(new BadClientCredentialsException());
    endpoints.getClientDetails(input.getClientId());
  }
  ...
  public void testChangeSecret() throws Exception {
    ...
    when(clientDetailsService.retrieve(detail.getClientId(), IdentityZoneHolder.get().getId()))
        .thenReturn(detail);
    ...
    verify(clientRegistrationService)
        .updateClientSecret(detail.getClientId(), "newpassword", "testzone");
  }
  ...
  public void testAddSecret() {
    ...
    endpoints.changeSecret(detail.getClientId(), change);
    verify(clientRegistrationService)
        .addClientSecret(detail.getClientId(), "newpassword", IdentityZoneHolder.get().getId());
  }
  ...
  public void testDeleteSecret() {
    ...
    endpoints.changeSecret(detail.getClientId(), change);
    verify(clientRegistrationService)
        .deleteClientSecret(detail.getClientId(), IdentityZoneHolder.get().getId());
  }
  ...
  public void testRemoveClientDetailsAdminCaller() throws Exception {
    ...
    Mockito.when(clientDetailsService.retrieve("foo", IdentityZoneHolder.get().getId()))
        .thenReturn(detail);
  ...
  public void testScopeIsRestrictedByCaller() throws Exception {
    BaseClientDetails caller =
        new BaseClientDetails("caller", null, "none", "client_credentials,implicit", "uaa.none");
    when(clientDetailsService.retrieve("caller", IdentityZoneHolder.get().getId()))
        .thenReturn(caller);
    setSecurityContextAccessor(
        new StubSecurityContextAccessor() {
          @Override
  ...
  public void testClientEndpointCannotBeConfiguredWithAnInvalidMaxCount() throws Exception {
    ...
    expected.expectMessage(
        containsString("Invalid \"clientMaxCount\" value (got 0). Should be positive number."));
  ...
  public void testAuthorityAllowedByCaller() throws Exception {
    BaseClientDetails caller =
        new BaseClientDetails(
            "caller", null, "uaa.none", "client_credentials,implicit", "uaa.none");
    when(clientDetailsService.retrieve("caller", IdentityZoneHolder.get().getId()))
        .thenReturn(caller);
    setSecurityContextAccessor(
        new StubSecurityContextAccessor() {
  ...
  public void testHandleNoSuchClient() throws Exception {
    ResponseEntity<Void> result =
        endpoints.handleNoSuchClient(new NoSuchClientException("No such client: foo"));
    assertEquals(HttpStatus.NOT_FOUND, result.getStatusCode());
  }

  @Test
  public void testHandleClientAlreadyExists() throws Exception {
    ResponseEntity<InvalidClientDetailsException> result =
        endpoints.handleClientAlreadyExists(
            new ClientAlreadyExistsException("No such client: foo"));
  ...
  public void testErrorHandler() throws Exception {
    ResponseEntity<InvalidClientDetailsException> result =
        endpoints.handleInvalidClientDetails(
            new InvalidClientDetailsException("No such client: foo"));
  ...
  public void testCreateClientWithAutoapproveScopesList() throws Exception {
    ...
    ClientDetails result = endpoints.createClientDetails(input);
    assertNull(result.getClientSecret());
    ArgumentCaptor<BaseClientDetails> clientCaptor =
        ArgumentCaptor.forClass(BaseClientDetails.class);
  ...
  private static void assertSetEquals(Collection<?> a, Collection<?> b) {
    assertTrue(
        ...
        a == null && b == null || a != null && b != null && a.containsAll(b) && b.containsAll(a));
  }
