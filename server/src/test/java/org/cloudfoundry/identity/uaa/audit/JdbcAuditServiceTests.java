...
public class JdbcAuditServiceTests extends JdbcTestBase {
  ...
  public void principalAuthenticationFailureAuditSucceeds() {
    auditService.log(
        getAuditEvent(PrincipalAuthenticationFailure, "clientA"),
        getAuditEvent(PrincipalAuthenticationFailure, "clientA").getIdentityZoneId());
  ...
  public void findMethodOnlyReturnsEventsWithinRequestedPeriod() throws Exception {
    ...
    auditService.log(
        ...
        getAuditEvent(PrincipalAuthenticationFailure, "clientA").getIdentityZoneId());
    auditService.log(
        getAuditEvent(PrincipalAuthenticationFailure, "clientB"),
        getAuditEvent(PrincipalAuthenticationFailure, "clientB").getIdentityZoneId());
    // Find events within last 2 mins
    List<AuditEvent> events =
        auditService.find("clientA", now - 120 * 1000, IdentityZoneHolder.get().getId());
    assertEquals(1, events.size());
  }
