...
public class ScimGroupBootstrapTests extends JdbcTestBase {
  ...
  public void canAddGroups() throws Exception {
    bootstrap.setGroups(
        StringUtils.commaDelimitedListToSet("org1.dev,org1.qa,org1.engg,org1.mgr,org1.hr")
            .stream()
            .collect(new MapCollector<>(s -> s, s -> null)));
  ...
  public void allowsBootstrapFromOtherInstance() throws Exception {
    // original bootstrap
    bootstrap.setGroups(
        StringUtils.commaDelimitedListToSet("multiple_bootstrap_group")
            .stream()
            .collect(new MapCollector<>(s -> s, s -> null)));
    bootstrap.afterPropertiesSet();
    ...
    when(gDB.update(anyString(), any(), anyString()))
    ...
    bootstrap.setGroups(
        StringUtils.commaDelimitedListToSet("multiple_bootstrap_group")
            .stream()
            .collect(new MapCollector<>(s -> s, s -> s)));
    bootstrap.afterPropertiesSet();
  ...
  public void canAddMembers() throws Exception {
    ...
    assertEquals(
        5,
        mDB.getMembers(
                bootstrap.getGroup("org1.dev").getId(), false, IdentityZoneHolder.get().getId())
            .size());
  }
  ...
  public void fallsBackToMessagesProperties() throws Exception {
    ...
    assertThat(
        ...
        PredicateMatcher.<ScimGroup>has(
            group ->
                "pets.cat".equals(group.getDisplayName())
                    && "Access the cat".equals(group.getDescription())));
    assertThat(
        bootstrappedGroups,
        PredicateMatcher.<ScimGroup>has(
            group ->
                "pets.dog".equals(group.getDisplayName())
                    && "Dog your data".equals(group.getDescription())));
    assertThat(
        bootstrappedGroups,
        PredicateMatcher.<ScimGroup>has(
            group ->
                "pony".equals(group.getDisplayName())
                    && "The magic of friendship".equals(group.getDescription())));
  }
  ...
  public void prefersNonBlankYmlOverMessagesProperties() throws Exception {
    ...
    assertThat(
        ...
        PredicateMatcher.<ScimGroup>has(
            group ->
                "print".equals(group.getDisplayName())
                    && "Access the network printer".equals(group.getDescription())));
    // records.read: exists in the message property source but should get its description from
    // configuration
    assertThat(
        bootstrappedGroups,
        PredicateMatcher.<ScimGroup>has(
            group ->
                "records.read".equals(group.getDisplayName())
                    && "Read important data".equals(group.getDescription())));
    // pets.cat: read: exists in the message property source but should get its description from
    // configuration
    assertThat(
        bootstrappedGroups,
        PredicateMatcher.<ScimGroup>has(
            group ->
                "pets.cat".equals(group.getDisplayName())
                    && "Pet the cat".equals(group.getDescription())));
    // pets.dog: specified in configuration with no description, so it should retain the default
    // description
    assertThat(
        bootstrappedGroups,
        PredicateMatcher.<ScimGroup>has(
    ...
    assertThat(
        bootstrappedGroups,
        PredicateMatcher.<ScimGroup>has(
            group -> "fish.nemo".equals(group.getDisplayName()) && group.getDescription() == null));
    assertThat(
        bootstrappedGroups,
        PredicateMatcher.<ScimGroup>has(
            group ->
                "water.drink".equals(group.getDisplayName())
