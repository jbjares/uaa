...
public class UaaAuthorizationEndpointParamaterizedTest {
  ...
  private final SessionAuthenticationException authException =
  ...
  public static Collection<Object[]> parameters() {
    return Arrays.asList(new Object[][] {{"code"}, {"token"}, {"id_token"}, {"token id_token"}});
  }

  @Before
  public void setup() {
    client =
        new BaseClientDetails("id", "", "openid", GRANT_TYPE_AUTHORIZATION_CODE, "", redirectUrl);
    ...
    when(clientDetailsService.loadClientByClientId(eq(client.getClientId()), eq(zoneID)))
        .thenReturn(client);
    when(redirectResolver.resolveRedirect(eq(redirectUrl), same(client))).thenReturn(redirectUrl);
    when(redirectResolver.resolveRedirect(eq(HTTP_SOME_OTHER_SITE_CALLBACK), same(client)))
        .thenThrow(new RedirectMismatchException(""));
    when(calculator.calculate(anyString(), anyString(), anyString()))
        .thenReturn("sessionstate.salt");
  ...
  public void test_client_not_found() throws Exception {
    ...
    when(clientDetailsService.loadClientByClientId(anyString(), anyString()))
        .thenThrow(new NoSuchClientException("not found"));
    uaaAuthorizationEndpoint.commence(request, response, authException);
    assertEquals(HttpStatus.BAD_REQUEST.value(), response.getStatus());
  }
  ...
  public void test_redirect_honors_ant_matcher() throws Exception {
    ...
    when(redirectResolver.resolveRedirect(eq("http://example.com/some/path"), same(client)))
        .thenReturn("http://example.com/some/path");
    uaaAuthorizationEndpoint.commence(request, response, authException);
  }
}
