...
public class JdbcScimGroupProvisioningTests extends JdbcTestBase {
  ...
  public void canRetrieveGroupsWithFilter() {
    ...
    assertEquals(1, dao.query("displayName EQ \"" + group3Description + "\"", zoneId).size());
    assertEquals(
        1, dao.query("displayName eq \"" + group3Description.toUpperCase() + "\"", zoneId).size());
    assertEquals(
        1,
        dao.query(
                "displayName co \""
                    + group1Description.substring(1, group1Description.length() - 1)
                    + "\"",
                zoneId)
            .size());
    ...
    assertEquals(
        1,
        dao.query("displayName eq \"" + group3Description + "\" and meta.version eq 0", zoneId)
            .size());
  ...
  public void canRemoveGroup() {
    ...
    List<ScimGroupMember> remainingMemberships =
        jdbcTemplate.query(
            "select " + MEMBERSHIP_FIELDS + " from " + MEMBERSHIP_TABLE,
  ...
  public void test_that_uaa_scopes_are_bootstrapped_when_zone_is_created() {
    ...
    List<String> groups =
        dao.retrieveAll(id).stream().map(g -> g.getDisplayName()).collect(Collectors.toList());
  ...
  public void sqlInjectionAttack3Fails() {
    dao.query(
        "displayName eq \"something\"; select "
            + SQL_INJECTION_FIELDS
            + " from groups where displayName='something'",
        zoneId);
  }
  ...
  public void sqlInjectionAttack4Fails() {
    dao.query(
        "displayName eq \"something\"; select id from groups where id='''; select "
            + SQL_INJECTION_FIELDS
            + " from groups where displayName='something'",
        zoneId);
  }
  ...
  private void validateGroupCount(int expected) {
    existingGroupCount =
        jdbcTemplate.queryForObject(
            "select count(id) from groups where identity_zone_id='" + zoneId + "'", Integer.class);
