...
public class ScopeAuthenticationManagerTests extends TestCase {
  ...
  protected void setUp() throws Exception {
    ...
    when(loginClient.getScope())
        .thenReturn(new HashSet<>(Arrays.asList("oauth.login", "oauth.approval")));
    ...
    AuthorizationRequest authorizationRequest =
        new DefaultOAuth2RequestFactory(service).createAuthorizationRequest(clientCredentials);
  ...
  public void testPasswordAuthenticateSucceed() throws Exception {
    UsernamePasswordAuthenticationToken userAuth =
        new UsernamePasswordAuthenticationToken(
            "username", "password", UaaAuthority.USER_AUTHORITIES);
    OAuth2Authentication auth = new OAuth2Authentication(request, userAuth);
    Authentication authentication = authenticationManager.authenticate(auth);
