...
public class LoginSamlAuthenticationProviderTests extends JdbcTestBase {
  ...
  public void configureProvider() throws Exception {
    ...
    ScimGroupProvisioning groupProvisioning =
        new JdbcScimGroupProvisioning(
            jdbcTemplate, new JdbcPagingListFactory(jdbcTemplate, limitSqlAdapter));
    IdentityZoneHolder.get()
        .getConfig()
        .getUserConfig()
        .setDefaultGroups(Arrays.asList("uaa.user"));
    groupProvisioning.createOrGet(
        new ScimGroup(null, "uaa.user", IdentityZoneHolder.get().getId()),
    ...
    uaaSamlUser =
        groupProvisioning.create(
            new ScimGroup(null, UAA_SAML_USER, IdentityZone.getUaa().getId()),
            IdentityZoneHolder.get().getId());
    uaaSamlAdmin =
        groupProvisioning.create(
            new ScimGroup(null, UAA_SAML_ADMIN, IdentityZone.getUaa().getId()),
            IdentityZoneHolder.get().getId());
    uaaSamlTest =
        groupProvisioning.create(
            new ScimGroup(null, UAA_SAML_TEST, IdentityZone.getUaa().getId()),
            IdentityZoneHolder.get().getId());

    JdbcScimGroupMembershipManager membershipManager =
        new JdbcScimGroupMembershipManager(jdbcTemplate);
    membershipManager.setScimGroupProvisioning(groupProvisioning);
    membershipManager.setScimUserProvisioning(userProvisioning);
    ScimUserBootstrap bootstrap =
        new ScimUserBootstrap(
            userProvisioning, groupProvisioning, membershipManager, Collections.EMPTY_LIST);
    ...
    externalManager.mapExternalGroup(
        uaaSamlUser.getId(), SAML_USER, OriginKeys.SAML, IdentityZoneHolder.get().getId());
    externalManager.mapExternalGroup(
        uaaSamlAdmin.getId(), SAML_ADMIN, OriginKeys.SAML, IdentityZoneHolder.get().getId());
    externalManager.mapExternalGroup(
        uaaSamlTest.getId(), SAML_TEST, OriginKeys.SAML, IdentityZoneHolder.get().getId());

    consumer = mock(WebSSOProfileConsumer.class);
    credential =
        getUserCredential(
            "marissa-saml", "Marissa", "Bloggs", "marissa.bloggs@test.com", "1234567890");
  ...
  private SAMLCredential getUserCredential(
      String username, String firstName, String lastName, String emailAddress, String phoneNumber) {
    return getUserCredential(username, firstName, lastName, emailAddress, phoneNumber, null);
  }
  ...
  public void test_relay_state_when_url() {
    ...
    assertTrue(
        "Authentication should be of type:" + UaaAuthentication.class.getName(),
        authentication instanceof UaaAuthentication);
    UaaAuthentication uaaAuthentication = (UaaAuthentication) authentication;
    assertThat(
        uaaAuthentication.getAuthContextClassRef(),
        containsInAnyOrder(AuthnContext.PASSWORD_AUTHN_CTX));
  ...
  public void test_multiple_group_attributes() throws Exception {
    providerDefinition.addAttributeMapping(
        GROUP_ATTRIBUTE_NAME, Arrays.asList("2ndgroups", "groups"));
    provider.setConfig(providerDefinition);
    providerProvisioning.update(provider, IdentityZoneHolder.get().getId());
    UaaAuthentication authentication = getAuthentication();
    assertEquals(
        "Four authorities should have been granted!", 4, authentication.getAuthorities().size());
    assertThat(
  ...
  public void test_group_mapping() throws Exception {
    ...
    UaaAuthentication authentication = getAuthentication();
    assertEquals(
        "Three authorities should have been granted!", 3, authentication.getAuthorities().size());
    assertThat(
        authentication.getAuthorities(),
        containsInAnyOrder(
            new SimpleGrantedAuthority(UAA_SAML_ADMIN),
  ...
  public void test_non_string_attributes() throws Exception {
    ...
    providerDefinition.addAttributeMapping(
        USER_ATTRIBUTE_PREFIX + "XSBase64Binary", "XSBase64Binary");

    provider.setConfig(providerDefinition);
    providerProvisioning.update(provider, IdentityZoneHolder.get().getId());
    UaaAuthentication authentication = getAuthentication();
    assertEquals(
        "http://localhost:8080/someuri", authentication.getUserAttributes().getFirst("XSURI"));
    ...
    assertEquals(
        new DateTime(0).toString(), authentication.getUserAttributes().getFirst("XSDateTime"));
  ...
  public void externalGroup_NotMapped_ToScope() throws Exception {
    try {
      externalManager.unmapExternalGroup(
          uaaSamlUser.getId(), SAML_USER, OriginKeys.SAML, IdentityZoneHolder.get().getId());
      externalManager.unmapExternalGroup(
          uaaSamlAdmin.getId(), SAML_ADMIN, OriginKeys.SAML, IdentityZoneHolder.get().getId());
      providerDefinition.addAttributeMapping(GROUP_ATTRIBUTE_NAME, "groups");
      ...
      UaaAuthentication authentication = getAuthentication();
      assertEquals(
          "Three authorities should have been granted!", 1, authentication.getAuthorities().size());
      assertThat(
          authentication.getAuthorities(),
          not(
              containsInAnyOrder(
  ...
  public void test_group_attribute_not_set() throws Exception {
    ...
    assertEquals(
        "Only uaa.user should have been granted", 1, uaaAuthentication.getAuthorities().size());
    assertEquals(
        UaaAuthority.UAA_USER.getAuthority(),
        uaaAuthentication.getAuthorities().iterator().next().getAuthority());
  }
  ...
  public void add_external_groups_to_authentication_with_wildcard_whitelist() throws Exception {
    ...
    UaaAuthentication authentication = getAuthentication();
    assertThat(
        authentication.getExternalGroups(),
        containsInAnyOrder(SAML_USER, SAML_ADMIN, SAML_NOT_MAPPED));
  }
  ...
  public void update_invitedUser_whose_username_is_notEmail() throws Exception {
    ...
    SAMLCredential credential =
        getUserCredential(
            "marissa-invited", "Marissa-invited", null, "marissa.invited@test.org", null);
    when(consumer.processAuthenticationResponse(any())).thenReturn(credential);
    getAuthentication();
  ...
  @Test
  public void invitedUser_authentication_whenAuthenticatedEmailDoesNotMatchInvitedEmail()
      throws Exception {
    ...
    SAMLCredential credential =
        getUserCredential("marissa-invited", "Marissa-invited", null, "different@test.org", null);
    when(consumer.processAuthenticationResponse(any())).thenReturn(credential);
    try {
      getAuthentication();
      fail();
  ...
  public void update_existingUser_if_attributes_different() throws Exception {
    ...
    SAMLCredential credential =
        getUserCredential(
            "marissa-saml", "Marissa-changed", null, "marissa.bloggs@change.org", null);
    when(consumer.processAuthenticationResponse(any())).thenReturn(credential);
    getAuthentication();
    ...
    credential =
        getUserCredential(
            "marissa-saml", "Marissa-changed", null, "marissa.bloggs@change.org", null, true);
    when(consumer.processAuthenticationResponse(any())).thenReturn(credential);
    getAuthentication();
  ...
  public void update_existingUser_if_username_different() throws Exception {
    ...
    UaaUser originalUser =
        userDatabase.retrieveUserByEmail("marissa.bloggs@test.com", OriginKeys.SAML);
    ...
    UaaPrincipal samlPrincipal =
        new UaaPrincipal(
            OriginKeys.NotANumber,
            "marissa-saml-changed",
            "marissa.bloggs@test.com",
            OriginKeys.SAML,
            "marissa-saml-changed",
            IdentityZoneHolder.get().getId());
    UaaUser user =
        authprovider.createIfMissing(
            samlPrincipal, false, new ArrayList<SimpleGrantedAuthority>(), attributes);
  ...
  public void have_attributes_changed() throws Exception {
    ...
    assertTrue(
        "Verified email modified", authprovider.haveUserAttributesChanged(existing, modified));
  ...
  public void custom_user_attributes_stored_if_configured() throws Exception {
    ...
    assertEquals("1234567890", user.getPhoneNumber());
    assertEquals(
        "marissa.bloggs@test.com", authentication.getUserAttributes().getFirst("secondary_email"));
    ...
    assertEquals(
        "marissa.bloggs@test.com", userInfo.getUserAttributes().getFirst("secondary_email"));
  ...
  @Test
  public void
      should_NotCreateShadowAccount_AndInstead_UpdateExistingUserUsername_if_userWithEmailExists()
          throws Exception {
    Map<String, Object> attributeMappings = new HashMap<>();
    attributeMappings.put("email", "emailAddress");
    providerDefinition.setAttributeMappings(attributeMappings);
  ...
  private ScimUser createSamlUser(
  ...
  public void shadowUser_GetsCreatedWithDefaultValues_IfAttributeNotMapped() throws Exception {
    ...
    assertEquals("marissa.bloggs@test.com", user.getEmail());
    assertEquals(
        "No custom attributes have been mapped", 0, authentication.getUserAttributes().size());
  }
  ...
  protected UaaAuthentication getAuthentication() {
    ...
    assertTrue(
        "Authentication should be UaaAuthentication", authentication instanceof UaaAuthentication);
