...
public class XOAuthProviderConfiguratorTests {
  ...
  public void setup() throws MalformedURLException {
    ...
    when(provisioning.retrieveAll(eq(true), anyString()))
        .thenReturn(
            Arrays.asList(oidcProvider, oauthProvider, new IdentityProvider<>().setType(LDAP)));
  }

  @Test
  public void retrieveAll() {
    List<IdentityProvider> activeXOAuthProviders =
        configurator.retrieveAll(true, IdentityZone.getUaa().getId());
  ...
  public void retrieveActive() {
    List<IdentityProvider> activeXOAuthProviders =
        configurator.retrieveActive(IdentityZone.getUaa().getId());
    assertEquals(2, activeXOAuthProviders.size());
    verify(configurator, times(1)).overlay(eq(config));
  ...
  public void retrieve_by_issuer() throws Exception {
    ...
    IdentityProvider<OIDCIdentityProviderDefinition> activeXOAuthProvider =
        configurator.retrieveByIssuer(issuer, IdentityZone.getUaa().getId());
  ...
  public void duplicate_issuer_found() throws Exception {
    ...
    when(provisioning.retrieveAll(eq(true), anyString()))
        .thenReturn(
            Arrays.asList(
                oidcProvider, oidcProvider, oauthProvider, new IdentityProvider<>().setType(LDAP)));
    doAnswer(
            invocation -> {
              OIDCIdentityProviderDefinition definition = invocation.getArgument(0);
              definition.setIssuer(issuer);
  ...
  public void getCompleteAuthorizationURI_includesNonceOnOIDC()
    ...
    String authzUri =
        configurator.getCompleteAuthorizationURI("alias", UaaUrlUtils.getBaseURL(request), oidc);
  ...
  public void getCompleteAuthorizationURI_doesNotIncludeNonceOnOAuth()
      throws UnsupportedEncodingException {
    String authzUri =
        configurator.getCompleteAuthorizationURI("alias", UaaUrlUtils.getBaseURL(request), oauth);
  ...
  @Test
  public void getCompleteAuthorizationURI_withOnlyDiscoveryUrlForOIDCProvider()
    ...
    assertThat(
        authorizationURI, Matchers.startsWith("https://accounts.google.com/o/oauth2/v2/auth"));
  ...
  public void getCompleteAuthorizationUri_hasAllRequiredQueryParametersForOidc() {
    ...
    assertThat(
        queryParams, hasEntry(is("redirect_uri"), containsString("login%2Fcallback%2Falias")));
