...
public class TokenKeyEndpointTests {
  ...
  private TokenKeyEndpoint tokenKeyEndpoint =
      new TokenKeyEndpoint(new KeyInfoService("https://localhost.uaa"));
  private Authentication validUaaResource;
  private final String SIGNING_KEY_1 =
      "-----BEGIN RSA PRIVATE KEY-----\n"
          ...
          + "-----END RSA PRIVATE KEY-----";
  private final String SIGNING_KEY_2 =
      "-----BEGIN RSA PRIVATE KEY-----\n"
          ...
          + "-----END RSA PRIVATE KEY-----";
  private final String SIGNING_KEY_3 =
      "-----BEGIN RSA PRIVATE KEY-----\n"
  ...
  public void keyIsReturnedForZone() {
    ...
    assertEquals(
        response.getModulus(), encoder.encodeToString(decoder.decode(response.getModulus())));
    assertEquals(
        response.getExponent(), encoder.encodeToString(decoder.decode((response.getExponent()))));
  ...
  public void listResponseContainsAllPublicKeysWhenUnauthenticated() {
    ...
    List<String> keyIds =
        keys.stream().map(VerificationKeyResponse::getId).collect(Collectors.toList());
    assertThat(keyIds, containsInAnyOrder("RsaKey1", "RsaKey2", "RsaKey3"));

    HashMap<String, VerificationKeyResponse> keysMap =
        keys.stream().collect(new MapCollector<>(k -> k.getId(), k -> k));
    ...
    keys.forEach(
        key ->
            assertFalse(
                ...
                key.getExponent().endsWith("=") || key.getModulus().endsWith("=")));
  }
  ...
  public void listResponseContainsAllKeysWhenAuthenticated() {
    ...
    VerificationKeyResponse symKeyResponse =
        keys.stream().filter(k -> k.getId().equals("SymmetricKey")).findAny().get();
  ...
  public void tokenKeyEndpoint_ReturnsAllKeysForZone() {
    ...
    List<String> keyIds =
        keysForZone.stream().map(VerificationKeyResponse::getId).collect(Collectors.toList());
  ...
  public void responseHeaderIncludesEtag() {
    ...
    ResponseEntity<VerificationKeyResponse> keyResponse =
        tokenKeyEndpoint.getKey(mock(Principal.class), "NaN");
    HttpHeaders headers = keyResponse.getHeaders();
    assertNotNull(headers.get("ETag"));

    ResponseEntity<VerificationKeysListResponse> keysResponse =
        tokenKeyEndpoint.getKeys(mock(Principal.class), "NaN");
  ...
  public void returns304IfUnmodified() {
    ...
    ResponseEntity<VerificationKeyResponse> keyResponse =
        tokenKeyEndpoint.getKey(mock(Principal.class), lastModified);
    assertEquals(keyResponse.getStatusCode(), HttpStatus.NOT_MODIFIED);

    ResponseEntity<VerificationKeysListResponse> keysResponse =
        tokenKeyEndpoint.getKeys(mock(Principal.class), lastModified);
