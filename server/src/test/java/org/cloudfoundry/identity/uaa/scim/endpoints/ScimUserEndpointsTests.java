...
public class ScimUserEndpointsTests {
  ...
  public void setUp() {
    ...
    Map<Class<? extends Exception>, HttpStatus> map =
  ...
  public void validate_password_for_uaa_origin_only(
      VerificationMode verificationMode, String origin, Matcher<String> expectedPassword) {
  ...
  public void approvalsIsSyncedCorrectlyOnGet() {
    assertEquals(
        0, endpoints.getUser(joel.getId(), new MockHttpServletResponse()).getApprovals().size());
    ...
    assertEquals(
        2, endpoints.getUser(joel.getId(), new MockHttpServletResponse()).getApprovals().size());
  }

  @Test
  public void createUser_whenPasswordIsInvalid_throwsException() {
    doThrow(new InvalidPasswordException("whaddup"))
        .when(mockPasswordValidator)
        .validate(anyString());
    ScimUserProvisioning mockDao = mock(ScimUserProvisioning.class);
    endpoints.setScimUserProvisioning(mockDao);
    String zoneId = IdentityZoneHolder.get().getId();
    when(mockDao.createUser(any(ScimUser.class), anyString(), eq(zoneId)))
        .thenReturn(new ScimUser());
  ...
  public void userWithNoEmailNotAllowed() {
    ...
    int count =
        jdbcTemplate.queryForObject(
            "select count(*) from users where userName=?", new Object[] {"dave"}, Integer.class);
  ...
  public void create_user_when_internal_user_management_is_disabled(String origin) {
    ...
    request.setAttribute(
        DisableInternalUserManagementFilter.DISABLE_INTERNAL_USER_MANAGEMENT, true);
    endpoints.createUser(user, request, new MockHttpServletResponse());
  }
  ...
  public void testHandleExceptionWithConstraintViolation() throws Exception {
    ...
    endpoints.setMessageConverters(
  ...
  public void userCanInitializePassword() {
    ...
    String password =
        jdbcTemplate.queryForObject(
            "select password from users where id=?", String.class, created.getId());
  ...
  public void deleteIsAllowedWithCorrectVersionInEtag() {
    ...
    endpoints.deleteUser(
        exGuy.getId(),
        Integer.toString(exGuy.getMeta().getVersion()),
        new MockHttpServletRequest(),
        new MockHttpServletResponse());
  }
  ...
  public void deleteIsAllowedWithQuotedEtag() {
    ...
    exGuy = dao.createUser(exGuy, "exguyspassword", IdentityZoneHolder.get().getId());
    endpoints.deleteUser(
        exGuy.getId(), "\"*", new MockHttpServletRequest(), new MockHttpServletResponse());
  }
  ...
  public void deleteIsNotAllowedWithWrongVersionInEtag() {
    ...
    endpoints.deleteUser(
        exGuy.getId(),
        Integer.toString(exGuy.getMeta().getVersion() + 1),
        new MockHttpServletRequest(),
        new MockHttpServletResponse());
  }
  ...
  public void deleteIsAllowedWithNullEtag() {
    ...
    exGuy = dao.createUser(exGuy, "exguyspassword", IdentityZoneHolder.get().getId());
    endpoints.deleteUser(
        exGuy.getId(), null, new MockHttpServletRequest(), new MockHttpServletResponse());
  }
  ...
  public void deleteUserUpdatesGroupMembership() {
    ...
    validateGroupMembers(
        groupEndpoints.getGroup(g.getId(), new MockHttpServletResponse()), exGuy.getId(), false);
  }
  ...
  public void testFindGroupsAndApprovals() {
    ...
    SearchResults<?> results =
        spy.findUsers("id,groups,approvals", "id pr", null, "ascending", 1, 100);
    assertEquals(2, results.getTotalResults());
  ...
  public void testFindAllAttributes() {
    ...
    SearchResults<Map<String, Object>> familyNames =
        (SearchResults<Map<String, Object>>)
            endpoints.findUsers("familyName", "id pr", "familyName", "ascending", 1, 100);
    SearchResults<Map<String, Object>> givenNames =
        (SearchResults<Map<String, Object>>)
            endpoints.findUsers("givenName", "id pr", "givenName", "ascending", 1, 100);
  ...
  public void testFindNonExistingAttributes() {
    ...
    List<Map<String, Object>> resources =
        (List<Map<String, Object>>)
            endpoints
                .findUsers(nonExistingAttribute, "id pr", null, "ascending", 1, 100)
                .getResources();
  ...
  public void testFindUsersGroupsSyncedByDefault() throws Exception {
    ...
    verify(mockgroupMembershipManager, atLeastOnce())
        .getGroupsWithMember(anyString(), anyBoolean(), eq(IdentityZoneHolder.get().getId()));
  ...
  public void testFindUsersApprovalsSyncedByDefault() throws Exception {
    ...
    endpoints.findUsers("", "id pr", null, "ascending", 1, 100);
    verify(mockApprovalStore, atLeastOnce())
        .getApprovalsForUser(anyString(), eq(IdentityZoneHolder.get().getId()));
  ...
  @Test
  public void whenSettingAnInvalidUserMaxCount_ScimUsersEndpointShouldThrowAnException()
      throws Exception {
  ...
  @Test
  public void whenSettingANegativeValueUserMaxCount_ScimUsersEndpointShouldThrowAnException()
      throws Exception {
    expected.expect(IllegalArgumentException.class);
    expected.expectMessage(
  ...
  public void testInvalidFilterExpression() {
    ...
    SearchResults<?> results =
        endpoints.findUsers("id", "userName qq 'd'", null, "ascending", 1, 100);
  ...
  public void testValidFilterExpression() {
    SearchResults<?> results =
        endpoints.findUsers("id", "userName eq \"d\"", "created", "ascending", 1, 100);
    assertEquals(0, results.getTotalResults());
  }
  ...
  public void testFindIdsByUserName() {
    SearchResults<?> results =
        endpoints.findUsers("id", "userName eq \"jdsa\"", null, "ascending", 1, 100);
    assertEquals(1, results.getTotalResults());
    assertEquals(1, results.getSchemas().size()); // System.err.println(results.getValues());
    assertEquals(
        joel.getId(), ((Map<String, Object>) results.getResources().iterator().next()).get("id"));
  }
  ...
  public void testFindIdsByEmailApostrophe() {
    SearchResults<?> results =
        endpoints.findUsers(
            "id", "emails.value eq \"" + JDSA_VMWARE_COM + "\"", null, "ascending", 1, 100);
    assertEquals(1, results.getTotalResults());
    assertEquals(1, results.getSchemas().size()); // System.err.println(results.getValues());
    assertEquals(
        joel.getId(), ((Map<String, Object>) results.getResources().iterator().next()).get("id"));
  ...
  public void testFindIdsByUserNameContains() {
    SearchResults<?> results =
        endpoints.findUsers("id", "userName co \"d\"", null, "ascending", 1, 100);
    assertEquals(2, results.getTotalResults());
    assertTrue(
        "Couldn't find id: " + results.getResources(),
        getSetFromMaps(results.getResources(), "id").contains(joel.getId()));
  }

  @Test
  public void testFindIdsByUserNameStartWith() {
    SearchResults<?> results =
        endpoints.findUsers("id", "userName sw \"j\"", null, "ascending", 1, 100);
    assertEquals(1, results.getTotalResults());
    assertTrue(
        "Couldn't find id: " + results.getResources(),
        getSetFromMaps(results.getResources(), "id").contains(joel.getId()));
  ...
  public void testFindIdsByEmailContains() {
    SearchResults<?> results =
        endpoints.findUsers("id", "emails.value sw \"j\"", null, "ascending", 1, 100);
    assertEquals(1, results.getTotalResults());
    assertTrue(
        "Couldn't find id: " + results.getResources(),
        getSetFromMaps(results.getResources(), "id").contains(joel.getId()));
  ...
  public void testFindIdsByEmailContainsWithEmptyResult() {
    SearchResults<?> results =
        endpoints.findUsers("id", "emails.value sw \"z\"", null, "ascending", 1, 100);
    assertEquals(0, results.getTotalResults());
  }
  ...
  public void testFindIdsWithBooleanExpression() {
    SearchResults<?> results =
        endpoints.findUsers("id", "userName co \"d\" and id pr", null, "ascending", 1, 100);
    assertEquals(2, results.getTotalResults());
    assertTrue(
        "Couldn't find id: " + results.getResources(),
        getSetFromMaps(results.getResources(), "id").contains(joel.getId()));
  ...
  public void testFindIdsWithBooleanExpressionIvolvingEmails() {
    SearchResults<?> results =
        endpoints.findUsers(
            "id", "userName co \"d\" and emails.value co \"vmware\"", null, "ascending", 1, 100);
    assertEquals(2, results.getTotalResults());
    assertTrue(
        "Couldn't find id: " + results.getResources(),
        getSetFromMaps(results.getResources(), "id").contains(joel.getId()));
  ...
  public void testUpdateIncludesETagHeader() throws Exception {
    ...
    MockHttpServletResponse httpServletResponse = new MockHttpServletResponse();
    endpoints.updateUser(
        joel, joel.getId(), "*", new MockHttpServletRequest(), httpServletResponse, null);
  ...
  public void testPatchUserNoChange() {
    ...
    ScimUser patchedUser =
        endpoints.patchUser(
            createdUser,
            createdUser.getId(),
            Integer.toString(user.getVersion()),
            new MockHttpServletRequest(),
            new MockHttpServletResponse(),
            null);
  ...
  public void testPatchUser() {
    ...
    ScimUser patchedUser =
        endpoints.patchUser(
            createdUser,
            createdUser.getId(),
            Integer.toString(createdUser.getVersion()),
            new MockHttpServletRequest(),
            new MockHttpServletResponse(),
            null);
  ...
  public void testPatchIncorrectVersionFails() {
    ...
    endpoints.patchUser(
        createdUser,
        createdUser.getId(),
        Integer.toString(createdUser.getVersion() + 1),
        new MockHttpServletRequest(),
        new MockHttpServletResponse(),
        null);
  }
  ...
  public void testPatchUserStatus() {
    ...
    UserAccountStatus updatedStatus =
  ...
  public void testCreateUserWithEmailDomainNotAllowedForOriginUaa() {
    ...
    IdentityProvider ldapProvider =
        new IdentityProvider()
            .setActive(true)
            .setType(OriginKeys.LDAP)
            .setOriginKey(OriginKeys.LDAP)
            .setConfig(new LdapIdentityProviderDefinition());
    ...
    when(identityProviderProvisioning.retrieveActive(anyString()))
        .thenReturn(asList(ldapProvider, oidcProvider));

    expected.expect(ScimException.class);
    expected.expectMessage(
        "The user account is set up for single sign-on. Please use one of these origin(s) : [ldap, oidc1]");
    endpoints.createUser(user, new MockHttpServletRequest(), new MockHttpServletResponse());
  ...
  public void testWhenEmailDomainConfiguredForUaaAllowsCreationOfUser() {
    ...
    IdentityProvider uaaProvider =
        new IdentityProvider()
            .setActive(true)
            .setType(OriginKeys.UAA)
            .setOriginKey(OriginKeys.UAA)
