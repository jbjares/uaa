    ...
    try {
      ...
      while (rs.next()) {
        ...
        if (tableName.equalsIgnoreCase(rstableName)
            && (scopeColumnName.equalsIgnoreCase(rscolumnName)
                || authoritiesColumnName.equalsIgnoreCase(rscolumnName))) {
          assertTrue(
              String.format(
                  "Table: %s Column: %s should be over 4000 chars", rstableName, rscolumnName),
              columnSize > 4000);
          foundTable = true;
          ...
          assertNotNull(
              String.format(
                  "Table: %s Column: %s should have a column type", rstableName, rscolumnName),
              columnType);
          assertThat(
              String.format(
                  "Table: %s Column: %s should be text, longtext, nvarchar or clob",
                  rstableName, rscolumnName),
              columnType.toLowerCase(),
              isIn(Arrays.asList("text", "longtext", "nvarchar", "clob")));
        } else {
          continue;
        }
      }
      ...
      assertTrue(
          "I was expecting to find column: " + authoritiesColumnName, foundColumnAuthorities);
