...
public class RevocableTokenTableTest extends JdbcTestBase {
  ...
  private List<TestColumn> TEST_COLUMNS =
      Arrays.asList(
  ...
  private List<TestColumn> TEST_INDEX =
      Arrays.asList(
  ...
  public boolean testColumn(List<TestColumn> columns, String name, String type, int size) {
    for (TestColumn c : columns) {
      if (c.name.equalsIgnoreCase(name)) {
        return ("varchar".equalsIgnoreCase(type) || "nvarchar".equalsIgnoreCase(type))
                && !("data".equalsIgnoreCase(name) || "scope".equalsIgnoreCase(name))
  ...
  public void validate_table() throws Exception {
    ...
    try {
      ...
      if (!rs.next()) {
        rs =
            meta.getIndexInfo(connection.getCatalog(), null, tableName.toUpperCase(), false, false);
        assertTrue(rs.next());
      }
      int indexCount = 0;
      do {
        ...
        if (shouldCompareIndex(indexName)) {
          assertTrue(
              "Testing index: " + indexName, testColumn(TEST_INDEX, indexName, "", indexType));
          indexCount++;
        }
      } while (rs.next());
