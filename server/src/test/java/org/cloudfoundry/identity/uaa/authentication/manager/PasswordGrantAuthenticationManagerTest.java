...
public class PasswordGrantAuthenticationManagerTest {
  ...
  public void setUp() throws Exception {
    ...
    when(identityProviderProvisioning.retrieveActive("uaa"))
        .thenReturn(Arrays.asList(idp, uaaProvider, ldapProvider));
    ...
    instance =
        new PasswordGrantAuthenticationManager(
  ...
  public void testOIDCPasswordGrant() {
    ...
    verify(rt, times(1))
        .exchange(
            eq("http://localhost:8080/uaa/oauth/token"),
            eq(HttpMethod.POST),
            httpEntityArgumentCaptor.capture(),
            eq(new ParameterizedTypeReference<Map<String, String>>() {}));
    ArgumentCaptor<XOAuthCodeToken> tokenArgumentCaptor =
        ArgumentCaptor.forClass(XOAuthCodeToken.class);
  ...
  public void testOIDCPasswordGrantProviderTypeNotOidc() {
    ...
    when(identityProviderProvisioning.retrieveActive("uaa"))
        .thenReturn(Arrays.asList(uaaProvider, ldapProvider, localIdp));
    UaaLoginHint loginHint = mock(UaaLoginHint.class);
    when(loginHint.getOrigin()).thenReturn("oidcprovider");
    Authentication auth = mock(Authentication.class);
    when(zoneAwareAuthzAuthenticationManager.extractLoginHint(auth)).thenReturn(loginHint);
  ...
  public void testOIDCPasswordGrantNoBody() {
    ...
    try {
      ...
      assertEquals(
          "Could not obtain id_token from external OpenID Connect provider.", e.getMessage());
    }
  }
  ...
  public void testOIDCPasswordGrant_NoLoginHintDefaultNotAllowedSingleIdpOIDC() {
    ...
    Map<String, Object> additionalInfo =
        Collections.singletonMap(
            ClientConstants.ALLOWED_PROVIDERS, Collections.singletonList("oidcprovider"));
  ...
  public void testOIDCPasswordGrant_NoLoginHintDefaultNotAllowedChainedAuth() {
    ...
    when(zoneAwareAuthzAuthenticationManager.extractLoginHint(auth)).thenReturn(null);
    Map<String, Object> additionalInfo =
        Collections.singletonMap(ClientConstants.ALLOWED_PROVIDERS, Arrays.asList("uaa", "ldap"));
    when(clientDetails.getAdditionalInformation()).thenReturn(additionalInfo);
  ...
  public void testOIDCPasswordGrant_NoLoginHintDefaultNotAllowedMultipleIdpsWithUaa() {
    ...
    Map<String, Object> additionalInfo =
        Collections.singletonMap(
            ClientConstants.ALLOWED_PROVIDERS, Arrays.asList("uaa", "oidcprovider"));
    when(clientDetails.getAdditionalInformation()).thenReturn(additionalInfo);
  ...
  public void testOIDCPasswordGrant_NoLoginHintDefaultNotAllowedMultipleIdpsOnlyOIDC() {
    ...
    Map<String, Object> additionalInfo =
        Collections.singletonMap(
            ClientConstants.ALLOWED_PROVIDERS, Arrays.asList("oidcprovider", "oidcprovider2"));
    when(clientDetails.getAdditionalInformation()).thenReturn(additionalInfo);
    ...
    when(identityProviderProvisioning.retrieveActive("uaa"))
        .thenReturn(Arrays.asList(uaaProvider, ldapProvider, idp, localIdp));
  ...
  public void testOIDCPasswordGrant_LoginHintProviderNotAllowed() {
    ...
    try {
      ...
      assertEquals(
          "Client is not authorized for specified user's identity provider.", e.getMessage());
    }
  }
}
