...
public class LoginInfoEndpointTests {
  ...
  public void setUpPrincipal() {
    ...
    when(identityProviderProvisioning.retrieveByOriginIgnoreActiveFlag(
            eq(OriginKeys.UAA), anyString()))
        .thenReturn(uaaProvider);
    when(identityProviderProvisioning.retrieveByOrigin(eq(OriginKeys.LDAP), anyString()))
    ...
    configurator =
        new XOAuthProviderConfigurator(identityProviderProvisioning, oidcMetadataFetcher);
  ...
  public void testLoginReturnsSystemZone() throws Exception {
    ...
    endpoint.loginForHtml(
        model, null, new MockHttpServletRequest(), Collections.singletonList(MediaType.TEXT_HTML));
  ...
  public void testSavedAccountsPopulatedOnModel() throws Exception {
    ...
    Cookie cookie1 =
        new Cookie(
            "Saved-Account-xxxx",
            URLEncoder.encode(JsonUtils.writeValueAsString(savedAccount), UTF_8.name()));
  ...
  public void testIgnoresBadJsonSavedAccount() throws Exception {
    ...
    savedAccount.setOrigin("uaa");
    Cookie cookieGood =
        new Cookie("Saved-Account-xxxx", JsonUtils.writeValueAsString(savedAccount));
  ...
  public void customSelfserviceLinks_ApplyToAllZone_Html() throws Exception {
    ...
    IdentityZoneHolder.get()
        ...
        .getLinks()
        .getSelfService()
        .setSignup("http://custom_signup_link");
    IdentityZoneHolder.get()
        .getConfig()
        .getLinks()
        .getSelfService()
        .setPasswd("http://custom_passwd_link");
    endpoint.loginForHtml(
        model, null, new MockHttpServletRequest(), Collections.singletonList(MediaType.TEXT_HTML));
    validateSelfServiceLinks("http://custom_signup_link", "http://custom_passwd_link", model);
    validateSelfServiceLinks(
        "http://custom_signup_link", "http://custom_passwd_link", endpoint.getSelfServiceLinks());
    ...
    endpoint.setGlobalLinks(
        new Links()
            .setSelfService(new Links.SelfService().setSignup("/signup").setPasswd("/passwd")));
        ...
        new Links()
            .setSelfService(
                new Links.SelfService()
                    .setSignup("/signup?domain={zone.subdomain}")
                    .setPasswd("/passwd?id={zone.id}")));
    validateSelfServiceLinks(
        "/signup?domain=" + zone.getSubdomain(),
        "/passwd?id=" + zone.getId(),
        endpoint.getSelfServiceLinks());
    ...
    validateSelfServiceLinks(
        "/local_signup?domain=" + zone.getSubdomain(),
        "/local_passwd?id=" + zone.getId(),
        endpoint.getSelfServiceLinks());
  }
  ...
  public void discoverIdentityProviderCarriesLoginHintIfProvided() throws Exception {
    ...
    endpoint.discoverIdentityProvider(
        "testuser@fake.com", "true", loginHint, model, session, request);
  ...
  public void discoverIdentityProviderWritesLoginHintIfOnlyUaa() throws Exception {
    ...
    when(identityProviderProvisioning.retrieveActive("uaa"))
        .thenReturn(Collections.singletonList(uaaProvider));
  ...
  public String check_links_urls(IdentityZone zone) throws Exception {
    ...
    assertEquals(
        addSubdomainToUrl(baseUrl), ((Map<String, String>) model.asMap().get("links")).get("uaa"));
    assertEquals(
        addSubdomainToUrl(baseUrl.replace("uaa", "login")),
        ((Map<String, String>) model.asMap().get("links")).get("login"));
    ...
    when(mockIDPConfigurator.getIdentityProviderDefinitions((List<String>) isNull(), eq(zone)))
        .thenReturn(idps);
    ...
    assertEquals(
        "Temporary Authentication Code ( Get one at "
            + addSubdomainToUrl(HTTP_LOCALHOST_8080_UAA)
            + "/passcode )",
        ((String[]) mapPrompts.get("passcode"))[1]);
    return baseUrl;
  }
  ...
  public void saml_links_for_json() throws Exception {
    ...
    Map<String, String> idpDefinitions =
        (Map<String, String>) model.get(LoginInfoEndpoint.IDP_DEFINITIONS);
    for (SamlIdentityProviderDefinition def : idps) {
      assertEquals(
          "http://someurl/saml/discovery?returnIDParam=idp&entityID="
              + endpoint.getZonifiedEntityId()
              + "&idp="
  ...
  public void saml_links_for_html() throws Exception {
    ...
    endpoint.setIdpDefinitions(mockIDPConfigurator);
    endpoint.loginForHtml(
        model, null, new MockHttpServletRequest("GET", endpoint.getBaseUrl()), null);
    Map<String, Object> links = (Map<String, Object>) model.asMap().get("links");
    assertNotNull(links);
    assertEquals("http://someurl", links.get("login"));
  ...
  public void no_usernamePasswordBoxes_if_internalAuth_and_ldap_disabled() throws Exception {
    ...
    when(identityProviderProvisioning.retrieveByOrigin(OriginKeys.LDAP, "uaa"))
        .thenReturn(ldapIdentityProvider);

    IdentityProvider uaaIdentityProvider = new IdentityProvider();
    uaaIdentityProvider.setActive(false);
    when(identityProviderProvisioning.retrieveByOriginIgnoreActiveFlag(OriginKeys.UAA, "uaa"))
        .thenReturn(uaaIdentityProvider);
  ...
  public void testGeneratePasscodeForUnknownUaaPrincipal() throws Exception {
    ...
    ExpiringUsernameAuthenticationToken token =
        new ExpiringUsernameAuthenticationToken("princpal", "");
  ...
  public void test_PromptLogic() throws Exception {
    ...
    endpoint.loginForHtml(
        model,
        null,
        new MockHttpServletRequest("GET", endpoint.getBaseUrl()),
    ...
    model.clear();
    when(mockIDPConfigurator.getIdentityProviderDefinitions(
            (List<String>) isNull(), eq(IdentityZone.getUaa())))
        .thenReturn(idps);
    endpoint.setIdpDefinitions(mockIDPConfigurator);
    endpoint.infoForJson(model, null, new MockHttpServletRequest("GET", endpoint.getBaseUrl()));
    assertNotNull("prompts attribute should be present", model.get("prompts"));
  ...
  public void testFilterIdpsForDefaultZone() throws Exception {
    ...
    Collection<SamlIdentityProviderDefinition> idpDefinitions =
        (Collection<SamlIdentityProviderDefinition>) model.asMap().get("idpDefinitions");
  ...
  public void testFilterIDPsForAuthcodeClientInDefaultZone() throws Exception {
    ...
    List<String> allowedProviders =
        Arrays.asList("my-client-awesome-idp1", "my-client-awesome-idp2", OriginKeys.LDAP);

    // mock Client service
    BaseClientDetails clientDetails = new BaseClientDetails();
    clientDetails.setClientId("client-id");
    clientDetails.addAdditionalInformation(
        ClientConstants.ALLOWED_PROVIDERS, new LinkedList<>(allowedProviders));
    ...
    when(mockIDPConfigurator.getIdentityProviderDefinitions(
            eq(allowedProviders), eq(IdentityZone.getUaa())))
        .thenReturn(clientIDPs);
    ...
    assertThat(
        idpDefinitions,
        PredicateMatcher.<SamlIdentityProviderDefinition>has(
            c -> c.getIdpEntityAlias().equals("my-client-awesome-idp1")));
    assertThat(
        idpDefinitions,
        PredicateMatcher.<SamlIdentityProviderDefinition>has(c -> c.isShowSamlLink()));
    assertEquals(true, model.asMap().get("fieldUsernameShow"));
  ...
  public void testFilterIDPsForAuthcodeClientInOtherZone() throws Exception {
    ...
    List<String> allowedProviders =
        Arrays.asList("my-client-awesome-idp1", "my-client-awesome-idp2");
    ...
    clientDetails.addAdditionalInformation(
        ClientConstants.ALLOWED_PROVIDERS, new LinkedList<>(allowedProviders));
    ClientServicesExtension clientDetailsService = mock(ClientServicesExtension.class);
    when(clientDetailsService.loadClientByClientId("client-id", "other-zone"))
    ...
    clientIDPs.add(createIdentityProviderDefinition("my-client-awesome-idp1", "uaa"));
    clientIDPs.add(createIdentityProviderDefinition("my-client-awesome-idp2", "uaa"));
    SamlIdentityProviderConfigurator mockIDPConfigurator =
        mock(SamlIdentityProviderConfigurator.class);
    when(mockIDPConfigurator.getIdentityProviderDefinitions(eq(allowedProviders), eq(zone)))
        .thenReturn(clientIDPs);
  ...
  public void testFilterIDPsForAuthcodeClientWithNoAllowedIDPsInOtherZone() throws Exception {
    ...
    BaseClientDetails clientDetails = new BaseClientDetails();
    ClientServicesExtension clientDetailsService = mock(ClientServicesExtension.class);
    when(clientDetailsService.loadClientByClientId(eq("client-id"), anyString()))
        .thenReturn(clientDetails);
  ...
  public void allowedIdpsforClientOIDCProvider() throws Exception {
    ...
    when(identityProviderProvisioning.retrieveAll(eq(true), anyString()))
        .thenReturn(clientAllowedIdps);

    LoginInfoEndpoint endpoint = getEndpoint();
    endpoint.setClientDetailsService(clientDetailsService);
    endpoint.loginForHtml(model, null, request, Collections.singletonList(MediaType.TEXT_HTML));

    Map<String, AbstractXOAuthIdentityProviderDefinition> idpDefinitions =
        (Map<String, AbstractXOAuthIdentityProviderDefinition>) model.asMap().get(OAUTH_LINKS);
    assertEquals(2, idpDefinitions.size());
  }
  ...
  public void oauth_provider_links_shown() throws Exception {
    ...
    IdentityProvider<AbstractXOAuthIdentityProviderDefinition> identityProvider =
        MultitenancyFixture.identityProvider("oauth-idp-alias", "uaa");
    identityProvider.setConfig(definition);

    when(identityProviderProvisioning.retrieveAll(anyBoolean(), anyString()))
        .thenReturn(Collections.singletonList(identityProvider));
    LoginInfoEndpoint endpoint = getEndpoint();
    endpoint.loginForHtml(
        model, null, new MockHttpServletRequest(), Collections.singletonList(MediaType.TEXT_HTML));
  ...
  public void passcode_prompt_present_whenThereIsAtleastOneActiveOauthProvider() throws Exception {
    RawXOAuthIdentityProviderDefinition definition =
        new RawXOAuthIdentityProviderDefinition()
    ...
    when(identityProviderProvisioning.retrieveAll(anyBoolean(), anyString()))
        .thenReturn(Collections.singletonList(identityProvider));
    LoginInfoEndpoint endpoint = getEndpoint();
    endpoint.infoForLoginJson(
        model, null, new MockHttpServletRequest("GET", endpoint.getBaseUrl()));
  ...
  public void we_return_both_oauth_and_oidc_providers() throws Exception {
    RawXOAuthIdentityProviderDefinition oauthDefinition =
        new RawXOAuthIdentityProviderDefinition()
            .setAuthUrl(new URL("http://auth.url"))
            .setTokenUrl(new URL("http://token.url"));
    OIDCIdentityProviderDefinition oidcDefinition =
        new OIDCIdentityProviderDefinition()
            .setAuthUrl(new URL("http://auth.url"))
            .setTokenUrl(new URL("http://token.url"));

    IdentityProvider<AbstractXOAuthIdentityProviderDefinition> oauthProvider =
        MultitenancyFixture.identityProvider("oauth-idp-alias", "uaa");
    oauthProvider.setConfig(oauthDefinition);

    IdentityProvider<AbstractXOAuthIdentityProviderDefinition> oidcProvider =
        MultitenancyFixture.identityProvider("oidc-idp-alias", "uaa");
  ...
  public void testLoginWithInvalidMediaType() throws Exception {
    LoginInfoEndpoint endpoint = getEndpoint();
    endpoint.loginForHtml(
        model, null, new MockHttpServletRequest(), Arrays.asList(MediaType.TEXT_XML));
  }
  ...
  public void testLoginHintEmailDomain() throws Exception {
    ...
    AbstractXOAuthIdentityProviderDefinition mockOidcConfig =
        mock(OIDCIdentityProviderDefinition.class);
    ...
    when(identityProviderProvisioning.retrieveAll(anyBoolean(), any()))
    ...
    SavedRequest savedRequest =
        (SavedRequest)
            mockHttpServletRequest.getSession().getAttribute(SAVED_REQUEST_SESSION_ATTRIBUTE);
    when(savedRequest.getParameterValues("login_hint")).thenReturn(new String[] {"example.com"});

    String redirect =
        endpoint.loginForHtml(
            model, null, mockHttpServletRequest, Collections.singletonList(MediaType.TEXT_HTML));
  ...
  public void testLoginHintOriginUaa() throws Exception {
    ...
    SavedRequest savedRequest =
        (SavedRequest)
            mockHttpServletRequest.getSession().getAttribute(SAVED_REQUEST_SESSION_ATTRIBUTE);
    when(savedRequest.getParameterValues("login_hint"))
        .thenReturn(new String[] {"{\"origin\":\"uaa\"}"});
  ...
  public void testLoginHintOriginUaaDoubleEncoded() throws Exception {
    ...
    SavedRequest savedRequest =
        (SavedRequest)
            mockHttpServletRequest.getSession().getAttribute(SAVED_REQUEST_SESSION_ATTRIBUTE);
    when(savedRequest.getParameterValues("login_hint"))
        .thenReturn(new String[] {URLEncoder.encode("{\"origin\":\"uaa\"}", "utf-8")});
  ...
  public void testLoginHintOriginUaaAllowedProvidersNull() throws Exception {
    ...
    List<String> allowedProviders =
        Arrays.asList("my-OIDC-idp1", "my-OIDC-idp2", OriginKeys.LDAP, OriginKeys.UAA);
    // mock Client service
    BaseClientDetails clientDetails = new BaseClientDetails();
    clientDetails.setClientId("client-id");
  ...
  public void testInvalidLoginHintErrorOnDiscoveryPage() throws Exception {
    ...
    SavedRequest savedRequest =
        (SavedRequest)
            mockHttpServletRequest.getSession().getAttribute(SAVED_REQUEST_SESSION_ATTRIBUTE);
    when(savedRequest.getParameterValues("login_hint"))
        .thenReturn(new String[] {"{\"origin\":\"invalidorigin\"}"});
  ...
  public void testLoginHintOriginOidc() throws Exception {
    ...
    SavedRequest savedRequest =
        (SavedRequest)
            mockHttpServletRequest.getSession().getAttribute(SAVED_REQUEST_SESSION_ATTRIBUTE);
    when(savedRequest.getParameterValues("login_hint"))
        .thenReturn(new String[] {"{\"origin\":\"my-OIDC-idp1\"}"});

    String redirect =
        endpoint.loginForHtml(
            model, null, mockHttpServletRequest, Arrays.asList(MediaType.TEXT_HTML));
  ...
  public void testGetPromptsFromOIDCProvider() throws Exception {
    ...
    customPrompts.add(
        new Prompt(
            "passcode",
            "text",
            "MyTemporary Authentication Code ( Get one at "
                + HTTP_LOCALHOST_8080_UAA
                + "/passcode )"));
    ...
    when(identityProviderProvisioning.retrieveByOrigin("OIDC-without-prompts", "uaa"))
        .thenReturn(provider);
  ...
  public void testGetPromptsFromNonExistentProvider() throws Exception {
    ...
    mockHttpServletRequest.setParameter("origin", "non-OIDC");
    LoginInfoEndpoint endpoint = getEndpoint();
    when(identityProviderProvisioning.retrieveByOrigin("non-OIDC", "uaa"))
  ...
  public void testLoginHintLdapOverridesDefaultProviderUaa() throws Exception {
    ...
    SavedRequest savedRequest =
        (SavedRequest)
            mockHttpServletRequest.getSession().getAttribute(SAVED_REQUEST_SESSION_ATTRIBUTE);
    when(savedRequest.getParameterValues("login_hint"))
        .thenReturn(new String[] {"{\"origin\":\"ldap\"}"});
  ...
  private MockHttpServletRequest getMockHttpServletRequest() {
    ...
    when(savedRequest.getRedirectUrl())
        .thenReturn(
            "http://localhost:8080/uaa/oauth/authorize?client_id=identity&redirect_uri=http%3A%2F%2Flocalhost%3A8888%2Flogin&response_type=code&state=8tp0tR");
    session.setAttribute(SAVED_REQUEST_SESSION_ATTRIBUTE, savedRequest);
    request.setSession(session);
    return request;
  }
  ...
  private LoginInfoEndpoint getEndpoint() {
    ...
    SamlIdentityProviderConfigurator emptyConfigurator =
        mock(SamlIdentityProviderConfigurator.class);
    when(emptyConfigurator.getIdentityProviderDefinitions()).thenReturn(Collections.EMPTY_LIST);
    when(emptyConfigurator.getIdentityProviderDefinitionsForZone(any()))
  ...
  private SamlIdentityProviderDefinition createIdentityProviderDefinition(
      String idpEntityAlias, String zoneId) {
    SamlIdentityProviderDefinition idp1 =
  ...
  private IdentityProvider createOIDCIdentityProvider(String originKey)
      throws MalformedURLException {
    IdentityProvider<AbstractXOAuthIdentityProviderDefinition> oidcIdentityProvider =
