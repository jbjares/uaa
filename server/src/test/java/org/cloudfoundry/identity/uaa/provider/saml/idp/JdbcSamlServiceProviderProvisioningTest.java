...
public class JdbcSamlServiceProviderProvisioningTest extends JdbcTestBase {
  ...
  public void testCreateAndUpdateSamlServiceProviderInDefaultZone() throws Exception {
    ...
    SamlServiceProvider createdSp = db.create(sp, sp.getIdentityZoneId());
    Map<String, Object> rawCreatedSp =
        jdbcTemplate.queryForMap("select * from service_provider where id = ?", createdSp.getId());
    ...
    assertEquals(
        sp.getConfig(),
        JsonUtils.readValue(
            (String) rawCreatedSp.get("config"), SamlServiceProviderDefinition.class));
    ...
    assertEquals(sp.getConfig(), createdSp.getConfig());
    assertTrue(
        Math.abs(sp.getLastModified().getTime() - createdSp.getLastModified().getTime()) < 1001);
    assertEquals(
        Integer.valueOf(rawCreatedSp.get("version").toString()) + 1, createdSp.getVersion());
