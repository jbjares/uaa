...
public class RemoteAuthenticationEndpointTests {
  ...
  public void setUp() throws Exception {
    UaaPrincipal principal =
        new UaaPrincipal("user-id-001", "joe", "joe@example.com", OriginKeys.UAA, null, null);
  ...
  public void successfulAuthenticationGives200Status() throws Exception {
    ...
    ResponseEntity response =
        (ResponseEntity) endpoint.authenticate(new MockHttpServletRequest(), "joe", "joespassword");
  ...
  public void accountNotVerifiedExceptionGives403Status() throws Exception {
    when(am.authenticate(any(Authentication.class)))
        .thenThrow(new AccountNotVerifiedException("failed"));
    @SuppressWarnings("rawtypes")
    ResponseEntity response =
        (ResponseEntity) endpoint.authenticate(new MockHttpServletRequest(), "joe", "joespassword");
  ...
  public void successfulLoginAuthenticationInvokesLoginAuthManager() throws Exception {
    ...
    when(am.authenticate(any(Authentication.class)))
        .thenThrow(new BadCredentialsException("Invalid authentication manager invoked"));
    when(loginAuthMgr.authenticate(any(Authentication.class)))
        .thenReturn(new UsernamePasswordAuthenticationToken("joe", null));
    when(loginAuthentication.isClientOnly()).thenReturn(Boolean.TRUE);
    @SuppressWarnings("rawtypes")
    ResponseEntity response =
        (ResponseEntity) endpoint.authenticate(new MockHttpServletRequest(), "joe", "origin", null);
    assertEquals(HttpStatus.OK, response.getStatusCode());
  }
}
