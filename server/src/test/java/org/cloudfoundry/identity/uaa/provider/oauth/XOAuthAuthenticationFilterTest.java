...
public class XOAuthAuthenticationFilterTest {
  ...
  public void testShouldAuthenticate() {
    XOAuthAuthenticationFilter filter =
        spy(
            new XOAuthAuthenticationFilter(
                mock(XOAuthAuthenticationManager.class), successHandler));
  ...
  public void getIdTokenInResponse() throws Exception {
    XOAuthAuthenticationManager xOAuthAuthenticationManager =
        mock(XOAuthAuthenticationManager.class);
    XOAuthAuthenticationFilter filter =
        spy(new XOAuthAuthenticationFilter(xOAuthAuthenticationManager, successHandler));

    HttpServletRequest request = mock(HttpServletRequest.class);
    when(request.getRequestURL())
        .thenReturn(new StringBuffer("http://localhost/uaa/login/callback/the_origin"));
    ...
    assertEquals(
        "http://localhost/uaa/login/callback/the_origin", xoAuthCodeToken.getRedirectUrl());
  ...
  public void getXOAuthCodeTokenFromRequest() throws Exception {
    ...
    XOAuthAuthenticationFilter filter =
        new XOAuthAuthenticationFilter(xOAuthAuthenticationManager, successHandler);
  ...
  public void redirectsToErrorPageInCaseOfException() throws Exception {
    ...
    Mockito.doThrow(new HttpClientErrorException(HttpStatus.BAD_REQUEST, "error from oauth server"))
        .when(xOAuthAuthenticationManager)
        .authenticate(any());
    filter.doFilter(request, response, chain);
    Assert.assertThat(
        ...
        Matchers.containsString(
            request.getContextPath()
                + "/oauth_error?error=There+was+an+error+when+authenticating+against+the+external+identity+provider%3A"));
  }
}
