...
public class TlsKeyGenerator {
  ...
  public static X509Certificate getCertificate(ServerEntry entry) throws NamingException {
    ...
    try {
      ...
      NamingException ne =
          new NamingException("Failed to get BC Certificate factory for algorithm: X.509");
      ne.setRootCause(e);
      throw ne;
    }
  ...
  public static KeyPair getKeyPair(ServerEntry entry) throws NamingException {
    ...
    try {
      ...
      NamingException ne =
          new NamingException("Failed to get key factory for algorithm: " + ALGORITHM);
      ne.setRootCause(e);
      throw ne;
    }
  ...
  /**
   * Adds a private key pair along with a self signed certificate to an entry making sure it
   * contains the objectClasses and attributes needed to support the additions. This function is
   * intended for creating a TLS key value pair and self signed certificate for use by the server to
   * authenticate itself during SSL handshakes in the course of establishing an LDAPS connection or
   * a secure LDAP connection using StartTLS. Usually this information is added to the administrator
   * user's entry so the administrator (effectively the server) can manage these security concerns.
   *
  ...
  public static void addKeyPair(ServerEntry entry) throws NamingException {
    ...
    if (objectClass == null) {
      entry.put(
          SchemaConstants.OBJECT_CLASS_AT, TLS_KEY_INFO_OC, SchemaConstants.INET_ORG_PERSON_OC);
    } else {
  ...
  public static void addKeyPair(
      ServerEntry entry, String issuerDN, String subjectDN, String keyAlgo) throws NamingException {
    EntryAttribute objectClass = entry.get(SchemaConstants.OBJECT_CLASS_AT);
