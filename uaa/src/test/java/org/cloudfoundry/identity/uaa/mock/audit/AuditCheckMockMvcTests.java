...
class AuditCheckMockMvcTests {
  ...
  @Autowired private ClientServicesExtension clientRegistrationService;
  ...
  void setUp(
    ...
    testUser =
        createUser(adminToken, "testUser", "Test", "User", "testuser@test.com", testPassword, true);

    testListener.clearEvents();
    listener = mock(new DefaultApplicationListener<AbstractUaaEvent>() {}.getClass());
    authSuccessListener =
        mock(new DefaultApplicationListener<UserAuthenticationSuccessEvent>() {}.getClass());
    ...
    dbTrueString =
        LimitSqlAdapterFactory.getLimitSqlAdapter()
                .getClass()
                .equals(SQLServerLimitSqlAdapter.class)
            ? "1"
            : "true";
  }
  ...
  void client_modification_logs_authorities_and_scopes() throws Exception {
    ...
    BaseClientDetails client =
        new BaseClientDetails(clientId, resource, scopes, grantTypes, authorities);
    ...
    assertClientEvents(
        ...
        new String[] {"uaa.resource", "uaa.admin"});
  ...
  void userLoginTest() throws Exception {
    ...
    IdentityProviderAuthenticationSuccessEvent passwordevent =
        getEventOfType(captor, IdentityProviderAuthenticationSuccessEvent.class);
    ...
    UserAuthenticationSuccessEvent userevent =
        getEventOfType(captor, UserAuthenticationSuccessEvent.class);
  ...
  void invalidPasswordLoginUnsuccessfulTest() throws Exception {
    ...
    IdentityProviderAuthenticationFailureEvent event1 =
        (IdentityProviderAuthenticationFailureEvent) captor.getAllValues().get(0);
    UserAuthenticationFailureEvent event2 =
        (UserAuthenticationFailureEvent) captor.getAllValues().get(1);
    PrincipalAuthenticationFailureEvent event3 =
        (PrincipalAuthenticationFailureEvent) captor.getAllValues().get(2);
  ...
  void unverifiedLegacyUserAuthenticationWhenAllowedTest(
      @Autowired List<JdbcTemplate> jdbcTemplates) throws Exception {
    ...
    ScimUser molly =
        createUser(
            adminToken, "molly", "Molly", "Collywobble", "molly@example.com", "wobblE3", false);
    jdbcTemplates.forEach(
        jdbc ->
            jdbc.execute(
                "update users set legacy_verification_behavior = "
                    + dbTrueString
                    + " where origin='uaa' and username = '"
                    + molly.getUserName()
                    + "'"));
    ...
    ArgumentCaptor<UserAuthenticationSuccessEvent> captor =
        ArgumentCaptor.forClass(UserAuthenticationSuccessEvent.class);
  ...
  void unverifiedPostLegacyUserAuthenticationWhenAllowedTest() throws Exception {
    ...
    UnverifiedUserAuthenticationEvent event =
        (UnverifiedUserAuthenticationEvent) allValues.get(allValues.size() - 1);
    assertEquals(molly.getUserName(), event.getUser().getUsername());
    assertTrue(event.getAuditEvent().getOrigin().contains("sessionId=<SESSION>"));
  }
  ...
  void findAuditHistory(@Autowired JdbcAuditService auditService) throws Exception {
    ...
    List<AuditEvent> events =
        auditService.find(
            jacobId, System.currentTimeMillis() - 10000, IdentityZoneHolder.get().getId());
  ...
  void userNotFoundLoginUnsuccessfulTest() throws Exception {
    ...
    assertTrue(event1.getAuditEvent().getOrigin().contains("sessionId=<SESSION>"));
    PrincipalAuthenticationFailureEvent event2 =
        (PrincipalAuthenticationFailureEvent) captor.getAllValues().get(1);
  ...
  void userChangePasswordTest() throws Exception {
    ...
    String marissaToken =
        testClient.getUserOAuthAccessToken(
            "app", "appclientsecret", testUser.getUserName(), testPassword, "password.write");
    ...
    MockHttpServletRequestBuilder changePasswordPut =
  ...
  void password_change_recorded_at_dao(@Autowired ScimUserProvisioning provisioning) {
    ScimUser user =
        new ScimUser(
            null, new RandomValueStringGenerator().generate() + "@test.org", "first", "last");
    user.setPrimaryEmail(user.getUserName());
    user = provisioning.createUser(user, "oldpassword", IdentityZoneHolder.get().getId());
    provisioning.changePassword(
        user.getId(), "oldpassword", "newpassword", IdentityZoneHolder.get().getId());
    ArgumentCaptor<AbstractUaaEvent> captor = ArgumentCaptor.forClass(AbstractUaaEvent.class);
    verify(listener, times(2)).onApplicationEvent(captor.capture());
  ...
  void changePassword_ReturnsSuccess_WithValidExpiringCode() throws Exception {
    ...
    MockHttpSession session = new MockHttpSession();
    MockHttpServletRequestBuilder changePasswordPost =
  ...
  void clientAuthenticationSuccess() throws Exception {
    ...
    String basicDigestHeaderValue =
        "Basic " + new String(Base64.encodeBase64(("login:loginsecret").getBytes()));
    ...
    verify(listener, times(2)).onApplicationEvent(captor.capture());
    ClientAuthenticationSuccessEvent event =
        (ClientAuthenticationSuccessEvent) captor.getAllValues().get(0);
  ...
  void clientAuthenticationFailure() throws Exception {
    ArgumentCaptor<AbstractUaaEvent> captor = ArgumentCaptor.forClass(AbstractUaaEvent.class);
    String basicDigestHeaderValue =
        "Basic " + new String(Base64.encodeBase64(("login:loginsecretwrong").getBytes()));
    MockHttpServletRequestBuilder oauthTokenPost =
        post("/oauth/token")
            .header("Authorization", basicDigestHeaderValue)
            .param("grant_type", "client_credentials")
  ...
  void clientAuthenticationFailureClientNotFound() throws Exception {
    ArgumentCaptor<AbstractUaaEvent> captor = ArgumentCaptor.forClass(AbstractUaaEvent.class);
    String basicDigestHeaderValue =
        "Basic " + new String(Base64.encodeBase64(("login2:loginsecret").getBytes()));
    MockHttpServletRequestBuilder oauthTokenPost =
        post("/oauth/token")
            .header("Authorization", basicDigestHeaderValue)
            .param("grant_type", "client_credentials")
    ...
    mockMvc.perform(oauthTokenPost).andExpect(status().isUnauthorized());
    verify(listener, atLeast(1)).onApplicationEvent(captor.capture());
    PrincipalAuthenticationFailureEvent event0 =
        (PrincipalAuthenticationFailureEvent) captor.getAllValues().get(0);
    assertEquals("login2", event0.getAuditEvent().getPrincipalId());
    ClientAuthenticationFailureEvent event1 =
        (ClientAuthenticationFailureEvent) captor.getAllValues().get(1);
  ...
  void testUserApprovalAdded() throws Exception {
    ...
    String marissaToken =
        testClient.getUserOAuthAccessToken(
            "login", "loginsecret", testUser.getUserName(), testPassword, "oauth.approvals");
    Approval[] approvals = {
      new Approval()
          .setUserId(null)
          .setClientId("app")
    ...
    MockHttpSession session = new MockHttpSession();
    MockHttpServletRequestBuilder approvalsPut =
        put("/approvals")
            .accept(APPLICATION_JSON_VALUE)
            .contentType(MediaType.APPLICATION_JSON)
            .session(session)
    ...
    ApprovalModifiedEvent approvalModifiedEvent =
        (ApprovalModifiedEvent) testListener.getLatestEvent();
  ...
  void testUserCreatedEvent() throws Exception {
    ...
    String username = "jacob" + new RandomValueStringGenerator().generate(),
        firstName = "Jacob",
        lastName = "Gyllenhammar",
        email = "jacob@gyllenhammar.non";
    ...
    MockHttpSession session = new MockHttpSession();
    MockHttpServletRequestBuilder userPost =
        post("/Users")
            .accept(APPLICATION_JSON_VALUE)
            .contentType(MediaType.APPLICATION_JSON)
            .session(session)
  ...
  void testUserModifiedAndDeleteEvent() throws Exception {
    ...
    user =
        JsonUtils.readValue(result.andReturn().getResponse().getContentAsString(), ScimUser.class);
    testListener.clearEvents();

    user.setName(new ScimUser.Name(modifiedFirstName, lastName));
    MockHttpServletRequestBuilder userPut =
    ...
    // delete the user
    testListener.clearEvents();
    MockHttpServletRequestBuilder userDelete =
  ...
  void testUserVerifiedEvent() throws Exception {
    ...
    MockHttpServletRequestBuilder verifyGet =
        get("/Users/" + user.getId() + "/verify")
            .accept(APPLICATION_JSON_VALUE)
            .session(session)
            .header("Authorization", "Bearer " + adminToken)
            .header("If-Match", user.getVersion());
  ...
  void testGroupEvents() throws Exception {
    ...
    MockHttpServletRequestBuilder groupPost =
        post("/Groups")
            .accept(APPLICATION_JSON_VALUE)
            .contentType(MediaType.APPLICATION_JSON)
            .header("Authorization", "Bearer " + adminToken)
    ...
    group =
        JsonUtils.readValue(result.andReturn().getResponse().getContentAsString(), ScimGroup.class);
    ...
    assertEquals(
        new GroupModifiedEvent.GroupInfo(
            group.getDisplayName(), ScimEventPublisher.getMembers(group)),
    ...
    MockHttpServletRequestBuilder groupPut =
    ...
    // delete the group
    MockHttpServletRequestBuilder groupDelete =
  ...
  private void verifyGroupAuditData(ScimGroup group, AuditEventType eventType) {
    ...
    Map<String, Object> auditObjects =
        JsonUtils.readValue(auditEventData, new TypeReference<Map<String, Object>>() {});
    assertEquals("testgroup", auditObjects.get("group_name"));
    assertThat(
        (Collection<String>) auditObjects.get("members"),
        containsInAnyOrder(ScimEventPublisher.getMembers(group)));
  }
  ...
  private ScimUser createUser(
    ...
    return JsonUtils.readValue(
        result.andReturn().getResponse().getContentAsString(), ScimUser.class);
  }

  private class DefaultApplicationListener<T extends ApplicationEvent>
  ...
  private String requestExpiringCode(String email, String token) throws Exception {
    MockHttpServletRequestBuilder resetPasswordPost =
        post("/password_resets")
            .accept(APPLICATION_JSON_VALUE)
            .contentType(MediaType.APPLICATION_JSON)
            ...
            .content(email);
    MvcResult mvcResult =
        mockMvc.perform(resetPasswordPost).andExpect(status().isCreated()).andReturn();
  ...
  private void assertClientEvents(AuditEventType eventType, String[] scopes, String[] authorities) {
    List<AbstractUaaEvent> events =
        testListener
            .getEvents()
            .stream()
            .filter(e -> e instanceof AbstractClientAdminEvent)
            .collect(Collectors.toList());
