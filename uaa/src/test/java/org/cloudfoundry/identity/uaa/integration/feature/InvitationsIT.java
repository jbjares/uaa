...
public class InvitationsIT {
  ...
  public void setup() throws Exception {
    scimToken =
        testClient.getOAuthAccessToken(
            "admin", "adminsecret", "client_credentials", "scim.read,scim.write,clients.admin");
    loginToken =
  ...
  public void invite_fails() {
    ...
    ResponseEntity<Void> response =
        uaaTemplate.exchange(
            uaaUrl + "/invite_users/?client_id=admin&redirect_uri={uri}",
            POST,
            request,
            Void.class,
  ...
  public void performInviteUser(String email, boolean isVerified) throws Exception {
    ...
    String invitedUserId =
        IntegrationTestUtils.getUserIdByField(scimToken, baseUrl, OriginKeys.UAA, "email", email);
    if (isVerified) {
    ...
    if (!isVerified) {
      ...
      assertTrue(
          IntegrationTestUtils.getUser(scimToken, baseUrl, OriginKeys.UAA, email).isVerified());
      ...
      Assert.assertThat(
          webDriver.findElement(By.cssSelector("h1")).getText(), containsString("Welcome!"));
    }
    String acceptedUserId =
        IntegrationTestUtils.getUserId(scimToken, baseUrl, OriginKeys.UAA, email);
  ...
  public void acceptInvitation_for_samlUser() throws Exception {
    ...
    String code =
        createInvitation(
            testInviteEmail, testInviteEmail, "http://localhost:8080/app/", "simplesamlphp");

    String invitedUserId =
        IntegrationTestUtils.getUserIdByField(
            scimToken, baseUrl, "simplesamlphp", "email", testInviteEmail);
    ...
    String acceptedUsername =
        IntegrationTestUtils.getUsernameById(scimToken, baseUrl, invitedUserId);
  ...
  public void invitedOIDCUserVerified() throws Exception {
    ...
    BaseClientDetails clientDetails =
        new BaseClientDetails(clientId, null, null, "client_credentials", "scim.invite");
    clientDetails.setClientSecret("invite-client-secret");
    testClient.createClient(scimToken, clientDetails);
    String inviteToken =
        testClient.getOAuthAccessToken(
            clientId, "invite-client-secret", "client_credentials", "scim.invite");
    IntegrationTestUtils.createOidcIdentityProvider(
        "oidc-invite-provider", "puppy-invite", baseUrl);
    ...
    ResponseEntity<InvitationsResponse> response =
        uaaTemplate.exchange(
            uaaUrl + "/invite_users?client_id=app&redirect_uri=" + appUrl,
            POST,
            request,
    ...
    IntegrationTestUtils.deleteProvider(
        getZoneAdminToken(baseUrl, serverRunning), baseUrl, "uaa", "puppy-invite");
  }
  ...
  private String createInvitation(
      String username, String userEmail, String redirectUri, String origin) {
    return createInvitation(
        baseUrl, uaaUrl, username, userEmail, origin, redirectUri, loginToken, scimToken);
  }
  ...
  public static String createInvitation(
    ...
    try {
      userId =
          IntegrationTestUtils.getUserIdByField(scimToken, baseUrl, origin, "email", userEmail);
    ...
    if (userId == null) {
      ...
      ResponseEntity<ScimUser> response =
          uaaTemplate.exchange(uaaUrl + "/Users", POST, request, ScimUser.class);
        ...
        new Timestamp(
            System.currentTimeMillis()
                + TimeUnit.MILLISECONDS.convert(
                    System.currentTimeMillis() + 24 * 3600, TimeUnit.MILLISECONDS));
    ExpiringCode expiringCode =
        new ExpiringCode(
            ...
            "{\"origin\":\""
                ...
                + "\"}",
            null);
    HttpEntity<ExpiringCode> expiringCodeRequest =
        new HttpEntity<>(expiringCode, invitationHeaders);
    ResponseEntity<ExpiringCode> expiringCodeResponse =
        uaaTemplate.exchange(uaaUrl + "/Codes", POST, expiringCodeRequest, ExpiringCode.class);
