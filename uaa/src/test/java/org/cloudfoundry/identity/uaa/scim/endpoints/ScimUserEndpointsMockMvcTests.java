...
class ScimUserEndpointsMockMvcTests {
  ...
  @Autowired private JdbcIdentityZoneProvisioning jdbcIdentityZoneProvisioning;
  @Autowired private JdbcMfaProviderProvisioning jdbcMfaProviderProvisioning;
  ...
  void setUp() throws Exception {
    ...
    mfaCredentialsProvisioning =
        webApplicationContext.getBean(JdbcUserGoogleMfaCredentialsProvisioning.class);
    ...
    usersMaxCount =
        Integer.parseInt(webApplicationContext.getEnvironment().getProperty("userMaxCount"));
  }
  ...
  void tearDown() {
    jdbcIdentityZoneProvisioning
        .retrieveAll()
        .forEach(
            identityZone -> jdbcMfaProviderProvisioning.deleteByIdentityZone(identityZone.getId()));
  }

  @Test
  void unauthorized_put_returns_401() throws Exception {
    mockMvc.perform(put("/Users/some-user")).andExpect(status().isUnauthorized());

    mockMvc.perform(put("/Users")).andExpect(status().isUnauthorized());
  }
  ...
  void default_password_policy_does_not_allow_empty_passwords() throws Exception {
    ...
    IdentityProvider<UaaIdentityProviderDefinition> uaa =
        webApplicationContext
            ...
            .retrieveByOrigin(OriginKeys.UAA, otherIdentityZone.getId());

    ResultActions result =
        createUserAndReturnResult(
            scimUser,
            uaaAdminToken,
            IdentityZone.getUaa().getSubdomain(),
            otherIdentityZone.getId());
    result
        .andExpect(status().isBadRequest())
        .andExpect(
            jsonPath("$.message").value("Password must be at least 1 characters in length."));
  }
  ...
  void verification_link_in_non_default_zone_using_switch() throws Exception {
    ...
    String zonedScimCreateToken =
        MockMvcUtils.getClientCredentialsOAuthAccessToken(
            mockMvc, zonedClientId, zonedClientSecret, "uaa.admin", null);
  ...
  void verification_link_user_not_found() throws Exception {
    MockHttpServletRequestBuilder get =
        MockMvcRequestBuilders.get("/Users/12345/verify-link")
  ...
  void listUsers_in_anotherZone() throws Exception {
    ...
    MockHttpServletRequestBuilder get =
        MockMvcRequestBuilders.get("/Users")
            .param("count", Integer.toString(usersMaxCountWithOffset))
  ...
  void testUserSelfAccess_Get_and_Post() throws Exception {
    ...
    String selfToken =
        testClient.getUserOAuthAccessToken("cf", "", savedUser.getUserName(), "secret", "");
  ...
  void testUnlockAccount() throws Exception {
    ...
    attemptLogin(userToLockout).andExpect(redirectedUrl("/"));
  }
  ...
  void testAccountStatusEmptyPatchDoesNotUnlock() throws Exception {
    ...
    attemptLogin(userToLockout).andExpect(redirectedUrl("/login?error=account_locked"));
  }
  ...
  void testUpdateStatusCannotLock() throws Exception {
    ...
    attemptLogin(user).andExpect(redirectedUrl("/"));
  }
  ...
  void testForcePasswordExpireAccountInvalid() throws Exception {
    ...
    assertFalse(
        usersRepository.checkPasswordChangeIndividuallyRequired(
            user.getId(), IdentityZoneHolder.get().getId()));
  }
  ...
  void testTryMultipleStatusUpdatesWithInvalidRemovalOfPasswordChange() throws Exception {
    ...
    attemptLogin(user).andExpect(redirectedUrl("/login?error=account_locked"));
  }
  ...
  class WhenSelfEditing {
    ...
    class WhenAnAdminSelfEdits {
      ...
      void beforeEach() {
        zoneSeeder
            .withClientWithImplicitPasswordRefreshTokenGrants(
                "admin_client", "uaa.admin,scim.write")
            .withUserWhoBelongsToGroups(
                "admin@test.org", Lists.newArrayList("uaa.admin", "scim.write"))
            .afterSeeding(
                zs -> {
      ...
      @Test
      void patch_usingAnAccessTokenWithScimWriteScope_aUserCanSelfUpdateAnything()
          throws Exception {
      ...
      private void performSelfEdit_shouldSucceed(
          String scopesToBeIncludedInToken, MockHttpServletRequestBuilder requestBuilder)
          throws Exception {
        String accessToken =
    ...
    class WhenARegularUserSelfEdits {
      ...
      @Test
      void patch_selfUpdate_WithAccessToken_WhenTryingToDeleteAField_shouldResultIn403()
          throws Exception {
      ...
      @Test
      void patch_selfUpdate_WithAccessToken_WhenFamilyNameAndGivenNameAreChanged_shouldResultIn200()
          throws Exception {
        test_patch_selfUpdate_WithAccessToken(
      ...
      @Test
      void patch_selfUpdate_WithAccessToken_WhenPrimaryEmailIsChanged_shouldResultIn403()
          throws Exception {
      ...
      void test_patch_selfUpdate_WithAccessToken(
      ...
      private String getAccessTokenForUser(ScimUser scimUser) throws Exception {
        return testClient.getUserOAuthAccessTokenForZone(
            ...
            zoneSeeder.getPlainTextClientSecret(
                zoneSeeder.getClientWithImplicitPasswordRefreshTokenGrants()),
  ...
  void testCreateUserWithEmailDomainNotAllowedForOriginUaa() throws Exception {
    ...
    IdentityProvider oidcProvider =
        new IdentityProvider()
            .setActive(true)
            .setName("OIDC_test")
            .setType(OriginKeys.OIDC10)
            .setOriginKey(OriginKeys.OIDC10)
            .setConfig(new OIDCIdentityProviderDefinition());
    ...
    try {
      ...
      mockMvc
          ...
          .andExpect(
              jsonPath("$.message")
                  .value(
                      "The user account is set up for single sign-on. Please use one of these origin(s) : [oidc1.0]"));
    } finally {
      identityProviderProvisioning.deleteByOrigin(
          oidcProvider.getOriginKey(), IdentityZoneHolder.getUaaZone().getId());
    }
  }
  ...
  void testDeleteMfaUserCredentials() throws Exception {
    ...
    UserGoogleMfaCredentials creds =
        new UserGoogleMfaCredentials(user.getId(), "ABCDEFGHIJKLMNOP", 1234, Arrays.asList(123456))
            .setMfaProviderId(provider.getId());
  ...
  void testDeleteMfaUserCredentialsWithZoneSwitching() throws Exception {
    ...
    String uaaAdminTokenFromOtherZone =
        testClient.getClientCredentialsOAuthAccessToken(
            "switchClientId", "switchClientSecret", authorities);
    ScimUser user = setUpScimUser(identityZone);
  ...
  void testDeleteMfaUserCredentialsNotAuthorized() throws Exception {
    ...
    mockMvc.perform(delete).andExpect(status().isForbidden());
  }
  ...
  private ScimUser updateUser(String token, int status, ScimUser user) throws Exception {
    ...
    if (status == HttpStatus.OK.value()) {
      ...
      mockMvc.perform(put).andExpect(status().is(status));
      return null;
    }
  }

  private ResultActions updateAccountStatus(ScimUser user, UserAccountStatus alteredAccountStatus)
      throws Exception {
  ...
  private void attemptUnsuccessfulLogin(int numberOfAttempts, String username, String subdomain)
      throws Exception {
    ...
    for (int i = 0; i < numberOfAttempts; i++) {
      mockMvc.perform(post).andExpect(redirectedUrl("/login?error=login_failure"));
    }
  }
  ...
  private ScimUser getAndReturnUser(int status, ScimUser user, String token) throws Exception {
    ...
    if (status == HttpStatus.OK.value()) {
      ...
      mockMvc.perform(get).andExpect(status().is(status));
      return null;
    }
  }
