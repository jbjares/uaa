...
public class MfaProviderEndpointsMockMvcTests {
  ...
  @Autowired public GenericWebApplicationContext webApplicationContext;
  String adminToken;
  ...
  public void testCreateGoogleMfaProviderConfigDefaults() throws Exception {
    ...
    MfaProvider<GoogleMfaProviderConfig> mfaProviderCreated =
        JsonUtils.readValue(mfaResponse.getResponse().getContentAsString(), MfaProvider.class);
  ...
  public void testCreateGoogleMfaProviderInvalidType() throws Exception {
    MfaProvider mfaProvider = constructGoogleMfaProvider();
    ObjectNode mfaAsJSON =
        (ObjectNode) JsonUtils.readTree(JsonUtils.writeValueAsString(mfaProvider));
    mfaAsJSON.put("type", "not-google-authenticator");
    ResultActions authorization =
        mockMvc.perform(
            post("/mfa-providers")
                .header("Authorization", "Bearer " + adminToken)
                .contentType(APPLICATION_JSON)
    ...
    assertEquals(
        ...
        authorization.andReturn().getResponse().getStatus());
  }
  ...
  public void testCreateMfaProviderInvalidIssuer() throws Exception {
    ...
    MfaProvider<GoogleMfaProviderConfig> mfaProvider =
        constructGoogleMfaProvider().setConfig(config);
    String name = new RandomValueStringGenerator(5).generate();
    mfaProvider.setName(name);
    MvcResult mfaResponse =
        mockMvc
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.error_description")
                .value("An MFA Provider with that name already exists."));
  }
  ...
  public void testRetrieveMfaProviders() throws Exception {
    int mfaProvidersCount =
        mfaProviderProvisioning.retrieveAll(IdentityZoneHolder.get().getId()).size();
    MvcResult authorization =
        mockMvc
            .perform(get("/mfa-providers").header("Authorization", "Bearer " + adminToken))
            .andReturn();

    assertEquals(HttpStatus.OK.value(), authorization.getResponse().getStatus());
    List<MfaProvider> mfaProviders =
        JsonUtils.readValue(authorization.getResponse().getContentAsString(), List.class);
  ...
  public void testRetrieveMfaProviderById() throws Exception {
    ...
    createdProvider =
        mfaProviderProvisioning.create(createdProvider, IdentityZoneHolder.get().getId());
    MvcResult result =
        mockMvc
            .perform(
    ...
    assertEquals(
        JsonUtils.writeValueAsString(createdProvider), result.getResponse().getContentAsString());
  }
  ...
  public void testGetMfaInOtherZone() throws Exception {
    ...
    MvcResult createResult =
        mockMvc
            ...
            .andReturn();
    mfaProvider =
        JsonUtils.readValue(createResult.getResponse().getContentAsString(), MfaProvider.class);

    MvcResult mfaListResult =
        mockMvc
            .perform(
                get("/mfa-providers")
                    .header("Authorization", "Bearer " + adminToken)
                    .header(IdentityZoneSwitchingFilter.HEADER, identityZone.getId()))
            .andReturn();
    List<Map> mfaProviders =
        JsonUtils.readValue(mfaListResult.getResponse().getContentAsString(), List.class);
    List providerIds =
        mfaProviders.stream().map(provider -> provider.get("id")).collect(Collectors.toList());
  ...
  public void testRetrieveMfaProviderByIdInvalid() throws Exception {
    MvcResult authorization =
        mockMvc
            .perform(get("/mfa-providers/abcd").header("Authorization", "Bearer " + adminToken))
            .andReturn();
  ...
  public void testDeleteMfaProvider() throws Exception {
    ...
    mockMvc
        .perform(
            delete("/mfa-providers/" + provider.getId())
                .header("Authorization", "Bearer " + adminToken))
  ...
  public void testDeleteZoneActiveMfaProviderShouldFail() throws Exception {
    ...
    String deleteResponse =
        mockMvc
            .perform(
                delete("/mfa-providers/" + mfaProvider.getId())
                    .header("Authorization", "Bearer " + adminToken)
                    .header("X-Identity-Zone-Id", identityZone.getId()))
            .andExpect(status().isConflict())
            .andReturn()
    ...
    assertThat(
        deleteResponse,
        containsString("MFA provider is currently active on zone: " + identityZone.getId()));
  }

  @Test
  public void testNonExistentMfaProviderDelete() throws Exception {
    mockMvc
        .perform(delete("/mfa-providers/invalid").header("Authorization", "Bearer " + adminToken))
  ...
  public void testDeleteForbiddenForNonAdmin() throws Exception {
    mockMvc
        .perform(
            delete("/mfa-providers/invalid").header("Authorization", "Bearer " + nonAdminToken))
        .andExpect(status().isForbidden())
        .andReturn();
    assertEquals(0, eventListener.getEventCount());
  }
  ...
  public void testDeleteZoneAlsoDeletesMfaProviderInThatZone() throws Exception {
    ...
    MfaProviderProvisioning providerProvisioning =
        webApplicationContext.getBean(JdbcMfaProviderProvisioning.class);
