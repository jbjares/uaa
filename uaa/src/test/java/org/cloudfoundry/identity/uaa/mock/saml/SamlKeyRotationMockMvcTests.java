...
class SamlKeyRotationMockMvcTests {
  ...
  @BeforeEach
  void createZone(@Autowired TestClient testClient, @Autowired MockMvc mockMvc) throws Exception {
    token =
  ...
  void key_rotation(String url) throws Exception {
    ...
    assertThat(
        ...
        containsInAnyOrder(clean(legacyCertificate), clean(certificate1), clean(certificate2)));
  ...
  private String getMetadata(String uri) throws Exception {
    return mockMvc
        .perform(
            get(uri).header("Host", zone.getSubdomain() + ".localhost").accept(APPLICATION_XML))
  ...
  private void evaluateSignatureKey(String metadata, String expectedKey) throws Exception {
    ...
    NodeList nodeList =
        SamlTestUtils.evaluateXPathExpression(SamlTestUtils.getMetadataDoc(metadata), xpath);
