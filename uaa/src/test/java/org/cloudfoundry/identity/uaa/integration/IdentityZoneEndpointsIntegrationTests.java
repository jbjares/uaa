...
public class IdentityZoneEndpointsIntegrationTests {
  ...
  public void cleanup() throws Exception {
    ...
    String groupId =
        IntegrationTestUtils.findGroupId(
            client, serverRunning.getBaseUrl(), String.format("zones.%s.admin", zoneId));
    IntegrationTestUtils.deleteGroup(
        clientCredentialsToken, "", serverRunning.getBaseUrl(), groupId);
  }
  ...
  public void testCreateZone() throws Exception {
    ...
    String clientCredentialsToken =
        IntegrationTestUtils.getClientCredentialsToken(serverRunning, "admin", "adminsecret");
    ScimGroup group =
        IntegrationTestUtils.createGroup(
            clientCredentialsToken, "", serverRunning.getBaseUrl(), scimGroup);
    IntegrationTestUtils.addMemberToGroup(
        adminClient, serverRunning.getBaseUrl(), user.getId(), group.getId());
    ...
    assertNull(
        ObjectUtils.castInstance(identityProvider.getConfig(), UaaIdentityProviderDefinition.class)
            .getPasswordPolicy());
  }
  ...
  public void testCreateZoneWithClient() throws IOException {
    ...
    BaseClientDetails clientDetails =
        new BaseClientDetails(
            "test123", null, "openid", GRANT_TYPE_AUTHORIZATION_CODE, "uaa.resource");
    clientDetails.setClientSecret("testSecret");
    clientDetails.addAdditionalInformation(
        ClientConstants.ALLOWED_PROVIDERS, Collections.singleton(OriginKeys.UAA));
