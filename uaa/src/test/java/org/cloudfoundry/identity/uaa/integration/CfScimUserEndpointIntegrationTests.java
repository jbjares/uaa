...
/**
 * Integration test to verify that the trusted client use cases are supported adequately for cf.
 *
 * @author Luke Taylor
 * @author Dave Syer
 */
...
public class CfScimUserEndpointIntegrationTests {
  ...
  public void setUpUserAccounts() {
    ...
    ResponseEntity<ScimUser> newuser =
        client.postForEntity(serverRunning.getUrl(usersEndpoint), user, ScimUser.class);
    ...
    ResponseEntity<Void> result =
        client.exchange(
            serverRunning.getUrl(usersEndpoint) + "/{id}/password",
            HttpMethod.PUT,
  ...
  private ResponseEntity<Map> deleteUser(RestOperations client, String id, int version) {
    ...
    return client.exchange(
        serverRunning.getUrl(usersEndpoint + "/{id}"),
        HttpMethod.DELETE,
