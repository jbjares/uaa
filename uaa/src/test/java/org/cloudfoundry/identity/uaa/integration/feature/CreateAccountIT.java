...
public class CreateAccountIT {
  ...
  public void testUserInitiatedSignup() throws Exception {
    ...
    assertEquals(
        "Please check email for an activation link.",
    ...
    webDriver.get(link);
    assertThat(
        webDriver.findElement(By.cssSelector("h1")).getText(), not(containsString("Where to?")));
  ...
  public void testEnteringContraveningPasswordShowsErrorMessage() {
    ...
    assertEquals(
        "Password must be no more than 255 characters in length.",
  ...
  public void testEmailDomainRegisteredWithIDPDoesNotAllowAccountCreation() throws Exception {
    ...
    try {
      ...
      assertEquals(
          "Account sign-up is not required for this email domain. Please login with the identity provider",
          webDriver.findElement(By.cssSelector(".alert-error")).getText());
      webDriver.findElement(By.xpath("//input[@value='Login with provider']")).click();
      assertThat(
          webDriver.getCurrentUrl(), startsWith(oidcProvider.getConfig().getAuthUrl().toString()));
    } finally {
