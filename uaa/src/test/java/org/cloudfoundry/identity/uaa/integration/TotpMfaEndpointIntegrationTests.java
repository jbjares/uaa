...
public class TotpMfaEndpointIntegrationTests {
  ...
  public void setup() throws Exception {
    ...
    adminClient = IntegrationTestUtils.getClientCredentialsTemplate(adminResource);
    ...
    adminAccessToken =
        testClient.getOAuthAccessToken("admin", "adminsecret", "client_credentials", "uaa.admin");
    zoneAdminToken =
  ...
  public void cleanup() {
    ...
    IntegrationTestUtils.createZoneOrUpdateSubdomain(
        adminClient, baseUrl, mfaZone.getId(), mfaZone.getSubdomain(), mfaZone.getConfig());
  }
  ...
  public void force_password_happens_after_MFA() throws Exception {
    IntegrationTestUtils.updateUserToForcePasswordChange(
        getRestTemplate(), baseUrl, adminAccessToken, user.getId(), mfaZone.getId());
  ...
  public void testQRCodeScreenAfterRegistrationDeletion() throws Exception {
    ...
    ResponseEntity<Map> exchange =
        client.exchange(
            serverRunning.getUrl("/Users?attributes=id&filter={filter}"),
            HttpMethod.GET,
            new HttpEntity<Void>(headers),
            Map.class,
            uriParams);
    String userId =
        (String) ((Map) ((java.util.List) exchange.getBody().get("resources")).get(0)).get("id");

    client.exchange(
        serverRunning.getUrl("/Users/{userId}/mfa"),
        HttpMethod.DELETE,
        new HttpEntity<Void>(headers),
        Map.class,
        userId);
  ...
  private String getSecretFromQrImageString(String imageSrc) throws Exception {
    ...
    for (String param : qparams) {
      if (param.contains("issuer=")) {
        assertEquals(
            "issuer=" + mfaProvider.getConfig().getIssuer(), URLDecoder.decode(param, "UTF-8"));
        break;
      }
    }
    String secretKey = "";
  ...
  private MfaProvider enableMfaInZone(String zoneAdminToken) {
    MfaProvider provider =
        IntegrationTestUtils.createGoogleMfaProvider(
            baseUrl, zoneAdminToken, MockMvcUtils.constructGoogleMfaProvider(), mfaZone.getId());
    mfaZone.getConfig().getMfaConfig().setEnabled(true).setProviderName(provider.getName());
    mfaZone =
        IntegrationTestUtils.createZoneOrUpdateSubdomain(
            adminClient, baseUrl, "testzone1", mfaZone.getSubdomain(), mfaZone.getConfig());
    return provider;
  }
  ...
  private ScimUser createRandomUser() {
    ...
    return IntegrationTestUtils.createUser(adminAccessToken, baseUrl, user, mfaZone.getId());
  }
}
