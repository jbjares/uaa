...
public class LoginIT {
  ...
  @Autowired SimpleSmtpServer simpleSmtpServer;
  ...
  public void check_JSESSIONID_defaults() throws Exception {
    ...
    ResponseEntity<String> loginResponse =
        template.exchange(
            baseUrl + "/login", HttpMethod.GET, new HttpEntity<>(null, headers), String.class);
  ...
  public void testBannerFunctionalityInDiscoveryPage() {
    ...
    assertEquals(
        "test banner", webDriver.findElement(By.cssSelector(".banner-header span")).getText());
    assertEquals(
        "rgba(68, 68, 68, 1)",
        webDriver.findElement(By.cssSelector(".banner-header")).getCssValue("background-color"));
    assertEquals(
        "rgba(17, 17, 17, 1)",
        webDriver.findElement(By.cssSelector(".banner-header span")).getCssValue("color"));
    ...
    assertEquals(
        ...
        webDriver.findElement(By.cssSelector(".banner-header img")).getAttribute("src"));
    assertEquals(
        2,
        webDriver
            .findElement(By.cssSelector(".banner-header"))
  ...
  public void testSuccessfulLoginNewUser() throws Exception {
    ...
    assertThat(
        ...
        Matchers.containsString("Where to?"));
    webDriver.get(baseUrl + "/logout.do");
    attemptLogin(newUserEmail, USER_PASSWORD);
  ...
  public void testLoginHint() throws Exception {
    ...
    assertThat(
        ...
        containsString("Unable to verify email or password. Please try again."));
  ...
  public void testNoZoneFound() throws Exception {
    assertTrue(
        "Expected testzone1/2/3/4/doesnotexist.localhost to resolve to 127.0.0.1",
        doesSupportZoneDNS());
  ...
  public void testPasscodeRedirect() throws Exception {
    ...
    assertThat(
        webDriver.findElement(By.cssSelector("h1")).getText(),
        Matchers.containsString("Temporary Authentication Code"));
  }
  ...
  public void testUnsuccessfulLogin() throws Exception {
    ...
    assertThat(
        webDriver.findElement(By.cssSelector("h1")).getText(), Matchers.containsString("Welcome!"));
  }
  ...
  public void testAccessDeniedIfCsrfIsMissing() throws Exception {
    ...
    ResponseEntity<String> loginResponse =
        template.exchange(
            baseUrl + "/login.do", HttpMethod.POST, new HttpEntity<>(body, headers), String.class);
    assertEquals(HttpStatus.FOUND, loginResponse.getStatusCode());
    assertTrue(
        "CSRF message should be shown",
        loginResponse.getHeaders().getFirst("Location").contains("invalid_login_request"));
  }
  ...
  public void testCsrfIsResetDuringLoginPageReload() {
    ...
    String csrf1 =
        webDriver
            .manage()
            .getCookieNamed(CookieBasedCsrfTokenRepository.DEFAULT_CSRF_COOKIE_NAME)
            .getValue();
    webDriver.get(baseUrl + "/login");
    String csrf2 =
        webDriver
  ...
  public void userLockedoutAfterUnsuccessfulAttempts() throws Exception {
    ...
    assertThat(
        webDriver.findElement(By.cssSelector(".alert-error")).getText(),
  ...
  public void testBuildInfo() throws Exception {
    ...
    assertTrue(
        webDriver
            .findElement(By.cssSelector(".footer .copyright"))
            .getAttribute("title")
            .matches(regex));
  }
  ...
  public void testAccountChooserManualLogin() throws Exception {
    ...
    assertEquals(
        ...
        webDriver.findElement(By.cssSelector("div.action a")).getText());
  ...
  public void testAccountChooserPopulatesUsernameNotEmailWhenOriginIsUAAorLDAP() throws Exception {
    String userUAA =
        "{\"userId\":\"1\",\"username\":\"userUAA\",\"origin\":\"uaa\",\"email\":\"user@uaa.org\"}";
    String userLDAP =
        "{\"userId\":\"2\",\"username\":\"userLDAP\",\"origin\":\"ldap\",\"email\":\"user@ldap.org\"}";
    String userExternal =
        "{\"userId\":\"3\",\"username\":\"userExternal\",\"origin\":\"external\",\"email\":\"user@external.org\"}";
    ...
    js.executeScript(
        "document.cookie = \"Saved-Account-1="
            + URLEncoder.encode(userUAA, StandardCharsets.UTF_8.name())
            + ";path=/;domain=testzone3.localhost\"");
    js.executeScript(
        "document.cookie = \"Saved-Account-2="
            + URLEncoder.encode(userLDAP, StandardCharsets.UTF_8.name())
            + ";path=/;domain=testzone3.localhost\"");
    js.executeScript(
        "document.cookie = \"Saved-Account-3="
            + URLEncoder.encode(userExternal, StandardCharsets.UTF_8.name())
            + ";path=/;domain=testzone3.localhost\"");
    ...
    assertEquals(
        "user@external.org", webDriver.findElement(By.id("username")).getAttribute("value"));
  ...
  public void testLoginReloadRetainsFormRedirect() {
    ...
    ((JavascriptExecutor) webDriver)
        .executeScript("document.getElementsByName('X-Uaa-Csrf')[0].value=''");
    ...
    assertThat(
        webDriver.findElement(By.name("form_redirect_uri")).getAttribute("value"),
        Matchers.containsString("redirect_uri=" + redirectUri));
  }
  ...
  private String createAnotherUser(String url) {
    return IntegrationTestUtils.createAnotherUser(
        webDriver, USER_PASSWORD, simpleSmtpServer, url, testClient);
  }
  ...
  private String createDiscoveryZone() {
    ...
    IntegrationTestUtils.createZoneOrUpdateSubdomain(
        identityClient, baseUrl, testzone3, testzone3, config);
