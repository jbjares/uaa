...
public class OIDCLoginIT {
  ...
  @Rule public ScreenshotOnFail screenShootRule = new ScreenshotOnFail();
  ...
  public void setUp() throws Exception {
    assertTrue(
        "/etc/hosts should contain the host 'oidcloginit.localhost' for this test to work",
        doesSupportZoneDNS());
    ...
    zone =
        IntegrationTestUtils.createZoneOrUpdateSubdomain(
            identityClient, baseUrl, subdomain, subdomain, zoneConfiguration);
    ...
    createdGroup =
        IntegrationTestUtils.createOrUpdateGroup(
            adminToken, subdomain, baseUrl, new ScimGroup(createdGroupName));
    ScimGroupExternalMember createdGroupExternalMapping =
        new ScimGroupExternalMember(createdGroup.getId(), "openid");
    createdGroupExternalMapping.setOrigin(identityProvider.getOriginKey());
    IntegrationTestUtils.mapExternalGroup(
        adminToken, subdomain, baseUrl, createdGroupExternalMapping);
    ...
    zoneClient =
        IntegrationTestUtils.createClientAsZoneAdmin(
            clientCredentialsToken, baseUrl, zone.getId(), zoneClient);
    zoneClient.setClientSecret("secret");
  ...
  public void updateProvider() {
    identityProvider =
        IntegrationTestUtils.createOrUpdateProvider(
            clientCredentialsToken, baseUrl, identityProvider);
  ...
  public static boolean doesSupportZoneDNS() {
    try {
      return Arrays.equals(
          Inet4Address.getByName("oidcloginit.localhost").getAddress(), new byte[] {127, 0, 0, 1});
  ...
  private void doLogout(String zoneUrl) {
    for (String url :
        Arrays.asList(
            "http://simplesamlphp.cfapps.io/module.php/core/authenticate.php?as=example-userpass&logout",
            baseUrl + "/logout.do",
            zoneUrl + "/logout.do")) {
      webDriver.get(url);
  ...
  public void successfulLoginWithOIDCProvider() throws Exception {
    ...
    String zoneAdminToken =
        IntegrationTestUtils.getClientCredentialsToken(serverRunning, "admin", "adminsecret");
    String origUserId =
        IntegrationTestUtils.getUserId(adminToken, baseUrl, "uaa", testAccounts.getUserName());
    ScimUser user =
        IntegrationTestUtils.getUserByZone(
            zoneAdminToken, baseUrl, subdomain, testAccounts.getUserName());
  ...
  public void testLoginWithInactiveProviderDoesNotWork() throws Exception {
    ...
    assertNotNull(beforeLogin.getValue());
    String linkLocation =
        webDriver.findElement(By.linkText("My OIDC Provider")).getAttribute("href");
    ...
    assertThat(
        ...
        containsString("Could not resolve identity provider with given origin."));
    webDriver.get(zoneUrl + "/");
  ...
  public void successfulLoginWithOIDCProviderWithExternalGroups() throws Exception {
    ...
    ScimUser user =
        IntegrationTestUtils.getUserByZone(
            adminToken, baseUrl, subdomain, testAccounts.getUserName());
    assertEquals(user.getGivenName(), user.getUserName());

    ScimGroup updatedCreatedGroup =
        IntegrationTestUtils.getGroup(
            adminToken, subdomain, baseUrl, createdGroup.getDisplayName());
  ...
  public void login_with_wrong_keys() throws Exception {
    identityProvider
        ...
        .setTokenKeyUrl(
            new URL(
                "https://login.microsoftonline.com/9bc40aaf-e150-4c30-bb3c-a8b3b677266e/discovery/v2.0/keys"));
    updateProvider();
    ...
    Assert.assertThat(
        ...
        containsString(
            zoneUrl
                + "/oauth_error?error=There+was+an+error+when+authenticating+against+the+external+identity+provider"));
  ...
  public void testShadowUserNameDefaultsToOIDCSubjectClaim() throws Exception {
    Map<String, Object> attributeMappings =
        new HashMap<>(identityProvider.getConfig().getAttributeMappings());
    ...
    BaseClientDetails client =
        new BaseClientDetails(
            clientId, null, "openid", GRANT_TYPE_AUTHORIZATION_CODE, "openid", baseUrl);
    ...
    Map<String, String> authCodeTokenResponse =
        IntegrationTestUtils.getAuthorizationCodeTokenMap(
            serverRunning,
    ...
    Map<String, Object> claims =
        JsonUtils.readValue(idTokenClaims.getClaims(), new TypeReference<Map<String, Object>>() {});
    String expectedUsername = (String) claims.get(SUB);

    String adminToken =
        IntegrationTestUtils.getClientCredentialsToken(
            zoneUrl, zoneClient.getClientId(), zoneClient.getClientSecret());
    ScimUser shadowUser =
        IntegrationTestUtils.getUser(
            adminToken, zoneUrl, identityProvider.getOriginKey(), expectedUsername);
  ...
  public void successfulLoginWithOIDC_and_SAML_Provider() throws Exception {
    SamlIdentityProviderDefinition saml =
        IntegrationTestUtils.createSimplePHPSamlIDP("simplesamlphp", OriginKeys.UAA);
    ...
    samlProvider =
        IntegrationTestUtils.createOrUpdateProvider(clientCredentialsToken, baseUrl, samlProvider);
    try {
      ...
      */
      ...
      Map<String, Object> claims =
          JsonUtils.readValue(
              idTokenClaims.getClaims(), new TypeReference<Map<String, Object>>() {});
      ...
      assertThat(
          (List<String>) acr.get("values"), containsInAnyOrder(AuthnContext.PASSWORD_AUTHN_CTX));

      UserInfoResponse userInfo =
          IntegrationTestUtils.getUserInfo(zoneUrl, authCodeTokenResponse.get("access_token"));
      ...
      IntegrationTestUtils.deleteProvider(
          clientCredentialsToken, baseUrl, OriginKeys.UAA, samlProvider.getOriginKey());
    }
  }

  @Test
  public void testResponseTypeRequired() throws Exception {
    BaseClientDetails uaaClient =
        new BaseClientDetails(
            new RandomValueStringGenerator().generate(),
            null,
            "openid,user_attributes",
    ...
    uriBuilder
        ...
        .append("?scope=openid&client_id=")
        .append(uaaClient.getClientId())
        .append("&redirect_uri=")
        .append(baseUrl);
    ...
    assertThat(
        webDriver.getCurrentUrl(),
        containsString("error_description=Missing%20response_type%20in%20authorization%20request"));
  }
  ...
  private OIDCIdentityProviderDefinition azureConfig() throws Exception {
    ...
    config.setAuthUrl(
        new URL(
            "https://login.microsoftonline.com/9bc40aaf-e150-4c30-bb3c-a8b3b677266e/oauth2/authorize"));
    config.setTokenUrl(
        new URL(
            "https://login.microsoftonline.com/9bc40aaf-e150-4c30-bb3c-a8b3b677266e/oauth2/token"));
    config.setTokenKeyUrl(
        new URL(
            "https://login.microsoftonline.com/9bc40aaf-e150-4c30-bb3c-a8b3b677266e/discovery/v2.0/keys"));
    config.setShowLinkText(true);
