...
/**
 * Integration test to verify that the userid translation use cases are supported adequately for cf.
 *
 * @author Luke Taylor
 */
...
public class CfUserIdTranslationEndpointIntegrationTests {
  ...
  public void findUsersWithExplicitFilterSucceeds() throws Exception {
    ...
    ResponseEntity<Map> response =
        serverRunning.getForObject(idsEndpoint + "?filter=userName eq \"" + JOE + "\"", Map.class);
  ...
  public void findUsersExplicitEmailFails() throws Exception {
    @SuppressWarnings("rawtypes")
    ResponseEntity<Map> response =
        serverRunning.getForObject(idsEndpoint + "?filter=emails.value sw \"joe\"", Map.class);
    @SuppressWarnings("unchecked")
    Map<String, Object> results = response.getBody();
  ...
  public void findUsersExplicitPresentFails() throws Exception {
    @SuppressWarnings("rawtypes")
    ResponseEntity<Map> response =
        serverRunning.getForObject(idsEndpoint + "?filter=pr userType", Map.class);
    @SuppressWarnings("unchecked")
    Map<String, Object> results = response.getBody();
    assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
    assertNotNull("There should be an error", results.get("error"));
  ...
  public void findUsersExplicitGroupFails() throws Exception {
    @SuppressWarnings("rawtypes")
    ResponseEntity<Map> response =
        serverRunning.getForObject(idsEndpoint + "?filter=groups.display co \"foo\"", Map.class);
    @SuppressWarnings("unchecked")
    Map<String, Object> results = response.getBody();
    assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
    assertNotNull("There should be an error", results.get("error"));
