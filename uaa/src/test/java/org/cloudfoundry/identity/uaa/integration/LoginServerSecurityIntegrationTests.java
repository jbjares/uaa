...
public class LoginServerSecurityIntegrationTests {
  ...
  private final String LOGIN_SERVER_JOE =
      "ls_joe" + new RandomValueStringGenerator().generate().toLowerCase();
  ...
  public void setUpUserAccounts() {
    ...
    ResponseEntity<ScimUser> newuser =
        client.postForEntity(serverRunning.getUrl(userEndpoint), user, ScimUser.class);
    userForLoginServer =
        client
            .postForEntity(serverRunning.getUrl(userEndpoint), userForLoginServer, ScimUser.class)
            .getBody();
    ...
    context.setParameters(
        Collections.singletonMap(
            "credentials", testAccounts.getJsonCredentials(joe.getUserName(), "Passwo3d")));
  }
  ...
  public void testLoginServerCanAuthenticateUserForCf() throws Exception {
    ...
    ResponseEntity<Map> response =
        serverRunning.postForMap(serverRunning.getAuthorizationUri(), params, headers);
  ...
  public void testLoginServerCfPasswordToken() throws Exception {
    ...
    @SuppressWarnings("rawtypes")
    ResponseEntity<Map> response =
        serverRunning.postForMap(serverRunning.getAccessTokenUri(), params, headers);
    assertEquals(HttpStatus.OK, response.getStatusCode());
  ...
  private static class AppClient extends ClientCredentialsResourceDetails {
    ...
    public AppClient(Object target) {
      LoginServerSecurityIntegrationTests test = (LoginServerSecurityIntegrationTests) target;
      ClientCredentialsResourceDetails resource =
          test.testAccounts.getClientCredentialsResource("app", "appclientsecret");
      setClientId(resource.getClientId());
      setClientSecret(resource.getClientSecret());
      setId(getClientId());
      setAccessTokenUri(test.serverRunning.getAccessTokenUri());
