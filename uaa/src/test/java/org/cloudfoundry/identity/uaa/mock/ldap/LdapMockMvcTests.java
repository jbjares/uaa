...
public class LdapMockMvcTests {
  ...
  public static void trustOurCustomCA() {
    ...
    File file =
        new File(classLoader.getResource("certs/truststore-containing-the-ldap-ca.jks").getFile());
  ...
  public void createTestZone() throws Exception {
    ...
    zone =
        MockMvcUtils.createZoneForInvites(
            getMockMvc(), getWebApplicationContext(), userId, REDIRECT_URI);
  ...
  public void acceptInvitation_for_ldap_user_whose_username_is_not_email() throws Exception {
    ...
    getWebApplicationContext()
        .getBean(JdbcTemplate.class)
        .update("DELETE FROM users WHERE email=?", email);
    ...
    code =
        getWebApplicationContext()
            .getBean(JdbcTemplate.class)
            .queryForObject("select code from expiring_code_store", String.class);

    MockHttpSession session = (MockHttpSession) result.getRequest().getSession(false);
    String expectRedirectToLogin =
        "/login?success=invite_accepted&form_redirect_uri=" + URLEncoder.encode(redirectUri);
    getMockMvc()
        .perform(
    ...
    getMockMvc()
        .perform(get(expectRedirectToLogin).with(cookieCsrf()).session(session).header(HOST, host))
        .andExpect(status().isOk())
  ...
  public void test_external_groups_whitelist() throws Exception {
    Assume.assumeThat(
        "ldap-groups-map-to-scopes.xml, ldap-groups-as-scopes.xml",
        StringContains.containsString(ldapGroup));
    AuthenticationManager manager =
        getWebApplicationContext().getBean(DynamicZoneAwareAuthenticationManager.class);
    UsernamePasswordAuthenticationToken token =
        new UsernamePasswordAuthenticationToken("marissa3", "ldap3");
  ...
  public void testCustomUserAttributes() throws Exception {
    ...
    UaaAuthentication authentication =
        (UaaAuthentication)
            ((SecurityContext)
                    result.getRequest().getSession().getAttribute(SPRING_SECURITY_CONTEXT_KEY))
                .getAuthentication();

    assertEquals("Expected two user attributes", 2, authentication.getUserAttributes().size());
    assertNotNull(
        "Expected cost center attribute", authentication.getUserAttributes().get(COST_CENTERS));
    ...
    assertEquals(
        "Expected 2 manager attribute values",
        2,
        authentication.getUserAttributes().get(MANAGERS).size());
    assertThat(
        authentication.getUserAttributes().get(MANAGERS),
  ...
  public void testLdapConfigurationBeforeSave() throws Exception {
    ...
    String identityAccessToken =
        MockMvcUtils.getClientOAuthAccessToken(getMockMvc(), "identity", "identitysecret", "");
    String adminAccessToken =
        MockMvcUtils.getClientOAuthAccessToken(getMockMvc(), "admin", "adminsecret", "");
    IdentityZone zone = MockMvcUtils.createZoneUsingWebRequest(getMockMvc(), identityAccessToken);
    String zoneAdminToken =
        MockMvcUtils.getZoneAdminToken(getMockMvc(), adminAccessToken, zone.getId());
    ...
    IdentityProviderValidationRequest request =
        new IdentityProviderValidationRequest(provider, token);
    ...
    MvcResult result = getMockMvc().perform(post).andExpect(status().isOk()).andReturn();
    ...
    result = getMockMvc().perform(post).andExpect(status().isExpectationFailed()).andReturn();
    ...
    result = getMockMvc().perform(post).andExpect(status().isForbidden()).andReturn();
    ...
    result = getMockMvc().perform(post).andExpect(status().isBadRequest()).andReturn();
    ...
    result = getMockMvc().perform(post).andExpect(status().isOk()).andReturn();
  ...
  public void testLoginInNonDefaultZone() throws Exception {
    assumeFalse(
        !(ldapProfile.contains("ldap-search-and-bind.xml")
            && ldapGroup.contains("ldap-groups-map-to-scopes.xml")));

    getMockMvc()
        .perform(get("/login").header(HOST, host))
        .andExpect(status().isOk())
    ...
    UaaUser user =
        getWebApplicationContext()
            ...
            .retrieveUserByName("marissa2", LDAP);
    IdentityZoneHolder.clear();
    assertNotNull(user);
  ...
  public void testLogin_partial_result_exception_on_group_search() throws Exception {
    ...
    UaaUser user =
        getWebApplicationContext()
            .getBean(UaaUserDatabase.class)
            .retrieveUserByName("marissa8", LDAP);
    IdentityZoneHolder.clear();
    assertNotNull(user);
    assertEquals(LDAP, user.getOrigin());
    assertEquals(zone.getZone().getIdentityZone().getId(), user.getZoneId());
  ...
  public void test_memberOf_search() throws Exception {
    ...
    String[] list = new String[] {"internal.read", "internal.everything", "internal.superuser"};
    ...
    UaaUser user =
        getWebApplicationContext()
            .getBean(UaaUserDatabase.class)
            .retrieveUserByName("marissa10", LDAP);
    IdentityZoneHolder.clear();
    assertNotNull(user);
    assertEquals(LDAP, user.getOrigin());
    assertEquals(zone.getZone().getIdentityZone().getId(), user.getZoneId());
  ...
  public ClassPathXmlApplicationContext getBeanContext() throws Exception {
    DynamicZoneAwareAuthenticationManager zm =
        getWebApplicationContext().getBean(DynamicZoneAwareAuthenticationManager.class);
    zm.getLdapAuthenticationManager(zone.getZone().getIdentityZone(), provider)
  ...
  public void test_read_and_write_config_then_login() throws Exception {
    ...
    IdentityProvider<LdapIdentityProviderDefinition> provider =
        JsonUtils.readValue(
            response, new TypeReference<IdentityProvider<LdapIdentityProviderDefinition>>() {});
  ...
  public void testLogin() throws Exception {
    ...
    IdentityProviderAuthenticationFailureEvent event =
        (IdentityProviderAuthenticationFailureEvent) allValues.get(4);
    ...
    IdentityProviderAuthenticationSuccessEvent successEvent =
        (IdentityProviderAuthenticationSuccessEvent) allValues.get(12);
  ...
  public void testLdapAuthenticationWithMfa() throws Exception {
    ...
    mfaProvider.setConfig(
        (GoogleMfaProviderConfig) new GoogleMfaProviderConfig().setIssuer("issuer"));
    mfaProvider =
        getWebApplicationContext()
            .getBean(JdbcMfaProviderProvisioning.class)
            .create(mfaProvider, zoneId);
    zone.getZone()
        .getIdentityZone()
        .getConfig()
        .setMfaConfig(new MfaConfig().setEnabled(true).setProviderName(mfaProvider.getName()));
    IdentityZone newZone =
        getWebApplicationContext()
            .getBean(JdbcIdentityZoneProvisioning.class)
            .update(zone.getZone().getIdentityZone());
    assertEquals(mfaProvider.getName(), newZone.getConfig().getMfaConfig().getProviderName());
    ResultActions actions =
        performMfaRegistrationInZone(
            "marissa7",
            "ldap7",
            getMockMvc(),
            host,
  ...
  public void testAuthenticate() throws Exception {
    ...
    assertThat(
        result.getResponse().getContentAsString(),
        containsString("\"email\":\"marissa3@test.com\""));
  }
  ...
  public void testAuthenticateFailure() throws Exception {
    ...
    IdentityProviderAuthenticationFailureEvent event =
        (IdentityProviderAuthenticationFailureEvent) allValues.get(3);
  ...
  public void validateOriginForNonLdapUser() throws Exception {
    ...
    MockMvcUtils.createUserInZone(
        getMockMvc(), zone.getAdminToken(), user, zone.getZone().getIdentityZone().getSubdomain());
    ...
    assertThat(
        result.getResponse().getContentAsString(),
        containsString("\"email\":\"marissa@test.org\""));
  ...
  public void validateCustomEmailForLdapUser() throws Exception {
    ...
    assertThat(
        result.getResponse().getContentAsString(),
        containsString("\"email\":\"marissa7@ldaptest.org\""));
    assertEquals(LDAP, getOrigin(username));
  ...
  private MvcResult performAuthentication(String username, String password, HttpStatus status)
      throws Exception {
    MockHttpServletRequestBuilder post =
        post("/authenticate")
            .header(HOST, host)
    ...
    return getMockMvc().perform(post).andExpect(status().is(status.value())).andReturn();
  }

  private MvcResult performUiAuthentication(
      String username, String password, HttpStatus status, boolean authenticated) throws Exception {
    MockHttpServletRequestBuilder post =
        post("/login.do")
            .with(cookieCsrf())
            .header(HOST, host)
  ...
  public void testLdapScopes() throws Exception {
    ...
    Authentication auth = manager.authenticate(token);
    assertNotNull(auth);
    String[] list = new String[] {"uaa.admin", "cloud_controller.read", "thirdmarissa"};
  ...
  public void testLdapScopesFromChainedAuth() throws Exception {
    ...
    AuthenticationManager manager =
        (AuthenticationManager)
            getWebApplicationContext().getBean("zoneAwareAuthzAuthenticationManager");
    UsernamePasswordAuthenticationToken token =
        new UsernamePasswordAuthenticationToken("marissa3", "ldap3");
    IdentityZoneHolder.set(zone.getZone().getIdentityZone());
    Authentication auth = manager.authenticate(token);
  ...
  public void testNestedLdapScopes() throws Exception {
    ...
    Set<String> defaultAuthorities =
        new HashSet(
            zone.getZone().getIdentityZone().getConfig().getUserConfig().getDefaultGroups());
    AuthenticationManager manager =
        getWebApplicationContext().getBean(DynamicZoneAwareAuthenticationManager.class);
    UsernamePasswordAuthenticationToken token =
        new UsernamePasswordAuthenticationToken("marissa4", "ldap4");
    IdentityZoneHolder.set(zone.getZone().getIdentityZone());
    Authentication auth = manager.authenticate(token);
    assertNotNull(auth);
    defaultAuthorities.addAll(Arrays.asList("test.read", "test.write", "test.everything"));
    assertThat(
        UaaStringUtils.getStringsFromAuthorities(auth.getAuthorities()),
        containsInAnyOrder(defaultAuthorities.toArray()));
    IdentityZoneHolder.clear();
  }

  public void transferDefaultMappingsToZone(IdentityZone zone) {
    JdbcScimGroupExternalMembershipManager exm =
        getWebApplicationContext().getBean(JdbcScimGroupExternalMembershipManager.class);
    ScimGroupProvisioning gp = getWebApplicationContext().getBean(ScimGroupProvisioning.class);
    List<String> defaultMappings =
  ...
  public void doTestNestedLdapGroupsMappedToScopes(
      String username, String password, String[] expected) throws Exception {
  ...
  public void testNestedLdapGroupsMappedToScopes() throws Exception {
    String[] list =
        new String[] {
          "internal.read", "internal.write", "internal.everything", "internal.superuser"
        };
  ...
  public void doTestNestedLdapGroupsMappedToScopesWithDefaultScopes(
      String username, String password, String[] expected) throws Exception {
    assumeTrue(ldapGroup.equals("ldap-groups-map-to-scopes.xml"));
    AuthenticationManager manager =
        getWebApplicationContext().getBean(DynamicZoneAwareAuthenticationManager.class);
