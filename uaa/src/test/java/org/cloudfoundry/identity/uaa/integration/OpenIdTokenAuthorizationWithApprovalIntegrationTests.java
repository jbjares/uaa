...
public class OpenIdTokenAuthorizationWithApprovalIntegrationTests {
  ...
  public void createRestTemplate() throws Exception {
    ...
    loginClient.setErrorHandler(
        new OAuth2ErrorHandler(clientCredentials) {
  ...
  public void testOpenIdTokenUsingLoginClientOauthTokenEndpoint() throws Exception {
    ...
    ResponseEntity<Map> responseEntity =
        loginClient.exchange(
            serverRunning.getBaseUrl() + "/oauth/token",
            HttpMethod.POST,
  ...
  public void testOpenIdHybridFlowIdTokenAndCode() throws Exception {
    // non approved
    doOpenIdHybridFlowIdTokenAndReturnCode(
        new HashSet<>(Arrays.asList("token", "code")), ".+access_token=.+code=.+");
    // approved
    doOpenIdHybridFlowIdTokenAndReturnCode(
        new HashSet<>(Arrays.asList("token", "code")), ".+access_token=.+code=.+");
    // approved using login client
    doOpenIdHybridFlowForLoginClient(
        new HashSet<>(Arrays.asList("token", "code")), ".+access_token=.+code=.+");
  }
  ...
  public void testOpenIdHybridFlowIdTokenAndTokenAndCode() throws Exception {
    // non approved
    doOpenIdHybridFlowIdTokenAndReturnCode(
        new HashSet<>(Arrays.asList("token", "id_token", "code")),
  ...
  public void testOpenIdHybridFlowIdTokenAndToken() throws Exception {
    // non approved
    doOpenIdHybridFlowIdTokenAndReturnCode(
        new HashSet<>(Arrays.asList("id_token", "code")), ".+id_token=.+code=.+");
    // approved
    doOpenIdHybridFlowIdTokenAndReturnCode(
        new HashSet<>(Arrays.asList("id_token", "code")), ".+id_token=.+code=.+");
    // approved using login client
  ...
  public void testOpenIdHybridFlowZoneDoesNotExist() {
    ...
    String uri =
        serverRunning
            ...
            .replace("localhost", "testzonedoesnotexist.localhost");
    ...
    String uri =
        serverRunning
            .getUrl(
                "/oauth/authorize?response_type={response_type}&"
                    + "state={state}&client_id={client_id}&redirect_uri={redirect_uri}")
            .replace("localhost", "testzoneinactive.localhost");
    RestTemplate restTemplate = serverRunning.createRestTemplate();
  ...
  private String doOpenIdHybridFlowIdTokenAndReturnCode(
    ...
    formData.add(
        ...
        extractCookieCsrf(response.getBody()));
    ...
    response =
        restTemplate.exchange(
            location, HttpMethod.GET, new HttpEntity<>(null, getHeaders(cookies)), String.class);
    if (response.getHeaders().containsKey("Set-Cookie")) {
      for (String cookie : response.getHeaders().get("Set-Cookie")) {
        int nameLength = cookie.indexOf('=');
        cookies.addCookie(
  ...
  private void doOpenIdHybridFlowForLoginClient(
      Set<String> responseTypes, String responseTypeMatcher) throws Exception {
