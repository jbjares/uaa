...
/**
 * An easy way to create and populate a new Identity Zone for tests to help you isolate your test
 * setup.
 *
 * <p>Can be injected into your before or test method as a parameter (see {@link
 * ZoneSeederExtension}).
 *
 * <p>Use the with*() methods to configure, then call {@link #seed()} to create the data in the db.
 * Add callbacks with the {@link #afterSeeding(AfterSeedCallback)} method if you need to do
 * additional setup after the seed but before your test starts. After seeding has happened, use the
 * get*() methods to query what was created, and use the create*() methods to keep creating more
 * objects in the zone.
 *
 * <p>{@link #seed()} will automatically be called by {@link
 * ZoneSeederExtension#beforeTestExecution(ExtensionContext)} which happens after all beforeEach
 * methods, just before the test itself is executed.
 *
 * <p>{@link #destroy()} will automatically be called by {@link
 * ZoneSeederExtension#afterEach(ExtensionContext)} to perform a cascading delete of the zone and
 * its contents after each test.
 */
public class ZoneSeeder {
  private static final String IMPLICIT_PASSWORD_REFRESH_TOKEN_CLIENT_ID =
      "implict_password_refresh_token";
  ...
  ZoneSeeder(ApplicationContext applicationContext) {
    ...
    jdbcScimGroupMembershipManager =
        applicationContext.getBean(JdbcScimGroupMembershipManager.class);
  ...
  public ZoneSeeder withClientWithImplicitPasswordRefreshTokenGrants(
      String clientId, String commaSeparatedScopeNames) {
  ...
  public ZoneSeeder withAdminClientWithClientCredentialsGrant() {
    BaseClientDetails newClient =
        new BaseClientDetails(
            ADMIN_CLIENT_CREDENTIALS_CLIENT_ID,
            "none",
            "uaa.none",
            "client_credentials",
  ...
  private ZoneSeeder withUserWhoBelongsToGroups(
      ScimUser scimUser, List<String> belongsToGroupNames) {
  ...
  ZoneSeeder seed() {
    ...
    uaaIdentityProviderDefinitionToCreate.setDisableInternalUserManagement(
    ...
    for (ClientDetails clientDetails : clientDetailsToCreate) {
      plainTextClientSecretsForClients.put(
          clientDetails.getClientId(), clientDetails.getClientSecret());
      this.clientDetails.put(
          clientDetails.getClientId(), jdbcClientDetailsService.create(clientDetails, zoneId));
    }
    ...
    for (ScimUser scimUser : usersInGroupsToCreate.keySet()) {
      ...
      ScimUser refreshedUser =
          jdbcScimUserProvisioning.retrieve(createdUser.getId(), getIdentityZoneId());
  ...
  void destroy() {
    UaaPrincipal justEnoughPrincipal =
        new UaaPrincipal(
            "id", "name", "", identityProvider.getOriginKey(), "external id", identityZone.getId());
    UaaAuthentication justEnoughAuthentication =
        new UaaAuthentication(justEnoughPrincipal, Lists.newArrayList(), null);
    applicationContext.publishEvent(
        new EntityDeletedEvent<>(identityZone, justEnoughAuthentication));
  }

  public ScimUser createUser() {
    String email =
        generator.generate().toLowerCase() + "@" + generator.generate().toLowerCase() + ".com";
  ...
  private ScimUser provisionScimUser(ScimUser scimUser) {
    ...
    ScimUser createdUser =
        jdbcScimUserProvisioning.createUser(scimUser, password, getIdentityZoneId());
