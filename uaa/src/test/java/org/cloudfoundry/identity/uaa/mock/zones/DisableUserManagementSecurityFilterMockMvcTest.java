...
class DisableUserManagementSecurityFilterMockMvcTest {
  ...
  @Autowired private ExpiringCodeStore codeStore;
  ...
  void setUp(@Autowired FilterChainProxy springSecurityFilterChain) throws Exception {
    ...
    token =
        testClient.getClientCredentialsOAuthAccessToken(
            "login", "loginsecret", "scim.write,password.write");
  }
  ...
  void userEndpointDeleteNotAllowed_For_Origin_UAA() throws Exception {
    ...
    MockMvcUtils.setDisableInternalUserManagement(true, webApplicationContext);
    mockMvc
        .perform(delete("/Users/" + createdUser.getId()).header("Authorization", "Bearer " + token))
        .andExpect(status().isForbidden())
        .andExpect(
            content()
                .string(
  ...
  void userEndpointVerifyUsersNotAllowed() throws Exception {
    ...
    mockMvc
        .perform(
            get("/Users/" + createdUser.getId() + "/verify")
                .header("Authorization", "Bearer " + token))
        .andExpect(status().isForbidden())
        .andExpect(
            content()
  ...
  void accountsControllerVerifyUserNotAllowed() throws Exception {
    ...
    MockMvcUtils.setDisableInternalUserManagement(true, webApplicationContext);
    mockMvc
        .perform(get("/verify_user").param("code", getExpiringCode(codeData).getCode()))
        .andExpect(status().isForbidden())
        .andExpect(
            content()
                .string(
  ...
  void changeEmailControllerChangeEmailPageNotAllowed() throws Exception {
    ...
    mockMvc
        .perform(
            get("/change_email").session(userSession).with(cookieCsrf()).accept(ACCEPT_TEXT_HTML))
        .andExpect(status().isForbidden())
        .andExpect(
            content()
                .string(
  ...
  void changeEmailControllerVerifyEmailNotAllowed() throws Exception {
    ...
    MockMvcUtils.setDisableInternalUserManagement(true, webApplicationContext);
    mockMvc
        .perform(get("/verify_email").param("code", code.getCode()))
        .andExpect(status().isForbidden())
        .andExpect(
            content()
                .string(
  ...
  void changePasswordControllerChangePasswordPageNotAllowed() throws Exception {
    ...
    mockMvc
        .perform(get("/change_password").session(getUserSession(createdUser.getUserName(), PASSWD)))
        .andExpect(status().isForbidden())
        .andExpect(
            content()
                .string(
  ...
  void resetPasswordControllerForgotPasswordNotAllowed() throws Exception {
    MockMvcUtils.setDisableInternalUserManagement(true, webApplicationContext);
    mockMvc
        .perform(post("/forgot_password.do").param("email", "another@example.com"))
        .andExpect(status().isForbidden())
        .andExpect(
            content()
                .string(
  ...
  void resetPasswordControllerResetPasswordPageNotAllowed() throws Exception {
    ...
    mockMvc
        .perform(
            get("/reset_password").param("code", "12345").param("email", "another@example.com"))
        .andExpect(status().isForbidden())
        .andExpect(
            content()
                .string(
  ...
  void resetPasswordControllerResetPasswordNotAllowed() throws Exception {
    ...
    PasswordChange change =
        new PasswordChange(
            ...
            createdUser.getPasswordLastModified(),
            "",
            "");
  ...
  private ExpiringCode getExpiringCode(Object data) {
    ...
    return codeStore.generateCode(
        JsonUtils.writeValueAsString(data), fiveMinutes, null, IdentityZoneHolder.get().getId());
  }
  ...
  private MockHttpSession getUserSession(String username, String password) throws Exception {
    ...
    MockHttpSession afterLoginSession =
        (MockHttpSession)
            mockMvc
                .perform(
                    post("/login.do")
    ...
    assertNotNull(
        afterLoginSession.getAttribute(
