...
public abstract class AbstractTokenMockMvcTests {
  ...
  @Autowired protected JdbcScimGroupMembershipManager groupMembershipManager;
  @Autowired protected UaaTokenServices tokenServices;
  @Autowired protected IdentityZoneProvisioning identityZoneProvisioning;
  @Autowired protected JdbcScimUserProvisioning jdbcScimUserProvisioning;
  @Autowired protected IdentityProviderProvisioning identityProviderProvisioning;
  @Autowired protected JdbcRevocableTokenProvisioning revocableTokenProvisioning;
  ...
  @Autowired protected TestClient testClient;

  @Autowired protected JdbcTemplate jdbcTemplate;

  @Autowired protected UaaUserDatabase uaaUserDatabase;

  @Autowired protected UserGoogleMfaCredentialsProvisioning userGoogleMfaCredentialsProvisioning;
  ...
  protected String setUpUserForPasswordGrant() {
    ...
    ScimUser scimUser =
        jdbcScimUserProvisioning.retrieve(user.getId(), IdentityZoneHolder.get().getId());
  ...
  IdentityZone setupIdentityZone(String subdomain, List<String> defaultUserGroups) {
    ...
    zone.getConfig()
        .getTokenPolicy()
        .setKeys(IdentityZone.getUaa().getConfig().getTokenPolicy().getKeys());
    ...
    List<String> defaultGroups =
        new LinkedList(zone.getConfig().getUserConfig().getDefaultGroups());
  ...
  protected BaseClientDetails setUpClients(
      String id, String authorities, String scopes, String grantTypes, Boolean autoapprove) {
  ...
  protected BaseClientDetails setUpClients(
    ...
    return setUpClients(
        id, authorities, scopes, grantTypes, autoapprove, redirectUri, allowedIdps, -1);
  }
  ...
  protected ScimUser syncGroups(ScimUser user) {
    ...
    Set<ScimGroup> directGroups =
        groupMembershipManager.getGroupsWithMember(
            user.getId(), false, IdentityZoneHolder.get().getId());
  ...
  private ScimGroup createIfNotExist(String scope, String zoneId) {
    List<ScimGroup> exists =
        groupProvisioning.query(
            "displayName eq \"" + scope + "\" and identity_zone_id eq \"" + zoneId + "\"",
            IdentityZoneHolder.get().getId());
    if (exists.size() > 0) {
      return exists.get(0);
    } else {
      return groupProvisioning.create(
          new ScimGroup(null, scope, zoneId), IdentityZoneHolder.get().getId());
    }
  }
}
