  ...
  public void setup() {
    String token =
        IntegrationTestUtils.getClientCredentialsToken(
            serverRunning.getBaseUrl(), "admin", "adminsecret");
  ...
  public void cleanup() {
    ...
    String groupId =
        IntegrationTestUtils.getGroup(
                token, "", serverRunning.getBaseUrl(), "zones.testzone1.admin")
            .getId();
  ...
  public void test_LDAP_Custom_User_Attributes_In_ID_Token() throws Exception {
    ...
    LdapIdentityProviderDefinition ldapIdentityProviderDefinition =
        LdapIdentityProviderDefinition.searchAndBindMapGroupToScopes(
            ...
            100,
            true);
    ldapIdentityProviderDefinition.addAttributeMapping(
        USER_ATTRIBUTE_PREFIX + COST_CENTERS, COST_CENTER);
    ...
    BaseClientDetails clientDetails =
        new BaseClientDetails(
            ...
            "password,authorization_code,client_credentials",
    ...
    clientDetails =
        IntegrationTestUtils.createClientAsZoneAdmin(
            zoneAdminToken, baseUrl, zoneId, clientDetails);
    clientDetails.setClientSecret("secret");
    ...
    Map<String, List<String>> userAttributes =
        (Map<String, List<String>>) claims.get(ClaimConstants.USER_ATTRIBUTES);
    assertThat(userAttributes.get(COST_CENTERS), containsInAnyOrder(DENVER_CO));
    assertThat(
        userAttributes.get(MANAGERS), containsInAnyOrder(JOHN_THE_SLOTH, KARI_THE_ANT_EATER));
