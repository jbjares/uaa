...
public class JwtBearerGrantMockMvcTests extends AbstractTokenMockMvcTests {
  ...
  public void setupJwtBearerTests() throws Exception {
    ...
    originZone =
        MockMvcUtils.createOtherIdentityZoneAndReturnResult(
            subdomain, mockMvc, webApplicationContext, originClient);
  ...
  void default_zone_jwt_grant() throws Exception {
    ...
    perform_grant_in_zone(
            defaultZone, getUaaIdToken(originZone.getIdentityZone(), originClient, originUser))
  ...
  void non_default_zone_jwt_grant() throws Exception {
    ...
    perform_grant_in_zone(
            zone, getUaaIdToken(originZone.getIdentityZone(), originClient, originUser))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.access_token").isNotEmpty());
  }
  ...
  void defaultZoneJwtGrantWithInternalIdp() throws Exception {
    BaseClientDetails defaultZoneClient =
        setUpClients(generator.generate(), "", "openid", "password", true);
    ...
    perform_grant_in_zone(
            defaultZone, getUaaIdToken(defaultZone, defaultZoneClient, defaultZoneUser))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.access_token").isNotEmpty());
  }
  ...
  ResultActions perform_grant_in_zone(IdentityZone theZone, String assertion) throws Exception {
    ...
    MockHttpServletRequestBuilder jwtBearerGrant =
        post("/oauth/token")
    ...
    return mockMvc.perform(jwtBearerGrant).andDo(print());
  }
  ...
  String getUaaIdToken(IdentityZone zone, ClientDetails client, ScimUser user) throws Exception {
    MockHttpServletRequestBuilder passwordGrant =
        post("/oauth/token")
            .header(HttpHeaders.ACCEPT, MediaType.APPLICATION_JSON_VALUE)
            .header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_FORM_URLENCODED_VALUE)
            .param("client_id", client.getClientId())
    ...
    Map<String, Object> token =
        JsonUtils.readValue(jsonToken, new TypeReference<Map<String, Object>>() {});
  ...
  public ScimUser createUser(IdentityZone zone) {
    ...
    try {
      return webApplicationContext
          ...
          .createUser(user, SECRET, IdentityZoneHolder.get().getId());
    } finally {
      IdentityZoneHolder.clear();
    }
  }
  ...
  ClientDetails createJwtBearerClient(IdentityZone zone) {
    BaseClientDetails details =
        new BaseClientDetails(
            generator.generate().toLowerCase(), "", "openid", GRANT_TYPE_JWT_BEARER, null);
  ...
  IdentityProvider<OIDCIdentityProviderDefinition> createOIDCProvider(
      IdentityZone zone, String tokenKey, String issuer, String relyingPartyId) throws Exception {
    ...
    try {
      return webApplicationContext
          ...
          .create(identityProvider, zone.getId());
    } finally {
      IdentityZoneHolder.clear();
    }
  }
