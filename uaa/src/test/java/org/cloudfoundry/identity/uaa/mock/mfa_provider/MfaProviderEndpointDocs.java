...
class MfaProviderEndpointDocs extends EndpointDocs {
  ...
  private static final String NAME_DESC =
  ...
  private static final FieldDescriptor NAME =
      fieldWithPath("name").required().type(STRING).description(NAME_DESC);
  private static final FieldDescriptor TYPE =
      fieldWithPath("type")
          .required()
          .type(STRING)
          .description("Type of MFA provider. Available types include `google-authenticator`.");
  ...
  private static final HeaderDescriptor MFA_AUTHORIZATION_HEADER =
      headerWithName("Authorization")
          .description("Bearer token containing `uaa.admin` or `zones.<zoneId>.admin`");
  ...
  void testCreateGoogleMfaProvider() throws Exception {
    ...
    mockMvc
        .perform(
            RestDocumentationRequestBuilders.post("/mfa-providers", mfaProvider.getId())
                .accept(APPLICATION_JSON)
                .header("Authorization", "Bearer " + adminToken)
                .contentType(APPLICATION_JSON)
                .content(
                    serializeExcludingProperties(
                        mfaProvider, "id", "created", "last_modified", "identityZoneId")))
        ...
        .andDo(
            document(
                ...
                requestHeaders(MFA_AUTHORIZATION_HEADER, IDENTITY_ZONE_ID_HEADER),
                requestFields,
                responseFields));
  }
  ...
  private FieldDescriptor[] getMfaProviderResponseFields(FieldDescriptor[] idempotentFields) {
    return (FieldDescriptor[])
        ArrayUtils.addAll(
            idempotentFields, new FieldDescriptor[] {ID, CREATED, LAST_MODIFIED, IDENTITY_ZONE_ID});
  }

  private MfaProvider<GoogleMfaProviderConfig> getGoogleMfaProvider() {
    return (MfaProvider<GoogleMfaProviderConfig>)
        new MfaProvider<GoogleMfaProviderConfig>()
            ...
            .setConfig(
                new GoogleMfaProviderConfig()
                    .setProviderDescription("Google MFA for default zone"));
  }
  ...
  void testGetMfaProvider() throws Exception {
    ...
    Snippet responseFields =
        responseFields(getMfaProviderResponseFields(getGoogleMfaProviderFields()));

    ResultActions getMFaResultAction =
        mockMvc.perform(
            RestDocumentationRequestBuilders.get("/mfa-providers/{id}", mfaProvider.getId())
                .header("Authorization", "Bearer " + adminToken)
                .accept(APPLICATION_JSON));

    getMFaResultAction.andDo(
        document(
            "{ClassName}/{methodName}",
            preprocessResponse(prettyPrint()),
  ...
  void testListMfaProviders() throws Exception {
    ...
    ResultActions listMfaProviderAction =
        mockMvc.perform(
            RestDocumentationRequestBuilders.get("/mfa-providers")
                .header("Authorization", "Bearer " + adminToken)
                .accept(APPLICATION_JSON));
  ...
  private MfaProvider createMfaProviderHelper(MfaProvider<GoogleMfaProviderConfig> mfaProvider)
      throws Exception {
    ...
    MfaProvider createdMfaProvider =
        JsonUtils.readValue(createResponse.getContentAsString(), MfaProvider.class);
