...
public class PasswordGrantIntegrationTests {
  ...
  public void testUserLoginViaPasswordGrant() throws Exception {
    ResponseEntity<String> responseEntity =
        makePasswordGrantRequest(
            testAccounts.getUserName(),
            testAccounts.getPassword(),
            "cf",
            "",
            serverRunning.getAccessTokenUri());
  ...
  public void password_grant_returns_correct_error() throws Exception {
    ...
    assertEquals(
        "application/json;charset=UTF-8", responseEntity.getHeaders().get("Content-Type").get(0));
    Map<String, Object> errors =
        JsonUtils.readValue(responseEntity.getBody(), new TypeReference<Map<String, Object>>() {});
    assertEquals(
        HtmlUtils.htmlEscape(
            "User does not meet the client's required group criteria.", "ISO-8859-1"),
        errors.get("error_description"));
  ...
  public void passwordGrantInactiveZone() {
    ...
    String accessTokenUri =
        serverRunning.getAccessTokenUri().replace("localhost", "testzoneinactive.localhost");
    ResponseEntity<String> response =
        makePasswordGrantRequest(
            testAccounts.getUserName(), testAccounts.getPassword(), "cf", "", accessTokenUri);
  ...
  public void passwordGrantNonExistingZone() {
    String accessTokenUri =
        serverRunning.getAccessTokenUri().replace("localhost", "testzonedoesnotexist.localhost");
    ResponseEntity<String> response =
        makePasswordGrantRequest(
            testAccounts.getUserName(), testAccounts.getPassword(), "cf", "", accessTokenUri);
    assertEquals(HttpStatus.NOT_FOUND, response.getStatusCode());
  ...
  protected BaseClientDetails addUserGroupsRequiredClient() throws Exception {
    ...
    BaseClientDetails client =
        new BaseClientDetails(generator.generate(), null, "openid", "password", null);
    ...
    ResponseEntity<String> response =
        new RestTemplate()
            .postForEntity(serverRunning.getUrl("/oauth/clients"), request, String.class);
    assertEquals(201, response.getStatusCodeValue());

    return JsonUtils.readValue(response.getBody(), BaseClientDetails.class);
  }

  private ResponseEntity<String> makePasswordGrantRequest(
      String userName, String password, String clientId, String clientSecret, String url) {
    HttpHeaders headers = new HttpHeaders();
    headers.setAccept(Arrays.asList(APPLICATION_JSON));
