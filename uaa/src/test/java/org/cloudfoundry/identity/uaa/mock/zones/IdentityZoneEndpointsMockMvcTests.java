...
class IdentityZoneEndpointsMockMvcTests {
  ...
  void setUp(
      ...
      @Autowired ScimGroupMembershipManager scimGroupMembershipManager)
      throws Exception {
    ...
    BaseClientDetails uaaAdminClient =
        new BaseClientDetails(
            "uaa-admin-" + generator.generate().toLowerCase(),
            null,
            "uaa.admin",
    ...
    uaaAdminClientToken =
        testClient.getClientCredentialsOAuthAccessToken(
            uaaAdminClient.getClientId(), "secret", "uaa.admin");

    ScimUser uaaAdminUser = createUser(uaaAdminClientToken, null);

    String groupId =
        scimGroupProvisioning.getByName("uaa.admin", IdentityZone.getUaa().getId()).getId();
    scimGroupMembershipManager.addMember(
        groupId, new ScimGroupMember(uaaAdminUser.getId()), IdentityZone.getUaa().getId());
    ...
    zoneModifiedEventListener =
        MockMvcUtils.addEventListener(
            configurableApplicationContext, IdentityZoneModifiedEvent.class);
    clientCreateEventListener =
        MockMvcUtils.addEventListener(configurableApplicationContext, ClientCreateEvent.class);
    clientDeleteEventListener =
        MockMvcUtils.addEventListener(configurableApplicationContext, ClientDeleteEvent.class);
    groupModifiedEventListener =
        MockMvcUtils.addEventListener(configurableApplicationContext, GroupModifiedEvent.class);
    userModifiedEventListener =
        MockMvcUtils.addEventListener(configurableApplicationContext, UserModifiedEvent.class);
    uaaEventListener =
        MockMvcUtils.addEventListener(configurableApplicationContext, AbstractUaaEvent.class);
  ...
  void readWith_Write_TokenShouldNotFail(String url) throws Exception {
    mockMvc
        .perform(get(url).header("Authorization", "Bearer " + identityClientZonesWriteToken))
        .andExpect(status().isOk());
  }
  ...
  void readWith_Read_TokenShouldSucceed(String url) throws Exception {
    mockMvc
        .perform(get(url).header("Authorization", "Bearer " + identityClientZonesReadToken))
        .andExpect(status().isOk());
  }
  ...
  void create_zone_no_links() throws Exception {
    ...
    IdentityZone created =
        createZone(id, HttpStatus.CREATED, identityClientToken, zoneConfiguration);
  ...
  void test_bootstrapped_system_scopes() throws Exception {
    ...
    List<String> groups =
        webApplicationContext
            .getBean(JdbcScimGroupProvisioning.class)
            .retrieveAll(id)
            .stream()
    ...
    ZoneManagementScopes.getSystemScopes()
        ...
        .forEach(
            scope ->
                assertTrue(
                    "Scope:" + scope + " should have been bootstrapped into the new zone",
                    groups.contains(scope)));
  }
  ...
  void testGetZonesAsIdentityClient() throws Exception {
    ...
    mockMvc
        .perform(get("/identity-zones/").header("Authorization", "Bearer " + lowPriviledgeToken))
        .andExpect(status().isForbidden());

    MvcResult result =
        mockMvc
            .perform(
                get("/identity-zones/").header("Authorization", "Bearer " + identityClientToken))
            .andExpect(status().isOk())
            .andReturn();

    List<IdentityZone> zones =
        JsonUtils.readValue(
            result.getResponse().getContentAsString(), new TypeReference<List<IdentityZone>>() {});
  ...
  void testCreateZoneWithMfaConfigWithIdentityProviders() throws Exception {
    ...
    assertThat(
        checkZone.getConfig().getMfaConfig().getIdentityProviders(), hasItems("uaa", "ldap"));
  }

  @Test
  void testCreateZoneWithMfaConfigWithoutIdentityProviders_returnsDefaultProviders()
      throws Exception {
    String id = generator.generate();
  ...
  void updateZoneCreatesGroups() throws Exception {
    ...
    ScimGroupProvisioning groupProvisioning =
        webApplicationContext.getBean(ScimGroupProvisioning.class);
  ...
  void createZoneWithNoNameFailsWithUnprocessableEntity() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.error_description").value("The identity zone must be given a name."));
  ...
  void testCreateZoneWithoutID() throws Exception {
    IdentityZone zone =
        createZone("", HttpStatus.CREATED, identityClientToken, new IdentityZoneConfiguration());
  ...
  void testUpdateWithDifferentDataReturns200() throws Exception {
    ...
    IdentityZoneConfiguration definition =
        new IdentityZoneConfiguration(new TokenPolicy(3600, 7200));
    ...
    assertEquals(
        ...
        JsonUtils.writeValueAsString(updated.getConfig()));
    checkZoneAuditEventInUaa(2, AuditEventType.IdentityZoneModifiedEvent);
  }
  ...
  void testUpdateWithEmptySamlKeyCertPairRetainsCurrentValue() throws Exception {
    ...
    created
        .getConfig()
        .getTokenPolicy()
        .setKeys(new HashMap<>(Collections.singletonMap("kid", "key")));
  ...
  void testUpdateZoneWithExistingSubdomain() throws Exception {
    ...
    IdentityZone created1 =
        createZone(id1, HttpStatus.CREATED, identityClientToken, new IdentityZoneConfiguration());
    checkZoneAuditEventInUaa(1, AuditEventType.IdentityZoneCreatedEvent);

    String id2 = generator.generate();
    IdentityZone created2 =
        createZone(id2, HttpStatus.CREATED, identityClientToken, new IdentityZoneConfiguration());
  ...
  void testCreateZoneAndIdentityProvider(String url) throws Exception {
    ...
    IdentityProviderProvisioning idpp =
        (IdentityProviderProvisioning)
            webApplicationContext.getBean("identityProviderProvisioning");
    ...
    IdentityZoneProvisioning identityZoneProvisioning =
        (IdentityZoneProvisioning) webApplicationContext.getBean("identityZoneProvisioning");
    IdentityZone createdZone = identityZoneProvisioning.retrieve(id);

    assertEquals(
        JsonUtils.writeValueAsString(definition),
        JsonUtils.writeValueAsString(createdZone.getConfig()));
  ...
  void testCreateZoneWithRefreshTokenConfig() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.config.tokenPolicy.refreshTokenFormat").value(OPAQUE.getStringValue()));

    IdentityZone createdZone = provisioning.retrieve(id);
    assertEquals(
        OPAQUE.getStringValue(), createdZone.getConfig().getTokenPolicy().getRefreshTokenFormat());
  ...
  void testUpdateZoneWithConsent() throws Exception {
    ...
    createdZone
        ...
        .getBranding()
        .getConsent()
        .setLink("http://localhost/some-updated-link");
    ...
    IdentityZone updatedZone =
        JsonUtils.readValue(mvcResult.getContentAsString(), IdentityZone.class);
  ...
  void testUpdateZoneWithNoConsentText() throws Exception {
    ...
    assertThat(
        mvcResult.getContentAsString(),
        containsString("Consent text must be set if configuring consent"));
  }
  ...
  void testCreateZoneWithInvalidBannerLogo() throws Exception {
    ...
    assertThat(
        mvcResult.getContentAsString(),
        containsString("Invalid banner logo. Must be in BASE64 format."));
  }
  ...
  void test_delete_zone_cleans_db() throws Exception {
    ...
    ScimUserProvisioning userProvisioning =
        webApplicationContext.getBean(ScimUserProvisioning.class);
    ScimGroupMembershipManager membershipManager =
        webApplicationContext.getBean(ScimGroupMembershipManager.class);
    ScimGroupExternalMembershipManager externalMembershipManager =
        webApplicationContext.getBean(ScimGroupExternalMembershipManager.class);
    ...
    BaseClientDetails client =
        new BaseClientDetails(
            "limited-client", null, "openid", GRANT_TYPE_AUTHORIZATION_CODE, "uaa.resource");
    client.setClientSecret("secret");
    client.addAdditionalInformation(
        ClientConstants.ALLOWED_PROVIDERS, Collections.singletonList(UAA));
    ...
    MvcResult result =
        mockMvc
            ...
            .andExpect(status().isCreated())
            .andReturn();
    BaseClientDetails created =
        JsonUtils.readValue(result.getResponse().getContentAsString(), BaseClientDetails.class);
    assertNull(created.getClientSecret());
    assertEquals(
        "zones.write", created.getAdditionalInformation().get(ClientConstants.CREATED_WITH));
    assertEquals(
        Collections.singletonList(UAA),
        created.getAdditionalInformation().get(ClientConstants.ALLOWED_PROVIDERS));
    ...
    membershipManager.addMember(
        group.getId(),
        new ScimGroupMember(user.getId(), ScimGroupMember.Type.USER),
    ...
    assertEquals(
        "Delete Test Group",
        groupProvisioning
            .retrieve(group.getId(), IdentityZoneHolder.get().getId())
            .getDisplayName());
    assertEquals(
        1,
        membershipManager
            .getMembers(group.getId(), false, IdentityZoneHolder.get().getId())
            .size());
    ...
    // assertThat(template.queryForObject("select count(*) from sec_audit where identity_zone_id=?",
    // new Object[] {user.getZoneId()}, Integer.class), greaterThan(0));
    // create an external group map
    IdentityZoneHolder.set(zone);
    externalMembershipManager.mapExternalGroup(
        group.getId(), "externalDeleteGroup", LOGIN_SERVER, IdentityZoneHolder.get().getId());
    assertEquals(
        1,
        externalMembershipManager
            .getExternalGroupMapsByGroupId(
                group.getId(), LOGIN_SERVER, IdentityZoneHolder.get().getId())
            .size());
    assertThat(
        template.queryForObject(
            "select count(*) from external_group_mapping where origin=?",
            new Object[] {LOGIN_SERVER},
            Integer.class),
        is(1));
    ...
    assertEquals(
        1,
        approvalStore
            .getApprovals(user.getId(), client.getClientId(), IdentityZoneHolder.get().getId())
            .size());
    ...
    assertThat(
        ...
        is(0));
    try {
      externalMembershipManager.getExternalGroupMapsByGroupId(
          group.getId(), LOGIN_SERVER, IdentityZoneHolder.get().getId());
  ...
  void testDeleteZonePublishesEvent() throws Exception {
        ...
        mockMvc
            .perform(
                delete("/identity-zones/{id}", zone.getId())
                    ...
                    .accept(APPLICATION_JSON))
            .andExpect(status().isOk());
    IdentityZone deletedZone =
        JsonUtils.readValue(
            result.andReturn().getResponse().getContentAsString(), IdentityZone.class);
    ...
    assertEquals(
        serviceProviderCertificate, deletedZone.getConfig().getSamlConfig().getCertificate());
  ...
  void testCreateAndDeleteLimitedClientInNewZoneUsingZoneEndpoint() throws Exception {
    ...
    assertEquals(
        ...
        created.getAdditionalInformation().get(ClientConstants.ALLOWED_PROVIDERS));
    assertEquals("bar", created.getAdditionalInformation().get("foo"));
    checkAuditEventListener(
        1,
        AuditEventType.ClientCreateSuccess,
        clientCreateEventListener,
        id,
    ...
    for (String url : Arrays.asList("", "/")) {
      mockMvc
          .perform(
              delete(
                      "/identity-zones/" + zone.getId() + "/clients/" + created.getClientId(),
                      IdentityZone.getUaa().getId() + url)
                  .header("Authorization", "Bearer " + identityClientZonesReadToken)
                  .accept(APPLICATION_JSON))
          .andExpect(status().isForbidden());
    }
    ...
    checkAuditEventListener(
        1,
        AuditEventType.ClientDeleteSuccess,
        clientDeleteEventListener,
        id,
        "http://localhost:8080/uaa/oauth/token",
        "identity");
  ...
  void testZoneAdminTokenAgainstZoneEndpoints() throws Exception {
    ...
    IdentityZoneCreationResult result1 =
        MockMvcUtils.createOtherIdentityZoneAndReturnResult(
            zone1, mockMvc, webApplicationContext, null);
    IdentityZoneCreationResult result2 =
        MockMvcUtils.createOtherIdentityZoneAndReturnResult(
            zone2, mockMvc, webApplicationContext, null);
  ...
  void testSuccessfulUserManagementInZoneUsingAdminClient() throws Exception {
    ...
    BaseClientDetails adminClient =
        new BaseClientDetails("admin", null, null, "client_credentials", "scim.read,scim.write");
    adminClient.setClientSecret("admin-secret");
    IdentityZoneCreationResult creationResult =
        MockMvcUtils.createOtherIdentityZoneAndReturnResult(
            subdomain, mockMvc, webApplicationContext, adminClient);
    ...
    checkAuditEventListener(
        ...
        identityZone.getId(),
        "http://localhost:8080/uaa/oauth/token",
        creationResult.getZoneAdminUser().getId());

    String scimAdminToken =
        testClient.getClientCredentialsOAuthAccessToken(
            "admin", "admin-secret", "scim.write,scim.read", subdomain);
    ...
    checkAuditEventListener(
        ...
        userModifiedEventListener,
        identityZone.getId(),
        "http://" + subdomain + ".localhost:8080/uaa/oauth/token",
        "admin");
  ...
  void testCreateAndListUsersInOtherZoneIsUnauthorized() throws Exception {
    ...
    String defaultZoneAdminToken =
        testClient.getClientCredentialsOAuthAccessToken(
            "admin", "adminsecret", "scim.write,scim.read");
    ...
    MockHttpServletRequestBuilder get =
        get("/Users").header("Authorization", "Bearer " + defaultZoneAdminToken);
  ...
  void testModifyandDeleteUserInOtherZoneIsUnauthorized() throws Exception {
    String scimWriteToken =
        testClient.getClientCredentialsOAuthAccessToken("admin", "adminsecret", "scim.write");
    ...
    mockMvc.perform(put).andExpect(status().isUnauthorized()).andReturn();
    ...
    mockMvc.perform(delete).andExpect(status().isUnauthorized()).andReturn();
  }
  ...
  void userCanReadAZone_withZoneZoneIdReadToken() throws Exception {
    ...
    String userAccessToken =
        MockMvcUtils.getUserOAuthAccessTokenAuthCode(
            mockMvc,
            "identity",
            "identitysecret",
            user.getId(),
    ...
    IdentityZone zoneResult =
        JsonUtils.readValue(
            result.getResponse().getContentAsString(), new TypeReference<IdentityZone>() {});
    ...
    String userAccessTokenReadAndAdmin =
        MockMvcUtils.getUserOAuthAccessTokenAuthCode(
            mockMvc,
            "identity",
            "identitysecret",
    ...
    assertNull(zoneResult.getConfig().getSamlConfig().getPrivateKey());
    assertEquals(
        serviceProviderCertificate, zoneResult.getConfig().getSamlConfig().getCertificate());
  ...
  void createZoneWithMfaConfigIsNotSupported() throws Exception {
    ...
    String zoneContent =
        "{\"id\" : \""
            ...
            + "\", \"subdomain\" : \""
            + zoneId
            + "\", \"config\" : { \"mfaConfig\" : {\"enabled\" : true, \"providerName\" : \""
            + mfaProvider.getName()
            + "\"}}}";
    mockMvc
        .perform(
  ...
  void updateZoneWithValidMfaConfig() throws Exception {
    IdentityZone identityZone =
        createZone(
            new RandomValueStringGenerator(5).generate(),
            HttpStatus.CREATED,
            adminToken,
            new IdentityZoneConfiguration());
    MfaProvider<GoogleMfaProviderConfig> mfaProvider =
        createGoogleMfaProvider(identityZone.getId());
  ...
  void updateZoneWithValidMfaConfigWithoutIdInBody_Succeeds() throws Exception {
    ...
    identityZone
        .getConfig()
        .setMfaConfig(new MfaConfig().setEnabled(true).setProviderName(mfaProvider.getName()));
  ...
  void testUpdateZone_withCustomIssuerAndSigningKeyWorks() throws Exception {
    ...
    IdentityZone identityZone =
        createZone(zoneId, HttpStatus.CREATED, adminToken, identityZoneConfiguration);
  ...
  private static class IdentityZonesBaseUrlsArgumentsSource implements ArgumentsProvider {
    ...
    public Stream<? extends Arguments> provideArguments(ExtensionContext context) {
      return Stream.of(Arguments.of("/identity-zones"), Arguments.of("/identity-zones/"));
    }
  }
  ...
  private MfaProvider<GoogleMfaProviderConfig> createGoogleMfaProvider(String zoneId)
      throws Exception {
    MfaProvider<GoogleMfaProviderConfig> mfaProvider =
        new MfaProvider().setName(new RandomValueStringGenerator(5).generate());
    MockHttpServletRequestBuilder createMfaRequest =
        post("/mfa-providers")
            .header("Authorization", "Bearer " + adminToken)
            .contentType(APPLICATION_JSON)
    ...
    MockHttpServletResponse mfaProviderResponse =
        mockMvc.perform(createMfaRequest).andReturn().getResponse();
    mfaProvider = JsonUtils.readValue(mfaProviderResponse.getContentAsString(), MfaProvider.class);
    return mfaProvider;
  }

  private IdentityZone getIdentityZone(String id, HttpStatus expect, String token)
      throws Exception {
    MvcResult result =
        mockMvc
            .perform(get("/identity-zones/" + id).header("Authorization", "Bearer " + token))
  ...
  private IdentityZone createZone(
      String id, HttpStatus expect, String token, IdentityZoneConfiguration zoneConfiguration)
      throws Exception {
    Map<String, String> keys = new HashMap<>();
    keys.put("kid", "key");
  ...
  private IdentityZone updateZone(
      String id, IdentityZone identityZone, HttpStatus expect, String expectedContent, String token)
      throws Exception {
    MvcResult result =
        mockMvc
            .perform(
  ...
  private IdentityZone updateZone(
      String id, IdentityZone identityZone, HttpStatus expect, String token) throws Exception {
    return updateZone(id, identityZone, expect, "", token);
  }

  private IdentityZone updateZone(IdentityZone identityZone, HttpStatus expect, String token)
      throws Exception {
    return updateZone(identityZone.getId(), identityZone, expect, token);
  }

  private <T extends AbstractUaaEvent> void checkZoneAuditEventInUaa(
      int eventCount, AuditEventType eventType) {
    checkAuditEventListener(
        eventCount,
        eventType,
        zoneModifiedEventListener,
  ...
  private <T extends AbstractUaaEvent> void checkAuditEventListener(
      int eventCount,
      AuditEventType eventType,
      TestApplicationEventListener<T> eventListener,
  ...
  private List<ScimUser> getUsersInZone(String subdomain, String token) throws Exception {
    ...
    return JsonUtils.readValue(
        root.get("resources").toString(), new TypeReference<List<ScimUser>>() {});
  }
}
