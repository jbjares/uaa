...
/**
 * Tests implicit grant using a direct posting of credentials to the /authorize endpoint and also
 * with an intermediate form login.
 *
 * @author Dave Syer
 */
public class ImplicitTokenGrantIntegrationTests {

  private static final String REDIRECT_URL_PATTERN =
      "http://localhost:8080/redirect/cf#token_type=.+access_token=.+";
  ...
  private String implicitUrl() {
    URI uri =
        serverRunning
            ...
            .queryParam("scope", "cloud_controller.read")
            .build();
    return uri.toString();
  }
  ...
  public void authzWithNonExistingIdentityZone() {
    ResponseEntity<Void> result =
        serverRunning.getForResponse(
            implicitUrl().replace("localhost", "testzonedoesnotexist.localhost"),
            new HttpHeaders());
  ...
  public void authzWithInactiveIdentityZone() {
    ...
    ResponseEntity<Void> result =
        serverRunning.getForResponse(
            implicitUrl().replace("localhost", "testzoneinactive.localhost"), new HttpHeaders());
    assertEquals(HttpStatus.NOT_FOUND, result.getStatusCode());
  }
}
