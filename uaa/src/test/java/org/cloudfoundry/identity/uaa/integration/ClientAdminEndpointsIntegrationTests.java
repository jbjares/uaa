...
public class ClientAdminEndpointsIntegrationTests {
  ...
  public static final String SECRET_TOO_LONG =
      "adfdfdasgdasgasdgafsgasfgfasgfadsgfagsagasddsafdsafsdfdafsdafdsfasdffasfasdfasdfdsfds"
    ...
    for (ClientDetailsModification clientDetailsModification : clientDetailsModifications) {
      serverRunning
          ...
          .exchange(
              ...
              HttpMethod.DELETE,
              new HttpEntity<BaseClientDetails>(clientDetailsModification, headers),
              Void.class,
  ...
  public void testListClientsWithExtremePagination_defaultsTo500() throws Exception {
    ...
    ResponseEntity<String> result =
        serverRunning.getForString("/oauth/clients?count=3000", headers);
    assertEquals(HttpStatus.OK, result.getStatusCode());

    SearchResults searchResults =
        new ObjectMapper().readValue(result.getBody(), SearchResults.class);
  ...
  public ClientDetailsModification[] doCreateClients() throws Exception {
    ...
    for (int i = 0; i < ids.length; i++) {
      ...
      detailsModification.setAuthorities(
          AuthorityUtils.commaSeparatedStringToAuthorityList("uaa.none"));
      clients[i] = detailsModification;
      clients[i].setClientSecret("secret");
      clients[i].setAdditionalInformation(
          Collections.<String, Object>singletonMap("foo", Arrays.asList("bar")));
  ...
  public void createClientWithoutSecretIsRejected() throws Exception {
    ...
    BaseClientDetails invalidSecretClient =
    ...
    ResponseEntity<UaaException> result =
        serverRunning
            .getRestTemplate()
            .exchange(
                serverRunning.getUrl("/oauth/clients"),
                HttpMethod.POST,
                new HttpEntity<BaseClientDetails>(invalidSecretClient, headers),
                UaaException.class);
    ...
    ResponseEntity<UaaException> result =
        serverRunning
            ...
            .exchange(
                serverRunning.getUrl("/oauth/clients"),
                HttpMethod.POST,
                new HttpEntity<BaseClientDetails>(client, headers),
                UaaException.class);
    assertEquals(HttpStatus.BAD_REQUEST, result.getStatusCode());
    assertEquals("invalid_client", result.getBody().getErrorCode());
  }
  ...
  public void createClientWithStrictSecretPolicyTest() throws Exception {
    ...
    // min length 5, max length 12, requires 1 uppercase lowercase digit and specialChar, expries 6
    // months.
    config.setClientSecretPolicy(new ClientSecretPolicy(5, 12, 1, 1, 1, 1, 6));
    IntegrationTestUtils.createZoneOrUpdateSubdomain(
        identityClient, serverRunning.getBaseUrl(), testZoneId, testZoneId, config);
    ...
    String zoneAdminToken =
        IntegrationTestUtils.getZoneAdminToken(
            serverRunning.getBaseUrl(), serverRunning, testZoneId);
    ...
    ResponseEntity<UaaException> result =
        serverRunning
            .getRestTemplate()
            .exchange(
                serverRunning.getBaseUrl() + "/oauth/clients",
                HttpMethod.POST,
                new HttpEntity<BaseClientDetails>(client, xZoneHeaders),
                UaaException.class);

    Assert.assertEquals(HttpStatus.CREATED, result.getStatusCode());

    // Negative Test
    BaseClientDetails failClient =
        new BaseClientDetails(
            new RandomValueStringGenerator().generate(),
            "",
            "foo,bar",
    ...
    result =
        serverRunning
            ...
            .exchange(
                serverRunning.getBaseUrl() + "/oauth/clients",
                HttpMethod.POST,
                new HttpEntity<BaseClientDetails>(failClient, xZoneHeaders),
                UaaException.class);
  ...
  public void authzCodeGrantAutomaticallyAddsRefreshToken() throws Exception {
    ...
    ResponseEntity<String> result =
        serverRunning.getForString("/oauth/clients/" + client.getClientId(), headers);
    assertEquals(HttpStatus.OK, result.getStatusCode());
    assertTrue(
        result
            .getBody()
            .contains("\"authorized_grant_types\":[\"authorization_code\",\"refresh_token\"]"));
  }
  ...
  public void passwordGrantAutomaticallyAddsRefreshToken() throws Exception {
    ...
    assertEquals(HttpStatus.OK, result.getStatusCode());
    assertTrue(
        result.getBody().contains("\"authorized_grant_types\":[\"password\",\"refresh_token\"]"));
  }
  ...
  public void testUpdateClient() throws Exception {
    ...
    client.setAdditionalInformation(
        Collections.<String, Object>singletonMap("foo", Arrays.asList("rab")));
  ...
  public void testDeleteClients() throws Exception {
    ...
    headers =
        getAuthenticatedHeaders(
            getClientCredentialsAccessToken(
                "clients.admin,clients.read,clients.write,clients.secret,clients.admin"));
    headers.add("Accept", "application/json");
    ResponseEntity<BaseClientDetails[]> result =
        serverRunning
            .getRestTemplate()
  ...
  public void testChangeSecret() throws Exception {
    headers =
        getAuthenticatedHeaders(
            getClientCredentialsAccessToken("clients.read,clients.write,clients.secret,uaa.admin"));
    BaseClientDetails client = createClient("client_credentials");
    ...
    ResponseEntity<Void> result =
        serverRunning
            ...
            .exchange(
                ...
                HttpMethod.PUT,
                new HttpEntity<SecretChangeRequest>(change, headers),
                Void.class,
                client.getClientId());
    assertEquals(HttpStatus.OK, result.getStatusCode());
  }
    ...
    ResponseEntity<Void> result =
        serverRunning
            ...
            .exchange(
                serverRunning.getUrl("/oauth/clients/{client}"),
                HttpMethod.DELETE,
                new HttpEntity<BaseClientDetails>(client, getAuthenticatedHeaders(token)),
                Void.class,
                client.getClientId());
    assertEquals(HttpStatus.OK, result.getStatusCode());
    ...
    ResponseEntity<Void> result =
        serverRunning
            ...
            .exchange(
                serverRunning.getUrl("/oauth/clients/tx/delete"),
                HttpMethod.POST,
                new HttpEntity<BaseClientDetails[]>(
                    new BaseClientDetails[] {client},
                    getAuthenticatedHeaders(getClientCredentialsAccessToken("clients.admin"))),
                Void.class);
    assertEquals(HttpStatus.OK, result.getStatusCode());
    // create a client that can read another clients approvals
    String deletedClientId = client.getClientId();
  ...
  private Approval[] getApprovals(String token, String clientId) throws Exception {
    ...
    approvalList =
        Arrays.asList(approvals.getBody())
            .stream()
  ...
  private Approval[] addApprovals(String token, String clientId) throws Exception {
    ...
    ResponseEntity<Approval[]> response =
        serverRunning
            .getRestTemplate()
            .exchange(
  ...
  private OAuth2AccessToken getUserAccessToken(
