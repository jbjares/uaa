...
class ExpiringCodeStoreMockMvcTests {
  ...
  void testRetrieveCodeThatIsExpired() throws Exception {
    ...
    mockMvc.perform(get).andExpect(status().isNotFound()).andReturn();
  }
  ...
  void testCodeThatIsExpiredIsDeletedOnCreateOfNewCode() throws Exception {
    ...
    mockMvc.perform(post).andExpect(status().isCreated()).andReturn();

    assertThat(
        jdbcTemplate.queryForObject("select count(*) from expiring_code_store", Integer.class),
        is(1));
  }
