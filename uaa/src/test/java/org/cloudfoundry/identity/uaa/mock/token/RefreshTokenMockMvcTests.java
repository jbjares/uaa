...
class RefreshTokenMockMvcTests extends AbstractTokenMockMvcTests {
  ...
  @Autowired private TokenValidityResolver refreshTokenValidityResolver;
  @Autowired private RefreshTokenCreator refreshTokenCreator;
  @Autowired private IdTokenCreator idTokenCreator;
  ...
  private void createClientAndUserInRandomZone() throws Exception {
    ...
    refreshToken =
        getJwtRefreshToken(
            client.getClientId(), SECRET, user.getUserName(), SECRET, getZoneHostUrl(zone));
  }
  ...
  void refreshTokenGrant_rejectsAccessTokens_ClientCredentialsGrantType() throws Exception {
    ...
    String accessToken =
        (String)
            JsonUtils.readValue(tokenResponse, new TypeReference<Map<String, Object>>() {})
                .get("access_token");
  ...
  void test_refresh_token_after_key_rotation() throws Exception {
    ...
    MockHttpServletResponse refreshResponse =
        useRefreshToken(refreshToken, client.getClientId(), SECRET, getZoneHostUrl(zone));
    ...
    MockHttpServletResponse refreshResponse2 =
        useRefreshToken(refreshToken, client.getClientId(), SECRET, getZoneHostUrl(zone));
    ...
    MockHttpServletResponse refreshResponse3 =
        useRefreshToken(refreshToken, client.getClientId(), SECRET, getZoneHostUrl(zone));
  ...
  void test_opaque_refresh_tokens_sets_revocable_claim() throws Exception {
    ...
    IdentityZoneHolder.set(zone);
    String token =
        revocableTokenProvisioning.retrieve(tokenId, IdentityZoneHolder.get().getId()).getValue();
  ...
  private void assertRefreshIdTokenCorrect(
      String originalIdTokenJwt, String idTokenJwtFromRefreshGrant) {
    ...
    // they still need to be populated according to
  ...
  void refreshTokenGrantType_returnsIdToken_toOpenIdClients() throws Exception {
    ...
    CompositeToken tokenResponse =
        getTokensWithPasswordGrant(
            client.getClientId(), SECRET, user.getUserName(), SECRET, "localhost", "jwt");
    ...
    MockHttpServletResponse refreshResponse =
        useRefreshToken(refreshToken, client.getClientId(), SECRET, "localhost");

    assertEquals(HttpStatus.SC_OK, refreshResponse.getStatus());
    CompositeToken compositeToken =
        JsonUtils.readValue(refreshResponse.getContentAsString(), CompositeToken.class);
    String idTokenJwt = compositeToken.getIdTokenValue();
    assertRefreshIdTokenCorrect(originalIdTokenJwt, idTokenJwt);
  }

  @Test
  void refreshTokenGrantType_returnsIdToken_toOpenIdClients_withOpaqueRefreshToken()
      throws Exception {
    when(timeService.getCurrentTimeMillis()).thenReturn(1000L);
    client = setUpClients("openidclient", "", "openid", "password,refresh_token", true);
    user = setUpUser("openiduser", "", OriginKeys.UAA, "uaa");
    CompositeToken tokenResponse =
        getTokensWithPasswordGrant(
            client.getClientId(), SECRET, user.getUserName(), SECRET, "localhost", "opaque");
    String refreshToken = tokenResponse.getRefreshToken().getValue();
    String originalIdTokenJwt = tokenResponse.getIdTokenValue();
    when(timeService.getCurrentTimeMillis()).thenReturn(5000L);
  ...
  void refreshTokenGrantType_withJwtTokens_preservesRefreshTokenExpiryClaim() throws Exception {
    ...
    CompositeToken tokenResponse =
        getTokensWithPasswordGrant(
            client.getClientId(), SECRET, user.getUserName(), SECRET, getZoneHostUrl(zone), "jwt");
    String refreshToken = tokenResponse.getRefreshToken().getValue();
    when(timeService.getCurrentTimeMillis()).thenReturn(5000L);
    ...
    CompositeToken newTokenResponse =
        getTokensWithPasswordGrant(
            client.getClientId(), SECRET, user.getUserName(), SECRET, getZoneHostUrl(zone), "jwt");
  ...
  void refreshTokenGrantType_withOpaqueTokens_preservesRefreshTokenExpiry() throws Exception {
    ...
    MockHttpServletResponse refreshResponse =
        useRefreshToken(firstRefreshToken, client.getClientId(), SECRET, getZoneHostUrl(zone));
    ...
    MockHttpServletResponse expiredResponse =
        useRefreshToken(firstRefreshToken, client.getClientId(), SECRET, getZoneHostUrl(zone));
    assertEquals(HttpStatus.SC_UNAUTHORIZED, expiredResponse.getStatus());
    MockHttpServletResponse alsoExpiredResponse =
        useRefreshToken(secondRefreshToken, client.getClientId(), SECRET, getZoneHostUrl(zone));
  ...
  void refreshTokenGrantType_doesNotReturnIdToken_toNonOpenIdClients() throws Exception {
    ...
    String refreshToken =
        getJwtRefreshToken(client.getClientId(), SECRET, user.getUserName(), SECRET, "localhost");
  ...
  void refreshTokenGrantType_requiresAuthorizedGrantType() throws Exception {
    ...
    ClientDetails clientWithoutRefresh =
        setUpClients("passwordclient", "", "scim.me", "password", true);
    user = setUpUser("joe-user", "", OriginKeys.UAA, "uaa");
    String refreshToken =
        getJwtRefreshToken(client.getClientId(), SECRET, user.getUserName(), SECRET, "localhost");
  ...
  MockHttpServletResponse useRefreshToken(
      String refreshToken, String clientId, String clientSecret, String host) throws Exception {
    return mockMvc
        .perform(
            post("/oauth/token")
                .header("Host", host)
  ...
  String getJwtRefreshToken(
      String clientId, String clientSecret, String userName, String password, String host)
      throws Exception {
    CompositeToken result =
        getTokensWithPasswordGrant(clientId, clientSecret, userName, password, host, "jwt");
    assertNotNull(result.getRefreshToken().getValue());
    return result.getRefreshToken().getValue();
  }

  private CompositeToken getTokensWithPasswordGrant(
      String clientId,
      String clientSecret,
      String userName,
      String password,
