...
public class LdapCertificateMockMvcTests extends InjectedMockContextTest {
  ...
  public static void startLdapsServers() throws Exception {
    ...
    if (started.compareAndSet(false, true)) {
      File expiredKeystore =
          new File(classLoader.getResource("certs/expired-self-signed-ldap-cert.jks").getFile());
      File validKeystore =
          new File(classLoader.getResource("certs/valid-self-signed-ldap-cert.jks").getFile());
      RandomValueStringGenerator generator = new RandomValueStringGenerator();
      LDAP_ROOT_DIRECTORY_VALID =
          new File(System.getProperty("java.io.tmpdir"), generator.generate());
      LDAP_ROOT_DIRECTORY_EXPIRED =
          new File(System.getProperty("java.io.tmpdir"), generator.generate());
      validLdapCertServer =
          new ApacheDsSSLContainer(
                  "dc=test,dc=com",
                  new Resource[] {
                    new ClassPathResource("ldap_init_apacheds.ldif"),
                    new ClassPathResource("ldap_init.ldif")
                  })
  ...
  public void createzones() throws Exception {
    trustedCertZone =
        MockMvcUtils.createOtherIdentityZoneAndReturnResult(
            gen.generate(), getMockMvc(), getWebApplicationContext(), null);
    ...
    trustedButExpiredCertZone =
        MockMvcUtils.createOtherIdentityZoneAndReturnResult(
            gen.generate(), getMockMvc(), getWebApplicationContext(), null);
    definition.setBaseUrl("ldaps://localhost:" + LDAP_EXPIRED_LDAPS_PORT);
    MockMvcUtils.createIdentityProvider(
        getMockMvc(), trustedButExpiredCertZone, OriginKeys.LDAP, definition);
  }
            ...
            post("/login.do")
                ...
                .with(
                    new SetServerNameRequestPostProcessor(
                        trustedCertZone.getIdentityZone().getSubdomain() + ".localhost"))
    ...
    getMockMvc()
        .perform(
            post("/login.do")
                ...
                .with(
                    new SetServerNameRequestPostProcessor(
                        trustedButExpiredCertZone.getIdentityZone().getSubdomain() + ".localhost"))
                .param("username", "marissa2")
                .param("password", LDAP))
        .andExpect(status().isFound())
