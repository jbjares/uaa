...
public class InvitationsServiceMockMvcTests {
  ...
  void inviteUserCorrectOriginSet() throws Exception {
    ...
    inviteUser(
        webApplicationContext, mockMvc, email, userInviteToken, null, clientId, OriginKeys.UAA);
  }
  ...
  void testAuthorizeWithInvitationLogin() throws Exception {
    ...
    assertEquals(
        ...
        jdbcTemplate.queryForObject(
            "SELECT origin FROM users WHERE username=?", new Object[] {email}, String.class));

    String code = extractInvitationCode(inviteLink.toString());
    MvcResult result =
        mockMvc
            .perform(get("/invitations/accept").param("code", code).accept(MediaType.TEXT_HTML))
    ...
    BaseClientDetails client =
        new BaseClientDetails(
            clientId, "", "openid", GRANT_TYPE_AUTHORIZATION_CODE, "", redirectUri);
    client.setClientSecret("secret");
    String adminToken =
        MockMvcUtils.getClientCredentialsOAuthAccessToken(
            mockMvc, "admin", "adminsecret", "", null);
  ...
  void acceptInvitationShouldNotLogYouIn() throws Exception {
    ...
    mockMvc
        .perform(get("/profile").session(session).accept(MediaType.TEXT_HTML))
  ...
  void acceptInvitationForVerifiedUserSendsRedirect() throws Exception {
    ...
    assertTrue(
        ...
        queryUserForField(jdbcTemplate, email, "verified", Boolean.class));
    assertEquals(
        OriginKeys.UAA, queryUserForField(jdbcTemplate, email, OriginKeys.ORIGIN, String.class));
  ...
  void acceptInvitationForUaaUserShouldExpireInvitelink() throws Exception {
    ...
    MockHttpServletRequestBuilder get =
        get("/invitations/accept").param("code", code).accept(MediaType.TEXT_HTML);
  ...
  void invalid_code() throws Exception {
    ...
    assertFalse(
        "User should be not yet be verified",
        queryUserForField(jdbcTemplate, email, "verified", Boolean.class));
  ...
  void acceptInvitationSetsYourPassword() throws Exception {
    ...
    result =
        mockMvc
            ...
            .andExpect(status().isFound())
            .andExpect(
                redirectedUrl("/login?success=invite_accepted&form_redirect_uri=" + REDIRECT_URI))
            .andReturn();

    assertTrue(
        "User should be verified after password reset",
        queryUserForField(jdbcTemplate, email, "verified", Boolean.class));
  ...
  void inviteLdapUsersVerifiesAndRedirects() throws Exception {
    ...
    IdentityProvider provider =
        createIdentityProvider(mockMvc, zone.getZone(), OriginKeys.LDAP, definition);
    ...
    assertFalse(
        ...
        queryUserForField(jdbcTemplate, email, "verified", Boolean.class));
    assertEquals(
        OriginKeys.LDAP, queryUserForField(jdbcTemplate, email, OriginKeys.ORIGIN, String.class));
    ...
    assertFalse(
        "LDAP user should not be verified after accepting invite until logging in",
        queryUserForField(jdbcTemplate, email, "verified", Boolean.class));
  }
  ...
  void inviteSamlUserWillRedirectUponAccept() throws Exception {
    ...
    SamlIdentityProviderDefinition definition =
        getSamlIdentityProviderDefinition(zone.getZone(), entityID);
    definition.setEmailDomain(Collections.singletonList(domain));
    definition.setIdpEntityAlias(originKey);
    IdentityProvider provider =
        createIdentityProvider(mockMvc, zone.getZone(), originKey, definition);
    ...
    assertFalse(
        ...
        queryUserForField(jdbcTemplate, email, "verified", Boolean.class));
    assertEquals(
        originKey, queryUserForField(jdbcTemplate, email, OriginKeys.ORIGIN, String.class));
    ...
    mockMvc
        ...
        .andExpect(
            redirectedUrl(
                String.format(
                    "/saml/discovery?returnIDParam=idp&entityID=%s.cloudfoundry-saml-login&idp=%s&isPassive=true",
                    zone.getZone().getIdentityZone().getId(), originKey)));

    assertEquals(
        provider.getOriginKey(),
        queryUserForField(jdbcTemplate, email, OriginKeys.ORIGIN, String.class));
    assertFalse(
        "Saml user should not yet be verified after clicking on the accept link",
        queryUserForField(jdbcTemplate, email, "verified", Boolean.class));
  }

  private static <T> T queryUserForField(
      JdbcTemplate jdbcTemplate, String email, String field, Class<T> type) {
    return jdbcTemplate.queryForObject(
        "SELECT " + field + " FROM users WHERE email=?", type, email);
  }

  private static ZoneScimInviteData createZoneForInvites(
      MockMvc mockMvc, WebApplicationContext webApplicationContext, String clientId)
      throws Exception {
    return MockMvcUtils.createZoneForInvites(
        mockMvc, webApplicationContext, clientId, REDIRECT_URI);
  }
  ...
  private static SamlIdentityProviderDefinition getSamlIdentityProviderDefinition(
      IdentityZoneCreationResult zone, String entityID) {
  ...
  private static URL inviteUser(
      ...
      throws Exception {
    return MockMvcUtils.inviteUser(
        webApplicationContext,
        mockMvc,
        email,
        userInviteToken,
