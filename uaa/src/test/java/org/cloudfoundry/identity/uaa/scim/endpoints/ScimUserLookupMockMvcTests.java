...
public class ScimUserLookupMockMvcTests extends InjectedMockContextTest {
  ...
  public void setUp() throws Exception {
    ...
    user =
        new ScimUser(
            null,
            new RandomValueStringGenerator().generate() + "@test.org",
            "PasswordResetUserFirst",
            "PasswordResetUserLast");
    user.setPrimaryEmail(user.getUserName());
    user.setPassword("secr3T");
    user = MockMvcUtils.createUser(getMockMvc(), adminToken, user);

    originalIsUserIdConversionEndpointsEnabled =
        getWebApplicationContext().getBean(UserIdConversionEndpoints.class).isEnabled();
    ...
    MockMvcUtils.createClient(
        ...
        clientId,
        clientSecret,
        Collections.singleton("scim"),
        scopes,
        Arrays.asList("client_credentials", "password"),
        "uaa.none");
    scimLookupIdUserToken =
        testClient.getUserOAuthAccessToken(
            clientId, clientSecret, user.getUserName(), "secr3T", "scim.userids");
    if (testUsers == null) {
  ...
  public void restoreEnabled() {
    getWebApplicationContext()
        .getBean(UserIdConversionEndpoints.class)
        .setEnabled(originalIsUserIdConversionEndpointsEnabled);
  }
  ...
  public void lookupId_DoesntReturnInactiveIdp_ByDefault() throws Exception {
    ScimUser scimUser =
        createInactiveIdp(new RandomValueStringGenerator().generate() + "test-origin");
    ...
    MockHttpServletResponse response =
        getMockMvc().perform(post).andExpect(status().isOk()).andReturn().getResponse();
  ...
  public void lookupIdFromUsernameWithIncorrectScope() throws Exception {
    String token =
        testClient.getUserOAuthAccessToken(
            clientId, clientSecret, user.getUserName(), "secr3T", "cloud_controller.read");
    String username = UaaTestAccounts.standard(null).getUserName();
  ...
  public void lookupIdFromUsernamePagination() throws Exception {
    ...
    for (int i = 0; i < testUsers.length; i += pageSize) {
      MockHttpServletRequestBuilder post =
          getIdLookupRequest(scimLookupIdUserToken, filter, i + 1, pageSize);
  ...
  private MockHttpServletRequestBuilder getIdLookupRequest(
      String token, String username, String operator) {
    String filter = String.format("username %s \"%s\"", operator, username);
    return getIdLookupRequest(token, filter, 1, 100);
  }

  private MockHttpServletRequestBuilder getIdLookupRequest(
      String token, String filter, int startIndex, int count) {
  ...
  private String[][] createUsers(String token) throws Exception {
    ...
    for (int i = 0; i < count; i++) {
      String id =
          i > 99 ? String.valueOf(i) : i > 9 ? "0" + String.valueOf(i) : "00" + String.valueOf(i);
  ...
  private ScimUser createInactiveIdp(String originKey) throws Exception {
    String tokenToCreateIdp =
        testClient.getClientCredentialsOAuthAccessToken("login", "loginsecret", "idps.write");
    IdentityProvider inactiveIdentityProvider =
        MultitenancyFixture.identityProvider(originKey, "uaa");
    inactiveIdentityProvider.setActive(false);
    MockMvcUtils.createIdpUsingWebRequest(
        getMockMvc(), null, tokenToCreateIdp, inactiveIdentityProvider, status().isCreated());

    ScimUser scimUser =
        new ScimUser(
            null, new RandomValueStringGenerator().generate() + "@test.org", "test", "test");
