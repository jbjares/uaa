...
public class ScimGroupEndpointsIntegrationTests {
  ...
  private final String DELETE_ME =
      "deleteme_" + new RandomValueStringGenerator().generate().toLowerCase();

  private final String CF_DEV =
      "cf_dev_" + new RandomValueStringGenerator().generate().toLowerCase();

  private final String CF_MGR =
      "cf_mgr_" + new RandomValueStringGenerator().generate().toLowerCase();
  ...
  public void createRestTemplate() throws Exception {
    ...
    JOEL =
        new ScimGroupMember(
            createUser(
                    "joel_" + new RandomValueStringGenerator().generate().toLowerCase(), "Passwo3d")
                .getId());
    DALE =
        new ScimGroupMember(
            createUser(
                    "dale_" + new RandomValueStringGenerator().generate().toLowerCase(), "Passwo3d")
                .getId());
    VIDYA =
        new ScimGroupMember(
            createUser(
                    "vidya_" + new RandomValueStringGenerator().generate().toLowerCase(),
                    "Passwo3d")
                .getId());
  }
  ...
  private ScimGroup createGroup(String name, ScimGroupMember... members) {
    ...
    List<ScimGroupMember> m =
        members != null ? Arrays.asList(members) : Collections.<ScimGroupMember>emptyList();
    g.setMembers(m);
    ScimGroup g1 =
        client.postForEntity(serverRunning.getUrl(groupEndpoint), g, ScimGroup.class).getBody();
  ...
  private ScimGroup updateGroup(String id, String name, ScimGroupMember... members) {
    ...
    ResponseEntity<Map> r =
        client.exchange(
            serverRunning.getUrl(groupEndpoint + "/{id}"),
            HttpMethod.PUT,
            new HttpEntity<>(g, headers),
            Map.class,
            id);
  ...
  private void validateUserGroups(String id, String... groups) {
    List<String> groupNames =
        groups != null ? Arrays.asList(groups) : Collections.<String>emptyList();
  ...
  private ScimUser getUser(String id) {
    return client
        .getForEntity(serverRunning.getUrl(userEndpoint + "/{id}"), ScimUser.class, id)
        .getBody();
  }
  ...
  public void getGroupsWithoutAttributesReturnsAllData() {
    ...
    ResponseEntity<Map> response =
        client.getForEntity(serverRunning.getUrl(groupEndpoint), Map.class);
    ...
    assertTrue(
        "There should be some resources", ((Collection<?>) results.get("resources")).size() > 0);
    @SuppressWarnings("rawtypes")
  ...
  public void createGroupSucceeds() throws Exception {
    ...
    ScimGroup g2 =
        client.getForObject(
            serverRunning.getUrl(groupEndpoint + "/{id}"), ScimGroup.class, g1.getId());
    assertEquals(g1, g2);
  }
  ...
  public void createGroupWithInvalidMembersFailsCorrectly() {
    ...
    Map<String, String> g2 =
        client.getForObject(
            serverRunning.getUrl(groupEndpoint + "?filter=displayName eq \"{name}\""),
            Map.class,
            CFID);
  ...
  public void createGroupWithMemberGroupSucceeds() {
    ...
    ScimGroup g3 =
        client.getForObject(
            serverRunning.getUrl(groupEndpoint + "/{id}"), ScimGroup.class, g2.getId());
    assertEquals(g2, g3);
  ...
  public void createExistingGroupFailsCorrectly() {
    ...
    @SuppressWarnings("unchecked")
    Map<String, String> g2 =
        client.postForEntity(serverRunning.getUrl(groupEndpoint), g1, Map.class).getBody();
  ...
  public void testAccessTokenReflectsGroupMembershipForPasswordGrant() throws Exception {
    ...
    OAuth2AccessToken token =
        getAccessTokenWithPassword(DELETE_ME, "secret", DELETE_ME, "Passwo3d");
    assertTrue("Wrong token: " + token, token.getScope().contains(CFID));
  ...
  public void testExtremeGroupPagination() {
    for (int i = 0; i < 502; i++) {
      ScimUser user =
          createUser(
              "deleteme_" + new RandomValueStringGenerator().generate().toLowerCase(), "Passwo3d");
      scimGroups.add(
          createGroup(
              "cfid_" + new RandomValueStringGenerator().generate().toLowerCase(),
              new ScimGroupMember(user.getId())));
    }

    ResponseEntity<Map> response =
        client.getForEntity(serverRunning.getUrl(groupEndpoint + "?count=502"), Map.class);
  ...
  private void createTestClient(String name, String secret, String scope) throws Exception {
    OAuth2AccessToken token =
        getClientCredentialsAccessToken("clients.read,clients.write,clients.admin");
  ...
  private OAuth2AccessToken getAccessTokenWithPassword(
  ...
  private OAuth2AccessToken getAccessToken(
    ...
    URI uri =
        serverRunning
            ...
            .queryParam("redirect_uri", "http://redirect.uri")
            .build();
    ResponseEntity<Void> result =
        serverRunning
            .createRestTemplate()
    ...
    assertTrue(
        "Wrong location: " + location, location.matches("http://redirect.uri" + ".*code=.+"));
