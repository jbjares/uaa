...
public class AuthorizeEndpointDocs extends EndpointDocs {
  ...
  private final ParameterDescriptor scopesParameter =
  ...
  private final ParameterDescriptor redirectParameter =
      parameterWithName(REDIRECT_URI)
          ...
          .attributes(key("constraints").value("Optional"), key("type").value(STRING));
  private final ParameterDescriptor promptParameter =
      parameterWithName(ID_TOKEN_HINT_PROMPT)
          .description(
              "specifies whether to prompt for user authentication. Only value `"
                  + ID_TOKEN_HINT_PROMPT_NONE
                  + "` is supported.")
          .attributes(key("constraints").value("Optional"), key("type").value(STRING));
  private final ParameterDescriptor responseTypeParameter =
  ...
  private final ParameterDescriptor loginHintParameter =
      parameterWithName("login_hint")
          ...
          .type(STRING)
          .description(
              "<small><mark>UAA 4.19.0</mark></small> Indicates the identity provider to be used. The passed string has to be a URL-Encoded JSON Object, containing the field `origin` with value as `origin_key` of an identity provider.");

  private UaaAuthentication principal;

  @Autowired private JdbcScimUserProvisioning userProvisioning;
  ...
  public void browserCodeRequest() throws Exception {
    ...
    Snippet requestParameters =
        requestParameters(
            responseTypeParameter.description(
  ...
  public void apiCodeRequest() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(status().isFound())
        .andDo(
            document("{ClassName}/{methodName}", requestParameters)
                .snippets(
                    requestHeaders(
  ...
  public void implicitGrant_browserRequest() throws Exception {
    ...
    MvcResult mvcResult =
        mockMvc
            .perform(get)
            .andExpect(status().isFound())
            .andDo(document("{ClassName}/{methodName}", responseHeaders, requestParameters))
            .andReturn();
  ...
  public void implicitGrantWithPromptParameter_browserRequest() throws Exception {
    ...
    Snippet responseHeaders =
        responseHeaders(
            headerWithName("Location")
                .description("Redirect url specified in the request parameters."));
  ...
  private static void resetMarissaPassword(ScimUserProvisioning scimUserProvisioning) {
    ...
    scimUserProvisioning.changePassword(
        marissa.getId(), null, UaaTestAccounts.DEFAULT_PASSWORD, IdentityZoneHolder.get().getId());
  }
}
