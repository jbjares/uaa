  ...
  public void populateDataUsingEndpoints() throws Exception {
    ...
    for (int i = 0; i < ENTITY_COUNT; i++) {
      ...
      webApplicationContext
          .getBean(IdentityZoneEndpoints.class)
          .createIdentityZone(
              zone,
              new AbstractBindingResult(null) {
                @Override
      ...
      for (int j = 0; j < ENTITY_COUNT; j++) {
        ...
        group =
            webApplicationContext
                ...
                .createGroup(group, new MockHttpServletResponse());
        groups.add(group);
      }
    ...
    for (Map.Entry<IdentityZone, List<ScimGroup>> zone : zones.entrySet()) {
      ...
      for (int i = 0; i < ENTITY_COUNT; i++) {
        ...
        try {
          ...
          user =
              webApplicationContext
                  .getBean(ScimUserEndpoints.class)
                  .createUser(user, new MockHttpServletRequest(), new MockHttpServletResponse());
          users.add(user);
          ScimGroupMember member = new ScimGroupMember(user.getId());
          ScimGroup group =
              webApplicationContext
                  .getBean(ScimGroupEndpoints.class)
                  .getGroup(zone.getValue().get(i).getId(), new MockHttpServletResponse());
          ...
          webApplicationContext
              ...
              .updateGroup(
                  group,
                  group.getId(),
                  String.valueOf(group.getVersion()),
  ...
  public void test_Ensure_That_New_Fields_NotNull() throws Exception {
    Assert.assertThat(
        webApplicationContext
            ...
            .queryForObject(
                "SELECT count(*) FROM external_group_mapping WHERE origin IS NULL", Integer.class),
        is(0));
    Assert.assertThat(
        webApplicationContext
            .getBean(JdbcTemplate.class)
            .queryForObject(
                "SELECT count(*) FROM groups WHERE identity_zone_id IS NULL", Integer.class),
        is(0));
  }
}
