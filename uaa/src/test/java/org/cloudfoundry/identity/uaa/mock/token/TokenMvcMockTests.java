...
public class TokenMvcMockTests extends AbstractTokenMockMvcTests {
  ...
  void setup() {
    ...
    IdentityZoneHolder.setProvisioning(
        webApplicationContext.getBean(IdentityZoneProvisioning.class));
  }
  ...
  void token_endpoint_post_query_string_by_default() throws Exception {
    ...
    mockMvc
        .perform(
            post(
                    "/oauth/token?client_id=cf&client_secret=&grant_type=password&username={username}&password=secret",
                    username)
  ...
  void token_endpoint_post_query_string() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.error_description")
                .value("Parameters must be passed in the body of the request"));
  }
  ...
  void refresh_grant_fails_because_missing_required_groups() throws Exception {
    ...
    BaseClientDetails clientDetails =
        new BaseClientDetails(
            clientId, null, "uaa.user,other.scope", "password,refresh_token", "uaa.resource", null);
  ...
  void authorization_code_missing_required_scopes() throws Exception {
    ...
    MultiValueMap<String, String> queryParams =
        UriComponentsBuilder.fromUri(new URI(location)).build().getQueryParams();
  ...
  void authorization_code_missing_required_scopes_during_token_fetch() throws Exception {
    ...
    assertThat(
        (String) errorResponse.get("error_description"),
        containsString(
            HtmlUtils.htmlEscape(
                "User does not meet the client's required group criteria.", "ISO-8859-1")));
  }
  ...
  void token_grant_missing_required_groups() throws Exception {
    ...
    BaseClientDetails clientDetails =
        new BaseClientDetails(
            clientId, null, "uaa.user,other.scope", "password", "uaa.resource", null);
    clientDetails.setClientSecret(SECRET);
    clientDetails.addAdditionalInformation(REQUIRED_USER_GROUPS, Arrays.asList("uaa.admin"));
    clientDetailsService.addClientDetails(clientDetails);
  ...
  void token_grant_required_groups_are_present() throws Exception {
    ...
    clientDetails.addAdditionalInformation(
        REQUIRED_USER_GROUPS, Arrays.asList("required.scope.1", "required.scope.2"));
    clientDetailsService.addClientDetails(clientDetails);
  ...
  void password_grant() throws Exception {
    ...
    assertEquals(
        ...
        webApplicationContext
            ...
            .update(
                "UPDATE users SET passwd_change_required = ? WHERE ID = ?", true, user.getId()));
  ...
  void passcode_with_client_parameters_when_password_change_required_for_user() throws Exception {
    ...
    jdbcScimUserProvisioning.updatePasswordChangeRequired(
        user.getId(), true, IdentityZoneHolder.get().getId());
    ...
    Map<String, String> error =
        (JsonUtils.readValue(response, new TypeReference<Map<String, String>>() {}));
  ...
  void refresh_access_token_and_user_group_removed() throws Exception {
    ...
    List<ScimGroup> groups =
        webApplicationContext
            .getBean(ScimGroupProvisioning.class)
            .query("displayName eq \"uaa.admin\"", IdentityZoneHolder.get().getId());
    assertEquals(1, groups.size());
    webApplicationContext
        .getBean(ScimGroupMembershipManager.class)
        .removeMemberById(
            groups.get(0).getId(), scimUser.getId(), IdentityZoneHolder.get().getId());
  ...
  void test_saml_bearer_grant() throws Exception {
    ...
    MockMvcUtils.IdentityZoneCreationResult zone =
        MockMvcUtils.createOtherIdentityZoneAndReturnResult(
            subdomain, mockMvc, webApplicationContext, null, false);
    ...
    SamlIdentityProviderDefinition idpDef =
        createLocalSamlIdpDefinition(origin, zone.getIdentityZone().getId(), idpMetadata);
    ...
    webApplicationContext
        ...
        .create(provider, provider.getIdentityZoneId());
    IdentityZoneHolder.clear();

    String assertion =
        samlTestUtils.mockAssertionEncoded(
            subdomain + ".cloudfoundry-saml-login",
            "urn:oasis:names:tc:SAML:1.1:nameid-format:unspecified",
            "Saml2BearerIntegrationUser",
            "http://"
                + subdomain
                + ".localhost:8080/uaa/oauth/token/alias/"
                + subdomain
    ...
    mockMvc
        .perform(post.param("scope", "uaa.admin"))
        .andDo(print())
        .andExpect(status().isBadRequest());
  }
  ...
  void test_two_zone_saml_bearer_grant() throws Exception {
    ...
    // create an IDP in the default zone
    SamlIdentityProviderDefinition idpDef =
        createLocalSamlIdpDefinition(idpOrigin, IdentityZone.getUaa().getId(), idpMetadata);
    IdentityProvider provider = new IdentityProvider();
    provider.setConfig(idpDef);
    provider.setActive(true);
  ...
  @Test
  void getOauthToken_usingAuthCode_withClientIdAndSecretInRequestBody_shouldBeOk()
      throws Exception {
    String clientId = "testclient" + generator.generate();
    setUpClients(
        clientId,
  ...
  void testRefreshTokenNotPresentWhenClientDoesNotHaveGrantType() throws Exception {
    BaseClientDetails clientWithoutRefreshTokenGrant =
        setUpClients(
            "testclient" + generator.generate(), "", "openid", GRANT_TYPE_AUTHORIZATION_CODE, true);
    String username = "testuser" + generator.generate();
    ...
    assertNotNull(
        JsonUtils.readValue(mvcResult.getResponse().getContentAsString(), Map.class)
            .get("access_token"));
    assertNull(
        JsonUtils.readValue(mvcResult.getResponse().getContentAsString(), Map.class)
            .get("refresh_token"));
  ...
  void refreshAccessToken_withClient_withAutoApproveField() throws Exception {
    ...
    clientDetails.setClientSecret("secret");
    clientDetails.addAdditionalInformation(
        ClientConstants.AUTO_APPROVE, Arrays.asList("other.scope"));
    ...
    OAuth2RefreshToken refreshToken =
        JsonUtils.readValue(mvcResult.getResponse().getContentAsString(), CompositeToken.class)
            .getRefreshToken();

    MockHttpServletRequestBuilder postForRefreshToken =
        post("/oauth/token")
            .with(httpBasic(clientId, SECRET))
  ...
  void authorizeEndpointWithPromptNone_WhenNotAuthenticated() throws Exception {
    ...
    assertTrue(
        url.startsWith(
            UaaUrlUtils.addQueryParameter(TEST_REDIRECT_URI, "error", "login_required")));
  }
  ...
  void testAuthorizeEndpointWithPromptNone_MfaRequired() throws Exception {
    ...
    assertTrue(
        url.startsWith(
            UaaUrlUtils.addQueryParameter(TEST_REDIRECT_URI, "error", "interaction_required")));
  ...
  @Test
  void getOauthToken_usingPassword_withClientIdAndSecretInRequestBody_shouldBeOk()
      throws Exception {
    String clientId = "testclient" + generator.generate();
    setUpClients(
        clientId,
  ...
  void getOauthToken_usingPassword_withNoCommonScopes_shouldBeUnauthorized() throws Exception {
    ...
    assertThat(
        result.getResponse().getContentAsString(),
        containsString(
            "[something_else] is invalid. This user is not allowed any of the requested scopes"));
  }

  @Test
  void getOauthToken_usingClientCredentials_withClientIdAndSecretInRequestBody_shouldBeOk()
      throws Exception {
    String clientId = "testclient" + generator.generate();
    setUpClients(
        clientId,
  ...
  @Test
  void testClientIdentityProviderWithoutAllowedProvidersForPasswordGrantWorksInOtherZone()
      throws Exception {
    ...
    String clientId = "testclient" + generator.generate();
    setUpClients(
        clientId, scopes, scopes, "authorization_code,password", true, TEST_REDIRECT_URI, null);

    String username = "testuser" + generator.generate();
    String userScopes =
        "space.1.developer,space.2.developer,org.1.reader,org.2.reader,org.12345.admin,scope.one,scope.two,scope.three,openid";
  ...
  @Test
  void
      testClientIdentityProviderClientWithoutAllowedProvidersForAuthCodeAlreadyLoggedInWorksInAnotherZone()
          throws Exception {
    // a client without allowed providers in non default zone should always be rejected
    String subdomain = "testzone" + generator.generate();
    IdentityZone testZone = setupIdentityZone(subdomain);
  ...
  void testClientIdentityProviderRestrictionForPasswordGrant() throws Exception {
    // a client with allowed providers in the default zone should be rejected if the client is not
    // allowed
    String clientId = "testclient" + generator.generate();
    String clientId2 = "testclient" + generator.generate();
    String scopes = "space.*.developer,space.*.admin,org.*.reader,org.123*.admin,*.*,*,openid";
    ...
    setUpClients(
        ...
        Arrays.asList(provider.getOriginKey()));
    setUpClients(
        clientId2, scopes, scopes, "authorization_code,password", true, TEST_REDIRECT_URI, null);
  ...
  void test_OAuth_Authorize_API_Endpoint() throws Exception {
    ...
    String uaaUserAccessToken =
        getUserOAuthAccessToken(mockMvc, clientId, SECRET, username, SECRET, "", testZone);

    String state = generator.generate();

    MockHttpServletRequestBuilder oauthAuthorizeGet =
        get("/oauth/authorize")
            ...
            .param(OAuth2Utils.CLIENT_ID, clientId);
    MvcResult result =
        mockMvc.perform(oauthAuthorizeGet).andExpect(status().is3xxRedirection()).andReturn();
    ...
    Map<String, Object> map =
        JsonUtils.readValue(body, new TypeReference<Map<String, Object>>() {});
    String accessToken = (String) map.get("access_token");
    OAuth2Authentication token = tokenServices.loadAuthentication(accessToken);
    assertTrue(
        "Must have uaa.user scope", token.getOAuth2Request().getScope().contains("uaa.user"));
  }
  ...
  void refreshAccessToken_whenScopeIsPresent_andRestrictedOnGrantType() throws Exception {
    ...
    mockMvc
        .perform(postForRefreshToken.param(REQUEST_TOKEN_FORMAT, OPAQUE.getStringValue()))
        .andExpect(status().isOk());
    mockMvc
        .perform(postForRefreshToken.param(REQUEST_TOKEN_FORMAT, OPAQUE.getStringValue()))
        .andExpect(status().isOk());
  ...
  void testOpenIdTokenHybridFlowWithNoImplicitGrant_When_IdToken_Disabled() throws Exception {
    try {
      webApplicationContext
          .getBean(DisableIdTokenResponseTypeFilter.class)
          .setIdTokenDisabled(true);
      ...
      MvcResult result =
          mockMvc.perform(oauthTokenPost).andExpect(status().is3xxRedirection()).andReturn();
      String location = result.getResponse().getHeader("Location");
  ...
  @Test
  void testOpenIdTokenHybridFlowWithNoImplicitGrantWhenLenientWhenAppNotApproved()
      throws Exception {
    String clientId = "testclient" + generator.generate();
    String scopes = "space.*.developer,space.*.admin,org.*.reader,org.123*.admin,*.*,*,openid";
  ...
  void test_subdomain_redirect_url() throws Exception {
    ...
    String state = generator.generate();
    MockHttpServletRequestBuilder authRequest =
        get("/oauth/authorize")
            .with(httpBasic(clientId, SECRET))
            .session(session)
            .param(OAuth2Utils.RESPONSE_TYPE, "code")
    ...
    MvcResult result =
        mockMvc.perform(authRequest).andExpect(status().is3xxRedirection()).andReturn();
    String location = result.getResponse().getHeader("Location");
  ...
  void invalidScopeErrorMessageIsNotShowingAllClientScopes() throws Exception {
    ...
    String username = "testuser" + generator.generate();
    ScimUser developer =
        setUpUser(username, "scim.write", OriginKeys.UAA, IdentityZoneHolder.getUaaZone().getId());
    MockHttpSession session = getAuthenticatedSession(developer);
    ...
    UriComponents locationComponents =
        UriComponentsBuilder.fromUri(URI.create(mvcResult.getResponse().getHeader("Location")))
            .build();
    MultiValueMap<String, String> queryParams = locationComponents.getQueryParams();
    String errorMessage =
        URIUtil.encodeQuery("scim.write is invalid. Please use a valid scope name in the request");
  ...
  void invalidScopeErrorMessageIsNotShowingAllUserScopes() throws Exception {
    String clientId = "testclient" + generator.generate();
    String scopes =
        "openid,password.write,cloud_controller.read,scim.userids,password.write,something.else";
    setUpClients(clientId, scopes, scopes, GRANT_TYPE_AUTHORIZATION_CODE, true);

    String username = "testuser" + generator.generate();
    ScimUser developer =
        setUpUser(username, "openid", OriginKeys.UAA, IdentityZoneHolder.getUaaZone().getId());
    MockHttpSession session = getAuthenticatedSession(developer);
  ...
  @Test
  void ensure_that_form_redirect_is_not_a_parameter_unless_there_is_a_saved_request()
      throws Exception {
    ...
    mockMvc
        .perform(get("/login").session(new MockHttpSession()))
        .andDo(print())
        .andExpect(content().string(not(containsString(FORM_REDIRECT_PARAMETER))));
  }
  ...
  void test_authorization_code_grant_session_expires_during_app_approval() throws Exception {
    ...
    MvcResult result =
        mockMvc
            .perform(get(new URI(url)).session(session))
            .andDo(print())
            .andExpect(status().isOk())
  ...
  void test_authorization_code_grant_redirect_when_session_expires() throws Exception {
    ...
    MvcResult result =
        mockMvc.perform(get(new URI(authUrl))).andExpect(status().is3xxRedirection()).andReturn();
    String location = result.getResponse().getHeader("Location");
  ...
  void test_invalid_redirect_uri() throws Exception {
    test_invalid_registered_redirect_uris(
        new HashSet(Arrays.asList("*", "*/*")), status().isBadRequest());
  }
  ...
  void test_valid_redirect_uri() throws Exception {
    ...
    test_invalid_registered_redirect_uris(
        new HashSet(Arrays.asList(redirectUri)), status().isFound());
  }
  ...
  void make_sure_Bootstrapped_users_Dont_Revoke_Tokens_If_No_Change() throws Exception {
    ...
    Map<String, Object> tokenResponse =
        JsonUtils.readValue(tokenString, new TypeReference<Map<String, Object>>() {});
  ...
  void testAuthorizationCode_ShouldNot_Throw_500_If_Client_Doesnt_Exist() throws Exception {
    ...
    MockHttpServletRequestBuilder login =
        get("/login").accept(MediaType.TEXT_HTML).session((MockHttpSession) session);
  ...
  void gettingOpenIdToken_withPasswordGrantType_usingBasicAuth() throws Exception {
    ...
    ScimUser developer =
        setUpUser(
            "testuser" + this.generator.generate(),
            "openid",
            OriginKeys.UAA,
  ...
  void legacyUserAuthentication_IdTokenRequest() throws Exception {
    ...
    String credentials =
        String.format(
            "{ \"username\":\"%s\", \"password\":\"%s\" }", developer.getUserName(), SECRET);
    MvcResult result =
        mockMvc
            .perform(
                post("/oauth/authorize")
    ...
    assertNotEquals(
        hashFragmentParams.get("access_token").get(0), hashFragmentParams.get("id_token").get(0));
    validateOpenIdConnectToken(
  ...
  void gettingOpenIdToken_withAuthorizationCodeGrantType_withBasicAuth() throws Exception {
    ...
    setUpClients(
        clientId, "", "openid", "authorization_code,refresh_token", true, TEST_REDIRECT_URI);
    ScimUser developer =
        setUpUser(
            "testuser" + this.generator.generate(),
            "openid",
    ...
    Map<String, String> tokenResponse =
        JsonUtils.readValue(result.getResponse().getContentAsString(), Map.class);
    ...
    // nonce must be in id_token if was in auth request, see
  ...
  void gettingOpenIdToken_withAuthorizationCodeGrantType() throws Exception {
    ...
    assertNotNull(
        "ID Token should be present when client has openid scope", tokenResponse.get("id_token"));
    assertNotEquals(tokenResponse.get(ACCESS_TOKEN), tokenResponse.get("id_token"));
    validateOpenIdConnectToken(tokenResponse.get("id_token"), developer.getId(), clientId);
    Map<String, Object> claims = getClaimsForToken(tokenResponse.get("id_token"));
    assertEquals("testnonce", claims.get(ClaimConstants.NONCE));
    assertEquals(
        "openid",
        ((ArrayList<String>) getClaimsForToken(tokenResponse.get(ACCESS_TOKEN)).get("scope"))
            .get(0));
  }

  @Test
  void attemptingToGetOpenIdToken_withAuthorizationCodeGrantType_whenClientMissingOpenidScope()
      throws Exception {
    String clientId = "authcode-client" + this.generator.generate();
    setUpClients(
        clientId,
    ...
    // http://openid.net/specs/openid-connect-core-1_0.html#TokenResponse
    assertNull(
        "ID Token should not be present when client is missing openid scope",
  ...
  void gettingOpenIdToken_HybridFlow_withTokenPlusIdToken() throws Exception {
    ...
    Map<String, String> tokenResponse =
        JsonUtils.readValue(result.getResponse().getContentAsString(), Map.class);
    assertNotNull(
        "ID Token should be present when response_type includes id_token",
        tokenResponse.get("id_token"));
    assertNotNull(tokenResponse.get("id_token"));
    assertNotNull(tokenResponse.get("access_token"));
    validateOpenIdConnectToken(tokenResponse.get("id_token"), developer.getId(), clientId);
  ...
  void test_Token_Expiry_Time() throws Exception {
    ...
    String userScopes =
        "space.1.developer,space.2.developer,org.1.reader,org.2.reader,org.12345.admin,scope.one,scope.two,scope.three";
    ScimUser developer =
        setUpUser(userId, userScopes, OriginKeys.UAA, IdentityZoneHolder.get().getId());
    ...
    String token =
        validatePasswordGrantToken(clientId, userId, subdomain, "", new ArrayList<>(allUserScopes));

    if (token.length() <= 36) {
      token =
          webApplicationContext
              .getBean(JdbcRevocableTokenProvisioning.class)
              .retrieve(token, IdentityZoneHolder.get().getId())
              .getValue();
    }
    ...
    assertTrue(
        "Expiration Date should be more than 9 years ahead.",
  ...
  void testWildcardPasswordGrant() throws Exception {
    ...
    validatePasswordGrantToken(
        clientId, userId, subdomain, "space.2.developer", Arrays.asList("space.2.developer"));
    validatePasswordGrantToken(
        clientId, userId, subdomain, "org.123*.admin", Arrays.asList("org.12345.admin"));
    validatePasswordGrantToken(
        clientId,
        userId,
        subdomain,
    ...
    validatePasswordGrantToken(
        clientId, userId, subdomain, "org.123*.admin,space.*.developer,*.*", new ArrayList<>(set1));
    validatePasswordGrantToken(
        clientId,
        userId,
        subdomain,
  ...
  void testLoginAuthenticationFilter() throws Exception {
    ...
    String userScopes =
        "space.1.developer,space.2.developer,org.1.reader,org.2.reader,org.12345.admin,scope.one,scope.two,scope.three";
    ScimUser developer =
        setUpUser(userId, userScopes, OriginKeys.LOGIN_SERVER, IdentityZoneHolder.get().getId());
    String loginToken = testClient.getClientCredentialsOAuthAccessToken("login", "loginsecret", "");
    String basicAuthForLoginClient =
        new String(Base64.encode(String.format("%s:%s", "login", "loginsecret").getBytes()));
  ...
  void testOtherOauthResourceLoginAuthenticationFilter() throws Exception {
    ...
    String oauthScopes =
        "space.*.developer,space.*.admin,org.*.reader,org.123*.admin,*.*,*,oauth.something";
  ...
  void testOtherClientAuthenticationMethods() throws Exception {
    ...
    String oauthClientId = "testclient" + generator.generate();
    String oauthScopes =
        "space.*.developer,space.*.admin,org.*.reader,org.123*.admin,*.*,*,oauth.something,uaa.user";
    setUpClients(oauthClientId, oauthScopes, oauthScopes, GRANT_TYPES, true);

    String userId = "testuser" + generator.generate();
    String userScopes =
        "space.1.developer,space.2.developer,org.1.reader,org.2.reader,org.12345.admin,scope.one,scope.two,scope.three,uaa.user";
    ScimUser developer =
        setUpUser(userId, userScopes, OriginKeys.UAA, IdentityZoneHolder.get().getId());
    String basicAuthForOauthClient =
        new String(Base64.encode(String.format("%s:%s", oauthClientId, SECRET).getBytes()));
  ...
  @Test
  void clientCredentials_byDefault_WillNotLockoutDuringUnsuccessfulBasicAuthAndFormData()
      throws Exception {
    String clientId = "testclient" + generator.generate();
    String scopes = "space.*.developer,space.*.admin,org.*.reader,org.123*.admin,*.*,*";
    setUpClients(clientId, scopes, scopes, GRANT_TYPES, true);
    ...
    mockMvc
        .perform(
            post("/check_token")
                .header(
                    "Authorization",
                    "Basic " + new String(Base64.encode("app:appclientsecret".getBytes())))
  ...
  void validateOldTokenAfterDeleteClientSecret() throws Exception {
    ...
    InvalidTokenException tokenRevokedException =
        JsonUtils.readValue(response.getContentAsString(), TokenRevokedException.class);
        ...
        mockMvc
            .perform(
                post("/oauth/token")
                    ...
                    .header(
                        "Authorization",
                        "Basic " + new String(Base64.encode((clientId + ":newSecret").getBytes())))
                    .param("grant_type", "client_credentials")
                    .param("client_id", clientId)
                    .param("client_secret", SECRET))
            .andExpect(status().isOk())
  ...
  void testGetClientCredentials_WithAuthoritiesExcluded_ForDefaultIdentityZone() throws Exception {
    Set<String> originalExclude =
        webApplicationContext.getBean(UaaTokenServices.class).getExcludedClaims();
    try {
      webApplicationContext
          .getBean(UaaTokenServices.class)
          .setExcludedClaims(
              new HashSet<>(Arrays.asList(ClaimConstants.AUTHORITIES, ClaimConstants.AZP)));
      String clientId = "testclient" + generator.generate();
      String scopes = "space.*.developer,space.*.admin,org.*.reader,org.123*.admin,*.*,*";
      setUpClients(clientId, scopes, scopes, GRANT_TYPES, true);
  ...
  void testGetClientCredentialsTokenForOtherIdentityZone() throws Exception {
    ...
    mockMvc
        .perform(
            post("http://" + subdomain + ".localhost/oauth/token")
                .accept(MediaType.APPLICATION_JSON_VALUE)
                .with(new SetServerNameRequestPostProcessor(subdomain + ".localhost"))
                .with(httpBasic(clientId, SECRET))
                .param("grant_type", "client_credentials")
  ...
  void misconfigured_jwt_keys_returns_proper_error() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.error_description")
                .value("Unable to sign token, misconfigured JWT signing keys"));
  }
  ...
  void testGetClientCredentialsTokenForOtherIdentityZoneFromDefaultZoneFails() throws Exception {
    ...
    mockMvc
        .perform(
            post("http://localhost/oauth/token")
                .accept(MediaType.APPLICATION_JSON_VALUE)
                // .header("Host", subdomain + ".localhost") - with updated Spring, this now works
                // for request.getServerName
                .with(httpBasic(clientId, SECRET))
                .param("grant_type", "client_credentials")
                .param("client_id", clientId)
                .param("client_secret", SECRET))
  ...
  void testGetPasswordGrantTokenExpiredPasswordForOtherZone() throws Exception {
    ...
    assertEquals(
        ...
        webApplicationContext
            .getBean(JdbcTemplate.class)
            .update("UPDATE users SET passwd_lastmodified = ? WHERE username = ?", t, username));
  ...
  void password_grant_with_default_user_groups_in_zone() throws Exception {
    ...
    List<String> defaultGroups =
        new LinkedList(Arrays.asList("custom.default.group", "other.default.group"));
    defaultGroups.addAll(UserConfig.DEFAULT_ZONE_GROUPS);
    createNonDefaultZone(
        username, subdomain, clientId, defaultGroups, "custom.default.group,openid");
    ...
    assertEquals(
        claims.getIss(), "http://" + subdomain.toLowerCase() + ".localhost:8080/uaa/oauth/token");
  ...
  void testGetTokenScopesNotInAuthentication() throws Exception {
    ...
    assertThat(
        ...
        containsInAnyOrder(
            zoneAdminGroup, "openid", "cloud_controller.read", "cloud_controller.write"));
  }
  ...
  void testGetTokenPromptLogin() throws Exception {
        ...
        get("/oauth/authorize")
            .header(
                ...
                "Basic "
                    + new String(
                        org.apache.commons.codec.binary.Base64.encodeBase64(
                            ("identity:identitysecret").getBytes())))
            .header("Accept", MediaType.APPLICATION_JSON_VALUE)
            .session(session)
            .param(OAuth2Utils.GRANT_TYPE, GRANT_TYPE_AUTHORIZATION_CODE)
            .param(OAuth2Utils.RESPONSE_TYPE, "code")
    ...
    MvcResult result =
        mockMvc.perform(authRequest).andExpect(status().is3xxRedirection()).andReturn();
    assertEquals(
        result.getRequest().getRequestURL().toString(),
    ...
    Map<String, String[]> mapResponse =
        UaaUrlUtils.getParameterMap(result.getResponse().getRedirectedUrl());
  ...
  void testRevocablePasswordGrantTokenForDefaultZone() throws Exception {
    ...
    Map<String, Object> tokenResponse =
        testRevocablePasswordGrantTokenForDefaultZone(new HashedMap());
    ...
    Map<String, Object> claims =
        JsonUtils.readValue(jwt.getClaims(), new TypeReference<Map<String, Object>>() {});
    assertNotNull(
        "Token revocation signature must exist", claims.get(ClaimConstants.REVOCATION_SIGNATURE));
    assertTrue(
        "Token revocation signature must be a string",
        claims.get(ClaimConstants.REVOCATION_SIGNATURE) instanceof String);
    assertTrue(
        "Token revocation signature must have data",
  ...
  void testPasswordGrantTokenForDefaultZone_Opaque() throws Exception {
    ...
    RevocableToken revocableToken =
        webApplicationContext
            .getBean(RevocableTokenProvisioning.class)
            .retrieve(token, IdentityZoneHolder.get().getId());
    ...
    assertTrue(
        "Token revocable claim must be set to true",
  ...
  void testRefreshGrant_returnsValidAccessToken() throws Exception {
    ...
    body =
        doRefreshGrant(refreshToken, clientId, SECRET, status().isOk())
            .getResponse()
            .getContentAsString();
  ...
  void testJkuHeaderIsSet_andNonRfcHeadersNotSet_forAccessToken() throws Exception {
    ...
    String accessTokenHeaderJson =
        new String(java.util.Base64.getDecoder().decode(accessTokenHeaderRaw));
    ...
    // `enc` and `iv` are not required by JWT or OAuth spec, so should not be set and thus not
    // returned in the token's header
  ...
  void testJkuHeaderIsSet_andNonRfcHeadersNotSet_forRefreshToken() throws Exception {
    ...
    String refreshTokenHeaderJson =
        new String(java.util.Base64.getDecoder().decode(refreshTokenHeaderRaw));
    Map<String, Object> headerMap =
  ...
  private String validatePasswordGrantToken(
      String clientId, String username, String requestedScopes, List<String> expectedScopes)
      throws Exception {
  ...
  private String validatePasswordGrantToken(
    ...
    if (zoneSubdomain == null) {
      pwdToken =
          testClient.getUserOAuthAccessToken(clientId, SECRET, username, SECRET, requestedScopes);
    } else {
      pwdToken =
          testClient.getUserOAuthAccessTokenForZone(
              clientId, SECRET, username, SECRET, requestedScopes, zoneSubdomain);
  ...
  private void test_invalid_registered_redirect_uris(
      Set<String> redirectUris, ResultMatcher resultMatcher) throws Exception {
    String redirectUri = "https://example.com/dashboard/?appGuid=app-guid&ace_config=test";
    String clientId = "authclient-" + generator.generate();
    String scopes = "openid";
  ...
  private void validateOpenIdConnectToken(String token, String userId, String clientId) {
    ...
    TokenEndpointBuilder tokenEndpointBuilder =
        (TokenEndpointBuilder) webApplicationContext.getBean("tokenEndpointBuilder");
    ...
    Long dbPreviousLogonTime =
        webApplicationContext
            .getBean(UaaUserDatabase.class)
  ...
  private static Map<String, List<String>> splitQuery(URL url) throws UnsupportedEncodingException {
    ...
    for (String pair : kv) {
      ...
      String value =
          i > 0 && pair.length() > i + 1 ? URLDecoder.decode(pair.substring(i + 1), "UTF-8") : null;
      params.get(key).add(value);
    }
    return params;
  }

  private MvcResult doPasswordGrant(
      String username,
      String password,
      String clientId,
      String clientSecret,
  ...
  private MvcResult doRefreshGrant(
      String refreshToken, String clientId, String clientSecret, ResultMatcher resultMatcher)
      throws Exception {
    return mockMvc
        .perform(
            post("/oauth/token")
  ...
  private void createNonDefaultZone(String username, String subdomain, String clientId) {
    createNonDefaultZone(
        username, subdomain, clientId, UserConfig.DEFAULT_ZONE_GROUPS, "cloud_controller.read");
  }

  private void createNonDefaultZone(
      String username,
      String subdomain,
      String clientId,
      List<String> defaultUserGroups,
      String scopes) {
  ...
  private ResultActions try_token_with_non_post(
      MockHttpServletRequestBuilder builder, ResultMatcher status) throws Exception {
    String username = setUpUserForPasswordGrant();
  ...
  private void validateRevocableJwtToken(Map<String, Object> tokenResponse, IdentityZone zone) {
    ...
    RevocableToken revocableToken =
        webApplicationContext
            .getBean(RevocableTokenProvisioning.class)
            .retrieve(tokenId, IdentityZoneHolder.get().getId());
    IdentityZoneHolder.clear();
    assertNotNull("Token should have been stored in the DB", revocableToken);
  ...
  private Map<String, Object> testRevocablePasswordGrantTokenForDefaultZone(
  ...
  private ScimUser setUpUser(String username) {
    ...
    return jdbcScimUserProvisioning.createUser(
        scimUser, "secret", IdentityZoneHolder.get().getId());
  }
    ...
    if (authorizationRequest.getAuthorities() != null) {
      authorizationRequestMap.put(
          ...
          Collections.unmodifiableSet(
              new HashSet<GrantedAuthority>(authorizationRequest.getAuthorities())));
    }
