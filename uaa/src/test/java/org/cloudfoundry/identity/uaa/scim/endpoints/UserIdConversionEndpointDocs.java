...
public class UserIdConversionEndpointDocs extends InjectedMockContextTest {
  ...
  public void setUp() throws Exception {
    ...
    dwayne =
        new ScimUser(null, "dwayne" + generator.generate() + "@test.org", "Dwayne", "Exemplar");
    ...
    userLookupToken =
        MockMvcUtils.getClientCredentialsOAuthAccessToken(
            getMockMvc(), clientId, clientSecret, "scim.userids", null, true);
  }
  ...
  public void lookUpIds() throws Exception {
    MockHttpServletRequestBuilder get =
        get("/ids/Users")
            ...
            .param(
                "filter",
                format("userName eq \"%s\" or id eq \"%s\"", bob.getUserName(), dwayne.getId()))
    ...
    Snippet requestHeader =
        requestHeaders(
            headerWithName("Authorization")
                .description("Bearer token with authorization for `scim.userids` scope"));

    Snippet requestParams =
        requestParameters(
            ...
            parameterWithName("sortOrder")
                .optional("ascending")
                .description("sort by username in `ascending` or `descending` order")
                .attributes(key("type").value(STRING)),
            ...
            parameterWithName("includeInactive")
                .optional("false")
                .description("include users from inactive identity providers")
    ...
    Snippet responseFields =
        responseFields(
            ...
            fieldWithPath("resources[].id")
                .description("The globally unique identifier for this user"),
            fieldWithPath("resources[].userName").description("The username"),
            fieldWithPath("resources[].origin")
                .description("The origin of the user, e.g. an identity provider alias"));
