...
class ScimExternalGroupMappingsEndpointDocs extends EndpointDocs {
  ...
  private final String DISPLAY_NAME_DESC =
      "The identifier specified upon creation of the group, unique within the identity zone";
  private final String EXTERNAL_GROUP_DESCRIPTION =
      "The identifier for the group in external identity provider that needs to be mapped to internal UAA groups";

  private static final HeaderDescriptor AUTHORIZATION_HEADER =
      headerWithName("Authorization")
          .description("Bearer token with authorization for `scim.write` scope");
  ...
  private final ParameterDescriptor externalGroup =
      parameterWithName("externalGroup").required().description(EXTERNAL_GROUP_DESCRIPTION);

  private final Snippet responseFields =
      responseFields(
          ...
          fieldWithPath("meta.lastModified")
              .description("The time the group mapping was last updated"),
          fieldWithPath("schemas").description(SCHEMAS_DESC));
  private final ParameterDescriptor origin =
      parameterWithName("origin").required().description(ORIGIN_DESC);
  ...
  void deleteExternalGroupMapping() throws Exception {
    ...
    ScimGroupExternalMember scimGroupExternalMember =
        JsonUtils.readValue(
            createExternalGroupMappingHelper(group).andReturn().getResponse().getContentAsString(),
    ...
    Snippet requestParameters =
        requestParameters(
            parameterWithName("startIndex")
                .optional("1")
                .type(NUMBER)
                .description("Display paged results beginning at specified index"),
            ...
            parameterWithName("origin")
                .optional(null)
                .type(STRING)
                .description("Filters results based on supplied origin. default is to return all"),
            parameterWithName("externalGroup")
                .optional(null)
                .type(STRING)
                .description(
                    "Filters results based on supplied externalGroup. default is to return all"),
            parameterWithName("filter")
                .optional(null)
                .type(STRING)
                .description(
                    "Deprecated - will be removed in future release. Use `externalGroup` and `origin` parameters instead."));

    Snippet requestHeaders =
        requestHeaders(
            headerWithName("Authorization")
                .description("Bearer token with authorization for `scim.read` scope"),
    ...
    Snippet responseFields =
        responseFields(
            ...
            fieldWithPath("startIndex")
                .description("The index of the first item of this page of results"),
            fieldWithPath("itemsPerPage")
                .description("The page size used in producing this page of results"),
            fieldWithPath("totalResults")
                .description("The number of results which matched the filter"),
            fieldWithPath("schemas").description(SCHEMAS_DESC));
