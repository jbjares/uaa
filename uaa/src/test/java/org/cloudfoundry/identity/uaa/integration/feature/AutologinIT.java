...
public class AutologinIT {
  ...
  public void testAutologinFlow(String contentType, Map body) throws Exception {
    ...
    ResponseEntity<Map> autologinResponseEntity =
        restOperations.exchange(
            baseUrl + "/autologin", HttpMethod.POST, new HttpEntity<>(body, headers), Map.class);
  ...
  public void testSimpleAutologinFlow() throws Exception {
    ...
    String tokenUrl =
        UriComponentsBuilder.fromHttpUrl(baseUrl).path("/oauth/token").build().toUriString();
    ...
    RequestEntity<MultiValueMap<String, String>> requestEntity =
        new RequestEntity<>(tokenParams, headers, HttpMethod.POST, new URI(tokenUrl));
    ...
    String revokeApprovalsUrl =
        UriComponentsBuilder.fromHttpUrl(baseUrl).path("/profile").build().toUriString();
    requestBody.clear();
    requestBody.add("clientId", "app");
    requestBody.add("delete", "");
    requestBody.add(
        DEFAULT_CSRF_COOKIE_NAME, IntegrationTestUtils.extractCookieCsrf(profilePage.getBody()));
  ...
  private void setCookiesFromResponse(
      BasicCookieStore cookieStore, ResponseEntity<String> loginResponse) {
  ...
  public void testClientAuthorization() throws Exception {
    ...
    try {
      restOperations.exchange(
          baseUrl + "/autologin", HttpMethod.POST, new HttpEntity<>(requestBody), Map.class);
    } catch (HttpClientErrorException e) {
      assertEquals(HttpStatus.UNAUTHORIZED, e.getStatusCode());
    }
  }
