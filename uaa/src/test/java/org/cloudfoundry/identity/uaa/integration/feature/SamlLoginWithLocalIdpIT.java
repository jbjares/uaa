...
public class SamlLoginWithLocalIdpIT {
  ...
  public void clearWebDriverOfCookies() throws Exception {
    ...
    assertTrue(
        "Expected testzone1.localhost and testzone2.localhost to resolve to 127.0.0.1",
        doesSupportZoneDNS());
  }
  ...
  public void cleanup() {
    String token = IntegrationTestUtils.getClientCredentialsToken(baseUrl, "admin", "adminsecret");
    String groupId =
        IntegrationTestUtils.getGroup(token, "", baseUrl, "zones.testzone1.admin").getId();
  ...
  public void testDownloadSamlIdpMetadata() {
    ...
    assertTrue(
        idpDefinition.getMetaDataLocation().contains(IDPSSODescriptor.DEFAULT_ELEMENT_LOCAL_NAME));
    assertTrue(idpDefinition.getMetaDataLocation().contains("entityID=\"" + entityId + "\""));
  }

  /**
   * Test that we can create an identity provider in UAA using UAA's own SAML identity provider
   * metadata.
   */
  @Test
  public void testCreateSamlIdp() throws Exception {
    SamlIdentityProviderDefinition idpDef =
        createLocalSamlIdpDefinition(IDP_ENTITY_ID, OriginKeys.UAA);
  ...
  public void testCreateSamlSp() throws Exception {
    SamlServiceProviderDefinition spDef =
        createLocalSamlSpDefinition("cloudfoundry-saml-login", "uaa");
    createSamlServiceProvider(
        "Local SAML SP", "cloudfoundry-saml-login", baseUrl, serverRunning, spDef);
  }
  ...
  public void testValidSaml2Bearer() throws Exception {
    ...
    ResponseEntity<CompositeToken> token =
        restOperations.exchange(
            baseUrl + "/oauth/token/alias/cloudfoundry-saml-login",
            HttpMethod.POST,
            new HttpEntity<>(postBody, headers),
  ...
  public void idp_initiated_login_invalid_sp() throws Exception {
    ...
    String adminToken =
        IntegrationTestUtils.getClientCredentialsToken(baseUrl, "admin", "adminsecret");

    IdentityZoneConfiguration configuration = new IdentityZoneConfiguration();
    IntegrationTestUtils.createZoneOrUpdateSubdomain(
        identityClient, baseUrl, zoneId1, zoneId1, configuration);
    ...
    assertNotNull(
        webDriver.findElement(By.xpath("//h2[contains(text(), 'Missing sp request parameter')]")));
    webDriver.get(testZone1Url + "/saml/idp/initiate?sp=invalid_entity_id");
    assertNotNull(
        webDriver.findElement(By.xpath("//h2[contains(text(), 'Invalid sp entity ID')]")));
  }
  ...
  public void idp_initiated_login() throws Exception {
    ...
    IdentityZone zone2 =
        IntegrationTestUtils.createZoneOrUpdateSubdomain(
            identityClient, baseUrl, zoneId2, zoneId2, null);
    ...
    SamlServiceProviderDefinition samlServiceProviderDefinition =
        createZone2SamlSpDefinition("cloudfoundry-saml-login");
    ...
    SamlIdentityProviderDefinition samlIdentityProviderDefinition =
        createZone1IdpDefinition("cloudfoundry-saml-login");
    getSamlIdentityProvider(zoneId2, adminToken, samlIdentityProviderDefinition);
    ...
    webDriver.findElement(
        By.xpath("//h1[contains(text(), 'Welcome to The Twiglet Zone[" + zoneId1 + "]!')]"));
    webDriver.findElement(By.name("username")).clear();
    webDriver.findElement(By.name("username")).sendKeys(email);
    webDriver.findElement(By.name("password")).sendKeys("secr3T");
    webDriver.findElement(By.xpath("//input[@value='Sign in']")).click();
  ...
  public void testLocalSamlIdpLoginInTestZone1Works() throws Exception {
    ...
    IdentityZone zone =
        IntegrationTestUtils.createZoneOrUpdateSubdomain(
            identityClient, baseUrl, zoneId, zoneId, null);
    String email = new RandomValueStringGenerator().generate() + "@samltesting.org";
    ScimUser user =
        IntegrationTestUtils.createUser(
            adminClient, baseUrl, email, "firstname", "lastname", email, true);

    String groupId =
        IntegrationTestUtils.findGroupId(adminClient, baseUrl, "zones.testzone1.admin");
    ...
    String zoneAdminClientId =
        new RandomValueStringGenerator().generate() + "-" + zoneId + "-admin";
    ...
    RestTemplate zoneAdminClient =
        IntegrationTestUtils.getClientCredentialsTemplate(
            IntegrationTestUtils.getClientCredentialsResource(
                testZone1Url, new String[0], zoneAdminClientId, "secret"));
    String zoneUserEmail = new RandomValueStringGenerator().generate() + "@samltesting.org";
    IntegrationTestUtils.createUser(
        zoneAdminClient, testZone1Url, zoneUserEmail, "Dana", "Scully", zoneUserEmail, true);

    SamlIdentityProviderDefinition samlIdentityProviderDefinition =
        createZone1IdpDefinition(IDP_ENTITY_ID);
    IdentityProvider<SamlIdentityProviderDefinition> provider =
        getSamlIdentityProvider(zoneId, zoneAdminToken, samlIdentityProviderDefinition);

    SamlServiceProviderDefinition samlServiceProviderDefinition =
        createZone1SamlSpDefinition("cloudfoundry-saml-login");
    getSamlServiceProvider(
        zoneId,
        zoneAdminToken,
        samlServiceProviderDefinition,
        "testzone1.cloudfoundry-saml-login",
        "Local SAML SP for testzone1",
        baseUrl);
    ...
    List<WebElement> elements =
        webDriver.findElements(
            By.xpath("//a[text()='" + samlIdentityProviderDefinition.getLinkText() + "']"));
    ...
    element.click();
    webDriver.findElement(
        By.xpath("//h1[contains(text(), 'Welcome to The Twiglet Zone[" + zoneId + "]!')]"));
    webDriver.findElement(By.name("username")).clear();
  ...
  /**
   * In this test testzone1 acts as the SAML IdP and testzone2 acts as the SAML SP. SP is first
   * configured with IDP Metadata that has three SSO Bindings, with Http-Artifact as the first one
   * and SamlAuthnRequest must succeed using the Second configured SSO binding. SP's IDP is updated
   * with Http-Artifact as the only binding configured. The SamlAuthnRequest must fail.
   */
  @SuppressWarnings("unchecked")
  @Test
  public void testWebSSOProfileWithArtifactInMetadataSamlIntegration() throws Exception {
    ...
    String groupIdTestZone1 =
        IntegrationTestUtils.findGroupId(adminClient, baseUrl, "zones.testzone1.admin");
    IntegrationTestUtils.addMemberToGroup(
        adminClient, baseUrl, idpZoneAdminUser.getId(), groupIdTestZone1);
    ...
    IdentityZone spZone =
        IntegrationTestUtils.createZoneOrUpdateSubdomain(
            identityClient, baseUrl, spZoneId, spZoneId, config);
    ...
    String groupIdTestZone2 =
        IntegrationTestUtils.findGroupId(adminClient, baseUrl, "zones.testzone2.admin");
    IntegrationTestUtils.addMemberToGroup(
        adminClient, baseUrl, spZoneAdminUser.getId(), groupIdTestZone2);
    ...
    SamlIdentityProviderDefinition samlIdentityProviderDefinition =
        createZone1IdpDefinition(IDP_ENTITY_ID);
    samlIdentityProviderDefinition.setMetaDataLocation(
        SamlTestUtils.SAML_IDP_METADATA_ARTIFACT_FIRST);
    IdentityProvider<SamlIdentityProviderDefinition> idp =
        getSamlIdentityProvider(spZoneId, spZoneAdminToken, samlIdentityProviderDefinition);
    ...
    performLogin(
        idpZoneId, idpZoneUserEmail, idpZoneUrl, spZone, spZoneUrl, samlIdentityProviderDefinition);
    ...
    samlIdentityProviderDefinition.setMetaDataLocation(
        SamlTestUtils.SAML_IDP_METADATA_ARTIFACT_ONLY);
    ...
    webDriver.get(url);
    String errorMessage =
        "No Supported binding was found for SAML SSO profile - browser. Supported SAML SSO browser profile bindings are HTTP-POST and HTTP-Redirect.";
  ...
  /**
   * In this test testzone1 acts as the SAML IdP and testzone2 acts as the SAML SP. SP makes the
   * first SAMLAUthnRequest using Http-Redirect binding and must succeed. SP's IDP is updated, so
   * that SP makes the Second SAMLAunthnRequest using Http-POST binding and must succeed.
   */
  @SuppressWarnings("unchecked")
  @Test
  public void testWebSSOProfileWithRedirectPostSamlIntegration() throws Exception {
    ...
    SamlIdentityProviderDefinition samlIdentityProviderDefinition =
        createZone1IdpDefinition(IDP_ENTITY_ID);
    samlIdentityProviderDefinition.setMetaDataLocation(
        SamlTestUtils.SAML_IDP_METADATA_REDIRECT_ONLY);
    ...
    SamlServiceProviderDefinition samlServiceProviderDefinition =
        createZone2SamlSpDefinition("cloudfoundry-saml-login");
    SamlServiceProvider sp = new SamlServiceProvider();
    samlServiceProviderDefinition.setMetaDataLocation(
        SamlTestUtils.SAML_SP_METADATA_TESTZONE2_FOR_REDIRECT);
    sp.setIdentityZoneId(idpZoneId);
    sp.setActive(true);
    sp.setConfig(samlServiceProviderDefinition);
    sp.setEntityId("testzone2.cloudfoundry-saml-login");
  ...
  public void testEntityIdFromZoneConfig() throws Exception {
    ...
    IdentityZone idpIdentityZone =
        IntegrationTestUtils.createZoneOrUpdateSubdomain(
            identityClient, baseUrl, idpZoneId, idpZoneId, null);
    String entityId = "testID1";
    idpIdentityZone.getConfig().getSamlConfig().setEntityID(entityId);
    IntegrationTestUtils.createZoneOrUpdateSubdomain(
        identityClient, baseUrl, idpZoneId, idpZoneId, idpIdentityZone.getConfig());
    ...
    String groupId =
        IntegrationTestUtils.findGroupId(adminClient, baseUrl, "zones." + idpZoneId + ".admin");
    ...
    ScimUser spZoneAdminUser = getSpZoneAdminUser(adminClient, spZoneAdminEmail);
    groupId =
        IntegrationTestUtils.findGroupId(adminClient, baseUrl, "zones." + spZoneId + ".admin");
    ...
    assertThat(
        idp.getConfig().getMetaDataLocation(), containsString("entityID=\"" + entityId + "\""));

    // Add SP definition to the IDP zone
    deleteSamlServiceProviders(idpZoneAdminToken, baseUrl, idpZoneId);
    SamlServiceProviderDefinition samlServiceProviderDefinition =
        createZone2SamlSpDefinition(IDP_ENTITY_ID);
    SamlServiceProvider sp = new SamlServiceProvider();
    sp.setIdentityZoneId(idpZoneId);
    sp.setActive(true);
    sp.setConfig(samlServiceProviderDefinition);
  ...
  public void testCrossZoneSamlIntegration() throws Exception {
    ...
    spZone =
        IntegrationTestUtils.createZoneOrUpdateSubdomain(
            identityClient, baseUrl, spZoneId, spZoneId, spZone.getConfig());
    assertEquals(2, spZone.getConfig().getSamlConfig().getKeys().size());
  ...
  private IdentityProvider<SamlIdentityProviderDefinition> getSamlIdentityProvider(
      String spZoneId,
  ...
  private void testLocalSamlIdpLogin(
      String firstUrl, String lookfor, String username, String password) throws Exception {
    SamlIdentityProviderDefinition idpDef = createLocalSamlIdpDefinition(IDP_ENTITY_ID, "uaa");
    @SuppressWarnings("unchecked")
    IdentityProvider<SamlIdentityProviderDefinition> provider =
        IntegrationTestUtils.createIdentityProvider(
    ...
    webDriver
        .findElement(By.xpath("//a[text()='" + provider.getConfig().getLinkText() + "']"))
        .click();
  ...
  private void deleteSamlServiceProviders(String zoneAdminToken, String url, String zoneId) {
    ...
    ResponseEntity<String> providerGet =
        client.exchange(url + "/saml/service-providers", HttpMethod.GET, getHeaders, String.class);
    if (providerGet != null && providerGet.getStatusCode() == HttpStatus.OK) {
      List<SamlServiceProvider> samlServiceProviders =
          JsonUtils.readValue(
              providerGet.getBody(), new TypeReference<List<SamlServiceProvider>>() {});
  ...
  private void deleteSamlServiceProvider(
      String id, String zoneAdminToken, String url, String zoneId) {
    RestTemplate client = new RestTemplate();
    MultiValueMap<String, String> headers = new LinkedMultiValueMap<>();
    headers.add("Accept", MediaType.APPLICATION_JSON_VALUE);
    headers.add("Authorization", "bearer " + zoneAdminToken);
    ...
    client.exchange(
        url + "/saml/service-providers/" + id, HttpMethod.DELETE, deleteHeaders, String.class);
  }

  private void getSamlServiceProvider(
      String idpZoneId,
      String idpZoneAdminToken,
      SamlServiceProviderDefinition samlServiceProviderDefinition,
      String entityId,
      String local_saml_sp_for_testzone2,
      String baseUrl) {
    SamlServiceProvider sp = new SamlServiceProvider();
    sp.setIdentityZoneId(idpZoneId);
    sp.setActive(true);
  ...
  private ScimUser getSpZoneAdminUser(RestTemplate adminClient, String spZoneAdminEmail) {
    return IntegrationTestUtils.createUser(
        adminClient, baseUrl, spZoneAdminEmail, "firstname", "lastname", spZoneAdminEmail, true);
  }
  ...
  private String getZoneAdminToken(RestTemplate adminClient, String zoneId) throws Exception {
    ...
    ScimUser idpZoneAdminUser =
        IntegrationTestUtils.createUser(
            adminClient, baseUrl, zoneAdminEmail, "firstname", "lastname", zoneAdminEmail, true);

    String groupId =
        IntegrationTestUtils.findGroupId(adminClient, baseUrl, "zones." + zoneId + ".admin");
  ...
  public void performLogin(
    ...
    try {
      webDriver.findElement(
          By.xpath("//h1[contains(text(), 'Welcome to The Twiglet Zone[" + idpZoneId + "]!')]"));
      webDriver.findElement(By.name("username")).clear();
      webDriver.findElement(By.name("username")).sendKeys(idpZoneUserEmail);
      webDriver.findElement(By.name("password")).sendKeys("secr3T");
      webDriver.findElement(By.xpath("//input[@value='Sign in']")).click();
      assertThat(
          webDriver.findElement(By.cssSelector("h1")).getText(), containsString("Where to?"));
  ...
  private ScimUser createZoneUser(
      String idpZoneId, String zoneAdminToken, String zoneUserEmail, String zoneUrl)
      throws Exception {
    String zoneAdminClientId =
        new RandomValueStringGenerator().generate() + "-" + idpZoneId + "-admin";
    BaseClientDetails clientDetails =
        new BaseClientDetails(
            zoneAdminClientId,
            null,
    ...
    RestTemplate zoneAdminClient =
        IntegrationTestUtils.getClientCredentialsTemplate(
            IntegrationTestUtils.getClientCredentialsResource(
                zoneUrl, new String[0], zoneAdminClientId, "secret"));
    return IntegrationTestUtils.createUserWithPhone(
        zoneAdminClient,
        zoneUrl,
        zoneUserEmail,
        "Dana",
  ...
  private ScimUser getZoneUser(
      String zoneId, String zoneAdminToken, String zoneUserEmail, String zoneUrl, String origin)
      throws Exception {
    String zoneAdminClientId =
        new RandomValueStringGenerator().generate() + "-" + zoneId + "-admin";
    BaseClientDetails clientDetails =
    ...
    String clientCredentialsToken =
        IntegrationTestUtils.getClientCredentialsToken(
            zoneUrl, clientDetails.getClientId(), clientDetails.getClientSecret());
  ...
  protected boolean doesSupportZoneDNS() {
    try {
      return Arrays.equals(
              Inet4Address.getByName("testzone1.localhost").getAddress(), new byte[] {127, 0, 0, 1})
          && Arrays.equals(
              Inet4Address.getByName("testzone2.localhost").getAddress(), new byte[] {127, 0, 0, 1})
          && Arrays.equals(
              Inet4Address.getByName("testzone3.localhost").getAddress(),
              new byte[] {127, 0, 0, 1});
  ...
  public static SamlIdentityProviderDefinition createLocalSamlIdpDefinition(
      String alias, String zoneId) {
    String url;
  ...
  public static String getIdpMetadata(String url) {
    ...
    ResponseEntity<String> metadataResponse =
        client.exchange(url, HttpMethod.GET, getHeaders, String.class);

    return metadataResponse.getBody();
  }

  public static SamlServiceProviderDefinition createLocalSamlSpDefinition(
      String alias, String zoneId) {
  ...
  public static SamlServiceProvider createSamlServiceProvider(
      String name,
      String entityId,
      String baseUrl,
    ...
    String uaaGroupIdZone =
        IntegrationTestUtils.findGroupId(adminClient, baseUrl, "zones.uaa.admin");
    IntegrationTestUtils.addMemberToGroup(adminClient, baseUrl, user.getId(), uaaGroupIdZone);

    String zoneAdminToken =
        IntegrationTestUtils.getAccessTokenByAuthCode(
            serverRunning,
            UaaTestAccounts.standard(serverRunning),
            "identity",
            "identitysecret",
  ...
  public static SamlServiceProvider createOrUpdateSamlServiceProvider(
      String accessToken, String url, SamlServiceProvider provider) {
    RestTemplate client = new RestTemplate();
    MultiValueMap<String, String> headers = new LinkedMultiValueMap<>();
    headers.add("Accept", MediaType.APPLICATION_JSON_VALUE);
    ...
    List<SamlServiceProvider> existing =
        getSamlServiceProviders(accessToken, url, provider.getIdentityZoneId());
    if (existing != null) {
      for (SamlServiceProvider p : existing) {
        if (p.getEntityId().equals(provider.getEntityId())
          ...
          HttpEntity<SamlServiceProvider> putHeaders =
              new HttpEntity<SamlServiceProvider>(provider, headers);
          ResponseEntity<String> providerPut =
              client.exchange(
                  url + "/saml/service-providers/{id}",
                  HttpMethod.PUT,
                  putHeaders,
                  String.class,
                  provider.getId());
    ...
    HttpEntity<SamlServiceProvider> postHeaders =
        new HttpEntity<SamlServiceProvider>(provider, headers);
    ResponseEntity<String> providerPost =
        client.exchange(
            url + "/saml/service-providers/{id}",
            HttpMethod.POST,
            postHeaders,
    ...
    throw new IllegalStateException(
        "Invalid result code returned, unable to create identity provider:"
            + providerPost.getStatusCode());
  }

  public static List<SamlServiceProvider> getSamlServiceProviders(
      String zoneAdminToken, String url, String zoneId) {
    RestTemplate client = new RestTemplate();
    MultiValueMap<String, String> headers = new LinkedMultiValueMap<>();
    headers.add("Accept", MediaType.APPLICATION_JSON_VALUE);
    headers.add("Authorization", "bearer " + zoneAdminToken);
    ...
    if (providerGet != null && providerGet.getStatusCode() == HttpStatus.OK) {
      return JsonUtils.readValue(
          providerGet.getBody(),
          new TypeReference<List<SamlServiceProvider>>() {
            // Do nothing.
          });
    }
    return null;
