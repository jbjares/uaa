...
class IdentityZoneSwitchingFilterMockMvcTest {
  ...
  void setUp() throws Exception {
    identityToken =
        testClient.getClientCredentialsOAuthAccessToken(
            "identity", "identitysecret", "zones.write,scim.zones");
  ...
  void switchingZones() throws Exception {
    ...
    ClientDetails client =
        createClientInOtherZone(
            mockMvc, generator, zoneAdminToken, status().isCreated(), HEADER, zoneId);
  ...
  void switchingZoneWithSubdomain() throws Exception {
    ...
    String zoneAdminToken =
        MockMvcUtils.getZoneAdminToken(mockMvc, adminToken, identityZone.getId());
    ClientDetails client =
        createClientInOtherZone(
            mockMvc,
            generator,
  ...
  void noSwitching() throws Exception {
    ...
    BaseClientDetails client =
        new BaseClientDetails(clientId, null, null, "client_credentials", null);
  ...
  void switchingToNonExistentZone() throws Exception {
    ...
    createClientInOtherZone(
        mockMvc, generator, zoneAdminToken, status().isNotFound(), HEADER, "i-do-not-exist");
  }

  @Test
  void switchingZonesWithoutAuthority() throws Exception {
    String identityTokenWithoutZonesAdmin =
        testClient.getClientCredentialsOAuthAccessToken(
            "identity", "identitysecret", "zones.write,scim.zones");
    final String zoneId = createZone(mockMvc, identityTokenWithoutZonesAdmin).getId();
    createClientInOtherZone(
        mockMvc, generator, identityTokenWithoutZonesAdmin, status().isForbidden(), HEADER, zoneId);
  }
  ...
  void scimReadInAnotherZone() throws Exception {
    ...
    String adminToken =
        testClient.getClientCredentialsOAuthAccessToken("admin", "adminsecret", "scim.write");
    String scimReadZoneToken =
        MockMvcUtils.getZoneAdminToken(
            mockMvc, adminToken, zoneId, "zones." + zoneId + ".scim.read");
    ScimUser readUser =
        MockMvcUtils.readUserInZone(mockMvc, scimReadZoneToken, user.getId(), "", zoneId);
  ...
  void scimCreateInAnotherZone() throws Exception {
    ...
    String adminToken =
        testClient.getClientCredentialsOAuthAccessToken("admin", "adminsecret", "scim.write");
    String scimCreateZoneToken =
        MockMvcUtils.getZoneAdminToken(
            mockMvc, adminToken, zoneId, "zones." + zoneId + ".scim.create");
  ...
  private static ScimUser createScimUserUsingZonesScimWrite(
      MockMvc mockMvc, RandomValueStringGenerator generator, TestClient testClient, String zoneId)
      throws Exception {
    String adminToken =
        testClient.getClientCredentialsOAuthAccessToken("admin", "adminsecret", "scim.write");
    String scimWriteZoneToken =
        MockMvcUtils.getZoneAdminToken(
            mockMvc, adminToken, zoneId, "zones." + zoneId + ".scim.write");
  ...
  private static ScimUser createUserInAnotherZone(
      MockMvc mockMvc, RandomValueStringGenerator generator, String accessToken, String zoneId)
      throws Exception {
    String username = generator.generate() + "@example.com";
    ScimUser user = getScimUser(username);
  ...
  private static ClientDetails createClientInOtherZone(
      MockMvc mockMvc,
