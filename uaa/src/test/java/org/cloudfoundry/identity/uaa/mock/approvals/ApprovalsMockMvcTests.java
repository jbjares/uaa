...
public class ApprovalsMockMvcTests extends AbstractTokenMockMvcTests {
  ...
  public void createData() {
    user1 =
        syncGroups(
            setUpUser(generator.generate(), scopes, OriginKeys.UAA, IdentityZone.getUaa().getId()));
    client1 =
        setUpClients(generator.generate(), null, scopes, GRANT_TYPE_AUTHORIZATION_CODE, false);
  }
  ...
  public void revoke_invalid_client() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(status().isFound())
        .andExpect(
            header().string("Location", "profile?error_message_code=request.invalid_parameter"));
  }
    ...
    mockMvc
        ...
        .andExpect(
            redirectedUrlPattern(
                "http://test.example.org/redirect?error=invalid_scope&error_description=The%20requested%20scopes%20are%20invalid.%20Please%20use%20valid%20scope%20names%20in%20the%20request*"));
  ...
  public void test_post_approval_csrf() throws Exception {
    ...
    MockHttpServletRequestBuilder post =
        post("/profile")
            ...
            .param(
                "checkScopes",
                client1.getClientId() + "-test.scope1",
                client1.getClientId() + "-test.scope2");
    mockMvc.perform(post).andDo(print()).andExpect(status().isForbidden());

    mockMvc.perform(post.with(cookieCsrf().useInvalidToken())).andExpect(status().isForbidden());

    mockMvc
        .perform(post.with(cookieCsrf()))
        .andExpect(status().isFound())
  ...
  public MockHttpSession getAuthenticatedSession(ScimUser user) {
    List<SimpleGrantedAuthority> authorities =
        user.getGroups()
            .stream()
            .map(g -> new SimpleGrantedAuthority(g.getValue()))
