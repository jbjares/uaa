...
class InvitationsEndpointMockMvcTests {
  ...
  void setUp() throws Exception {
    ...
    adminToken =
        MockMvcUtils.getClientCredentialsOAuthAccessToken(
            ...
            "admin",
            "adminsecret",
            "clients.read clients.write clients.secret scim.read scim.write clients.admin uaa.admin",
            null);
    ...
    scimInviteToken =
        MockMvcUtils.getClientCredentialsOAuthAccessToken(
            mockMvc, clientId, clientSecret, "scim.read scim.invite", null);
    emailDomain = generator.generate().toLowerCase() + ".com";
    IdentityProvider uaaProvider =
        jdbcIdentityProviderProvisioning.retrieveByOrigin(UAA, IdentityZone.getUaa().getId());
  ...
  void inviteUserWithClientCredentials() throws Exception {
    ...
    assertResponseAndCodeCorrect(
        expiringCodeStore, new String[] {email}, redirectUrl, null, response, clientDetails);
  }
  ...
  void inviteMultipleUsersWithClientCredentials() throws Exception {
    ...
    InvitationsResponse response =
        sendRequestWithTokenAndReturnResponse(
            webApplicationContext, mockMvc, scimInviteToken, null, clientId, redirectUri, emails);
    assertResponseAndCodeCorrect(
        expiringCodeStore, emails, redirectUri, null, response, clientDetails);
  }
  ...
  void inviteUserWithUserCredentials() throws Exception {
    ...
    String redirectUri = "example.com";
    String userToken =
        MockMvcUtils.getScimInviteUserToken(
            mockMvc, clientId, clientSecret, null, "admin", "adminsecret");
    InvitationsResponse response =
        sendRequestWithTokenAndReturnResponse(
            webApplicationContext, mockMvc, userToken, null, clientId, redirectUri, email);
    assertResponseAndCodeCorrect(
        expiringCodeStore, new String[] {email}, redirectUri, null, response, clientDetails);
  }
  ...
  class WithOtherIdentityZone {
    ...
    void inviteUserInZoneWithDefaultZoneZoneAdmin() throws Exception {
      ...
      ClientDetails zonifiedScimInviteClientDetails =
          MockMvcUtils.createClient(
              mockMvc,
              adminToken,
      ...
      String zonifiedScimInviteToken =
          MockMvcUtils.getClientCredentialsOAuthAccessToken(
              mockMvc,
              zonifiedAdminClientId,
              zonifiedAdminClientSecret,
      ...
      InvitationsResponse invitationsResponse =
          readValue(mvcResult.getResponse().getContentAsString(), InvitationsResponse.class);
      assertResponseAndCodeCorrect(
          expiringCodeStore,
          new String[] {email},
          redirectUrl,
          ...
          invitationsResponse,
          zonifiedScimInviteClientDetails);
    }
          ...
          (BaseClientDetails)
              MockMvcUtils.createClient(
                  ...
                  MockMvcUtils.getZoneAdminToken(
                      mockMvc, adminToken, zoneSeeder.getIdentityZoneId()),
                  zonedClientId,
                  zonedClientSecret,
                  Collections.singleton("oauth"),
                  Arrays.asList("scim.read", "scim.invite"),
    ...
    @Test
    void acceptInvitationEmailWithinZone(
        @Autowired MultitenantJdbcClientDetailsService multitenantJdbcClientDetailsService)
        throws Exception {
      ...
      String acceptInvitationLink =
          getAcceptInvitationLink(
              ...
              zoneSeeder.getPlainTextClientSecret(
                  zoneSeeder.getAdminClientWithClientCredentialsGrant()));

      mockMvc
          .perform(
              get(acceptInvitationLink)
  ...
  void multipleUsersEmailExistsWithOneOrigin() throws Exception {
    String clientAdminToken =
        MockMvcUtils.getClientOAuthAccessToken(mockMvc, "admin", "adminsecret", "");
    ...
    InvitationsResponse response =
        sendRequestWithTokenAndReturnResponse(
            webApplicationContext, mockMvc, userToken, null, clientId, "example.com", email);
  ...
  void inviteUserWithInvalidEmails() throws Exception {
    ...
    assertEquals(
        invalidEmail1 + " is invalid email.", response.getFailedInvites().get(0).getErrorMessage());
    assertEquals(
        invalidEmail2 + " is invalid email.", response.getFailedInvites().get(1).getErrorMessage());
    assertEquals(
        invalidEmail3 + " is invalid email.", response.getFailedInvites().get(2).getErrorMessage());
  }
  ...
  void invitationsAcceptGetSecurity(@Autowired JdbcTemplate jdbcTemplate) throws Exception {
    ...
    sendRequestWithToken(
        webApplicationContext, mockMvc, userToken, clientId, "user1@" + emailDomain);
    ...
    MockHttpServletRequestBuilder accept = get("/invitations/accept").param("code", code);
    ...
    mockMvc
        ...
        .andExpect(
            content()
                .string(
                    containsString(
                        "<form action=\"/invitations/accept.do\" method=\"post\" novalidate=\"novalidate\">")));
  }

  private static InvitationsResponse sendRequestWithTokenAndReturnResponse(
      ...
      String... emails)
      throws Exception {
    return MockMvcUtils.sendRequestWithTokenAndReturnResponse(
        webApplicationContext, mockMvc, token, subdomain, clientId, redirectUri, emails);
  }
  ...
  private static void sendRequestWithToken(
    ...
    InvitationsResponse response =
        sendRequestWithTokenAndReturnResponse(
            webApplicationContext, mockMvc, token, null, clientId, "example.com", emails);
    assertThat(response.getNewInvites().size(), is(emails.length));
    assertThat(response.getFailedInvites().size(), is(0));
  }

  private static void assertResponseAndCodeCorrect(
    ...
    for (int i = 0; i < emails.length; i++) {
      ...
      if (zone != null && StringUtils.hasText(zone.getSubdomain())) {
        assertThat(
            link, startsWith("http://" + zone.getSubdomain() + ".localhost/invitations/accept"));
      ...
      assertThat(
          expiringCode.getExpiresAt().getTime(), is(greaterThan(System.currentTimeMillis())));
      assertThat(expiringCode.getIntent(), is(ExpiringCodeType.INVITATION.name()));
      Map<String, String> data =
          readValue(expiringCode.getData(), new TypeReference<Map<String, String>>() {});
  ...
  private static String getAcceptInvitationLink(
    ...
    String userToken =
        MockMvcUtils.getScimInviteUserToken(
            mockMvc, clientId, clientSecret, zone, adminClientId, adminClientSecret);
