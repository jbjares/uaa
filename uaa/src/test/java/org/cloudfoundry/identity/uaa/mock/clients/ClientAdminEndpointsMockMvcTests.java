...
public class ClientAdminEndpointsMockMvcTests {
  ...
  @AfterEach
  void restorePublisher(@Autowired ClientAdminEventPublisher eventPublisher) {
  ...
  void testCreateClient() throws Exception {
    ...
    assertEquals(
        ...
        abstractUaaEventCaptor.getValue().getAuditEvent().getType());
  ...
  void testCreateClientWithJwtBearerGrant() throws Exception {
    ...
    ClientDetails client =
        createBaseClient(
            id,
            SECRET,
            Collections.singletonList(GRANT_TYPE_JWT_BEARER),
            null,
            Collections.singletonList(id + ".read"));
  ...
  void testCreateClientWithJwtBearerGrantInvalid() throws Exception {
    String id = new RandomValueStringGenerator().generate();
    ClientDetails client =
        createBaseClient(id, SECRET, Collections.singletonList(GRANT_TYPE_JWT_BEARER), null, null);
    MockHttpServletRequestBuilder createClientPost =
        post("/oauth/clients")
            .header("Authorization", "Bearer " + adminToken)
            .accept(APPLICATION_JSON)
    ...
    assertTrue(
        mvcResult
            .getResponse()
            .getContentAsString()
            .contains("Scope cannot be empty for grant_type " + GRANT_TYPE_JWT_BEARER));
  ...
  void testCreateClientWithInvalidRedirectUrl() throws Exception {
    BaseClientDetails client =
        createBaseClient(
            new RandomValueStringGenerator().generate(), SECRET, Collections.singleton("implicit"));
  ...
  void createClient_withClientAdminToken_withAuthoritiesExcluded(
      @Autowired @Qualifier("excludedClaims") LinkedHashSet excludedClaims) throws Exception {
    String clientId = generator.generate().toLowerCase();
    excludedClaims.add("authorities");
    try {
      String clientAdminToken =
          testClient.getClientCredentialsOAuthAccessToken(
              testAccounts.getAdminClientId(),
              testAccounts.getAdminClientSecret(),
              "clients.admin");
      ...
      ClientDetailsModification client =
          createBaseClient(
              clientId, SECRET, Collections.singleton("client_credentials"), authorities, scopes);
      MockHttpServletRequestBuilder createClientPost =
          post("/oauth/clients")
      ...
      BaseClientDetails clientDetails =
          JsonUtils.readValue(
              createResult.andReturn().getResponse().getContentAsString(), BaseClientDetails.class);
      MockHttpServletRequestBuilder getClientMetadata =
          get("/oauth/clients/" + clientDetails.getClientId() + "/meta")
              .header("Authorization", "Bearer " + clientAdminToken)
              .accept(APPLICATION_JSON)
              .contentType(APPLICATION_JSON);
      ResultActions getResult = mockMvc.perform(getClientMetadata).andExpect(status().isOk());
      JsonUtils.readValue(
          getResult.andReturn().getResponse().getContentAsString(), ClientMetadata.class);
    } finally {
  ...
  void testClientCRUDAsAdminUser() throws Exception {
    ...
    MockHttpServletRequestBuilder getClient =
        get("/oauth/clients/" + client.getClientId())
            .header("Authorization", "Bearer " + adminUserToken)
            .accept(APPLICATION_JSON);
    MvcResult mvcResult = mockMvc.perform(getClient).andExpect(status().isOk()).andReturn();
    BaseClientDetails clientDetails =
        JsonUtils.readValue(mvcResult.getResponse().getContentAsString(), BaseClientDetails.class);
    ...
    MockHttpServletRequestBuilder updateClient =
        put("/oauth/clients/" + client.getClientId())
            .header("Authorization", "Bearer" + adminUserToken)
            .accept(APPLICATION_JSON)
            .contentType(APPLICATION_JSON)
    ...
    BaseClientDetails updatedClientDetails =
    ...
    assertThat(
        updatedClientDetails.getAuthorizedGrantTypes(),
        PredicateMatcher.has(m -> m.equals(GRANT_TYPE_AUTHORIZATION_CODE)));

    MockHttpServletRequestBuilder deleteClient =
        delete("/oauth/clients/" + client.getClientId())
            .header("Authorization", "Bearer" + adminUserToken)
            .accept(APPLICATION_JSON);
    MvcResult deleteResult = mockMvc.perform(deleteClient).andExpect(status().isOk()).andReturn();
    BaseClientDetails deletedClientDetails =
        JsonUtils.readValue(
            deleteResult.getResponse().getContentAsString(), BaseClientDetails.class);
  ...
  void create_client_and_check_created_by() throws Exception {
    ...
    BaseClientDetails clientDetails =
        createClient(Arrays.asList("password.write", "scim.write", "scim.read", "clients.write"));
    ...
    SearchResults<Map<String, Object>> marissa =
        (SearchResults<Map<String, Object>>)
            scimUserEndpoints.findUsers(
                "id,userName",
                "userName eq \"" + testUser.getUserName() + "\"",
                "userName",
                "asc",
                0,
                1);
  ...
  void testCreate_RestrictedClient_Fails() throws Exception {
    ...
    BaseClientDetails clientWithAuthorities =
        createBaseClient(id, SECRET, grantTypes, new UaaScopes().getUaaScopes(), null);
    BaseClientDetails clientWithScopes =
        createBaseClient(id, SECRET, grantTypes, null, new UaaScopes().getUaaScopes());
  ...
  void testCreate_RestrictedClient_Succeeds() throws Exception {
    ...
    BaseClientDetails client =
        createBaseClient(
            id, SECRET, Arrays.asList("client_credentials", "password"), scopes, scopes);
    ...
    createClientPost =
        put("/oauth/clients/restricted/" + id)
            .header("Authorization", "Bearer " + adminToken)
            .accept(APPLICATION_JSON)
            .contentType(APPLICATION_JSON)
            .content(JsonUtils.writeValueAsString(client));
  ...
  void testCreateClientsTxSuccess() throws Exception {
    ...
    ClientDetails[] clients =
        clientArrayFromString(result.andReturn().getResponse().getContentAsString());
    for (ClientDetails client : clients) {
      ClientDetails c = getClient(client.getClientId());
      assertNotNull(c);
      assertNull(c.getClientSecret());
    }
    verify(mockApplicationEventPublisher, times(count))
        .publishEvent(abstractUaaEventCaptor.capture());
    for (AbstractUaaEvent event : abstractUaaEventCaptor.getAllValues()) {
      assertEquals(AuditEventType.ClientCreateSuccess, event.getAuditEvent().getType());
    }
  }
  ...
  void test_InZone_ClientWrite_Failure_with_Min_Length_Secret() throws Exception {
    ...
    MockMvcUtils.IdentityZoneCreationResult result =
        MockMvcUtils.createOtherIdentityZoneAndReturnResult(
            subdomain, mockMvc, webApplicationContext, null);
    result
        .getIdentityZone()
        .getConfig()
        .setClientSecretPolicy(new ClientSecretPolicy(7, 255, 0, 0, 0, 0, 6));
    MockMvcUtils.setZoneConfiguration(
        webApplicationContext,
        result.getIdentityZone().getId(),
        result.getIdentityZone().getConfig());

    String clientId = generator.generate();
    BaseClientDetails client =
        new BaseClientDetails(
            clientId, "", "openid", GRANT_TYPE_AUTHORIZATION_CODE, "", "http://sample.redirect");
    client.setClientSecret("secret");
    MockMvcUtils.createClient(
        mockMvc,
        result.getZoneAdminToken(),
        client,
        result.getIdentityZone(),
        status().isBadRequest());
  }
  ...
  void test_InZone_ClientWrite_Failure_with_Secret_Too_Long() throws Exception {
    ...
    result
        .getIdentityZone()
        .getConfig()
        .setClientSecretPolicy(new ClientSecretPolicy(0, 5, 0, 0, 0, 0, 6));
    MockMvcUtils.setZoneConfiguration(
        webApplicationContext,
        result.getIdentityZone().getId(),
        result.getIdentityZone().getConfig());
  ...
  void test_InZone_ClientWrite_Failure_with_Secret_Requires_Uppercase_Character() throws Exception {
    ...
    result
        .getIdentityZone()
        .getConfig()
        .setClientSecretPolicy(new ClientSecretPolicy(0, 255, 1, 0, 0, 0, 6));
    MockMvcUtils.setZoneConfiguration(
        webApplicationContext,
        result.getIdentityZone().getId(),
        result.getIdentityZone().getConfig());
  ...
  void test_InZone_ClientWrite_Failure_with_Secret_Requires_Lowercase_Character() throws Exception {
    ...
    result
        .getIdentityZone()
        .getConfig()
        .setClientSecretPolicy(new ClientSecretPolicy(0, 255, 0, 1, 0, 0, 6));
    MockMvcUtils.setZoneConfiguration(
        webApplicationContext,
        result.getIdentityZone().getId(),
        result.getIdentityZone().getConfig());
  ...
  void test_InZone_ClientWrite_Success_with_Complex_Secret_Policy() throws Exception {
    ...
    result
        .getIdentityZone()
        .getConfig()
        .setClientSecretPolicy(new ClientSecretPolicy(6, 255, 1, 1, 1, 1, 6));
    MockMvcUtils.setZoneConfiguration(
        webApplicationContext,
        result.getIdentityZone().getId(),
        result.getIdentityZone().getConfig());
  ...
  void test_InZone_ClientWrite_Failure_with_Secret_Requires_Special_Character() throws Exception {
    ...
    result
        .getIdentityZone()
        .getConfig()
        .setClientSecretPolicy(new ClientSecretPolicy(0, 255, 0, 0, 0, 1, 6));
    MockMvcUtils.setZoneConfiguration(
        webApplicationContext,
        result.getIdentityZone().getId(),
        result.getIdentityZone().getConfig());
  ...
  void test_InZone_ClientWrite_Failure_with_Secret_Requires_Digit() throws Exception {
    ...
    result
        .getIdentityZone()
        .getConfig()
        .setClientSecretPolicy(new ClientSecretPolicy(0, 255, 0, 0, 1, 0, 6));
    MockMvcUtils.setZoneConfiguration(
        webApplicationContext,
        result.getIdentityZone().getId(),
        result.getIdentityZone().getConfig());
  ...
  void test_InZone_ClientWrite_Using_ZonesDotClientsDotAdmin() throws Exception {
    ...
    BaseClientDetails client =
        new BaseClientDetails(
            ...
            "",
            "client_credentials",
            "zones." + id + ".clients.admin",
            "http://some.redirect.url.com");
    client.setClientSecret("secret");
    client = MockMvcUtils.createClient(mockMvc, adminToken, client);
    client.setClientSecret("secret");

    String zonesClientsAdminToken =
        MockMvcUtils.getClientOAuthAccessToken(
            mockMvc,
            client.getClientId(),
  ...
  void manageClientInOtherZone_Using_AdminUserTokenFromDefaultZone() throws Exception {
    ...
    BaseClientDetails client =
        new BaseClientDetails(
            ...
            "http://some.redirect.url.com");
    client.setClientSecret("secret");
    BaseClientDetails createdClient =
        MockMvcUtils.createClient(
            mockMvc, adminUserToken, client, result.getIdentityZone(), status().isCreated());
  ...
  void test_InZone_ClientRead_Using_ZonesDotClientsDotAdmin() throws Exception {
    ...
    newclient.setClientSecret("secret");
    MockMvcUtils.createClient(
        mockMvc, zonesClientsAdminToken, newclient, result.getIdentityZone(), status().isCreated());
  }
  ...
  void test_InZone_ClientRead_Using_ZonesDotClientsDotRead() throws Exception {
    ...
    BaseClientDetails client =
        new BaseClientDetails(
            ...
            "",
            "client_credentials",
            "zones." + id + ".clients.read",
            "http://some.redirect.url.com");
    client.setClientSecret("secret");
    client = MockMvcUtils.createClient(mockMvc, adminToken, client);
    client.setClientSecret("secret");

    String zonesClientsReadToken =
        MockMvcUtils.getClientOAuthAccessToken(
            mockMvc,
            client.getClientId(),
            client.getClientSecret(),
    ...
    MockMvcUtils.getClient(
        mockMvc, zonesClientsReadToken, newclient.getClientId(), result.getIdentityZone());
  }
  ...
  void testCreateClientsTxClientCredentialsWithoutSecret() throws Exception {
    ...
    details[details.length - 1].setAuthorizedGrantTypes(
        StringUtils.commaDelimitedListToSet("client_credentials"));
  ...
  void testUpdateClientsTxSuccess() throws Exception {
    ...
    verify(mockApplicationEventPublisher, times(count * 2))
        .publishEvent(abstractUaaEventCaptor.capture());
    int index = 0;
    for (AbstractUaaEvent event : abstractUaaEventCaptor.getAllValues()) {
      if (index < count) {
  ...
  void testDeleteClientsTxSuccess() throws Exception {
    ...
    MockHttpServletRequestBuilder deleteClientsPost =
  ...
  void testAddUpdateDeleteClientsTxSuccess() throws Exception {
    ...
    MockHttpServletRequestBuilder modifyClientsPost =
    ...
    verify(mockApplicationEventPublisher, times(count * 5))
        .publishEvent(abstractUaaEventCaptor.capture());
    int index = 0;
    for (AbstractUaaEvent event : abstractUaaEventCaptor.getAllValues()) {
        ...
        case 4:
          {
            ...
            assertEquals(
                details[index < 10 ? index : (index - count * 2)].getClientId(),
                event.getAuditEvent().getPrincipalId());
            break;
          }
        case 2:
          {
            ...
            assertEquals(
                details[index - (count * 2)].getClientId(), event.getAuditEvent().getPrincipalId());
            break;
          }
        case 3:
          {
    ...
    for (int i = 0; i < 5; i++) {
      details[i] =
          (ClientDetailsModification)
              createClient(adminToken, null, SECRET, Collections.singleton("password"));
      details[i].setRefreshTokenValiditySeconds(120);
      details[i].setAction(ClientDetailsModification.UPDATE);
    }
    for (int i = 5; i < 10; i++) {
  ...
  void testApprovalsAreDeleted() throws Exception {
    ...
    ClientDetailsModification[] deleted =
        (ClientDetailsModification[])
            arrayFromString(
  ...
  void testApprovalsAreDeleted2() throws Exception {
    ...
    MockHttpServletRequestBuilder deleteClientsPost =
        delete("/oauth/clients/" + details.getClientId())
            .header("Authorization", "Bearer " + adminToken)
            .accept(APPLICATION_JSON);
    ResultActions result = mockMvc.perform(deleteClientsPost);
    result.andExpect(status().isOk());
  ...
  void testSecretChangeTxApprovalsNotDeleted() throws Exception {
    int count = 3;
    // create clients
    ClientDetailsModification[] clients =
        createBaseClients(count, SECRET, Arrays.asList("client_credentials", "password"));
    ...
    for (ClientDetailsModification c : clients) {
      String userToken =
          testClient.getUserOAuthAccessToken(
              c.getClientId(), "secret", testUser.getUserName(), testPassword, "oauth.approvals");
  ...
  void testSecretChangeEvent() throws Exception {
    ...
    MockHttpServletRequestBuilder modifyClientsPost =
        put("/oauth/clients/" + id + "/secret")
            .header("Authorization", "Bearer " + token)
            .accept(APPLICATION_JSON)
            .contentType(APPLICATION_JSON)
            .content(JsonUtils.writeValueAsString(request));
    mockMvc.perform(modifyClientsPost).andExpect(status().isOk());
    verify(mockApplicationEventPublisher, times(2)).publishEvent(abstractUaaEventCaptor.capture());
  ...
  void testAddNewClientSecret() throws Exception {
    ...
    MockHttpServletResponse response =
        mockMvc
            .perform(
                put("/oauth/clients/{client_id}/secret", client.getClientId())
                    .header("Authorization", "Bearer " + token)
                    .accept(APPLICATION_JSON)
                    .contentType(APPLICATION_JSON)
                    .content(JsonUtils.writeValueAsString(request)))
    ...
    ActionResult actionResult =
        JsonUtils.readValue(response.getContentAsString(), ActionResult.class);
  ...
  void testAddMoreThanTwoClientSecret() throws Exception {
    ...
    UaaException invalidClientDetailsException =
        JsonUtils.readValue(response.getContentAsString(), UaaException.class);
    ...
    assertEquals(
        ...
        invalidClientDetailsException.getMessage());
  ...
  void testSecretChange_UsingAdminClientToken() throws Exception {
    String adminToken =
        testClient.getClientCredentialsOAuthAccessToken(
            testAccounts.getAdminClientId(), testAccounts.getAdminClientSecret(), "uaa.admin");
    String id = generator.generate();
    ...
    MockHttpServletRequestBuilder modifySecret =
        put("/oauth/clients/" + id + "/secret")
            .header("Authorization", "Bearer " + adminToken)
            .accept(APPLICATION_JSON)
            .contentType(APPLICATION_JSON)
  ...
  void testSecretChange_UsingClientAdminToken() throws Exception {
    String adminToken =
        testClient.getClientCredentialsOAuthAccessToken(
            testAccounts.getAdminClientId(), testAccounts.getAdminClientSecret(), "clients.admin");
    String id = generator.generate();
    createClient(adminToken, id, SECRET, Collections.singleton("client_credentials"));
  ...
  void testUnsuccessfulSecretChangeEvent() throws Exception {
    ...
    BaseClientDetails client =
        createBaseClient(
            null, SECRET, Arrays.asList("password", "client_credentials"), scopes, scopes);
    MockHttpServletRequestBuilder createClientPost =
        post("/oauth/clients")
            .header("Authorization", "Bearer " + adminToken)
            .accept(APPLICATION_JSON)
    ...
    String clientSecretToken =
        testClient.getClientCredentialsOAuthAccessToken(
            client.getClientId(), client.getClientSecret(), "clients.secret");

    SecretChangeRequest request =
        new SecretChangeRequest(client.getClientId(), "invalidsecret", "newsecret");
    MockHttpServletRequestBuilder modifyClientsPost =
        put("/oauth/clients/" + client.getClientId() + "/secret")
            .header("Authorization", "Bearer " + clientSecretToken)
            .accept(APPLICATION_JSON)
            .contentType(APPLICATION_JSON)
            .content(JsonUtils.writeValueAsString(request));
    mockMvc.perform(modifyClientsPost).andExpect(status().isBadRequest());
    verify(mockApplicationEventPublisher, times(2)).publishEvent(abstractUaaEventCaptor.capture());
    assertEquals(SecretFailureEvent.class, abstractUaaEventCaptor.getValue().getClass());
    SecretFailureEvent event = (SecretFailureEvent) abstractUaaEventCaptor.getValue();
  ...
  void testSecretChangeModifyTxApprovalsDeleted() throws Exception {
    ...
    for (ClientDetailsModification c : clients) {
      String userToken =
          testClient.getUserOAuthAccessToken(
              c.getClientId(), "secret2", testUser.getUserName(), testPassword, "oauth.approvals");
      assertEquals(0, getApprovals(c.getClientId()).length);
      assertTrue(c.isApprovalsDeleted());
    }

    // verify(mockApplicationEventPublisher,
    // times(count*3)).publishEvent(abstractUaaEventCaptor.capture());
  ...
  void testSecretChangeModifyTxApprovalsNotDeleted() throws Exception {
    // create clients
    ClientDetailsModification[] clients =
        createBaseClients(3, SECRET, Arrays.asList("client_credentials", "password"));
    for (ClientDetailsModification c : clients) {
      c.setAction(ClientDetailsModification.ADD);
    }
    MockHttpServletRequestBuilder modifyClientsPost =
  ...
  void testClientsAdminPermissions() throws Exception {
    ...
    // create clients
    ClientDetailsModification[] clients =
        createBaseClients(3, SECRET, Arrays.asList("client_credentials", "refresh_token"));
    for (ClientDetailsModification c : clients) {
    ...
    String token =
        testClient.getClientCredentialsOAuthAccessToken(
            adminsClient.getClientId(), "secret", "clients.admin");
  ...
  void testNonClientsAdminPermissions() throws Exception {
    ...
    String token =
        testClient.getClientCredentialsOAuthAccessToken(
            adminsClient.getClientId(), "secret", "clients.write");
  ...
  void testCreateAsAdminPermissions() throws Exception {
    ...
    // create clients
    ClientDetailsModification[] clients =
        createBaseClients(1, SECRET, Arrays.asList("client_credentials", "refresh_token"));
    for (ClientDetailsModification c : clients) {
      c.setScope(Collections.singletonList("oauth.approvals"));
      c.setAction(ClientDetailsModification.ADD);
    }
  ...
  void testCreateAsReadPermissions() throws Exception {
    ...
    String token =
        testClient.getClientCredentialsOAuthAccessToken(
            adminsClient.getClientId(), "secret", "clients.read");
  ...
  void testGetClientDetailsSortedByLastModified() throws Exception {
    ...
    Collection<BaseClientDetails> clientDetails =
        JsonUtils.readValue(body, new TypeReference<SearchResults<BaseClientDetails>>() {})
            .getResources();
    ...
    for (ClientDetails clientDetail : clientDetails) {
      ...
      Date currentDate =
          JsonUtils.convertValue(
              clientDetail.getAdditionalInformation().get("lastModified"), Date.class);
  ...
  void testGetClientsLargerThanMax_whenCountParamIsProvided() throws Exception {
    ...
    SearchResults<BaseClientDetails> clientDetailsSearchResults =
        JsonUtils.readValue(body, new TypeReference<SearchResults<BaseClientDetails>>() {});
  ...
  void testGetClientsLargerThanMax_whenNoCountParamIsProvided() throws Exception {
    ...
    MockHttpServletRequestBuilder get =
        get("/oauth/clients").header("Authorization", "Bearer " + token).accept(APPLICATION_JSON);
  ...
  void testClientWithDotInID() throws Exception {
    ...
    ClientDetails detailsv2 =
        createClient(
            adminToken, "testclient.v2", SECRET, Collections.singleton("client_credentials"));
  ...
  void testPutClientModifyAuthorities() throws Exception {
    ClientDetails client =
        createClient(
            adminToken,
            "testClientForModifyAuthorities",
            SECRET,
            Collections.singleton("client_credentials"));
  ...
  void testPutClientModifyAccessTokenValidity() throws Exception {
    ClientDetails client =
        createClient(
            adminToken,
            "testClientForModifyAccessTokenValidity",
            SECRET,
            Collections.singleton("client_credentials"));
  ...
  void testPutClientModifyName() throws Exception {
    ClientDetails client =
        createClient(
            adminToken,
            "testClientForModifyName",
            SECRET,
            Collections.singleton("client_credentials"));

    Map<String, Object> requestBody =
        JsonUtils.readValue(
            JsonUtils.writeValueAsString(new BaseClientDetails(client)),
    ...
    assertThat(
        map, hasEntry(is("name"), PredicateMatcher.is(value -> value.equals("New Client Name"))));
    ...
    assertThat(
        ...
        hasEntry(is("name"), PredicateMatcher.is(value -> value.equals("New Client Name"))));
  }
  ...
  private Approval[] getApprovals(String clientId) {
    return jdbcApprovalStore
        .getApprovalsForClient(clientId, IdentityZoneHolder.get().getId())
        .toArray(new Approval[0]);
  }
  ...
  private void setupAdminUserToken() throws Exception {
    ...
    uaaAdmin =
        (SearchResults<Map<String, Object>>)
            scimGroupEndpoints.listGroups(
                "id,displayName", "displayName eq \"clients.write\"", "displayName", "asc", 1, 1);
    ...
    uaaAdmin =
        (SearchResults<Map<String, Object>>)
            scimGroupEndpoints.listGroups(
                "id,displayName", "displayName eq \"clients.read\"", "displayName", "asc", 1, 1);
    groupId = (String) uaaAdmin.getResources().iterator().next().get("id");
    group = scimGroupEndpoints.getGroup(groupId, mockResponse);
    gm = new ScimGroupMember(marissaId, ScimGroupMember.Type.USER);
    group.getMembers().add(gm);
    ...
    adminUserToken =
        testClient.getUserOAuthAccessToken(
            adminClient.getClientId(), "secret", testUser.getUserName(), testPassword, "uaa.admin");
  }
  ...
  private ClientDetailsModification createBaseClient(
      String id,
      String clientSecret,
  ...
  protected ClientDetails createClient(
      String token, String id, String clientSecret, Collection<String> grantTypes)
      throws Exception {
  ...
  private ClientDetails createAdminClient(String token) throws Exception {
    List<String> scopes =
        Arrays.asList("uaa.admin", "oauth.approvals", "clients.read", "clients.write");
    BaseClientDetails client =
        createBaseClient(
            null, SECRET, Arrays.asList("password", "client_credentials"), scopes, scopes);
  ...
  private ClientDetailsModification createBaseClient(
      String id, String clientSecret, Collection<String> grantTypes) {
    return createBaseClient(
        id,
        clientSecret,
        grantTypes,
        Collections.singletonList("uaa.none"),
        Arrays.asList("foo", "bar", "oauth.approvals"));
  }

  private ClientDetailsModification[] createBaseClients(
      int length, String clientSecret, Collection<String> grantTypes) {
