...
public class OpenIdTokenGrantsIT {
  ...
  private ScimUser createUser(
    ...
    return IntegrationTestUtils.createUser(
        client, uaaUrl, username, firstName, lastName, email, verified);
  }
  ...
  private void validateToken(String paramName, Map params, String[] scopes, String[] aud)
    ...
    Assert.assertThat(
        ((List<String>) claims.get(ClaimConstants.SCOPE)), containsInAnyOrder(scopes));
  ...
  public void testOpenIdHybridFlowIdTokenAndCode() throws Exception {
    doOpenIdHybridFlowIdTokenAndCode(
  ...
  private void doOpenIdHybridFlowIdTokenAndCode(
    ...
    result =
        restOperations.exchange(
            loginUrl + "/login.do",
            HttpMethod.POST,
            new HttpEntity<>(formData, getHeaders(cookies)),
            Void.class);
    assertEquals(HttpStatus.FOUND, result.getStatusCode());
