...
public class RemoteAuthenticationEndpointTests {
  ...
  public void remoteAuthenticationSucceedsWithCorrectCredentials() throws Exception {
    ...
    ResponseEntity<Map> response =
        authenticate(testAccounts.getUserName(), testAccounts.getPassword(), null);
  ...
  public void validateOrigin(
      String username, String password, String origin, Map<String, Object> info) throws Exception {
    ...
    ResponseEntity<Map> response =
        serverRunning.getForObject(
            "/Users" + "?filter=userName eq \"" + username + "\"&attributes=id,userName,origin",
            Map.class,
            headers);
  ...
  private String getScimReadBearerToken() {
    ...
    String basicDigestHeaderValue =
        "Basic "
            + new String(
                Base64.encodeBase64(
                    (testAccounts.getAdminClientId() + ":" + testAccounts.getAdminClientSecret())
                        .getBytes()));
    ...
    ResponseEntity<Map> tokenResponse =
        serverRunning.postForMap(serverRunning.getAccessTokenUri(), params, accessTokenHeaders);
  ...
  @SuppressWarnings("rawtypes")
  ResponseEntity<Map> authenticate(
      String username, String password, Map<String, Object> additionalParams) {
    ...
    if (restTemplate instanceof OAuth2RestTemplate) {
      ...
      oAuth2RestTemplate.setErrorHandler(
          new UaaOauth2ErrorHandler(
              oAuth2RestTemplate.getResource(), HttpStatus.Series.SERVER_ERROR));
    } else {
    ...
    ResponseEntity<Map> result =
        restTemplate.exchange(
            serverRunning.getUrl("/authenticate"),
            HttpMethod.POST,
