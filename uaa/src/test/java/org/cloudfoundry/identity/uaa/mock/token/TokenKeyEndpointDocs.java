...
class TokenKeyEndpointDocs extends EndpointDocs {
  ...
  private static final String signKey =
      "-----BEGIN RSA PRIVATE KEY-----\n"
          ...
          + "-----END RSA PRIVATE KEY-----";
  private static final String ETAG_HEADER_DESCRIPTION =
  ...
  private static final String IF_NONE_MATCH_DESCRIPTION =
      "Optional. See [Ref: RFC 2616](https://tools.ietf.org/html/rfc2616#section-14.26) ";
  private static final RequestHeadersSnippet SYMM_TOKEN_KEY_REQUEST_HEADERS =
      requestHeaders(
          headerWithName("Authorization")
              .description("No authorization is required for requesting public keys."),
          headerWithName("If-None-Match").description(IF_NONE_MATCH_DESCRIPTION).optional());
  private static final RequestHeadersSnippet ASYMM_TOKEN_KEY_REQUEST_HEADERS =
      requestHeaders(
          headerWithName("If-None-Match").description(IF_NONE_MATCH_DESCRIPTION).optional());
  private static final Snippet TOKEN_KEY_RESPONSE_HEADERS =
      HeaderDocumentation.responseHeaders(
          headerWithName("ETag").description(ETAG_HEADER_DESCRIPTION));
  ...
  void getTokenAsymmetricAuthenticated() throws Exception {
    Snippet responseFields =
        responseFields(
            fieldWithPath("kid")
                .type(STRING)
                .description("Key ID of key to be used for verification of the token."),
    ...
    mockMvc
        .perform(
            get("/token_key").accept(APPLICATION_JSON).header("If-None-Match", "1501570800000"))
  ...
  void getTokenSymmetricAuthenticated() throws Exception {
    ...
    try {
      String basicDigestHeaderValue =
          "Basic " + new String(Base64.encodeBase64(("app:appclientsecret").getBytes()));
