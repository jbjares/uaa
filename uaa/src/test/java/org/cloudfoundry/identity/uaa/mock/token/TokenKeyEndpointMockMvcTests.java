...
public class TokenKeyEndpointMockMvcTests {
                ...
                get("/token_key")
                    .with(
                        new SetServerNameRequestPostProcessor(
                            testZone.getSubdomain() + ".localhost"))
  ...
  public void checkTokenKey_IsNotFromDefaultZone() throws Exception {
    MvcResult nonDefaultZoneResponse =
        mockMvc
            .perform(
                get("/token_key")
                    .with(
            ...
            .andExpect(status().isOk())
            .andReturn();
    Map<String, Object> nonDefaultKey =
        JsonUtils.readValue(nonDefaultZoneResponse.getResponse().getContentAsString(), Map.class);
    VerificationKeyResponse nonDefaultKeyResponse = new VerificationKeyResponse(nonDefaultKey);

    MvcResult defaultZoneResponse =
        mockMvc
            .perform(
                get("/token_key")
                    .accept(MediaType.APPLICATION_JSON)
    ...
    Map<String, Object> defaultKey =
        JsonUtils.readValue(defaultZoneResponse.getResponse().getContentAsString(), Map.class);
  ...
  public void checkTokenKey_WhenKeysAreAsymmetric_asAuthenticatedUser() throws Exception {
    ...
    webApplicationContext
        .getBean(ClientServicesExtension.class)
        .addClientDetails(client, testZone.getSubdomain());
  ...
  @Test
  public void checkTokenKey_WhenKeysAreAsymmetric_asAuthenticatedUser_withoutCorrectScope()
      throws Exception {
  ...
  private void setSigningKeyAndDefaultClient(String signKey) throws Exception {
    ...
    webApplicationContext
        ...
        .addClientDetails(defaultClient, subdomain);
  }
  ...
  private String getBasicAuth(BaseClientDetails client) {
    return "Basic "
        + new String(
            Base64.encodeBase64(
                (client.getClientId() + ":" + client.getClientSecret()).getBytes()));
  }
  ...
  private void validateKeys(Map<String, Object> response) {
    ...
    Map<String, ? extends Map<String, Object>> keysMap =
        keys.stream().collect(new MapCollector<>(k -> (String) k.get("kid"), k -> k));
