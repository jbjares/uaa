  ...
  void test_Get_UserInfo() throws Exception {
    String userInfoToken =
        testClient.getUserOAuthAccessToken(
            clientId, clientSecret, user.getUserName(), "secr3T", "openid");

    Snippet requestHeaders =
        requestHeaders(
            headerWithName("Authorization")
                .description(
                    "Access token with openid required. If the `"
                        + USER_ATTRIBUTES
                        + "` scope is in the token, "
    ...
    Snippet responseFields =
        responseFields(
            ...
            fieldWithPath("email_verified")
                .description("Indicates whether the user has verified their email address."),
            fieldWithPath("user_name")
                .description("User name of the user, typically an email address."),
            ...
            fieldWithPath(ClaimConstants.PREVIOUS_LOGON_TIME)
                .description(
