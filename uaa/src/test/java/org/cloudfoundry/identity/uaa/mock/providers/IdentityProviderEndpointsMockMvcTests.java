...
class IdentityProviderEndpointsMockMvcTests {
  ...
  void setUp(
    ...
    eventListener =
        MockMvcUtils.addEventListener(
            configurableApplicationContext, IdentityProviderModifiedEvent.class);
  ...
  void test_delete_through_event() throws Exception {
    ...
    IdentityProviderBootstrap bootstrap =
        webApplicationContext.getBean(IdentityProviderBootstrap.class);
    assertNotNull(
  ...
  void testCreateAndUpdateIdentityProviderWithMissingConfig() throws Exception {
    ...
    IdentityProvider identityProvider =
        MultitenancyFixture.identityProvider("testnoconfig", IdentityZone.getUaa().getId());
    ...
    identityProvider =
        JsonUtils.readValue(create.getResponse().getContentAsString(), IdentityProvider.class);
  ...
  void test_Create_and_Delete_SamlProvider() throws Exception {
    ...
    IdentityProvider<SamlIdentityProviderDefinition> created =
        createIdentityProvider(null, provider, accessToken, status().isCreated());
    ...
    // no access token
    mockMvc
        .perform(delete("/identity-providers/{id}", created.getId()))
  ...
  void testCreateIdentityProviderWithInsufficientScopes() throws Exception {
    IdentityProvider identityProvider =
        MultitenancyFixture.identityProvider("testorigin", IdentityZone.getUaa().getId());
  ...
  void testUpdateUaaIdentityProviderDoesUpdateOfPasswordPolicy() throws Exception {
    ...
    IdentityProvider modifiedIdentityProvider =
        identityProviderProvisioning.retrieveByOrigin(
            OriginKeys.UAA, IdentityZone.getUaa().getId());
    assertEquals(
        newConfig,
  ...
  @Test
  void testUpdateUaaIdentityProviderDoesUpdateOfPasswordPolicyWithPasswordNewerThan()
      throws Exception {
    IdentityProvider identityProvider =
        identityProviderProvisioning.retrieveByOrigin(
            OriginKeys.UAA, IdentityZone.getUaa().getId());
  ...
  void invalid_ldap_origin_returns_UnprocessableEntity() throws Exception {
    IdentityProvider identityProvider =
        identityProviderProvisioning.retrieveByOriginIgnoreActiveFlag(
            OriginKeys.LDAP, IdentityZone.getUaa().getId());
    String accessToken = setUpAccessToken();
    updateIdentityProvider(null, identityProvider, accessToken, status().isOk());
  ...
  void testCreateAndUpdateIdentityProviderInOtherZone() throws Exception {
    ...
    ScimUser user =
        MockMvcUtils.createAdminForZone(
            mockMvc, adminToken, "zones." + zone.getId() + ".idps.write");
    ...
    String userAccessToken =
        MockMvcUtils.getUserOAuthAccessTokenAuthCode(
            ...
            user.getUserName(),
            "secr3T",
            "zones." + zone.getId() + ".idps.write");
    eventListener.clearEvents();
    IdentityProvider createdIDP =
        createIdentityProvider(
            zone.getId(), identityProvider, userAccessToken, status().isCreated());
  ...
  void test_Create_Duplicate_Saml_Identity_Provider_In_Other_Zone() throws Exception {
    ...
    IdentityProvider<SamlIdentityProviderDefinition> identityProvider =
        MultitenancyFixture.identityProvider(origin1, zone.getId());
    ...
    IdentityProvider<SamlIdentityProviderDefinition> createdIDP =
        createIdentityProvider(
            zone.getId(), identityProvider, userAccessToken, status().isCreated());
    ...
    assertEquals(identityProvider.getOriginKey(), createdIDP.getOriginKey());
    assertEquals(
        identityProvider.getConfig().getIdpEntityAlias(),
  ...
  void test_Create_Duplicate_Saml_Identity_Provider_In_Default_Zone() throws Exception {
    ...
    IdentityProvider identityProvider =
        MultitenancyFixture.identityProvider(origin1, IdentityZone.getUaa().getId());
    identityProvider.setType(OriginKeys.SAML);
    ...
    IdentityProvider createdIDP =
        createIdentityProvider(null, identityProvider, userAccessToken, status().isCreated());
  ...
  void testReadIdentityProviderInOtherZone_Using_Zones_Token() throws Exception {
    ...
    user =
        MockMvcUtils.createAdminForZone(
            mockMvc, adminToken, "zones." + zone.getId() + ".idps.read");
    userAccessToken =
        MockMvcUtils.getUserOAuthAccessTokenAuthCode(
            mockMvc,
            "identity",
    ...
    MockHttpServletRequestBuilder requestBuilder =
        get("/identity-providers/" + createdIDP.getId())
    ...
    IdentityProvider retrieved =
        JsonUtils.readValue(result.getResponse().getContentAsString(), IdentityProvider.class);
  ...
  void testListIdpsInZone() throws Exception {
    ...
    String accessToken =
        MockMvcUtils.getUserOAuthAccessToken(
            ...
            user.getUserName(),
            "secr3T",
            "idps.read,idps.write");

    int numberOfIdps =
        identityProviderProvisioning.retrieveAll(false, IdentityZone.getUaa().getId()).size();
  ...
  void testListIdpsInOtherZoneFromDefaultZone() throws Exception {
    ...
    String zoneAdminToken =
        MockMvcUtils.getUserOAuthAccessTokenAuthCode(
            ...
            "identity",
            "identitysecret",
            userInDefaultZone.getId(),
            userInDefaultZone.getUserName(),
            "secr3T",
            "zones."
                + identityZone.getId()
                + ".idps.read "
    ...
    IdentityProvider otherZoneIdp =
        MockMvcUtils.createIdpUsingWebRequest(
            mockMvc,
            identityZone.getId(),
            zoneAdminToken,
            MultitenancyFixture.identityProvider(
                new RandomValueStringGenerator().generate(), IdentityZone.getUaa().getId()),
            status().isCreated());
  ...
  void testRetrieveIdpInZone() throws Exception {
    ...
    MockHttpServletRequestBuilder requestBuilder =
        get("/identity-providers/" + newIdp.getId())
            .header("Authorization", "Bearer" + accessToken)
            .contentType(APPLICATION_JSON);
    ...
    assertTrue(
        ((AbstractXOAuthIdentityProviderDefinition)
                webApplicationContext
                    ...
                    .retrieve(identityProvider.getId(), identityProvider.getIdentityZoneId())
                    .getConfig())
            .isClientAuthInBody());
  ...
  void testUpdatePasswordPolicyWithPasswordNewerThan() throws Exception {
    ...
    MvcResult mvcResult =
        mockMvc
            .perform(
                patch("/identity-providers/" + identityProvider.getId() + "/status")
                    .header("Authorization", "Bearer " + accessToken)
    ...
    IdentityProviderStatus updatedStatus =
        JsonUtils.readValue(
            mvcResult.getResponse().getContentAsString(), IdentityProviderStatus.class);
    assertEquals(
        identityProviderStatus.getRequirePasswordChange(),
        updatedStatus.getRequirePasswordChange());
  }
  ...
  private BaseClientDetails getBaseClientDetails() throws Exception {
    ...
    BaseClientDetails client =
        new BaseClientDetails(clientId, null, "idps.read,idps.write", "password", null);
  ...
  private IdentityProvider createIdentityProvider(
      String zoneId, IdentityProvider identityProvider, String token, ResultMatcher resultMatcher)
      throws Exception {
    return MockMvcUtils.createIdpUsingWebRequest(
        mockMvc, zoneId, token, identityProvider, resultMatcher);
  }

  private MvcResult updateIdentityProvider(
      String zoneId, IdentityProvider identityProvider, String token, ResultMatcher resultMatcher)
      throws Exception {
    MockHttpServletRequestBuilder requestBuilder =
        put("/identity-providers/" + identityProvider.getId())
  ...
  private void testRetrieveIdps(boolean retrieveActive) throws Exception {
    String clientId = RandomStringUtils.randomAlphabetic(6);
    BaseClientDetails client =
        new BaseClientDetails(clientId, null, "idps.write,idps.read", "password", null);
    client.setClientSecret("test-client-secret");
    MockMvcUtils.createClient(mockMvc, adminToken, client);
    ...
    IdentityProvider identityProvider =
        MultitenancyFixture.identityProvider(randomOriginKey, IdentityZone.getUaa().getId());
    IdentityProvider createdIDP =
        createIdentityProvider(null, identityProvider, accessToken, status().isCreated());

    String retrieveActiveParam = retrieveActive ? "?active_only=true" : "";
    MockHttpServletRequestBuilder requestBuilder =
        get("/identity-providers" + retrieveActiveParam)
            .header("Authorization", "Bearer" + accessToken)
            .contentType(APPLICATION_JSON);

    int numberOfIdps =
        identityProviderProvisioning
            .retrieveAll(retrieveActive, IdentityZone.getUaa().getId())
            .size();
  ...
  private IdentityProvider createAndUpdateIdentityProvider(String accessToken) throws Exception {
    ...
    IdentityProvider persisted =
        identityProviderProvisioning.retrieve(createdIDP.getId(), createdIDP.getIdentityZoneId());
