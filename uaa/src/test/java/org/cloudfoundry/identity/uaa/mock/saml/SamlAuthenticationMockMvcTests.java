...
class SamlAuthenticationMockMvcTests {
  ...
  void createSamlRelationship(
    ...
    BaseClientDetails adminClient =
        new BaseClientDetails("admin", "", "", "client_credentials", "uaa.admin");
  ...
  void validateStaticAttributes(
    ...
    samlServiceProvider
        .getConfig()
        .getStaticCustomAttributes()
        .put("portal_emails", Arrays.asList("portal1@portal.test", "portal2@portal.test"));
    ...
    String emails =
        (String)
            xpath.evaluate(
                "//*[local-name()='Attribute'][@*[local-name()='Name' and .='portal_emails']]",
  ...
  void validateCustomEmailAttribute(
    ...
    String emails =
        (String)
            xpath.evaluate(
                "//*[local-name()='Attribute'][@*[local-name()='Name' and .='primary-email']]",
                new InputSource(new StringReader(xml)),
                XPathConstants.STRING);
  ...
  private String performIdpAuthentication() throws Exception {
    RequestPostProcessor marissa =
        securityContext(getUaaSecurityContext("marissa", webApplicationContext, idpZone));
    return mockMvc
        .perform(
  ...
  private String getSamlMetadata(String subdomain, String url) throws Exception {
    return mockMvc
        .perform(get(url).header("Host", subdomain + ".localhost"))
        .andReturn()
        .getResponse()
        .getContentAsString();
  }
  ...
  private static void createUser(
      JdbcScimUserProvisioning jdbcScimUserProvisioning, IdentityZone identityZone) {
  ...
  private void createIdp(IdentityProviderProvisioning idpProvisioning) throws Exception {
    ...
    SamlIdentityProviderDefinition idpDefinition =
  ...
  private IdentityZone createZone(BaseClientDetails adminClient) throws Exception {
    return MockMvcUtils.createOtherIdentityZoneAndReturnResult(
            generator.generate(), mockMvc, webApplicationContext, adminClient)
        .getIdentityZone();
  }
  ...
  private static String extractFormParameter(String searchFor, String response, boolean decode) {
    ...
    assertThat(
        "Must find the SAML response in output\n" + response,
        start,
        greaterThan(searchFor.length()));
