...
public class ScimUserEndpointsIntegrationTests {
  ...
  private ResponseEntity<Map> deleteUser(String id, int version) {
    ...
    return client.exchange(
        serverRunning.getUrl(userEndpoint + "/{id}"),
        HttpMethod.DELETE,
  ...
  private ResponseEntity<ScimUser> createUser(
      String username, String firstName, String lastName, String email, boolean verified) {
    ScimUser user = new ScimUser();
    user.setPassword("password");
    user.setUserName(username);
    user.setName(new ScimUser.Name(firstName, lastName));
  ...
  public void createUserSucceeds() throws Exception {
    ...
    ScimUser joe2 =
        client.getForObject(
            serverRunning.getUrl(userEndpoint + "/{id}"), ScimUser.class, joe1.getId());
  ...
  public void verifyUser() throws Exception {
    ...
    ScimUser joe3 =
        client.getForObject(
            serverRunning.getUrl(userEndpoint + "/{id}/verify"), ScimUser.class, joe1.getId());
  ...
  public void verifyUserNotFound() throws Exception {
    ...
    ResponseEntity<Map> response =
        client.exchange(
            serverRunning.getUrl(userEndpoint + "/{id}/verify"),
            HttpMethod.GET,
            new HttpEntity<Void>(headers),
            Map.class,
  ...
  public void createUserWithNoEmailFails() throws Exception {
    ...
    @SuppressWarnings("rawtypes")
    ResponseEntity<Map> response =
        client.postForEntity(serverRunning.getUrl(userEndpoint), user, Map.class);
    @SuppressWarnings("unchecked")
    Map<String, String> error = response.getBody();
  ...
  public void getUserHasEtag() throws Exception {
    ...
    ResponseEntity<ScimUser> result =
        client.getForEntity(
            serverRunning.getUrl(userEndpoint + "/{id}"), ScimUser.class, joe.getId());
  ...
  public void updateUserSucceeds() throws Exception {
    ...
    response =
        client.exchange(
            serverRunning.getUrl(userEndpoint) + "/{id}",
            HttpMethod.PUT,
            new HttpEntity<ScimUser>(joe, headers),
            ScimUser.class,
            joe.getId());
    ScimUser joe1 = response.getBody();
    assertEquals(JOE, joe1.getUserName());
  ...
  public void updateUserWithBadAttributeFails() throws Exception {
    ...
    Map<String, Object> map =
        new HashMap<String, Object>(
            JsonUtils.readValue(JsonUtils.writeValueAsString(joe), Map.class));
    map.put("nottheusername", JOE + "0");
    ResponseEntity<Map> response =
        client.exchange(
            serverRunning.getUrl(userEndpoint) + "/{id}",
            HttpMethod.PUT,
            new HttpEntity<Map>(map, headers),
            Map.class,
            joe.getId());
  ...
  public void createUserWithJustACaseChangeFails() throws Exception {
    ...
    response =
        client.postForEntity(serverRunning.getUrl(userEndpoint), userDifferentCase, Map.class);
    @SuppressWarnings("unchecked")
    Map<String, String> error = response.getBody();
  ...
  public void findUsersWithAttributes() throws Exception {
    ResponseEntity<Map> response =
        serverRunning.getForObject(usersEndpoint + "?attributes=id,userName", Map.class);
  ...
  public void findUsersWithSortBy() throws Exception {
    @SuppressWarnings("rawtypes")
    ResponseEntity<Map> response =
        serverRunning.getForObject(usersEndpoint + "?sortBy=emails.value", Map.class);
    @SuppressWarnings("unchecked")
    Map<String, Object> results = response.getBody();
    assertEquals(HttpStatus.OK, response.getStatusCode());
    assertTrue("There should be more than zero users", (Integer) results.get("totalResults") > 0);
  ...
  public void findUsersWithPagination() throws Exception {
    @SuppressWarnings("rawtypes")
    ResponseEntity<Map> response =
        serverRunning.getForObject(usersEndpoint + "?startIndex=2&count=3", Map.class);
    @SuppressWarnings("unchecked")
    Map<String, Object> results = response.getBody();
    assertEquals(HttpStatus.OK, response.getStatusCode());
    assertTrue("There should be more than zero users", (Integer) results.get("totalResults") > 0);
  ...
  public void findUsersWithExtremePagination() throws Exception {
    for (int i = 0; i < 501; i++) {
      ResponseEntity<ScimUser> scimUserResponseEntity =
          createUser(
    ...
    @SuppressWarnings("rawtypes")
    ResponseEntity<Map> response =
        serverRunning.getForObject(usersEndpoint + "?startIndex=0&count=501", Map.class);
    @SuppressWarnings("unchecked")
    Map<String, Object> results = response.getBody();
    assertEquals(HttpStatus.OK, response.getStatusCode());
