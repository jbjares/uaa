...
public class PasswordChangeEndpointMockMvcTests {
  ...
  public void setUp() throws Exception {
    ...
    BaseClientDetails clientDetails =
        new BaseClientDetails(clientId, null, null, "client_credentials", "password.write");
    clientDetails.setClientSecret(clientSecret);

    MockMvcUtils.createClient(mockMvc, adminToken, clientDetails);

    passwordWriteToken =
        testClient.getClientCredentialsOAuthAccessToken(clientId, clientSecret, "password.write");
  }
  ...
  @Test
  public void changePassword_NewPasswordSameAsOld_ReturnsUnprocessableEntityWithJsonError()
      throws Exception {
    ScimUser user = createUser();
    PasswordChangeRequest request = new PasswordChangeRequest();
    request.setOldPassword(password);
  ...
  public void changePassword_WithBadOldPassword_ReturnsUnauthorizedError() throws Exception {
    ...
    String userToken =
        testClient.getUserOAuthAccessToken(
            "cf", "", user.getUserName(), password, "password.write");
  ...
  public void changePassword_Resets_Session() throws Exception {
    ...
    MockHttpSession afterPasswordChange =
        (MockHttpSession)
            mockMvc
                .perform(
    ...
    assertNotNull(
        afterPasswordChange.getAttribute(
            HttpSessionSecurityContextRepository.SPRING_SECURITY_CONTEXT_KEY));
  ...
  public void changePassword_Resets_All_Sessions() throws Exception {
    ...
    MockHttpSession session = new MockHttpSession();
    MockHttpSession afterLoginSessionA =
        (MockHttpSession)
            mockMvc
                .perform(
                    post("/login.do")
    ...
    MockHttpSession afterLoginSessionB =
        (MockHttpSession)
            mockMvc
                .perform(
                    post("/login.do")
    ...
    assertNotNull(
        afterLoginSessionA.getAttribute(
            HttpSessionSecurityContextRepository.SPRING_SECURITY_CONTEXT_KEY));
    assertNotNull(
        afterLoginSessionB.getAttribute(
            HttpSessionSecurityContextRepository.SPRING_SECURITY_CONTEXT_KEY));

    mockMvc.perform(get("/profile").session(afterLoginSessionB)).andExpect(status().isOk());
    ...
    mockMvc
        .perform(get("/profile").session(afterLoginSessionB).accept(TEXT_HTML))
        .andExpect(status().isFound())
