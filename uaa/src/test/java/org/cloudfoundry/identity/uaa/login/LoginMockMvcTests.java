...
public class LoginMockMvcTests {
  ...
  void setUpContext(
    ...
    String adminToken =
        MockMvcUtils.getClientCredentialsOAuthAccessToken(
            mockMvc, "admin", "adminsecret", null, null);
    ...
    identityZone =
        MockMvcUtils.createOtherIdentityZone(subdomain, mockMvc, webApplicationContext, false);
  ...
  void tearDown() throws Exception {
    ...
    MockMvcUtils.setZoneConfiguration(
        webApplicationContext, getUaa().getId(), originalConfiguration);
    SecurityContextHolder.clearContext();
  ...
  void invalid_accept_media_type() throws Exception {
    mockMvc
        .perform(get("/login").header("Accept", MediaType.TEXT_XML_VALUE))
  ...
  @Test
  void testLoginMfaRedirect(@Autowired ScimUserProvisioning scimUserProvisioning) throws Exception {
    ...
    mockMvc
        .perform(
            post("/login.do")
                .with(cookieCsrf())
                .with(
                    new SetServerNameRequestPostProcessor(
  ...
  IdentityZone createZoneLinksZone() throws Exception {
    ...
    zone.getConfig()
        .getLinks()
        .setSelfService(new Links.SelfService().setPasswd(null).setSignup(null));
    return MockMvcUtils.updateIdentityZone(zone, webApplicationContext);
  }

  @Test
  void self_service_zone_variable_links(@Autowired LoginInfoEndpoint loginInfoEndpoint)
      throws Exception {
    IdentityZone zone = createZoneLinksZone();

    mockMvc
        .perform(get("/login").header("Host", zone.getSubdomain() + ".localhost"))
        .andExpect(status().isOk())
        .andExpect(view().name("login"))
        .andExpect(model().attribute("links", hasEntry("forgotPasswordLink", "/forgot_password")))
        .andExpect(model().attribute("links", hasEntry("createAccountLink", "/create_account")))
    ...
    mockMvc
        ...
        .andExpect(
            model()
                .attribute("links", hasEntry("forgotPasswordLink", "/passwd?id=" + zone.getId())))
        .andExpect(
            model()
                .attribute(
                    "links",
                    hasEntry("createAccountLink", "/signup?subdomain=" + zone.getSubdomain())))
    ...
    mockMvc
        ...
        .andExpect(
            model()
                .attribute(
                    "links", hasEntry("forgotPasswordLink", "/local_passwd?id=" + zone.getId())))
        .andExpect(
            model()
                .attribute(
                    "links",
                    hasEntry(
                        "createAccountLink", "/local_signup?subdomain=" + zone.getSubdomain())))
        .andExpect(content().string(containsString("/local_passwd?id=" + zone.getId())))
        .andExpect(
            content().string(containsString("/local_signup?subdomain=" + zone.getSubdomain())));
  }
  ...
  void global_zone_variable_home_redirect(
    ...
    mockMvc
        .perform(
            get("/")
                .with(
                    securityContext(
                        getUaaSecurityContext(marissa.getUserName(), webApplicationContext, zone)))
    ...
    mockMvc
        ...
        .andExpect(
            redirectedUrl("http://" + zone.getSubdomain() + ".redirect.to/z/" + zone.getId()));

    zone.getConfig()
        .getLinks()
        .setHomeRedirect("http://configured.{zone.subdomain}.redirect.to/z/{zone.id}");
    zone = MockMvcUtils.updateIdentityZone(zone, webApplicationContext);
    mockMvc
        .perform(
            get("/")
        ...
        .andExpect(
            redirectedUrl(
                "http://configured." + zone.getSubdomain() + ".redirect.to/z/" + zone.getId()));
  }
        ...
        .andExpect(
            cookie()
                .maxAge(
                    ...
                    CookieBasedCsrfTokenRepository.DEFAULT_COOKIE_MAX_AGE));
  }
  ...
  void testLogin_Csrf_Reset_On_Refresh() throws Exception {
    ...
    Cookie csrf1 =
        mvcResult.getResponse().getCookie(CookieBasedCsrfTokenRepository.DEFAULT_CSRF_COOKIE_NAME);

    mvcResult = mockMvc.perform(get("/login").cookie(csrf1)).andReturn();
    Cookie csrf2 =
        mvcResult.getResponse().getCookie(CookieBasedCsrfTokenRepository.DEFAULT_CSRF_COOKIE_NAME);
    assertNotNull(csrf2);
  ...
  void testLoginPageReloadOnCsrfExpiry(
      @Autowired CookieBasedCsrfTokenRepository cookieBasedCsrfTokenRepository) throws Exception {
    ...
    assertThat(
        ...
        containsString("http-equiv=\"refresh\" content=\"3\""));
    cookieBasedCsrfTokenRepository.setCookieMaxAge(
        CookieBasedCsrfTokenRepository.DEFAULT_COOKIE_MAX_AGE);
  }

  @Test
  void test_cookie_csrf(@Autowired JdbcScimUserProvisioning jdbcScimUserProvisioning)
      throws Exception {
    MockHttpSession session = new MockHttpSession();

    MockHttpServletRequestBuilder invalidPost =
        post("/login.do").session(session).param("username", "marissa").param("password", "koala");
    ...
    jdbcScimUserProvisioning
        .query("username eq 'marissa'", IdentityZoneHolder.get().getId())
        .get(0);
  ...
  @Test
  void test_case_insensitive_login(@Autowired ScimUserProvisioning scimUserProvisioning)
      throws Exception {
    ...
    MockHttpServletRequestBuilder loginPost =
        post("/authenticate")
  ...
  void test_previous_login_time_upon_authentication(
      @Autowired ScimUserProvisioning scimUserProvisioning) throws Exception {
    ...
    Long lastLoginTime =
        ((UaaAuthentication) securityContext.getAuthentication()).getLastLoginSuccessTime();
    ...
    void testDefaultLogo(@Autowired MockMvc mockMvc) throws Exception {
      mockMvc
          ...
          .andExpect(
              content()
                  .string(
                      containsString("url(//cdn.example.com/resources/images/product-logo.png)")));
    }
  }

  @Test
  void testCustomLogo() throws Exception {
    setZoneFavIconAndProductLogo(
        webApplicationContext, identityZoneConfiguration, null, "/bASe/64+");
    ...
    mockMvc
        ...
        .andExpect(
            content()
                .string(
                    allOf(
                        containsString("url(data:image/png;base64,/bASe/64+)"),
                        not(containsString("url(/uaa/resources/oss/images/product-logo.png)")))));
  }

  @Test
  void testCustomFavIcon() throws Exception {
    setZoneFavIconAndProductLogo(
        webApplicationContext, identityZoneConfiguration, "/sM4lL==", null);
        ...
        .andExpect(
            content()
                .string(
                    allOf(
                        containsString(
                            "<link href='data:image/png;base64,/sM4lL==' rel='shortcut icon' />"),
                        not(containsString("square-logo.png")))));
  }

  @Test
  void testCustomFavIcon_With_LineBreaks() throws Exception {
    setZoneFavIconAndProductLogo(
        webApplicationContext, identityZoneConfiguration, "/sM4\n\nlL==", "/sM4\n\nlL==");
        ...
        .andExpect(
            content()
                .string(
                    allOf(
                        containsString(
                            "<link href='data:image/png;base64,/sM4\n\nlL==' rel='shortcut icon' />"),
                        not(containsString("square-logo.png")))))
        .andExpect(
            content()
                .string(
                    allOf(
                        containsString(
                            "<style>.header-image {background-image: url(data:image/png;base64,/sM4lL==);}</style>"),
                        not(containsString("product-logo.png")))));
  }
  ...
  void testCustomizedFooter() throws Exception {
    ...
    MockMvcUtils.setZoneConfiguration(
        webApplicationContext, getUaa().getId(), identityZoneConfiguration);
    ...
    mockMvc
        ...
        .andExpect(
            content()
                .string(
                    allOf(containsString(customFooterText), not(containsString(cfCopyrightText)))))
        .andExpect(content().string(not(containsString(CF_LAST_LOGIN))));
  }
  ...
  @Test
  void testCustomCompanyNameInZone(@Autowired IdentityZoneProvisioning identityZoneProvisioning)
      throws Exception {
    String companyName = "Big Company";
    BrandingInformation branding = new BrandingInformation();
    branding.setCompanyName(companyName);
    ...
    IdentityZone identityZone =
        setupZone(webApplicationContext, mockMvc, identityZoneProvisioning, generator, config);
    ...
    mockMvc
        ...
        .andExpect(
            content()
                .string(
                    containsString(
                        "<a href=\"/privacy\">Privacy</a> &mdash; <a href=\"/terms.html\">Terms of Use</a>")));
  }

  @Test
  void testForgotPasswordPageDoesNotHaveCsrf() throws Exception {
    mockMvc
        .perform(get("/forgot_password"))
        .andExpect(status().isOk())
  ...
  void testChangePasswordPageDoesHaveCsrf() throws Exception {
    mockMvc
        .perform(
            get("/change_password")
                .with(
                    securityContext(MockMvcUtils.getMarissaSecurityContext(webApplicationContext))))
        .andExpect(status().isOk())
    ...
    mockMvc
        .perform(
            post("/change_password.do")
                .with(
                    securityContext(
                        MockMvcUtils.getUaaSecurityContext(
                            user.getUserName(), webApplicationContext)))
  ...
  void testLogOut() throws Exception {
    mockMvc
        .perform(get("/uaa/logout.do").contextPath("/uaa"))
        .andExpect(status().isFound())
  ...
  void testLogOutIgnoreRedirectParameter() throws Exception {
    mockMvc
        .perform(
            get("/uaa/logout.do").param("redirect", "https://www.google.com").contextPath("/uaa"))
        .andExpect(status().isFound())
        .andExpect(redirectedUrl("/uaa/login"))
        .andExpect(emptyCurrentUserCookie());
  }
    ...
    try {
      ...
      BaseClientDetails client =
          new BaseClientDetails(
              ...
              "client_credentials",
              "uaa.none",
              "http://*.wildcard.testing,http://testing.com");
  ...
  @Test
  void testLogOut_Config_For_Zone(@Autowired IdentityZoneProvisioning identityZoneProvisioning)
      throws Exception {
    ...
    // other zone
    mockMvc
        .perform(get("/uaa/logout.do").contextPath("/uaa").header("Host", zoneId + ".localhost"))
        .andExpect(status().isFound())
  ...
  class LoginWithAnalytics {
    ...
    void testLoginWithAnalytics(@Autowired MockMvc mockMvc) throws Exception {
      mockMvc
          .perform(get("/login").accept(TEXT_HTML))
          .andExpect(status().isOk())
  ...
  void testDefaultBranding() throws Exception {
    mockMvc
        ...
        .andExpect(
            xpath("//head/link[@rel='shortcut icon']/@href")
                .string("/resources/oss/images/square-logo.png"))
        .andExpect(
            xpath("//head/link[@href='/resources/oss/stylesheets/application.css']").exists())
        .andExpect(
            xpath("//head/style[text()[contains(.,'/resources/oss/images/product-logo.png')]]")
                .exists());
  }
  ...
  class Branding {
    ...
    void testExternalizedBranding(@Autowired MockMvc mockMvc) throws Exception {
      mockMvc
          ...
          .andExpect(
              xpath("//head/link[@rel='shortcut icon']/@href")
                  .string("//cdn.example.com/pivotal/images/square-logo.png"))
          .andExpect(
              xpath("//head/link[@href='//cdn.example.com/pivotal/stylesheets/application.css']")
                  .exists())
          .andExpect(
              xpath(
                      "//head/style[text()[contains(.,'//cdn.example.com/pivotal/images/product-logo.png')]]")
                  .exists());
    }
  }

  @Test
  void testAccessConfirmationPage(@Autowired JdbcScimUserProvisioning jdbcScimUserProvisioning)
      throws Exception {
    ScimUser marissa =
        jdbcScimUserProvisioning
            .query(
                "username eq \"marissa\" and origin eq \"uaa\"", IdentityZoneHolder.get().getId())
            .get(0);
    UaaPrincipal uaaPrincipal =
        new UaaPrincipal(
            marissa.getId(),
            marissa.getUserName(),
            marissa.getPrimaryEmail(),
            marissa.getOrigin(),
            marissa.getExternalId(),
            IdentityZoneHolder.get().getId());

    UaaAuthentication principal =
        new UaaAuthentication(
            uaaPrincipal, singletonList(UaaAuthority.fromAuthorities("uaa.user")), null);
    MockHttpSession session = new MockHttpSession();
  ...
  void testSignupsAndResetPasswordDisabledWithSomeLinksConfigured() throws Exception {
    ...
    identityZoneConfiguration
        .getLinks()
        .getSelfService()
        .setPasswd("http://example.com/reset_passwd");
  ...
  void testSignupsAndResetPasswordEnabledWithCustomLinks() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            xpath("//a[text()='Reset password']/@href").string("http://example.com/reset_passwd"));
  }
  ...
  void testLoginWithExplicitJsonPrompts() throws Exception {
    ...
    try {
      ...
      mockMvc
          .perform(get("/login").accept(APPLICATION_JSON))
          .andExpect(status().isOk())
          .andExpect(view().name("login"))
          .andExpect(model().attribute("prompts", hasKey("how")))
          .andExpect(model().attribute("prompts", hasKey("where")))
  ...
  void testInfoWithRemoteUaaJsonPrompts() throws Exception {
    mockMvc
        .perform(get("/info").accept(APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(view().name("login"))
        .andExpect(model().attribute("prompts", hasKey("username")))
        .andExpect(model().attribute("prompts", hasKey("password")));
  ...
  void testDefaultAndCustomSignupLink() throws Exception {
    ...
    identityZoneConfiguration
        .getLinks()
        .getSelfService()
        .setSignup("http://www.example.com/signup");
    MockMvcUtils.setZoneConfiguration(
        webApplicationContext, getUaa().getId(), identityZoneConfiguration);
    mockMvc
        .perform(get("/login").accept(TEXT_HTML))
        ...
        .andExpect(
            model()
                .attribute(
                    "links", hasEntry("createAccountLink", "http://www.example.com/signup")));
  }
  ...
  void testSamlLoginLinksShowActiveProviders(
    ...
    BaseClientDetails zoneAdminClient =
        new BaseClientDetails(
            "admin", null, null, "client_credentials", "clients.admin,scim.read,scim.write");
    ...
    String metadata =
        String.format(MockMvcUtils.IDP_META_DATA, new RandomValueStringGenerator().generate());
    SamlIdentityProviderDefinition activeSamlIdentityProviderDefinition =
    ...
    metadata =
        String.format(MockMvcUtils.IDP_META_DATA, new RandomValueStringGenerator().generate());
    SamlIdentityProviderDefinition inactiveSamlIdentityProviderDefinition =
        new SamlIdentityProviderDefinition()
            .setMetaDataLocation(metadata)
    ...
    createIdentityProvider(
        jdbcIdentityProviderProvisioning, identityZone, inactiveIdentityProvider);
    ...
    mockMvc
        ...
        .andExpect(status().isOk())
        .andExpect(
            xpath("//a[text()='" + activeSamlIdentityProviderDefinition.getLinkText() + "']")
                .exists())
        .andExpect(
            xpath("//a[text()='" + inactiveSamlIdentityProviderDefinition.getLinkText() + "']")
                .doesNotExist());
  }
  ...
  void testSamlRedirectWhenTheOnlyProvider(
    ...
    zoneAdminClient.addAdditionalInformation(
        ClientConstants.ALLOWED_PROVIDERS,
        Collections.singletonList(activeIdentityProvider.getOriginKey()));
    ...
    IdentityProvider uaaProvider =
        jdbcIdentityProviderProvisioning.retrieveByOriginIgnoreActiveFlag(
            UAA, identityZone.getId());
    try {
  ...
  void samlRedirect_onlyOneProvider_noClientContext(
    ...
    jdbcIdentityProviderProvisioning.update(
        uaaIdentityProvider, uaaIdentityProvider.getIdentityZoneId());
  ...
  void xOAuthRedirect_onlyOneProvider_noClientContext_and_ResponseType_Set(
    ...
    String oauthAlias =
        createOIDCProviderInZone(jdbcIdentityProviderProvisioning, identityZone, null);
    ...
    assertThat(
        ...
        hasEntry(
            ...
            "http%3A%2F%2F"
                + identityZone.getSubdomain()
                + ".localhost%2Flogin%2Fcallback%2F"
                + oauthAlias));
  ...
  void testLoginHintRedirect(
    ...
    IdentityProvider<OIDCIdentityProviderDefinition> oauthIdentityProvider =
        MultitenancyFixture.identityProvider(oauthAlias, "uaa");
  ...
  void testNoCreateAccountLinksWhenUAAisNotAllowedProvider(
    ...
    SamlIdentityProviderDefinition activeSamlIdentityProviderDefinition3 =
        new SamlIdentityProviderDefinition()
            .setMetaDataLocation(
                String.format(
                    BootstrapSamlIdentityProviderDataTests.xmlWithoutID,
                    "http://example3.com/saml/metadata"))
    ...
    activeIdentityProvider3 =
        createIdentityProvider(
            jdbcIdentityProviderProvisioning, identityZone, activeIdentityProvider3);
    ...
    SamlIdentityProviderDefinition activeSamlIdentityProviderDefinition2 =
        new SamlIdentityProviderDefinition()
            .setMetaDataLocation(
                String.format(
                    BootstrapSamlIdentityProviderDataTests.xmlWithoutID,
                    "http://example2.com/saml/metadata"))
    ...
    activeIdentityProvider2 =
        createIdentityProvider(
            jdbcIdentityProviderProvisioning, identityZone, activeIdentityProvider2);

    zoneAdminClient.addAdditionalInformation(
        ClientConstants.ALLOWED_PROVIDERS,
        asList(activeIdentityProvider3.getOriginKey(), activeIdentityProvider2.getOriginKey()));
    MockMvcUtils.updateClient(webApplicationContext, zoneAdminClient, identityZone);

    MockHttpSession session = new MockHttpSession();
    SavedRequest savedRequest =
        new DefaultSavedRequest(new MockHttpServletRequest(), new PortResolverImpl()) {
          @Override
  ...
  void testDeactivatedProviderIsRemovedFromSamlLoginLinks(
    ...
    identityProvider =
        createIdentityProvider(jdbcIdentityProviderProvisioning, identityZone, identityProvider);
    ...
    mockMvc
        ...
        .andExpect(status().isOk())
        .andExpect(
            xpath("//a[text()='" + samlIdentityProviderDefinition.getLinkText() + "']").exists());
    ...
    mockMvc
        ...
        .andExpect(status().isOk())
        .andExpect(
            xpath("//a[text()='" + samlIdentityProviderDefinition.getLinkText() + "']")
                .doesNotExist());
  }
  ...
  void testChangeEmailWithoutAuthenticationReturnsRedirect() throws Exception {
    mockMvc
        .perform(get("/change_email").accept(TEXT_HTML))
        .andExpect(status().isFound())
  ...
  void testChangeEmailPageHasCsrf() throws Exception {
    ...
    MockHttpServletRequestBuilder get =
        get("/change_email").accept(TEXT_HTML).with(securityContext(marissaContext));
    mockMvc
        .perform(get)
        .andExpect(status().isOk())
  ...
  void testChangeEmailSubmitWithMissingCsrf() throws Exception {
    ...
    MockHttpServletRequestBuilder changeEmail =
  ...
  void testChangeEmailDoNotLoggedIn() throws Exception {
    ...
    MockHttpServletRequestBuilder changeEmail =
        post("/change_email.do").accept(TEXT_HTML).with(cookieCsrf());
    mockMvc
        .perform(changeEmail)
        .andExpect(status().isFound())
        .andExpect(redirectedUrl("http://localhost/login"));

    changeEmail = post("/change_email.do").accept(TEXT_HTML).with(cookieCsrf());
    mockMvc
        .perform(changeEmail)
        .andExpect(status().isFound())
        .andExpect(redirectedUrl("http://localhost/login"));
  ...
  @Test
  void testCsrfForInvitationAcceptPost(@Autowired ExpiringCodeStore expiringCodeStore)
      throws Exception {
    assumeFalse(isLimitedMode(limitedModeUaaFilter), "Test only runs in non limited mode.");
    SecurityContext marissaContext = getMarissaSecurityContext(webApplicationContext);
    AnonymousAuthenticationToken inviteToken =
        new AnonymousAuthenticationToken(
            "invited-test",
            marissaContext.getAuthentication().getPrincipal(),
            singletonList(UaaAuthority.UAA_INVITED));
    ...
    codeData.put(
        "user_id", ((UaaPrincipal) marissaContext.getAuthentication().getPrincipal()).getId());
    codeData.put(
        "email", ((UaaPrincipal) marissaContext.getAuthentication().getPrincipal()).getEmail());
  ...
  /**
   * Positive test case that exercises the CORS logic for dealing with the "X-Requested-With"
   * header.
   *
   * @throws Exception
   */
   ...
   */
  @Test
  void testLogOutCorsPreflightWithStandardHeader(@Autowired CorsFilter corsFilter)
      throws Exception {
  ...
  /**
   * The endpoint is not white-listed to allow CORS requests with the "X-Requested-With" header so
   * the CorsFilter returns a 403.
   *
   * @throws Exception on test failure
   */
  @Test
  void testLogOutCorsPreflightWithUnallowedEndpoint(@Autowired CorsFilter corsFilter)
      throws Exception {
    corsFilter.setCorsXhrAllowedOrigins(singletonList("^localhost$"));
    corsFilter.setCorsXhrAllowedUris(singletonList("^/logout\\.do$"));
    corsFilter.initialize();
  ...
  /**
   * The access control request method is not a GET therefore CORS requests with the
   * "X-Requested-With" header are not allowed and the CorsFilter returns a 405.
   *
   * @throws Exception on test failure
   */
  @Test
  void testLogOutCorsPreflightWithUnallowedMethod(@Autowired CorsFilter corsFilter)
      throws Exception {
    corsFilter.setCorsXhrAllowedOrigins(singletonList("^localhost$"));
    corsFilter.setCorsXhrAllowedUris(singletonList("^/logout\\.do$"));
    corsFilter.initialize();
    ...
    httpHeaders.add("Origin", "localhost");
    mockMvc
        .perform(options("/logout.do").headers(httpHeaders))
        .andExpect(status().isMethodNotAllowed());
  }

  /**
   * The request origin is not white-listed to allow CORS requests with the "X-Requested-With"
   * header so the CorsFilter returns a 403.
   *
   * @throws Exception on test failure
   */
  @Test
  void testLogOutCorsPreflightWithUnallowedOrigin(@Autowired CorsFilter corsFilter)
      throws Exception {
    corsFilter.setCorsXhrAllowedOrigins(singletonList("^localhost$"));
    corsFilter.setCorsXhrAllowedUris(singletonList("^/logout\\.do$"));
    corsFilter.initialize();
  ...
  void autologin_with_validCode_RedirectsToSavedRequest_ifPresent(
      @Autowired JdbcExpiringCodeStore jdbcExpiringCodeStore) throws Exception {
    ...
    mockMvc
        .perform(
            post("/autologin")
                .header(
                    "Authorization",
                    "Basic " + new String(Base64.encode("admin:adminsecret".getBytes())))
                .contentType(APPLICATION_JSON)
  ...
  void idpDiscoveryPageDisplayed_IfFlagIsEnabled(
      @Autowired IdentityZoneProvisioning identityZoneProvisioning) throws Exception {
    IdentityZoneConfiguration config = new IdentityZoneConfiguration();
  ...
  void idpDiscoveryPageNotDisplayed_IfFlagIsEnabledAndDiscoveryUnsuccessfulPreviously(
    ...
    mockMvc
        .perform(
            get("/login?discoveryPerformed=true")
                .header("Accept", TEXT_HTML)
                .with(new SetServerNameRequestPostProcessor(zone.getSubdomain() + ".localhost")))
        .andExpect(status().isOk())
  ...
  @Test
  void accountChooserEnabled(@Autowired IdentityZoneProvisioning identityZoneProvisioning)
      throws Exception {
    String clientName = "woohoo";
    IdentityZoneConfiguration config = new IdentityZoneConfiguration();
    config.setIdpDiscoveryEnabled(true);
    ...
    mockMvc
        .perform(
            get("/login")
                ...
                .cookie(
                    new Cookie(
                        "Saved-Account-12345678",
                        URLEncoder.encode(JsonUtils.writeValueAsString(savedAccount))))
                .header("Accept", TEXT_HTML)
                .with(new SetServerNameRequestPostProcessor(zone.getSubdomain() + ".localhost")))
        .andDo(print())
        .andExpect(status().isOk())
  ...
  void emailPageIdpDiscoveryEnabled_SelfServiceLinksDisabled(
    ...
    config.setIdpDiscoveryEnabled(true);
    config.setLinks(
        new Links().setSelfService(new Links.SelfService().setSelfServiceLinksEnabled(false)));
    IdentityZone zone =
        setupZone(webApplicationContext, mockMvc, identityZoneProvisioning, generator, config);
  ...
  void idpDiscoveryRedirectsToSamlExternalProvider_withClientContext(
    ...
    MockHttpSession session =
        setUpClientAndProviderForIdpDiscovery(
            webApplicationContext, jdbcIdentityProviderProvisioning, generator, originKey, zone);

    mockMvc
        .perform(
            post("/login/idp_discovery")
                .with(cookieCsrf())
                .header("Accept", TEXT_HTML)
                .session(session)
        ...
        mockMvc
            .perform(
                post("/login/idp_discovery")
                    ...
                    .param("email", "marissa@test.org")
                    .with(
                        new SetServerNameRequestPostProcessor(zone.getSubdomain() + ".localhost")))
            .andExpect(status().isFound())
            .andReturn();
    String location = mvcResult.getResponse().getHeader("Location");
    Map<String, String> queryParams =
  ...
  void multiple_oidc_providers_use_response_type_in_url(
    ...
    mockMvc
        ...
        .andExpect(
            content()
                .string(
                    containsString("http://myauthurl.com?client_id=id&amp;response_type=code&")))
        .andExpect(
            content()
                .string(
                    containsString(
                        "http://myauthurl.com?client_id=id&amp;response_type=code+id_token&")));
  }
  ...
  void idpDiscoveryWithNoEmailDomainMatch_withClientContext(
    ...
    IdentityProvider identityProvider =
        jdbcIdentityProviderProvisioning.retrieveByOrigin("uaa", zone.getId());
    identityProvider.setConfig(
        new AbstractIdentityProviderDefinition()
            .setEmailDomain(Collections.singletonList("totally-different.org")));
  ...
  void idpDiscoveryWithUaaFallBack_withClientContext(
    ...
    mockMvc
        .perform(
            get("/login?discoveryPerformed=true&email=marissa%40other.domain")
                .with(cookieCsrf())
                .header("Accept", TEXT_HTML)
                .session(session)
                .with(new SetServerNameRequestPostProcessor(zone.getSubdomain() + ".localhost")))
  ...
  void idpDiscoveryWithLdap_withClientContext(
    ...
    identityProvider.setConfig(
        new LdapIdentityProviderDefinition()
            .setEmailDomain(Collections.singletonList("testLdap.org")));
  ...
  void passwordPageDisplayed_ifUaaIsFallbackIDPForEmailDomain(
    ...
    mockMvc
        .perform(
            get("/login?discoveryPerformed=true&email=marissa@koala.com")
                .with(cookieCsrf())
                .with(new SetServerNameRequestPostProcessor(zone.getSubdomain() + ".localhost"))
                .header("Accept", TEXT_HTML))
        .andExpect(view().name("idp_discovery/password"))
  ...
  void passwordPageIdpDiscoveryEnabled_SelfServiceLinksDisabled() throws Exception {
    ...
    mockMvc
        .perform(
            get("/login?discoveryPerformed=true&email=marissa%40koala.org")
                .with(cookieCsrf())
                .header("Accept", TEXT_HTML))
        .andExpect(status().isOk())
  ...
  @Test
  void userNamePresentInPasswordPage(@Autowired IdentityZoneProvisioning identityZoneProvisioning)
      throws Exception {
    IdentityZoneConfiguration config = new IdentityZoneConfiguration();
    config.setIdpDiscoveryEnabled(true);
    IdentityZone zone =
    ...
    mockMvc
        .perform(
            get("/login?discoveryPerformed=true&email=test@email.com")
                .with(cookieCsrf())
                .with(new SetServerNameRequestPostProcessor(zone.getSubdomain() + ".localhost")))
  ...
  void authorizeForClientWithIdpNotAllowed(
    ...
    SetServerNameRequestPostProcessor inZone =
        new SetServerNameRequestPostProcessor(subdomain + ".localhost");
    ...
    mockMvc.perform(post).andExpect(redirectedUrl("/uaa/"));
    ...
    client.addAdditionalInformation(
        ClientConstants.ALLOWED_PROVIDERS, singletonList("a-different-provider"));
    ...
    MockHttpServletRequestBuilder authorize =
        get("/oauth/authorize")
            .with(inZone)
            .session(session)
    ...
    String expectedUrl =
        "http://"
            + subdomain
            + ".localhost/oauth/authorize?client_id=different-provider-client&response_type=code&client_secret=secret&garbage=this-should-be-preserved";
    String html =
        mockMvc
            .perform(authorize)
            .andDo(print())
  ...
  private static MockHttpSession setUpClientAndProviderForIdpDiscovery(
    ...
    String metadata =
        String.format(MockMvcUtils.IDP_META_DATA, new RandomValueStringGenerator().generate());
    SamlIdentityProviderDefinition config =
    ...
    IdentityProvider identityProvider =
        MultitenancyFixture.identityProvider(originKey, zone.getId());
    identityProvider.setType(OriginKeys.SAML);
    ...
    client.addAdditionalInformation(
        ClientConstants.ALLOWED_PROVIDERS, asList(originKey, "other-provider", UAA, LDAP));
    MockMvcUtils.createClient(webApplicationContext, client, zone);
  ...
  private static void changeLockoutPolicyForIdpInZone(
      JdbcIdentityProviderProvisioning jdbcIdentityProviderProvisioning, IdentityZone zone) {
    IdentityProvider identityProvider =
        jdbcIdentityProviderProvisioning.retrieveByOrigin(UAA, zone.getId());
  ...
  private static void attemptUnsuccessfulLogin(
      MockMvc mockMvc, int numberOfAttempts, String username, String subdomain) throws Exception {
  ...
  private static ScimUser createUser(
      ScimUserProvisioning scimUserProvisioning, String username, String zoneId) {
  ...
  private String createOIDCProviderInZone(
      JdbcIdentityProviderProvisioning jdbcIdentityProviderProvisioning,
      IdentityZone identityZone,
      String discoveryUrl)
      throws Exception {
  ...
  private static final String cfCopyrightText =
      String.format(defaultCopyrightTemplate, "CloudFoundry.org Foundation, Inc.");
  private static final String CF_LAST_LOGIN = "Last Login";

  private static IdentityProvider createIdentityProvider(
      JdbcIdentityProviderProvisioning jdbcIdentityProviderProvisioning,
      IdentityZone identityZone,
      IdentityProvider activeIdentityProvider) {
