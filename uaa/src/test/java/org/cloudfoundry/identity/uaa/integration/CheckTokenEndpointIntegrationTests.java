...
public class CheckTokenEndpointIntegrationTests {
  ...
  public void testDecodeToken() {
    ...
    response =
        serverRunning.getForString(
            result.getHeaders().getLocation().toString(), getHeaders(cookies));
    if (response.getHeaders().containsKey("Set-Cookie")) {
      for (String cookie : response.getHeaders().get("Set-Cookie")) {
        int nameLength = cookie.indexOf('=');
        cookies.addCookie(
  ...
  public void testTokenKey() {
    ...
    ClientCredentialsResourceDetails resource =
        testAccounts.getClientCredentialsResource("app", null, "app", "appclientsecret");
    headers.set(
        "Authorization",
        testAccounts.getAuthorizationHeader(resource.getClientId(), resource.getClientSecret()));
  ...
  public void testInvalidScope() {
    ...
    String requestBody =
        String.format("token=%s&scopes=%s", accessToken.getValue(), "uaa.resource%");
  ...
  public void testValidPasswordGrant() {
    ...
    @SuppressWarnings("rawtypes")
    ResponseEntity<Map> tokenResponse =
        serverRunning.postForMap("/check_token", formData, tokenHeaders);
    assertEquals(HttpStatus.OK, tokenResponse.getStatusCode());
