...
public class UaaMetricsEmitterIT {
  ...
  private static List<String> metricFragments =
      Arrays.asList(
  ...
  public void assert_generic_metrics() throws IOException {
    ...
    assertThat(
        statsDKey + " first value must have a positive value.", first, greaterThanOrEqualTo(0l));
    ...
    assertThat(
        statsDKey + " second value must have a positive value.", second, greaterThanOrEqualTo(0l));
  }

  private static Map<String, String> getMessages(List<String> fragments, int timeout)
      throws IOException {
    ...
    } while (results.size() < fragments.size()
  ...
  private static void performLogin(String username) {
    ...
    ResponseEntity<String> loginResponse =
        template.exchange(
            UAA_BASE_URL + "/login", HttpMethod.GET, new HttpEntity<>(null, headers), String.class);
