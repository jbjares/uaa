...
public class UaaMetricsEmitter {
  ...
  private static final RequestMetricSummary MISSING_METRICS =
      new RequestMetricSummary(0l, 0d, 0l, 0d, 0l, 0d, 0l, 0d);
  ...
  public UaaMetricsEmitter(
      MetricsUtils metricsUtils, StatsDClient statsDClient, MBeanServerConnection server) {
    ...
    for (Map.Entry entry : result.entrySet()) {
      ...
      if (properties != null) {
        properties
            ...
            .forEach(
                e -> statsDClient.gauge(prefix + e.getKey(), ((Integer) e.getValue()).longValue()));
      }
    }
  }
  ...
  private void emitUrlGroupRequestMetrics(UaaMetrics metrics) {
    ...
    for (String key : perUrlMetrics.keySet()) {
      ...
      statsDClient.gauge(
          String.format(prefix + "completed.count", prefixName), metricTotals.getCount());
      statsDClient.gauge(
          String.format(prefix + "completed.time", prefixName),
          (long) metricTotals.getAverageTime());
    }
  }
  ...
  public void emitGlobalRequestMetrics(UaaMetrics metrics) {
    ...
    statsDClient.count(
        prefix + "completed.count", getMetricDelta(prefix + "completed.count", totals.getCount()));
    statsDClient.count(
        prefix + "unhealthy.count",
        getMetricDelta(prefix + "unhealthy.count", totals.getIntolerableCount()));
    ...
    for (StatusCodeGroup family : StatusCodeGroup.values()) {
      RequestMetricSummary summary =
          ofNullable(globals.getDetailed().get(family)).orElse(MISSING_METRICS);
    ...
    statsDClient.count(
        prefix + "completed.count",
        getMetricDelta(prefix + "completed.count", totals.getDatabaseQueryCount()));
    statsDClient.count(
        prefix + "unhealthy.count",
        getMetricDelta(prefix + "unhealthy.count", totals.getDatabaseIntolerableQueryCount()));
    statsDClient.gauge(
        prefix + "unhealthy.time", (long) totals.getAverageDatabaseIntolerableQueryTime());
  }
  ...
  public void emitJvmVitals() {
    ...
    invokeIfPresent(
        prefix + "cpu.load", osBean, "getProcessCpuLoad", d -> (long) (d.doubleValue() * 100));
  ...
  public void invokeIfPresent(
      String metric, Object mbean, String getter, Function<Number, Long> valueModifier) {
  ...
  public void throwIfOtherThanNotFound(Exception x) throws Exception {
    ...
    if (x instanceof UndeclaredThrowableException
        && x.getCause() instanceof InstanceNotFoundException) {
      // normal - the statsd server may have
      // started before the UAA
      logger.info(
          "Could not find UaaMetrics object on MBean server. Please deploy UAA in the same JVM.");
    } else {
      throw x;
    }
  }
    ...
    try {
      ...
      emitter.addNotificationListener(
          (notification, handback) -> {
            ...
            statsDClient.time(
                String.format("requests.%s.latency", prefix), (Long) notification.getSource());
          },
          null,
          null);
